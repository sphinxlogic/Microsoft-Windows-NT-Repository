#ifndef lint
static char rcsid[] = "$Header: /usr/people/sam/tiff/libtiff/RCS/mkg3states.c,v 1.6 91/09/27 09:16:20 sam Exp $";
#endif
 ...
 *
 *	Reference: $Id: mkg3states.c,v 1.6 91/09/27 09:16:20 sam Exp $
 *
...
DECLARE3(write_null_mode_table, FILE*, fd, NullModeTable, table, char*, name)
{
    ...
    fprintf(fd, "u_char\t%s[%d][256] = {", name, null_mode_prefix_count);
    outersep = "";
    for (i = 0; i < null_mode_prefix_count; i++) {
	fprintf(fd, "%s\n/* prefix %d */ {\n", outersep, i);
	sep = "    ";
	for (j = 0; j < 256; j++) {
	    fprintf(fd, "%s%2d", sep, (short) table[i][j]);
	    if (((j+1) % 16) == 0) {
		fprintf(fd, ", /* %3d-%3d */\n", j-15, j);
		sep = "    ";
	    } else
...
DECLARE3(write_horiz_mode_table, FILE*, fd, HorizModeTable, table, char*, name)
{
    ...
    fprintf(fd, "u_char\t%s[%d][256] = {", name, horiz_mode_prefix_count);
    outersep = "";
    for (i = 0; i < horiz_mode_prefix_count; i++) {
	fprintf(fd, "%s\n/* prefix %d */ {\n", outersep, i);
	sep = "    ";
	for (j = 0; j < 256; j++) {
	    fprintf(fd, "%s%3d", sep, (short) table[i][j]);
	    if (((j+1) % 14) == 0) {
		fprintf(fd, ", /* %3d-%3d */\n", j-13, j);
		sep = "    ";
	    } else
