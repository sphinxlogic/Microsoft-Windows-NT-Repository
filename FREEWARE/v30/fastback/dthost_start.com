...
$ask:==inquire
$oldprv=f$setprv("cmkrnl,sysprv,log_io,phy_io,tmpmbx,volpro,detach")
$tprv=f$privilege("cmkrnl,sysprv,log_io,phy_io,tmpmbx,volpro,detach")
...
$sysgen:==$sysgen
$sysgen connect DTA0:/noadapter/driver=dtdriver
$dvc="DTA0:"
$if(f$getdvi(dvc,"EXISTS")) THEN GOTO DVCEXIST
...
$ctf=0
$if f$extract(0,1,f$edit(ctyp,"UPCASE,COLLAPSE")) .EQS. "Y" THEN CTF=1
...
$open/write cmf sys$login:dth.com
$write cmf "$set verify"
$write cmf "$set command sys$system:dthstimg"
$write cmf "$set proc/name=dt_control"
$write cmf "$set proc/priv=all"
$mfr = "$mount/for/multi "
$mfr2:='mfr''tp'
$write cmf mfr2
$fh1 = "$dthost/tcache:"
$if ctf.eq.0 then fh1 = "$dthost/cache:"
$fh2 = "/skip:"
$fhi := 'fh1''csiz''fh2''skip' 'dvc' 'tp'
$write cmf fhi
$write cmf "$eoj"
$close cmf
$ifil=f$search("sys$login:dth.com")
$open/write ofl sys$login:dthost.log
$write ofl "$!log of ",ifil
$close ofl
$ofil=f$search("sys$login:dthost.log")
$run/det/auth/out='ofil'/input='ifil' sys$system:loginout
$write sys$output "Waiting for disk to come online now"
$! wait for 2 3/4 minutes
$lpctr=0
$wtonl:
$wait 0:0:15
...
$lpctr = lpctr+1
$if lpctr.lt.12 then goto wtonl
