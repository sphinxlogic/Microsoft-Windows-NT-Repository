    ...
    Bool            falldown;
{
	...
	while (flag) {
		...
		case '>':
		case '.':
		    if (!paused && LEGAL(hexagons + 1, corners, mixed, bonus)) {
			hexagons++;
			redoNext();
			if (showNext) {
			    clearNext();
		...
		case 'c':
		case 'C':
		    if (!paused && LEGAL(hexagons, !corners, mixed, bonus)) {
			corners = !corners;
			redoNext();
			if (showNext) {
			    clearNext();
    ...
    move_t          move;
{
	...
	score += (showNext ? 3 : 5) * level * level * lines * lines *
          hexagons * hexagons * (bonus + corners + 1) / (16 * (1 + mixed));
	rows += lines;
    ...
    int             num;
{
    ...
    (void) fprintf(stderr, "            GENERIC HEXTRIS  HALL OF FAME\n\n");
    (void) fprintf(stderr,
