...
static void mk_gsc P_((Widget w));
...
#define	SMALL	(1e-6)
	...
	while (fgets (buf, sizeof(buf), pfp)) {
		...
		int ytop = y0 - h + asc;
		XDrawString(dsp,win,plt_gcc[i], ix+2, (iy<ytop ? ytop : iy)-2,
									&c, 1);
	    }
	    lx[i] = ix;
	    ly[i] = iy;
...
Widget w;
{
	...
	unsigned long da_bgp;/* the background color of the drawing area */
...
int x, y;
{
	...
	XDrawLine (dsp, win, plt_gc, x, y+TL, x, y+2*TL);
	XDrawString (dsp, win, plt_gc, x-ovl.width/2, y+2*TL+asc+1, buf, l);
