...
static void esat_main P_((double CrntTime, Obj *op));
...
static void InitOrbitRoutines P_((double EpochDay));
...
Obj *op;
{
	...
	/* precess ra/dec to current epoch if it's not EOD */
	if (epoch != EOD) {
	    double dra = op->s_ra, ddec = op->s_dec;
	    precess (mjd, epoch, &dra, &ddec);
	    op->s_ra = dra;
	    op->s_dec = ddec;
	}

	/* allow for refraction of altitude */
	dalt = op->s_alt;
	refract (pressure, temp, dalt, &dalt);
	op->s_alt = dalt;

	/* just make up a size and brightness */
	op->s_mag = ESAT_MAG *MAGSCALE;
	op->s_size = 0;
...
Obj *op;
{
	...
	/* ECD: range checking */
	if (CurrentMotion <= 0) {
	    xe_msg ("Satellite elements no longer valid -- bogus results.", 0);
	    CurrentMotion = Epsilon;
	}
	...
	range (&dtmp, 2*PI);
	op->s_ra = dtmp;
	op->s_dec = atan2 (SatZ, sqrt(SatX*SatX + SatY*SatY));
