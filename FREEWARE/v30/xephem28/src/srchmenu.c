...
static srch_isup P_((void));
static srching_now P_((void));
static void srch_set_buttons P_((int whether));
static void srch_use_cb P_((Widget w, XtPointer client, XtPointer call));
static void srch_funcactivate_cb P_((Widget w, XtPointer client,
    XtPointer call));
...
static srch_minmax P_((double Mjd, double v, double *tmincp));
static srch_solve0 P_((double Mjd, double v, double *tmincp));
static srch_binary P_((double Mjd, double v, double *tmincp));
...
static char srchvname[] = "SrchValue";
...
Cursor c;
{
	...
	if (srchform_w && (win = XtWindow(srchform_w))) {
...
srch_create_form()
{
	 ...
	 */
	n = 0;
	XtSetArg (args[n], XmNtitle, "xephem Search Control"); n++;
	XtSetValues (XtParent(srchform_w), args, n);

	/* create a RowColumn to hold stuff */
	n = 0;
	...
	rc_w = XmCreateRowColumn (srchform_w, "SrchRC", args, n);
	XtManageChild (rc_w);

	/* searching on/off toggle button */

	n = 0;
	on_w = XmCreateToggleButton (rc_w, "Searching is Active", args, n);
	...
	str = XmStringCreate ("Search Function:",
						    XmSTRING_DEFAULT_CHARSET);
	n = 0;
	XtSetArg (args[n], XmNalignment, XmALIGNMENT_BEGINNING); n++;
	XtSetArg (args[n], XmNlabelString, str); n++;
	w = XmCreateLabel (rc_w, "FuncL", args, n);
	XtManageChild (w);
	...
	n = 0;
	func_w = XmCreateText (rc_w, "SrchFunction", args, n);
	XtAddCallback (func_w, XmNactivateCallback, srch_funcactivate_cb, NULL);
	...
	n = 0;
	XtSetArg (args[n], XmNalignment, XmALIGNMENT_BEGINNING); n++;
	err_w = XmCreateLabel (rc_w, "SrchErrsL", args, n);
	XtManageChild (err_w);
	...
	n = 0;
	XtSetArg (args[n], XmNlabelString, str); n++;
	field_w = XmCreateToggleButton (rc_w, "SrchFEnable", args, n);
	...
	compile_w = XmCreatePushButton (rc_w, "Compile", args, n);
	...
	str = XmStringCreate ("Search goal:", XmSTRING_DEFAULT_CHARSET);
	n = 0;
	XtSetArg (args[n], XmNalignment, XmALIGNMENT_BEGINNING); n++;
	XtSetArg (args[n], XmNlabelString, str); n++;
	w = XmCreateLabel (rc_w, "SrchRBL", args, n);
	XtManageChild(w);
	XmStringFree(str);

	n = 0;
	XtSetArg (args[n], XmNorientation, XmVERTICAL); n++;
	rb_w = XmCreateRadioBox (rc_w, "SrchGoalRadioBox", args, n);
	XtManageChild (rb_w);
	...
	n = 0;
	w = XmCreateSeparator (rc_w, "Sep3", args, n);
	XtManageChild (w);

	/* Accuracy label and its push button in a form */

	n = 0;
	f_w = XmCreateForm (rc_w, "SrchAF", args, n);
	XtManageChild (f_w);
	...
	n = 0;
	f_w = XmCreateForm (rc_w, "SrchVF", args, n);
	XtManageChild (f_w);
	    ...
	    XtSetArg (args[n], XmNbottomAttachment, XmATTACH_FORM); n++;
	    XtSetArg (args[n], XmNrightAttachment, XmATTACH_FORM); n++;
	    valu_w = XmCreateLabel (f_w, "SrchValu", args, n);
	    XtManageChild (valu_w);
	    set_xmstring (valu_w, XmNlabelString, "");
	    ...
	    /* add the name in the userData field in case it's plotted or
	     * listed
	     */
	    set_something (valu_w, XmNuserData, (XtArgVal)srchvname);

	/* button to select using for plotting. 
	 * ONLY MANAGED WHEN WE ARE TOLD TO SELECT FOR PLOTTING.
	 */

	n = 0;
	use_w = XmCreatePushButton (rc_w, "SrchUse", args, n);
	XtAddCallback (use_w, XmNactivateCallback, srch_use_cb, 0);
	set_xmstring (use_w, XmNlabelString, "Use for plotting");
	...
	f_w = XmCreateForm (rc_w, "SrchCF", args, n);
	XtManageChild (f_w);
...
int whether;	/* whether setting up for plotting or for not plotting */
{
	if (whether)
	    ...
	    XtUnmanageChild (use_w);
}

/* callback from the "use" button.
 * if plotting or listing has put us in selecting mode, we look like a button
 * and we should inform them we have been picked.
...
/* callback when Return is typed in the function text widget.
 * we just stimulate the Compile button as per Heller, pg 219.
 */
/* ARGSUSED */
static void
srch_funcactivate_cb (w, client, call)
Widget w;
XtPointer client;
XtPointer call;
{
	...
	XtCallActionProc (compile_w, "ArmAndActivate", cbs->event, NULL, 0);
}
...
XtPointer call;
{
	...
	if (!dw) {
	    ...
	    str = XmStringCreate ("search accuracy (h:m:s):",
						    XmSTRING_DEFAULT_CHARSET);
	    title = XmStringCreate ("xephem Search Accuracy",
						    XmSTRING_DEFAULT_CHARSET);
	    n = 0;
	    XtSetArg(args[n], XmNdefaultPosition, False);  n++;
