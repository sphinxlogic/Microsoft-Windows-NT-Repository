...
extern void mm_setsite P_((double slat, double slng, double selev,char *sname,
    int update));
...
static sites_cmpf P_((qsort_arg v1, qsort_arg v2));
 ...
 * if found, return index into sites[], else return -1.
 */
int
...
set_selected ()
{
	...
	if (XmListGetSelectedPos (sql_w, &pos, &npos)) {
		...
		mm_setsite(sip->si_lat,sip->si_lng,sip->si_elev,sip->si_name,0);
	    } else
...
XtPointer call;
{
	...
	if (pos >= 0) {
	    XmListSetPos (sql_w, pos+1);		/* scroll it to top */
	    XmListSelectPos (sql_w, pos+1, True);	/* highlight it */
