...
1000  if (on_code) then
         if (entry(i).nohol) then
            ...
            mon = m2i ( now(4:6) )
            read(now,900) mday
            pushit = .not. btest(cbits(mon),(mday-1))
         endif
      endif
c
      return
...
10    ieval = ival
      ...
      return
c
c --- literal string.  Note that ISSTRING is false because the string
c ---  returned is permanently stored in the parsed table.
c
20    ieval = ival
      isstring = .false.
      return
