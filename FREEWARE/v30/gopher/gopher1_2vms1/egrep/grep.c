...
/* June 11, 1992	Hunter Goatley	Modified to run under VMS
					(using #ifdef VMS)		*/

#include <stdio.h>
...
#ifdef VMS
#include <ssdef.h>
#define MATCHES_FOUND 1		/* Matches were found */
#define NO_MATCHES_FOUND 3	/* No matches -- success */
#define ERROR SS$_ABORT		/* Abort */
#else
...
#ifdef VMS
  printed_limit[count-1] = '\0';
  printf("%s", printed_limit);		/* printf is used because it's faster */
  printed_limit[count-1] = '\n';	/* than multiple putchar's.           */
  putchar('\n');
#else
  do
    {
...
#ifdef VMS
  	  (char *) index(reg.must, '\0') != reg.must + reg.mustn)
#else
...
#ifdef VMS
	fd = open(filename, 0, 0, "shr=get,put,del,upd");
#else
