/*****************************************************************************
 * JL Wilkinson 16-July-1994 Merged in TGV's syslog() functions to allow the
 *			     CLIENT_LOGGER and TELNET_TRACE functionality to
 *			     work with Multinet.  Not tested against other VMS
 *			     TCP/IP agents, and *NOT SUPPORTED* thru TGV -- 
 *			     please don't anybody ask TGV for help in using
 *			     this.  Somebody who wants to use it can Email me
 *			     or the VMSGopher-L@trln.lib.unc.edu group and
 *			     I'll try to assist them for MULTINET only
 *			     (JLW@psulias.psu.edu).  Basic documentation is in
 *			     [.DOC]CLIENTLOGGING.VMS.
 */
/*****************************************************************************
 * F.Macrides 01-Jul-1994   Added Lance's post-pager RETURN synonyms.
 * F.Macrides 20-Jun-1994   From the way Paul implemented my NOMAIL code in
 *			     pager.c, it appears that downloading was not
 *			     intended to by blocked as well, so I fixed code
 *			     here to indicate 'D'ownload as an option on exit
 *			     from the pager in SecureMode and NoShellMode.
 ****************************************************************************/

/********************************************************************
  Changes by Alan Coopersmith's patches:
    - fix Choose_View() so it doesn't crash when try to deal with more
	than 9 views for an item
    - make the first "text/plain" in the default language
	the default view in Choose_View()
 ********************************************************************/
  ...
  GopherObj *gs;
{
     ...
     if (GSisGplus(gs)) {
	  ...
	  if (GSgetNumViews(gs) >1) {
	       ...
	       char *default_lang =  Gtxt(DEFAULT_LANG, 218);
	       char **views = malloc((1 + GSgetNumViews(gs)) * sizeof(char *));
	       char **choices = malloc((1 + GSgetNumViews(gs)) * sizeof(char *));
  ...
  GopherObj *gs;
{
     ...
     if (strncasecmp(command, "builtin",7) == 0) {
     ...
     if (!SecureMode && !NoShellMode) {
	  ...
	  printf(Gtxt(", %sD%s to download:",219),
		 CURgetHighon(CursesScreen), CURgetHighoff(CursesScreen));
     }
#endif
#else /*** Unix: ****/
