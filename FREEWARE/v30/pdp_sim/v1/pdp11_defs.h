...
#define VA_DS		(1 << VA_V_DS)			/* data space flag */
...
struct fpac {
	unsigned int	l;				/* low 32b */
	unsigned int	h;				/* high 32b */
};
...
#define CSR_GO		(1 << CSR_V_GO)
#define CSR_IE		(1 << CSR_V_IE)
#define CSR_DONE	(1 << CSR_V_DONE)
#define CSR_BUSY	(1 << CSR_V_BUSY)
#define CSR_ERR		(1 << CSR_V_ERR)
...
#define TRAP_RED	(1 << TRAP_V_RED)
#define TRAP_ODD	(1 << TRAP_V_ODD)
#define TRAP_MME	(1 << TRAP_V_MME)
#define TRAP_NXM	(1 << TRAP_V_NXM)
#define TRAP_PAR	(1 << TRAP_V_PAR)
#define TRAP_PRV	(1 << TRAP_V_PRV)
#define TRAP_ILL	(1 << TRAP_V_ILL)
#define TRAP_BPT	(1 << TRAP_V_BPT)
#define TRAP_IOT	(1 << TRAP_V_IOT)
#define TRAP_EMT	(1 << TRAP_V_EMT)
#define TRAP_TRAP	(1 << TRAP_V_TRAP)
#define TRAP_TRC	(1 << TRAP_V_TRC)
#define TRAP_YEL	(1 << TRAP_V_YEL)
#define TRAP_PWRFL	(1 << TRAP_V_PWRFL)
#define TRAP_FPE	(1 << TRAP_V_FPE)
#define TRAP_INT	(1 << TRAP_V_MAX)
#define TRAP_ALL	((1 << TRAP_V_MAX) - 1)		/* all traps */
