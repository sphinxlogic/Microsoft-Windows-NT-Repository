...
#define RL_MAXMEM	(MEMSIZE / sizeof (short))	/* words/memory */
...
#define UNIT_W_UF	3				/* saved flags width */
#define UNIT_HWLK	(1 << UNIT_V_HWLK)
#define UNIT_RL02	(1 << UNIT_V_RL02)
...
#define RLDA_HD1	(1 << RLDA_V_TRACK)
...

extern int int_req;
extern unsigned short M[];				/* memory */
int rlcs = 0;						/* control/status */
int rlba = 0;						/* memory address */
int rlbae = 0;						/* mem addr extension */
int rlda = 0;						/* disk addr */
int rlmp = 0, rlmp1 = 0, rlmp2 = 0;			/* mp register queue */
int rl_swait = 10;					/* seek wait */
int rl_rwait = 10;					/* rotate wait */
int rl_stopioe = 1;					/* stop on error */
int rl_svc (UNIT *uptr);
int rl_reset (DEVICE *dptr);
void rl_set_done (int error);
int rl_boot (int unitno);
int rl_attach (UNIT *uptr, char *cptr);
int rl_set_size (UNIT *uptr, int value);
...
UNIT rl_unit[] = {
	{ UDATA (&rl_svc, UNIT_FIX+UNIT_ATTABLE, RL01_SIZE) },
	{ UDATA (&rl_svc, UNIT_FIX+UNIT_ATTABLE, RL01_SIZE) },
	{ UDATA (&rl_svc, UNIT_FIX+UNIT_ATTABLE, RL01_SIZE) },
	{ UDATA (&rl_svc, UNIT_FIX+UNIT_ATTABLE, RL01_SIZE) } };
...
REG rl_reg[] = {
	...
	{ GRDATA (FLG0, rl_unit[0].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG1, rl_unit[1].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG2, rl_unit[2].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG3, rl_unit[3].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
...
MTAB rl_mod[] = {
	...
	{ UNIT_AUTO, UNIT_AUTO, NULL, "AUTOSIZE", &rl_set_size },
...
int rl_rd (int *data, int PA, int access)
{
UNIT *uptr;
...

int rl_wr (int data, int PA, int access)
{
int curr, offs, newc, maxc;
UNIT *uptr;
...
static unsigned short fill[RL_NUMWD] = { 0 };
int rl_svc (UNIT *uptr)
{
int comp, err, awc, wc, maxwc;
int func, pa, da, fillc;
...
if ((uptr -> flags & UNIT_ATT) == 0) {			/* attached? */
	...
	return IORETURN (rl_stopioe, SCPE_NOATT);  }
...
err = fseek (uptr -> fileref, da * sizeof (short), SEEK_SET);
...
if ((func == RLCS_WRITE) && (err == 0)) {		/* write? */
	...
	if ((err == 0) && (fillc = (wc & (RL_NUMWD - 1)))) {
		fwrite (fill, sizeof (short), fillc, uptr -> fileref);
		err = ferror (uptr -> fileref);  }  }

if ((func == RLCS_WCHK) && (err == 0)) {		/* write check? */
	fillc = wc;					/* xfer length */
	for (wc = 0; (err == 0) && (wc < fillc); wc++)  {
		awc = fread (&comp, sizeof (short), 1, uptr -> fileref);
		if (awc == 0) comp = 0;
