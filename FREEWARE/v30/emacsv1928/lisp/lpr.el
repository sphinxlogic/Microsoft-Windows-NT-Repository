...
(defvar lpr-headers-switches
  (if (memq system-type '(usg-unix-v dgux hpux irix)) nil
    (if (memq system-type '(axp-vms vax-vms)) "/HEADER" "-p"))
...
(defvar print-region-function
  (if (memq system-type '(axp-vms vax-vms))
	 ...
	 (let ((name (make-temp-name "sys$scratch:emacs_print.tmp")))
	   (write-region start end name)
	   (call-process command
			 nil 1 nil
			 (concat name " /delete /name=\"GNUprintbuffer\" "
    ...
    (save-excursion
	  ...
	  (if lpr-headers-switches
	      ...
	      (setq switches (append (if (stringp lpr-headers-switches)
				     ...
				     switches))
	    (if (not (memq system-type '(axp-vms vax-vms)))
		(progn
		  (print-region-new-buffer start end)
      ...
      (apply (or print-region-function 'call-process-region)
	     (nconc (list start end lpr-command
		    ...
		    (nconc (and lpr-add-options
				(not (memq system-type '(axp-vms vax-vms)))
				(list "-J" name "-T" title))
			   switches)))
      (if (markerp end)
	  (set-marker end nil))
