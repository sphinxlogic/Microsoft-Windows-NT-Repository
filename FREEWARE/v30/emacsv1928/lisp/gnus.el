...
(defvar gnus-nntp-service (if (eq system-type 'vax-vms) 119 "nntp")
	  ...
	  (expand-file-name
	   (if (eq system-type 'vax-vms)
	       (concat "[."
			   ...
			   (make-legal-file-name newsgroup 'take-all-dots)
			 (make-legal-file-name newsgroup 'leave-dots))
		       "]"
		       (int-to-string (nntp-header-number headers)))
	     (concat (if gnus-use-long-file-name
			 (gnus-capitalize newsgroup)
		       (gnus-newsgroup-directory-form newsgroup))
		     "/" (int-to-string (nntp-header-number headers))))
	   (or gnus-article-save-directory
	       (if (eq system-type 'vax-vms)
		   (concat (expand-file-name "~") "[.News]")
		 "~/News")))))
    (if (and last-file
...
If optional argument RAWFILE is non-nil, the raw startup file is read."
  ...
  (let* ((newsrc-file gnus-current-startup-file)
	 (tmp-dir (file-name-directory newsrc-file))
	 (tmp-file-name (concat (file-name-nondirectory newsrc-file) ".el"))
	 (quick-file (concat tmp-dir
			     (if (eq system-type 'vax-vms)
				 (make-legal-file-name tmp-file-name)
			       tmp-file-name)))
	 (quick-loaded nil))
    (save-excursion
...
(defun gnus-make-newsrc-file (file)
  ...
  (let* ((file (expand-file-name file nil))
	 (real-file (concat file "-"
			    (if (eq system-type 'vax-vms)
				(make-legal-file-name gnus-nntp-server
						      'take-all-dots)
			      gnus-nntp-server))))
  ...
  (and (or gnus-newsrc-assoc gnus-killed-assoc)
	 ...
	 (if (not (buffer-modified-p))
	   ...
	   (let ((make-backup-files nil)
		 ...
		 (vms-stmlf-recfm t)  ; Otherwise lines get broken after 512
				      ; chars on VMS.
		 (tmp-dir (file-name-directory gnus-current-startup-file))
		 (tmp-file-name (concat (file-name-nondirectory gnus-current-startup-file)
					".el")))
	     (write-file (concat tmp-dir
				 (if (eq system-type 'vax-vms)
				     (make-legal-file-name tmp-file-name)
				   tmp-file-name))))
