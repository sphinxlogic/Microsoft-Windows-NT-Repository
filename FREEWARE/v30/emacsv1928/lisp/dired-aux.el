...
;; Hacked by Richard Levitte to load `dired-vmsaux' when on VMS.
		...
		(insert-directory filename
				  (concat dired-actual-switches 
					  (if (eq system-type 'vax-vms)
					      "/NOHEADER/NOTRAILER/SELECT=FILE=(NONODE,NODEVICE,NODIRECTORY)"
					    "d"))))
...
(defun dired-insert-subdir-newpos (new-dir)
  ...
  ;;(goto-char (point-max))
  (let ((alist dired-subdir-alist) elt dir pos (new-pos (point-max)))
    (while alist
      (setq elt (car alist)
	    alist (cdr alist)
	    dir (car elt)
