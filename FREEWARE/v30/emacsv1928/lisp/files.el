...
colon-separated list of directories when resolving a relative directory name."
  ...
  (if (file-name-absolute-p dir)
	...
	(let ((trypath (parse-colon-path (getenv "CDPATH"))))
	  (setq cd-path (or trypath (if (not (memq system-type
						   '(vax-vms axp-vms)))
					(list "./"))))))
    (if (not (catch 'found
	       (mapcar
		...
		cd-path)
		   ...
		   (let ((f (expand-file-name (file-name-as-directory dir))))
		     (if (file-directory-p f)
			 (progn
			   (cd-absolute f)
			   (throw 'found t))))
...
(defvar find-file-expand-completelly-list nil
  "*List of logicals names which should be completelly expanded.
On VMS, vms-expand-search-paths will normally expand paths until it is
unique, that is until the device name only designates one place.  There
might be cases, though, when you want the logical names to be expanded
completelly, and only get a real device name as device.  In that case,
add that logical name to this list.")

;;; The following function is needed, mostly to make it more elegant
(defun file-truename-wrap (filename)
  (if (eq system-type 'vax-vms)
      (file-truename filename
		     (and (string-match ":" (file-name-directory filename))
			  (member (substring (file-name-directory filename)
					     0
					     (match-beginning 0))
				  find-file-expand-completelly-list)))
    (file-truename filename)))
...
The buffer is not selected, just returned to the caller."
  ...
  (if (file-directory-p filename)
    ...
    (let* ((buf (get-file-buffer filename))
	   (truename (abbreviate-file-name (file-truename-wrap filename)))
...
if you wish to pass an empty string as the argument."
  ...
  (let (truename)
    ...
    (if filename
	(progn
	  (setq truename (file-truename-wrap filename))
	  (if find-file-visit-truename
  ...
  (let (tempsetmodes setmodes)
    ...
    (let ((dir (file-name-directory buffer-file-name))) 
	  ...
	  (let ((realname buffer-file-name)
	    ...
	    (while nogood
	      (setq tempname (format (if (eq system-type 'vax-vms)
					 "%s$tmp$%d"
				       "%s#tmp#%d") dir i))
...
to create parent directories if they don't exist."
  ...
  (let ((handler (find-file-name-handler dir 'make-directory)))
	...
	(let ((dir1 (directory-file-name (expand-file-name dir)))
	      create-list)
	  (while (not (file-exists-p dir1))
	    (setq create-list (cons (if (eq system-type 'vax-vms)
					dir
				      dir1)
				    create-list)
		  dir (file-name-directory dir1)
		  dir1 (directory-file-name dir)))
	  (while create-list
...
(defconst list-directory-brief-switches
  (if (eq system-type 'vax-vms)
      "/NODATE/NOSIZE/NOPROTECTION/NOOWNER/NOACL"
    "-CF")
  "*Switches for list-directory to pass to `ls' for brief listing,")

(defconst list-directory-verbose-switches
  (if (eq system-type 'vax-vms)
      "/PROTECTION/SIZE/DATE/OWNER/WIDTH=(FILENAME=32,SIZE=5,OWNER=10)"
    "-l")
...
(defvar insert-directory-program
  (if (eq system-type 'vax-vms) "DIRECTORY" "ls")
...
On VMS, if FILE ends with a semicolon, only the highest version will
be inserted."
