...
\\([0-9]+\\):\\([0-9]+\\)[: \t]" 2 3 4)
    ...
    ;; VMS, any compiler:
    ;; In compilation-parse-errors, this will trigger special error handling.
    ("^%\\([A-Z0-9]+\\)-[FE]-\\([A-Z]+\\), \\(.*\\)$"
...
easily repeat a grep command."
  ...
  (compile-internal (concat command-args
                            (if (memq system-type '(vax-vms axp))
                                " nla0:null.txt"
                              " /dev/null"))
	...
	(if (fboundp 'start-process)
	    (let (proc)
		  ...
		  (progn
		    ...
		    (process-send-string proc "eoj"))
		(setq proc (start-process-shell-command (downcase mode-name)
							outbuf
							command)))
...
See variable `compilation-parse-errors-function' for the interface it uses."
  ...
  (let (text-buffer orig orig-expanded parent-expanded
    ...
    ;; There is a special case.  If the error line is supposed to be parsed
    ;; further by some function (see compilation-error-regexp-alist), the
    ;; corresponding item in the alist becomes (IDX ERR-IDX FN), where IDX
    ;; still means the same thing, but ERR-IDX is the index of the error
    ;; message, and FN is the name of the function to call for further parsing.
    ...
    (while (and (not found-desired)
      ...
      (cond ((match-beginning enter-group)
	    ...
	    ((match-beginning error-group)
	     ...
	     ;; Extract the file name and line and column number from the
	     ;; error message.
	     (let ((beginning-of-match (match-beginning 0)) ;looking-at nukes
		   filename linenum column)
 	       (if (symbolp (nth 2 alist))
 		   (let ((tmp (funcall (nth 2 alist) (nth 1 alist))))
 		     (if tmp
 			 (setq filename (nth 0 tmp)
 			       linenum (nth 1 tmp)
			       column (nth 2 tmp))))
		 (setq
		  ...
		  column (and (nth 3 alist)
		   ...
		   (progn
		     ...
		     (and (or (and find-at-least
				   (> compilation-num-errors-found
				      find-at-least))
			      (and limit-search (>= (point) limit-search)))
			  ;; We have found as many new errors as the user
			  ;; wants, or past the buffer position he indicated.
			  ;; We continue to parse until we have seen all the
			  ;; consecutive errors in the same file, so the error
			  ;; positions will be recorded as markers in this
			  ;; buffer that might change.
			  (cdr compilation-error-list) ; Must check at least two.
			  (not (equal
				(car (cdr (nth 0 compilation-error-list)))
			  ...
			  (progn
			    ...
			    ;; in the same file all at once.  If we didn't do
			    ;; this, we would have the same problem we are
			    ;; trying to avoid with the test above, just
