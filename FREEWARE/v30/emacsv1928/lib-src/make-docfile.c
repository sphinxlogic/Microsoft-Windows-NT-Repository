     ...
     char **argv;
{
  ...
  if (argc > i + 1 && !strcmp (argv[i], "-d"))
    {
      ...
      getcwd (buffer, 256);
      fprintf (stderr, "Position before chdir:  %s\n", buffer);
#endif
	...
	*directory = '\0';
      ...
      {
	char dummy[256];
	unsigned long s_errno = errno, s_vaxc$errno = vaxc$errno;
	getcwd (dummy, 256);
	fprintf (stderr, "Position after chdir to %s (status %d (%x, %x):  %s\n",
		 argv[i + 1], status, s_errno, s_vaxc$errno, dummy);
	if (status)
	  {
	    errno = s_errno;
	    vaxc$errno = s_vaxc$errno;
	    perror ("chdir");
#if 0
	    exit (0);
#endif
  ...
  for (; i < argc; i++)
    err_count += scan_file (directory, argv[i]);	/* err_count seems to be {mis,un}used */
     ...
     char *filename;
{
  ...
  if (len > 0 && filename[len - 1] == ',')
    filename[--len] = '\0';
  if (*filename == '\0')
    return 0; /* don't generate an error. It was just a comma! */
#endif
  ...
  if (!strcmp (buf + len - 4, ".elc"))
    return scan_lisp_file (buf, READ_BINARY);
  else if (!strcmp (buf + len - 3, ".el"))
    return scan_lisp_file (buf, READ_TEXT);
  else
    return scan_c_file (buf, READ_TEXT);
}

char buf[128];
...
#ifdef VMS
  if (strcmp (filename + strlen (filename) - 3, "obj") == 0)
    strcpy (filename + strlen (filename) - 3, "c");
#else
