...
get_time ()
{
  ...
  EMACS_GET_TIME (TV2);
  if (EMACS_USECS (TV1) > EMACS_USECS (TV2))
    {
      EMACS_TIME temp;
      EMACS_SET_USECS (temp, 1000000);
      EMACS_ADD_TIME(TV2, TV2, temp);
      EMACS_SET_SECS (temp, 1);
      EMACS_SUB_TIME (TV2, TV2, temp);
    }
  sprintf (time_string, "%lu.%06lu",
	   EMACS_SECS (TV2) - EMACS_SECS (TV1),
	   EMACS_USECS (TV2) - EMACS_USECS (TV1));
  return time_string;
}
