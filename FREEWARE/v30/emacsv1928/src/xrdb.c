...
static char *
magic_file_p (string, string_len, class,
	      type, filename, escaped_suffix, suffix)
     char *string;
     int string_len;
     char *class, *type, *filename, *escaped_suffix, *suffix;
{
      ...
      if (*p == '%')
	{
	  ...
	  if (p >= string + string_len)
		...
		break;
	      ...
	      case 'N':
		next = (filename ? filename : class);
		next_len = strlen (next);
		break;

	      case 'T':
		next = (type ? type : "app-defaults");
		next_len = strlen (next);
		break;

	      default:
	      case 'S':
		next = (suffix ? suffix : 0);
		next_len = (next ? strlen (next) : 0);
		break;
...
#ifdef DEBUG_XRDB
  fprintf (stderr, "magic_file_p (\"%s\",%d,\"%s\",\"%s\",\"%s\"):\n",
	   (string ? string : ""), string_len, (class ? class : ""),
	   (escaped_suffix ? escaped_suffix : ""), (suffix ? suffix : ""));
  fprintf (stderr, "\tpath = \"%s\"", (path ? path : ""));
#endif
  if (! file_p (path))
    {
      free (path);
#ifdef DEBUG_XRDB
      fprintf (stderr, "  (exists NOT)\n");
#endif
      return NULL;
    }
...
static char *
search_magic_path (search_path, class, type, filename, escaped_suffix, suffix)
     char *search_path, *class, *type, *filename, *escaped_suffix, *suffix;
{
  register char *s, *p;

  for (s = search_path; *s; s = p)
    {
      for (p = s; *p && *p != SEPCHAR; p++)
	;
      
      if (p > s)
	{
	  char *path = magic_file_p (s, p - s, class, type, filename, escaped_suffix, suffix);
	  if (path)
	    return path;
	}
      else if (*p == SEPCHAR)
...
#ifdef VMS
#define X_DEFAULT_SEARCH_PATH "DECW$%T_%L:%N_%C.%S,DECW$%T_%l:%N_%C.%S,DECW$%T:%N_%C.%S,DECW$%T_%L:%N.%S,DECW$%T_%l:%N.%S,DECW$%T:%N.%S"
#define X_DEFAULT_TYPE "SYSTEM_DEFAULTS"
#define X_DEFAULT_SUFFIX "DAT"
#else
...
#define X_DEFAULT_TYPE "app-defaults"
#define X_DEFAULT_SUFFIX 0
#endif
     ...
     char *class;
{
  ...
  path = search_magic_path (path, class, X_DEFAULT_TYPE, 0, 0, X_DEFAULT_SUFFIX);
  if (path)
    {
     ...
     char *class;
{
  ...
  if (file = search_magic_path (path, class, "USER_DEFAULTS", 0, 0, X_DEFAULT_SUFFIX))
#else
  /* Check for XUSERFILESEARCHPATH.  It is a path of complete file
     names, not directories.  */
  if (((path = getenv ("XUSERFILESEARCHPATH"))
       && (file = search_magic_path (path, class, 0, 0, 0, 0)))
	 ...
	 we have to search for LANG/CLASS and then CLASS.  */
	  ...
	  && ((file = search_magic_path (path, class, 0, 0, "/%L/%N", 0))
	      || (file = search_magic_path (path, class, 0, 0, "/%N", 0))))
...
#ifdef DEBUG_XRDB
  fprintf (stderr, "get_user_db(): xdefs = \"%s\"\n", xdefs);
#endif
...
#ifdef VMS
#define XDEFAULT_FILE_NAME "DECW$XDEFAULTS.DAT"
#else
#define XDEFAULT_FILE_NAME ".Xdefaults"
#endif
      home = gethomedir ();
      xdefault = (char *) malloc (strlen (home) + sizeof (XDEFAULT_FILE_NAME));
      strcpy (xdefault, home);
      strcat (xdefault, XDEFAULT_FILE_NAME);
#ifdef DEBUG_XRDB
      fprintf (stderr, "get_user_db(): xdefault = \"%s\"\n", xdefault);
#endif
...
#ifdef DEBUG_XRDB
  fprintf (stderr, "get_user_db(): db = %p\n", db);
#endif
  return db;
}
...
#ifdef DEBUG_XRDB
      fprintf (stderr, "get_environ_db(): p = \"%s\"\n", p);
#endif
...
#ifdef DEBUG_XRDB
  fprintf (stderr, "get_environ_db(): db = %p\n", db);
#endif
  return db;
}

...
#ifdef DEBUG_XRDB
  fprintf (stderr, "x_get_resource(): class = \"%s\", name = \"%s\"\n",
	   class, name);
#endif
    ...
    char **argv;
{
  ...
  if (!NIL (lp))
    ...
    {
#ifdef VMS
      displayname = getenv ("DECW$DISPLAY");
#else
