...
#define EMACS_TIME vms_emacs_time
#define EMACS_SECS(__time)		    (__vms_time_secs (__time))
#define EMACS_USECS(__time)		    (__vms_time_usecs (__time))
#define EMACS_SET_SECS(__time, __seconds)    (__vms_set_secs (&(__time), \
							     (__seconds)))
#define EMACS_SET_USECS(__time, __usecs)    (__vms_set_usecs (&(__time), \
							      (__usecs)))

#define EMACS_GET_TIME(__t) (__vms_set_secs (&(__t), time ((long *) 0)))
#define EMACS_ADD_TIME(__dest, __src1, __src2) (__vms_add_time (&(__dest), \
								(__src1), \
								(__src2)))
#define EMACS_SUB_TIME(__dest, __src1, __src2) (__vms_sub_time (&(__dest), \
								(__src1), \
								(__src2)))
#define EMACS_TIME_NEG_P(__tim) ((long)((__tim).t[1]) < 0)
#define EMACS_NEGATE_TIME(__dest, __t) (__vms_negate_time (&(__dest), (__t)))
