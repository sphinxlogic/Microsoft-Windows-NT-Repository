...
#endif /* not VMS4_4 */
#if 0
  /* Now when we have several architectures to keep track of, we might have a problem
     with the fact that doc files from different compilations will reside in the same
     directory, and because we use the file version number as compilation one of them
     will obviously be purged, so it's better to convert the last dot in the file to
     a dash, which is consistent with the way versions are generated on VMS anyway...  */
  fprintf (stderr, "DEBUG: name -> \"%s\"\n", name);
  p = strrchr (name, '/');
  fprintf (stderr, "DEBUG: p = %p\n", p);
  if (p) fprintf (stderr, "DEBUG: p -> \"%s\"\n", p); else fprintf ("DEBUG!!! \"%s\" contains no slash!\n", name);
  {
    char *q;
    q = strrchr (p, '.');
    fprintf (stderr, "DEBUG: q = %p\n", q);
    if (q)
      {
	p = q;
	fprintf (stderr, "DEBUG: p -> \"%s\"\n", p);
      ...
      {
	fprintf ("DEBUG!!! \"%s\" contains no dot!\n", p);
	p = q;
      }
  }
  if (p) *p = '-';
