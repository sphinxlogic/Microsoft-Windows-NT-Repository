...
#ifdef VMS
  /* lib$stop (SS$_ABORT); makes it impossible to check what went wrong... */
  lib$signal (SS$_DEBUG); /* Is a lot better, we might even be able to get
			     a traceback! */
#else
...
#ifdef VMS /* On VMS, we don't want the extension and the version number */
  {
    Lisp_Object s = Ffile_name_nondirectory (build_string (argv[0]));
    char *cp;
    cp = index (XSTRING (s)->data, '_');
    if (cp != 0 && cp[1] >='0' && cp[1] <= '9')
      *cp = '\0';
    cp = index (XSTRING (s)->data, '-');
    if (cp != 0 && cp[1] >='0' && cp[1] <= '9')
      *cp = '\0';
    cp = index (XSTRING (s)->data, '.');
    if (cp == 0)
    ...
    if (cp != 0)
      *cp = '\0';
    Vinvocation_name = build_string (XSTRING (s)->data);
  }
#else
  ...
  if (NILP (Vinvocation_directory))
    {
      ...
      if (yes == 1)
	Vinvocation_directory = Fexpand_file_name (Ffile_name_directory (found), Qnil);
    }
  else
...
#ifdef VMS
  {
    extern void __vms_allocate_initial_buffer ();
...
#if 0 && defined(VMS) && defined(USE_X_TOOLKIT)
  /* For some reason, there's a bug with Xt on AXP, so we take some
     corrective measures.  /Richard Levitte  */
  {
    extern void reinit_emacsFrameClass ();
    reinit_emacsFrameClass ();
  }
#endif
...
#if 0 /* We don't need it, and even mustn't do this, otherwise
	 the terminal will not be reset to its old bits when
	 shut_down_emacs () is called --- Richard Levitte */
#ifdef VMS
  stop_vms_input ();
#endif
#endif
...
#ifdef VMS
#define dump_emacs_max_args 3
#else
#define dump_emacs_max_args 2
#endif

DEFUN ("dump-emacs", Fdump_emacs, Sdump_emacs, 2, dump_emacs_max_args, 0,
...
and announce itself normally when it is run.")
  (intoname, symname, version)
     Lisp_Object intoname, symname, version;
{
  extern int my_edata;
  Lisp_Object tem;
  int major=0, minor=0, edit=0;

  if (!NILP (version))
    {
      major = XINT (Fcar (version));
      minor = XINT (Fcar (Fcdr (version)));
      edit = XINT (Fcdr (Fcdr (version)));
    }
  CHECK_STRING (intoname, 0);
  intoname = Fexpand_file_name (intoname, Qnil);
  if (!NILP (symname))
  ...
  fflush (stdout);
#ifdef VMS
  mapout_data (XSTRING (intoname)->data, major, minor, edit);
#else
  /* Tell malloc where start of impure now is */
  /* Also arrange for warnings when nearly out of space.  */
#ifndef SYSTEM_MALLOC
