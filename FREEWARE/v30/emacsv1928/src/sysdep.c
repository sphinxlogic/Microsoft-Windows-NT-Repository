...
sys_suspend ()
{
  ...
  if (parent_id && parent_id != 0xffffffff)
	...
	char	*a;
      ...
      lib$spawn (0, 0, 0, &CLI$M_AUTHPRIV, 0, 0, 0, 0, 0, 0, &d_prompt, 0);
      return 1;
    }
  return -1;
#else
...
#endif /* HAVE_GETHOSTNAME */
#endif /* VMS */
#if defined(HAVE_GETHOSTNAME) || defined(VMS)
  Vsystem_name = build_string (hostname);
#endif /* HAVE_GETHOSTNAME || VMS */
#endif /* BSD4_1 */
  {
    unsigned char *p;
     ...
     int newd;
{
  ...
  if (fd != newd)
    error ("can't dup2 (%i,%i) : %s", oldd, newd, err_str (errno));
#else
  fd = dup (old);
  if (fd == -1)
    return -1;
...
#ifdef VMS
#if 0	/* not used any more...  I think...  Richard Levitte  */
     ...
     int mode;
{
    ...
    return -1;
  {
    ...
    if (! (stat & 1))
      return -1;
    /* If ACL was not empty (2512) or invalid buffer length, and error
       occurred, then deny access.  */
    if (xab.xab$l_aclsts != SS$_ACLEMPTY &&
	xab.xab$l_aclsts != SS$_IVBUFLEN &&
	! (xab.xab$l_aclsts & 1))
      return -1;
     ...
     char *old, *new;
{
  ...
  /* As this is a "replacement" for creat, return a file descriptor
     opened for writing. */
  return open (new, O_WRONLY, 0, "mrs=32767");
}
