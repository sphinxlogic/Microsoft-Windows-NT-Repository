...
#if 0
  /* This expression makes select return -1 if there were unexpected ASTs,
     and there were NO expected ones.  */
#define testexpression ((mask & readMask == 0) && (mask & allMask) != 0)
#else
...
#endif
#if 0
  mask &= allMask;		/* Just examine what we need */
#endif
#if 0 /* I see no reason to clear this event, but testing so far... */
...
#if 0
	  if (waitMask & (1 << (KEYBOARD_EVENT_FLAG % 32)))
	    {
	      sys$clref (KEYBOARD_EVENT_FLAG);
...
#define NET_BUF(vs) ((vs)->a.net.net_buffer)
...
#ifdef HAVE_SOCKETS
#ifdef MULTINET
      status = SYS$QIO(vs->eventFlag, vs->chan, IO$_RECEIVE,
     ...
     int len;
{
...
#ifdef UCX
  /* do the write */
  status = SYS$QIOW(0, vs->chan, IO$_WRITEVBLK, iosb, 0, 0, buf, len,
                    0, 0, 0, 0);
  if (!(status & 1))
    {
      errno = EVMSERR;
...
#ifdef MULTINET
  {
    ...
    if (socket_ioctl (vs->chan, FIONBIO, &dum_1) != 0)
      {
	errno = socket_errno;
	return -1;
      }
     ...
     int fd;
{
  ...
  if (vs->is_pty)
    ...
    {
#ifdef HAVE_SOCKETS
#if defined(MULTINET) || defined(VMS)
      socket_close (vs->chan);
#else
      close (vs->chan);
#endif
      if (NET_BUF (vs))
