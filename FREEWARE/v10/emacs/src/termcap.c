...
#ifdef VMS
    {
      /* This whole thing could be done with the ANSI C construct
	 PATH_DATA "termcap.dat", using string concatenation, but stupid
	 VAX C will insert a comma between the strings... sigh.  */
      char *temp = PATH_DATA;
      name_size = strlen (temp) + 12; /* 11 for "termcap.dat", one for '\0' */
      termcap_name = xmalloc (name_size);
      strcpy (termcap_name, temp);
      strcpy (termcap_name + name_size - 12, "termcap.dat");
    }
#else
