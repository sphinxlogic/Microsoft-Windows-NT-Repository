...
read_avail_input (expected)
     int expected;
{
#ifndef VMS			/* This function is not used on VMS */
...
reinvoke_input_signal ()
{
  ...
  /* send ourselves an AST */
  sys$dclast (kbd_input_ast, input_ef, 0/*acmode*/);
#endif
#ifdef SIGIO  
  kill (0, SIGIO);
#endif
 ...
 */
      ...
      if (immediate_quit && NILP (Vinhibit_quit))
	{
	  immediate_quit = 0;
#ifdef VMS
	  /* This (undocumented) service will clear the AST active bit and
	     allow additional ASTs to be delivered.  This is required, since
	     the signal can cause arbitrary lisp code to be executed, which
	     could include process i/o functions. */
	  sys$clrast ();
#endif
          sigfree ();
	  Fsignal (Qquit, Qnil);
