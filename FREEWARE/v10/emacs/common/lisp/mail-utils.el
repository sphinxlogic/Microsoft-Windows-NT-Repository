...
Return a modified address list."
  (if (null address)
	...
	(progn (require 'rfc822)
	 ...
	 (if (or (and (> (length address) (match-end 0))
		      (= (aref address (match-end 0)) ?@))
		 ;; For VMS systems, if the previous char is "%", we
		 ;; probably have FOO%"xyzzy@host".  Leave that too. -- RL
...
;;; An extra function to strip mail addresses from possible transport
;;; wrapping, like FOO%"..." or BAR::"..."
(defun vms-mail-remove-transport (address)
  (or (and (or (string-match "^[a-zA-Z0-9_]+\\(%\\|::\\)\"\\(.*\\)\"$"
			     address)
	       (string-match "^[a-zA-Z0-9_]+\\(%\\|::\\)\\([^@\\.]*\\)$"
			     address))
	   (substring address (match-beginning 2) (match-end 2)))
      address))
