...
#define PROT_START	{ int protect$$sts= SYS$SETAST (0);
#define PROT_END	if (protect$$sts== SS$_WASSET) SYS$SETAST (1);}
#define UNPROTECT 	if (protect$$sts== SS$_WASSET) SYS$SETAST (1);
	/*--- macros to access AST protected variables */

/****** ACCOUNTING routines *******************
*/
...
static	unsigned long objtyp= ACL$C_FILE;
	...
	log_fab.fab$b_rfm = FAB$C_UDF;		/* FAB$C_VAR */
	log_fab.fab$b_rat = 0;			/* FAB$M_CR */
...
int bufsize;
{
	...
	if (stream.debug & DBG_DUMP)
		dprintf ("->:%.*s\n", min (256, bufsize), bufadr);

	for (len = bufsize, cp=bufadr; len > 0; len -= smb_maxbuf, 
						cp += smb_maxbuf) {

		int l= min (smb_maxbuf, len);
...
/* 	PROT_START	/* protect against ^Y AST */
...
pagecount pstack pop flush end \r\n\004"};
	...
	if ((stream.lw_flushcnt!=0)&&(stream.lw_flushcnt <= stream.lw_eotcnt)){
		...
		PROT_START	/* stop_reason && stop_task must be */
				/* consistent */
...
static const unsigned long libdtop= LIB$K_DELTA_SECONDS;
		...
		if (duree==0) dprintf ("Durée nulle ??");
		else 	dprintf ("Envoi de %d bytes en %d secondes (%d bauds)", 
					delta_bytes, duree, 
