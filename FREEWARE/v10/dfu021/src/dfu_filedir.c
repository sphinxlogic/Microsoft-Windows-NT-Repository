...
extern unsigned int ctrlc, keyb_id;
...
static unsigned int status, version, alloc;
static Boolean matalias, matcheck, matversion, matdump, mattest, matempty;
static FILE *fp;
...
{ struct FAB fab;
  ...
  static char res_str[255], exp_str[255], devnam[15], 
    fname[255], dummy[80], ans[4];
  unsigned int func, x, bad, dir, lock, log, cont, rvcount,
    bucksize, vfcsize, maxrec, eblock, ebyte, hiblk, modifiers;
  unsigned short chan, rtype,fileorg, recsize;
  static unsigned long uchar;
  ...
  Boolean matfile, matbadacl, matdir, matupdate, matlock, matcont, matbakdate,
    ...
    matebyte, mateblock, matconfirm;
  static struct { unsigned short atr_size, atr_type;
                  int atr_address; } 
    acb[8] =  { {32, ATR$C_RECATTR, (int) &attrib }, 
...
/* Check file name */
  status = parse_item("p1", &fname_descr, 0 , &matfile, 0);
  if (matfile == FALSE) 
  { printf("%%DFU-E-NOFILSPEC, No file specified on command line\n");
...
/* get the rest of the fab fields */
    ...
    if ((status & 1 ) != 1) 
    { printf("%%DFU-E-PARSEERR, Error parsing filename from %s,\n", fname);
      singlemsg(status); status = RMS$_NMF; goto next_set;
    }
...
/* now search for the file */
    ...
    { res_str[nam.nam$b_rsl] = '\0';
      if ((status & 1 ) != 1) 
      { printf("%%DFU-E-SEARCHERR, Error searching file %s,\n", res_str);
        singlemsg(status); status = RMS$_NMF; goto next_set;
      }
      res_str[nam.nam$b_rsl] = '\0';
...
/* Open a channel to the device */
      ...
      if ((status & 1 ) != 1) 
      { printf("%%DFU-E-ASSIGN, Error assigning channel,\n");
...
/* Now fill in File id and function code for the ACP call */
      ...
      if ((status & 1) != 1) 
      { printf("%%DFU-E-OPENERR, Error accesing file %s,\n", res_str);
        singlemsg(status); status = 1; goto next_set;
      }

/* Confirm this file ? */
      ans[0] = 'y';
      if(matconfirm)
      { printf("Modify %s ? : ",res_str);
        ans[0] = 'n'; x = 4;
        status = SMG$READ_STRING(&keyb_id, &answer, 0 , 
      ...
      { rtype = attrib[0] % 16;
        ...
        if (matcont) 
         if (cont == 0) uchar = uchar | FH2$M_CONTIGB; /* Set the bit */
           else uchar = uchar & ~FH2$M_CONTIGB; /* Clear the bit */
...
/* Now rewrite the file header */
        ...
        if ((status & 1) == 1) status = iostat.iosb_1;
        if ((status & 1) != 1) 
        { printf("%%DFU-E-SETERR, Error modifying file %s,\n", res_str);
          singlemsg(status); 
        }
         else
          if (log == 0) printf("%%DFU-S-MODIFIED, File %s modified \n",res_str);
      }
...
/* Now do a IO$_ACCESS without IO$M_ACCESS because we need the file name.
   A little bit clumsy but I know no other way to solve this */
      ...
      printf("%%DFU-E-NOTDEL, Error deleting file %s,\n",file);
      singlemsg(status); goto end_del;
    }
...
/* And delete the file */  
  ...
  if (!deldir)
  { if (!nolog) printf("%%DFU-S-DELETED, File %s deleted \n", file);
...
{ unsigned int func;
  int i, j, k, n, nr, vbn, x, e_blk, h_blk, num_files, lastsize;
  static unsigned int uchar;
...
/* Open the directory */
  ...
  if ((status & 1 ) == 1) status = iostat.iosb_1;
  if ((status & 1) !=1)
  { printf("%%DFU-E-OPENERR, Error opening directory %s,\n",r_str);
    singlemsg(status); return(status);
  }
...
/* Check if directory is valid */
  ...
  { printf("%%DFU-E-INVDIR, %s is not a valid directory\n",r_str);
     status = SYS$QIOW(0, dchan, IO$_DEACCESS,0,0,0,0,0,0,0,0,0);
     return(1);
  }
  ...
  { /* Allocate dynamic space to read the directory file */
    ...
    if ((status & 1) != 1)
    { printf("%%DFU-E-INSFMEM, Error allocating %d pages memory for %s\n",
        e_blk, r_str);
      singlemsg(status); lastsize = 0;
      status = SYS$QIOW(0, dchan, IO$_DEACCESS,0,0,0,0,0,0,0,0,0);
      return(1);
    }
     else lastsize = e_blk;
...
/* Pointer to space for directory blocks (1000 max) */
  ...
  if ((status & 1 ) == 1) status = iostat.iosb_1;
  if ((status & 1) !=1)
  { printf("%%DFU-E-READERR, Error reading directory %s,\n",r_str);
    singlemsg(status);  
    status = SYS$QIOW(0, dchan, IO$_DEACCESS,0,0,0,0,0,0,0,0,0);
    goto next_del;
  }
/* Check for corrupted directory */
  if (i_dir->block[0] == 0)
  { printf("%%DFU-E-JUNKDIR, Corrupted directory (byte offset 0) %s\n",
     r_str);
    status = SYS$QIOW(0, dchan, IO$_DEACCESS,0,0,0,0,0,0,0,0,0);
    goto next_del;
  }
...
/* Align count field */
	  ...
	  { num_files++;
	    if (!nolog) 
              printf("%%DFU-S-DELETED, File %.*s;%d deleted \n",
  ...
  printf("%%DFU-S-TOTAL, %d file(s) deleted\n", num_files);
...
/* get the rest of the fab fields */
  ...
  printf("%%DFU-I-PARSEDIR, Parsing directory tree...\n");
  ...
  { if ((status & 1 ) != 1) 
    { sort_rec.rx_str[xnam.nam$b_rsl] = '\0';
      printf("%%DFU-W-NOSUBDIR, no subdirectories found in this tree\n");
...
/* get the file id */
    ...
    { printf("%%DFU-E-NOFID, Missing /FILE_ID or /DIRECTORY qualifier on command line\n");
      return(SS$_NOSUCHFILE);
    }
    ...
    status = CLI$GET_VALUE(&item_descr, &dummy_descr, &dummy_descr.dsc$w_length);
    if ((status & 1) != 1) 
    { printf("%%DFU-E-ERRFID, Incorrect File id on command line\n");
      return(SS$_NOSUCHFILE);
    }
    tmp = atoi(&dummy);
    f_id.seq = tmp;
...
/* Check the privileges */
  ...
  if ( x > 0 )
  { printf("%%DFU-E-ILLIOFUNC, This function not supported on this version of VMS\n");
    return(1);
  }
...
/* Check file name */
  ...
  printf("%%DFU-I-DEFRAG, Start defragmenting file(s) ...\n");
  while((matfile) && (ctrlc ==0))/* Loop through the file list */
  { if (fname[0] == '@')
    { ip = fopen(&fname[1], "r");
      if (ip == 0) 
        printf("%%DFU-E-OPENERR, Error opening file %s\n",&fname[1]);
       else
...
/* Now defrag the file */
    ...
    if ((status & 1) == 1) status = iostat.iosb_1;
    if ((status & 1) != 1)
    { printf("%%DFU-E-NOTMOVED, Error moving %s, \n",res_str);
      singlemsg(status);
    }
     else
      printf("%%DFU-S-MOVED, File %s defragmented\n",res_str);
    status = SYS$DASSGN(chan);
/* Next file if any...*/
    status = SYS$SEARCH(&fab , 0 , 0);
  }  
...
/*
   ...
   There are 3 major functions :
	...
	- search all directories of the disk for either files with many
	  ...
	  V2.1 added /check to test correct alphabetical order
...
/* All qualifiers parsed */
  ...
  { printf("%%DFU-E-SYNTAXERR, missing required qualifier\n");
    return(1);
  }
...
/* Follow different code path for this function */
  ...
  { if (matstat) status = lib$init_timer();
    ...
    { if (dname[0] == '@')
      { ip = fopen(&dname[1],"r");
        if (ip==0) printf("%%DFU-E-OPENERR, Error opening file %s\n",&dname[1]);
         else
        { status = (int) fgets(file, 160, ip);
          while ((status != 0) && (ctrlc ==0))
          { x = strindex(&file," ",160); file[x] = '\0';
...
{ static struct fibdef fib;
  ...
  if ((status & 1) == 1) status = iostat.iosb_1;
  if ((status & 1) != 1)
  { printf("%%DFU-E-NOTCREATE, Error creating directory %s,\n", crea_file);
    singlemsg(status);
  }
   else printf("%%DFU-S-CREATED, Created directory %.*s\n", 
     nam.nam$b_rsl, res_str);
}

int compdir (char *comp_file)
/*
...
{ struct FAB fab;
  ...
  struct {int fiblen; struct fibdef *fibadr;} fibdescr;
  static char res_str[255], exp_str[255], devnam[15];
  int i, j, k, n, nr, newi, newj, vbn, e_blk, h_blk, oldeblk, lastsize;
  char deflt[] = ".DIR;1";
  struct _quad {unsigned int address[2];} retadr;
...
/* Pointer to space for directory blocks */
    if (lastsize <= 1000)
    { o_dir = i_dir = (struct _sh *) &header[0].block[0];
    }
     else
    { o_dir = i_dir = (struct _sh *) retadr.address[0];
    }
...
/* Now processs the directory blocks */
    ...
    { i = 1; 
      found = FALSE;   
      if (matdump) fprintf(fp,"DUMP of directory block %d\n",k);
      while (!found)
      { if ((i > 512) || i_dir->block[i-1] == -1) found = TRUE; /* End of block */
         else /* parse this entry */
        { dir_entry = (struct _d_e *) &i_dir->block[i-1]; 
...
/* If /dump entered display this entry */
          if (matdump)
          { fprintf(fp,"Size: %d, Version limit: %d, Type: %d, Name(%d): %.*s\n",
...
/* Align count field */
            ...
            { fid_num = (fid_entry->fid_nmx << 16) + fid_entry->fid_num;
              fprintf(fp,"   Version: %d, FID : (%d,%d,%d)\n",
...
/* Fill in new directory */
          if ((newj + j - i) >= 512) /* Check if we reached  block bounds */
...
/* Rewrite the new , compressed directory */
    { e_blk = newi + 1;
      ...
      if ((status & 1 ) == 1) status = iostat.iosb_1;
      if ((status & 1) !=1)
      { printf("%%DFU-E-WRITEERR, Error writing directory %s,\n",res_str);
        singlemsg(status); 
        status = SYS$QIOW(0, chan, IO$_DEACCESS,0,0,0,0,0,0,0,0,0);
        goto next_dir;
      }
/* Write the new attributes back , truncate and close the file */
    ...
    if (mattest)
      printf("%%DFU-I-TESTONLY, %s : %d files; old size : %d/%d, new : %d blocks\n",
        res_str, n, oldeblk, h_blk, newi);
     else 
      if (matdump)
        printf("%%DFU-I-TOTAL, %s : %d files\n", res_str, n);
       else
        printf("%%DFU-S-DONE, %s : %d files; was : %d/%d, now : %d/%d blocks\n",
...

int scan_directories(int *rvt, int rvn)
/* Scan all directories found on this disk. Implicitly called
     by the diskwide DIRECTORY commands. RVT is the relative
     volume table which contains the adresses for the dynamic array
...
{ 
  ...
  struct _t { unsigned short fid[3]; unsigned int bakfid; 
...
/* Check if directory is valid */
    if ( ((attrib[0] & 2048 ) != 2048) /*NOSPAN attribute */
      ...
      || ((uchar & 128) != 128) ) /* Contiguous bit */
      goto next;
    if (strstr(&dirrec.dirnam,".DIR;1") == 0) goto next; 
    lastsize = 0;
    if (e_blk > 1000) 
    { /* Allocate dynamic space to read the directory file */
      status = SYS$EXPREG(e_blk, &retadr.address[0], 0, 0);
...
/* Check for corrupted directory */
    if (i_dir->block[0] == 0) 
    { if (matcheck) fprintf(fp,"%s%s first byte is corrupted \n",
       fulldevnam, &dirrec.dirnam);
      goto next;
    }
...
/* Now processs the directory blocks */
    ...
    { if (matempty) fprintf(fp,"%s%s is empty \n", fulldevnam, &dirrec.dirnam);
      goto next;
    } 
...
/* Pointer to directory record */
	  ...
	  if (matcheck)
          { x = strncmp(&oldnam, dir_entry->name, dir_entry->count);
            ...
            if (x > 0) fprintf(fp,"%s%s%s out of alphabetical order\n",
	  ...
	  if (matversion)
          { if (!flag) nr = 0;
              ...
              { x = strncmp(&savnam, dir_entry->name, dir_entry->count);
                ...
                { if (nr >= version)  
   	           fprintf(fp,"%s%s%s , %d versions\n",
                     fulldevnam, &dirrec.dirnam, &savnam, nr);
                  nr = 0;
                }
              }
/* # of files entries */ 
            nr += (dir_entry->size - 4 - dir_entry->count) / 8; 
	    if ((j > 512) || i_dir->block[j-1] == -1) flag = TRUE;
              else flag = FALSE;
/* Save name if last entry in this block */
	    if (flag) 
   ...
   the dynamic array (because of concurrent disk activity). */
	          { if ( ((did.fid_nmx << 16) + did.fid_num) != l)
                    { x = strindex(&filrec.dirnam,"[",198); 
                      ...
                      fprintf(fp,"%s%s%.*s;%d is alias for %s%s\n",
                       fulldevnam, &dirrec.dirnam, dir_entry->count, 
...
/* Report this file */
      fprintf(fp,"%s%s%.*s , %d versions\n",
        fulldevnam, &dirrec.dirnam, dir_entry->count, dir_entry->name, nr);
    }
