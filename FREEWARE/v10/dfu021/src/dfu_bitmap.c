/*
     ...
     Called by REPORT, UNDELETE and VERIFY
...
/* Open bitmap. (Channel already opened in calling procedure)
  ...
  if ((status & 1) != 1) 
  { printf("%%DFU-E-BITMAP, Error opening bitmap,\n");
...
/* Read in bitmap starting from VBN 2 (VBN 1 is Storage control block ) */
  ...
  if ((status & 1) == 1) status = iostat.iosb_1;
  if ((status & 1) != 1)
  { printf("%%DFU-E-READERR, Error reading bitmap,\n");
    singlemsg(status); 
  }
  return(status);
}
...
/* Allocate dynamic space */
  ...
  if ((status & 1) != 1)
  { printf("%%DFU-E-EXPREG, Error creating dynamic space for bitmap,\n");
    singlemsg(status); return(status);  
  }
...
/* Now we have the bitmap read in, process it */
  ...
  { size = 32 - offset;
    ...
    if ((status &1) != 1) /* Clear bits are USED clusters */
...
/* We now have a consecutive chunk of multiple allocated blocks, report them */
	      ...
	      if (pass == 2) /* now report the file to the user */
              { memcpy(&name[0],id->filename,20);
                ...
                fprintf(fp,"%%DFU-E-MULTALLOC, file (%d,%d,%d) %.*s blocks LBN %d through %d multiple allocated\n",
...
/* 
	...
	V2.1: modify this routine so that we can set/clr bits
...
/* We now have a consecutive chunk of multiple allocated blocks, report them */
              ...
              { report = FALSE; /* Don't report twice */
                ...
                if (name[19] != ' ') memcpy(&name[20],id->filenamext,66);
                namelen = strindex(&name[0]," ",86); 
                printf("%%DFU-E-MULTALLOC, file %.*s blocks LBN %d through %d already allocated\n",
                  namelen, name, lbne1, lbnc1);
              }
              ibit++;
            } /* End k loop */
...
int compare_bitmap(short chan, int *addr, int pages, Boolean locked, 
                   int cluster, FILE *fp, Boolean rebuild)
/*
...
/* Allocate dynamic space */
  fprintf(fp,"%%DFU-I-CHKBITMAP, Checking BITMAP.SYS...\n");
            ...
            { chk = lib$extzv(&k, &l, &(bmap+i)->block[j]) +
              ...
              { if (reports == TRUE)
                { reports = FALSE; trigger = TRUE;
                  fprintf(fp,"%%DFU-E-ALLOCSET, blocks LBN %d through %d incorrectly marked free\n",
                    blkss, blkse);
                  if (rebuild) 
              ...
              { if (reportc == TRUE)
                { reportc = FALSE; trigger = TRUE;
                  fprintf(fp,"%%DFU-E-ALLOCCLR, blocks LBN %d through %d incorrectly marked allocated\n",
                    blkas, blkae);
                  if (rebuild) 
...
/* Check if we have unreported blocks left */
  ...
  { /* Now rewrite the bitmap */
    ...
    if ((status & 1) == 1) status = iostat.iosb_1;
    if ((status & 1) != 1)
    { printf("%%DFU-E-RBDBMAP, Error rewriting BITMAP.SYS,\n");
      singlemsg(status);
    }
     else
      fprintf(fp,"%%DFU-S-RBDBITMAP, BITMAP.SYS succesfully rebuild\n");
  }
  if (rebuild) SYS$QIOW(0, chan, IO$_DEACCESS, 0,0,0,0,0,0,0,0,0);
...
/* Check if quota is enabled */
  ...
  { qfib.fib$w_exctl = FIB$C_MOD_QUOTA;
    ...
    printf("%%DFU-I-ADDQUOTA, updating diskquota...\n");
...
{ struct _usg *usage_table;
  ...
  Boolean matquota;
  static char ident[30], outbuf[255];
  static struct fibdef qfib; /* File information block for quota file */
  static struct _qb qblock;
  struct { int fiblen; struct fibdef *fibadr; } fibdescr;
  struct { int qlen; struct _qb *qadr; } qb_descr;
...
/* Check if quota is enabled */
  ...
  { fprintf(fp,"\n      ***** Disk Usage Table (from INDEXF.SYS and QUOTA.SYS) *****\n"); 
    fprintf(fp," Identifier/UIC                   Used/Allocated   Headers   Quota Used/Perm\n");
  }
   else
  { fprintf(fp,"\n        ***** Disk Usage Table (from INDEXF.SYS) *****\n");
    fprintf(fp," Identifier/UIC                   Used/Allocated   Headers\n");
  }
  fprintf(fp," ---------------------------------------------------------------------------\n");
  i = 1;
  usage_table = table;
  ...
  { status = SYS$FAO(&ctrstr, &id_descr.dsc$w_length, &id_descr,
      usage_table->owner);
    if ((status &1) != 1) singlemsg(status);
...
/* Check if quota is enabled */
  ...
  if(matquota==TRUE) 
  { usage_table = table;
    fprintf(fp,"%%DFU-I-CHKQUOTA, Checking QUOTA.SYS...\n");
    ...
    { trigger = FALSE;
      ...
      { SYS$FAO(&ctrstr, &id_descr.dsc$w_length, &id_descr,
         usage_table->owner);
        fprintf(fp,"%%DFU-W-QUOTAERR, %.*s has %d blocks used, QUOTA indicates %d blocks\n"
  ...
  if ((reb) && (reb_done))
    fprintf(fp,"%%DFU-S-REBQUOTA, QUOTA.SYS succesfully rebuild\n");
}
