/*
	...
	and diskwide DIRECTORY functions
...
static unsigned int volcount, maxvol, volnum, orig_rvn, curvol;
...

{
  static char dummy_item[80], name[86], dir[200], 
    fname[80], ident[30], date_s[23], outbuf[255];
  struct header_area_struct *hdr;
  struct ident_area_struct *id;
  ...
  int size_min, size_max, parse_type, free_hdr, ivbn, k, j, x,
    ...
    retr_min, retr_max, hdrs, rtvptr, lbn;
...
/* Search on allocated filesize */
  ...
  { /* Check minimum size entered */
    ...
    if (mat_a_minsize == TRUE && mat_a_maxsize == TRUE )
      ...
      { printf("%%DFU-E-SIZEERR, Minimum size > maximum size\n");
	cleanup();
	return(1);
      }
  }
...
/* Ident or UIC */
  ...
  { status = SYS$ASCTOID(&id_descr, &uic, 0);
    if ((status & 1) != 1) 
    { printf("%%DFU-E-INVID, Invalid identifier entered,\n");
      singlemsg(status);
      return(status); 
    }
  }
...
/* Get output file */
  ...
  { status = sor$pass_files(0,&file_descr,0,0,0,0,0,0,0,0,0);
    if ((status &1 ) != 1)
    { singlemsg(status); return(status); }
    status = sor$begin_sort(&keybuf,&reclen,0,0,0,0,SOR$GK_RECORD,0,0,0);
    if ((status &1 ) != 1)
    { singlemsg(status); return(status); }
    clean_flags.sort = 1;
  }
...
/* Open the volume (set) , read in the home block */
  ...
  { ctx.i = -1; /* Clear context */
	...
	printf("\n%%DFU-I-SEARCH, Start search on %.*s (%s) , RVN = %d\n",
...
/* Display home block info if needed */
	...
	{ ctx.end = TRUE;
	  ...
	  { printf("%%DFU-E-FIDTOOBIG, File id specified > max file id\n");
	    goto nexti;
	  }
...
/* Check corresponding bitmap bit (must be set) */
        bitblk = headers / 4096;
        bitje = (headers - bitblk*4096) / 32;
        bitval = headers - bitblk*4096 - bitje*32 - 1;
...
/* Check for specific LBN */
	...
	{ found = FALSE;
	  ...
	  if (hdr->seg_num > 0 ) /* read backlink for primary header */
	    ...
	    if ((status & 1) != 1) 
	    { printf("%%DFU-E-READERR, Error reading backlink header,\n");
	      singlemsg(status);
	      goto nexti;
	    }
	    status = verify_header(hdr);
...
/* Post the results to user */
	...
	{ if (matfragment == FALSE)
	  { faolen = 56 + 80*((dir_descr.dsc$w_length + 24 - 1)/80);
            status = SYS$FAO(&ctrstr4, &outlen, &out_descr,
              faolen, &dir_descr, r_size, a_size);
	  }
	   else
	  { faolen = 46 + 80*((dir_descr.dsc$w_length + 34 - 1)/80);
...
nexti: status = get_next_header();
  ...
  if (ctrlc == 1) 
  { ctrlc = 0;
      printf("%%DFU-W-ABORT, Command aborted by CTRL/C\n");
  }
  if (matsort == TRUE)
  { printf("%%DFU-I-SORT, Sorting ... \n");
    sor$sort_merge();
    printf("\n");
  }
  if (matfragment == FALSE) 
    printf("%%DFU-S-FND , Files found : %d, Size : %d/%d\n",
      fcount,tot_r_size,tot_a_size);
   else
     printf("%%DFU-S-FND , Files found : %d, Size : %d/%d, Hdr/Frag : %d/%d\n",
...
{
  static char dummy_item[80], fname[80], outbuf[255];
  ...
  int ivbn, bitblk, bitje, bitval, find, rvn, x, 
    ...
    tot_r_size, tot_a_size, map_in_use , k, size; 
...
/* Open the volume (set) , read in the home block */
  ...
  { ctx.i = -1; /* Clear context */
    ...
    { if (maxvol == 1)
      ...
      {	printf("\n%%DFU-I-REPORT, Reporting on %.*s (%s) , RVN = %d\n",
        rvt[curvol].devnam_len, rvt[curvol].devnam, 
        &rvt[curvol].fulldevnam[1], curvol); 
      }
      headers=0;
...
/* Report home block info */
      fprintf(fp,"\n      ***** Volume info for %.*s (from HOME block) *****\n",
        rvt[curvol].devnam_len, rvt[curvol].devnam);
      fprintf(fp," Volume name                      :  %.12s\n",
         home[curvol].volname);
      fprintf(fp," Volume owner                     :  %.12s\n",
         home[curvol].ownername);
      fprintf(fp," Structure name                   :  %.12s\n",
         home[curvol].strucname);
      fprintf(fp," Cluster size                     :  %d\n",
         home[curvol].cluster);
      fprintf(fp," Maximum # files                  :  %d\n",
         home[curvol].maxfiles);
      fprintf(fp," Header count                     :  %d\n",
         rvt[curvol].if_size);
      fprintf(fp," First header VBN                 :  %d\n",
         rvt[curvol].vbn_file_1);                   
      fprintf(fp," Free headers                     :  %d\n\n",free_hdr);
...
/* We have a valid header, proceed...*/
	fcount +=1;
        if (headers == 1) map_in_use = hdr->map_inuse;
...
next_rep: status = get_next_header();
    ...
    { fprintf(fp,"      ***** File Statistics (from INDEXF.SYS) *****\n");
      fprintf(fp," INDEXF.SYS fragments/ map_in_use :  %d /%d\n",
        if_frag, map_in_use);
      fprintf(fp," Total files                      :  %d\n", fcount);
      fcount -= acount;
      fprintf(fp," Empty files                      :  %d\n", fcount);
      fprintf(fp," Files with allocation            :  %d\n", acount);
      fprintf(fp," Files with extension headers     :  %d\n", ucount);
      if (mcount == 0)
        fprintf(fp," Files marked for delete          :  %d\n", mcount);
       else
        fprintf(fp," Files marked for delete          :  %d, size %d/%d blocks\n", 
          mcount, mark_r_size, mark_a_size);
      fprintf(fp," Directory files                  :  %d\n", dcount);
      fprintf(fp," Contiguous files                 :  %d\n", ccount);
      fprintf(fp," Total used/ allocated size       :  %d /%d\n", 
        tot_r_size, tot_a_size);
      fprintf(fp," Total fragments                  :  %d\n", totfrag);
      a = totfrag; b = acount;
      avefrag = a/b;
      fprintf(fp," Average fragments per file       :  %.3f\n", avefrag);
      a = mmp_hdr; b = mmp_blks / home[curvol].cluster;
      avefrag = (a > 0) ? (avefrag*50 + a/b*5000)/100 : 0;
      ...
      fprintf(fp," File fragmentation index         :  %.3f %s\n",
        avefrag,dummy_item);
      tot_a_size = tot_a_size / totfrag;
      fprintf(fp," Average size per fragment        :  %d\n", tot_a_size);
      status = lib$fid_to_name(&device_descr, &badfid[0],&out_descr,
         &outlen, 0, 0);
      fprintf(fp," Most fragmented file             : \n");
      x = strindex(outbuf,"[",255);
      outlen -=x;
      fprintf(fp,"    %s%.*s ( %d/%d blocks; %d fragments)\n\n", 
       &rvt[curvol].fulldevnam[1],
       outlen, &outbuf[x], bad_r_size, bad_a_size, badrtv);
    }
    if (ctrlc !=1)
/* Now continue with bitmap processing */
    { strcpy(device, rvt[curvol].devnam);
      ...
      if ((status & 1) != 1) 
      { printf("%%DFU-E-ASSIGN, Error assign channel to disk,\n");
        singlemsg(status); cleanup(); return(status);
      }      
      ...
      { largest = largest * home[curvol].cluster; /* Adjust for cluster size */
        ...
        fprintf(fp,"      ***** Free space statistics (from BITMAP.SYS) *****\n");
        fprintf(fp," Total blocks on disk             :  %d\n",
          rvt[curvol].maxblocks);
        fprintf(fp," Total free blocks                :  %d\n",freeblocks);
        tot_a_size = (100*freeblocks) / rvt[curvol].maxblocks;
        fprintf(fp," Percentage free (rounded)        :  %d\n",tot_a_size);
        fprintf(fp," Total free extents               :  %d\n",freeext);
        fprintf(fp," Largest free extent              :  %d  at LBN: %d\n",
          largest, large_lbn);
        tot_a_size = (freeext == 0) ? 0 : freeblocks / freeext;
        fprintf(fp," Average extent size (rounded)    :  %d\n",tot_a_size);
        a = freeext - 1;
        ...
        if (avefrag < 2.0) strcpy(dummy_item," (good) ");      
        if (avefrag < 1.0) strcpy(dummy_item," (excellent) ");
        fprintf(fp," Free space fragmentation index   :  %.3f %s\n",
        avefrag,dummy_item);
        if (matgraph == TRUE)
        {
/* Now print the graph table */
        ...
        fprintf(fp," Free space distribution; each * = %d free blocks\n",
           tot_a_size);
        }
      }
      else 
...
/* Now continue with usage table processing */
  if ((matusage == TRUE) && (ctrlc != 1))
    report_usage(&usage_table, rvt[curvol].bchan, fp); 
  if (ctrlc == 1) printf("%%DFU-W-ABORT, Command aborted by CTRL/C\n"); 
  cleanup();  
  if (matstat == TRUE) status = lib$show_timer(0,0,display_stat,0);
  return(1);
}
...

{
  ...
  Boolean dummy, matstat, matname, matlist, matuic, matnoconfirm, found;
  register int i;
...
/* Check the privileges */
  if (mask > -1) 
  { printf("%%DFU-E-NOPRIV, No privilege to perform this function\n");
    return(SS$_NOPRIV);
  }
/* Fill in FIB */
...
/* Open the volume (set) , read in the home block */
  ...
  if ((status & 1) != 1) return(status);
  if (rvt[1].wlk == TRUE) 
  { printf("%%DFU-E-WRITELK, Cannot undelete on write-locked device\n");
    cleanup(); return(1);
  }
  if (home[1].volchar_overlay.volchar_bits.erase == 1) /*Erase on delete */
  { printf("%%DFU-E-ERASED, Cannot undelete ; erase-on-delete set on device\n");
    cleanup(); return(1);
  }
  ...
  { desblk.handler_addr = (int) &dfu_handler;
    ...
    if ((status & 1) !=1) 
    { printf("%%DFU-E-EXHFAIL, Failed to setup exit handler,\n");
      singlemsg(status); cleanup(); return(status);
    }
  }
...
/* Read in the BITMAP.SYS we need it anyway... */
  printf("%%DFU-I-READBMAP, Reading BITMAP.SYS...\n");
  for (x=1; x <=maxvol; x++) 
/* Assign a channel for BITMAP, create dynamic space, and read in the BITMAP*/
  { strcpy(device, rvt[x].devnam);
    ...
    if ((status & 1) != 1) 
    { printf("%%DFU-E-ASSIGN, Error assigning channel to BITMAP.SYS,\n");
      singlemsg(status); cleanup(); return(status);
    }
    ...
    status = SYS$EXPREG(bmap[x].pagecnt, &rvt[x].bmap_addr[0],
      0 , 0);
    if ((status & 1) != 1) 
    { printf("%%DFU-E-EXPREG, Error creating dynamic space,\n");
      singlemsg(status); cleanup(); return(status);
    }
    status = read_bitmap(rvt[x].bchan, bmap[x].pagecnt, 
      rvt[x].wlk, &rvt[x].bmap_addr[0]);
    if ((status & 1) != 1) 
    { printf("%%DFU-E-NOTUNDEL, File undeletes not possible,\n");
...
/* Loop for all volumes in the set */
  ...
  { ctx.i = -1;
    if (rvt[curvol].i_open ==1) 
    { if (maxvol == 1)
      {	printf("%%DFU-I-UNDEL, Start search on %.*s (%s)\n",
        device_descr.dsc$w_length, device, &rvt[curvol].fulldevnam[1]); 
      }
      else
      {	printf("%%DFU-I-UNDEL, Start search on %.*s (%s) , RVN = %d\n",
        rvt[curvol].devnam_len, rvt[curvol].devnam, 
        &rvt[curvol].fulldevnam[1], curvol); 
      }
      headers=0; qchan = 0;
...
/* Now rearrange the directory name found */
	...
	fcount++;         
	fprintf(fp,"Recoverable file %s%s found \n", dirrec.dirnam, name);
	x = 4; ans[0] = 'n';
	if (!matlist)
	  if(!matnoconfirm)
	    status = SMG$READ_STRING(&keyb_id, &answer, &prompt, 
	      &x, &modifiers ,0,0,0,0,0,0,0,0,0);
	   else
	    strcpy(ans,"Y");
	if ((ans[0] == 'y') || (ans[0] == 'Y')) 
	{  
...
/* Now rebuild the mapping pointers into the bitmap.
   ...
   off the whole undelete process */
	    status = rebuild_bitmap(hdr, rvt[rvn].bmap_addr[0],
            ...
            if ((status & 1) != 1) 
	    { printf("%%DFU-E-NOTUNDEL, File cannot be undeleted\n");
...
/* Read in extension header */
	    { x = 0; 
	      ...
	      hdr = (struct header_area_struct *) &header[x];   
              ...
              if ((status & 1) == 1) status = iostat[iocnt].iosb_1;
              if ((status & 1) != 1) 
              { printf("%%DFU-E-READERR, Error reading extension header,\n");
                singlemsg(status); 
	        printf("%%DFU-E-NOTUNDEL, File cannot be undeleted\n");
                ctx.end = TRUE; curvol = maxvol; goto next_und;
	      }                       
/* Check if this header is valid */
	      if ((hdr->bk_fid_overlay.bk_fid_fields.bk_fid_num != 
	          ...
	          (hdr->bk_fid_overlay.bk_fid_fields.bk_fid_nmx != 
                   oldhdr->fid_overlay.fid_fields.fid_nmx))
              { printf("%%DFU-E-BADEXTHDR, Extension header linkage broken\n");
	        printf("%%DFU-E-NOTUNDEL, File cannot be undeleted\n");
                ctx.end = TRUE; curvol = maxvol; goto next_und;
	      }
	    } 
...
/* So we have rebuild the bitmap(s), now rewrite them */
	  for (j = 1; j <= maxvol; j++) 
	      ...
	      status = SYS$QIOW(0,rvt[j].bchan, IO$_WRITEVBLK, &iostat[iocnt],
              ...
              if ((status & 1) == 1) status = iostat[iocnt].iosb_1;
              if ((status & 1) != 1) 
              { printf("%%DFU-E-BITMAPERR, Error writing BITMAP.SYS,\n");
                singlemsg(status); 
		printf("- Use ANALYZE/DISK/REPAIR to repair the volume\n");
                ctx.end = TRUE; curvol = maxvol; goto next_und;
	      }                       
	    }
          }
...
/* Check if we are dealing with a directory file */ 
	    ...
	    vbn = rvt[rvn].vbn_file_1 + fidnum - 1;
            ...
            if ((status & 1) == 1) status = iostat[iocnt].iosb_1;
            if ((status & 1) != 1) 
            { printf("%%DFU-E-INDEXFERR, Error writing INDEXF.SYS,\n");
              singlemsg(status); 
	      printf("- Use ANALYZE/DISK/REPAIR to repair the volume\n");
              ctx.end = TRUE; curvol = maxvol; goto next_und;
	    }                       
...
/* Read in extension header */
	  ...
	  printf("%%DFU-S-RECOVER, File succesfully recovered\n");
...
/* Add quota if needed */
	  ...
	  { status = SYS$ASSIGN(&device_descr, &qchan, 0, 0);
            if ((status & 1) != 1) 
            { printf("%%DFU-E-ASSIGN, Error assigning channel to QUOTA.SYS,\n");
              singlemsg(status); 
            }
	  }
...
/* Set up work entry for entering file in directory */
	  if (hdr->bk_fid_overlay.bk_fid_fields.bk_fid_num == 0)
	  { printf("%%DFU-E-INVBAKFID, Invalid backlink, file cannot be entered in directory\n");
	  }
           else
          { rvn = hdr->bk_fid_overlay.bk_fid_fields.bk_fid_rvn;
            if (rvn ==0) rvn = curvol;
...
/* Now enter files in directories */
  ...
  if (first != NULL) printf("%%DFU-I-ENTER, Entering file(s) in directory...\n");
  for (i=1; i <=maxvol ; i++)
  ...
  { list = first; rvn = list->rvn;
    ...
    { if (syslost == 0) 
      ...
      { copy_fid(&fib.fib$w_did[0], &lost_fid.fid_num, FALSE);
        ...
        if ((status & 1) == 1) status = iostat[ctx.thread].iosb_1;
        if ((status & 1) == 1)
         printf("%%DFU-S-ENTERED, File %s entered in [SYSLOST]\n",name);
      }
      if (syslost == 2) 
      { printf("%%DFU-E-NOTENTER, File %s not entered in directory,\n",name);
        singlemsg(status); 
        printf("- Use ANALYZE/DISK/REPAIR to move the file to [SYSLOST]\n");
      }
    }
     else printf("%%DFU-S-ENTERED, File %s entered in original directory\n",
	   name);
  }

  curvol = 1;
  cleanup();  
  if (matlist) printf("%%DFU-S-FND , %d recoverable files found\n",fcount);
  if (matstat == TRUE) status = lib$show_timer(0,0,display_stat,0);
  return(1);
}

...
/* Go on and create this directory */
    ...
    if ((status & 1) == 1 ) status = io_stat.iosb_1;
    if ((status & 1) != 1 )
    { printf("%%DFU-E-MKLOST, Error creating/accessing SYSLOST.DIR,\n");
      singlemsg(status);
      status = SYS$DASSGN(l_chan); return(2);
    }
     else
...
{
  static char dummy_item[80], fname[80], name[86], outbuf[255];
  struct header_area_struct *hdr;
  struct ident_area_struct *id;
...
/* Open the volume (set) , read in the home block */
  ...
  { printf("%%DFU-W-WRITELK, Cannot rebuild or lock on write-locked device\n");
  ...
  { ctx.i = -1; trigger = FALSE;
    if (rvt[curvol].i_open ==1) 
    { if (maxvol == 1)
      {	printf("%%DFU-I-VERIFY, Verifying %.*s (%s) \n",
        device_descr.dsc$w_length, device, &rvt[curvol].fulldevnam[1]); 
      }
      else
      {	printf("%%DFU-I-VERIFY, Verifying %.*s (%s) , RVN = %d\n",
        rvt[curvol].devnam_len, rvt[curvol].devnam, 
        &rvt[curvol].fulldevnam[1], curvol); 
      }
      headers=0;
...
/* Report home block info */
      if ((home[curvol].altidxlbn !=0) &&
        ...
        fprintf(fp,"%%DFU-S-CHKHOME, Home block info verified OK\n");
       else
        fprintf(fp,"%%DFU-E-ERRHOME, Home block info not OK\n");
...
/* Check corresponding bitmap bit (must be set) */
	...
	if (status != SS$_NORMAL)
        { if (bitset == TRUE) 
          { fprintf(fp,"%%DFU-W-NOBITCLR, file (%d,%d,%d) deleted file header marked BUSY in Index File bitmap\n"
...
/* Check marked for delete bit */
	if ((hdr->filechar & FH2$M_MARKDEL) == FH2$M_MARKDEL)
        { (dyn_array+headers)->bitje = 0;
          fprintf(fp,"%%DFU-W-DELETED, file (%d,%d,%d) %.*s marked for delete\n"
            ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol
            ,namelen,name);
...
/* Check locked bit */
	if ((hdr->filechar & FH2$M_LOCKED) == FH2$M_LOCKED)
        { fprintf(fp,"%%DFU-W-LOCKED, file (%d,%d,%d) %.*s is deaccess locked \n"
           ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol,namelen,name);
        }

/* Check badblock bit */
	if ((hdr->filechar & FH2$M_BADBLOCK) == FH2$M_BADBLOCK)
        { fprintf(fp,"%%DFU-W-BADBLOCK, file (%d,%d,%d) %.*s has suspected bad blocks\n"
           ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol,namelen,name);
        }
...
/* Check corresponding bit in bitmap */
	if (bitset == FALSE)
        { fprintf(fp,"%%DFU-W-NOBITSET, file (%d,%d,%d) %.*s Index File bitmap bit not set\n"
           ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol,namelen,name);
	  if (matreb)
...
/* Check file owner */
	if (hdr->fileowner_overlay.fileowner == 0)
          fprintf(fp,"%%DFU-W-NOOWNER, file (%d,%d,%d) %.*s has no owner\n"
            ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol,namelen,name);

/* Check backlink ; special case for the MFD */
        if ( (bakfid == 0) || ((headers == 4) && (bakfid != 4)) )
        { fprintf(fp,"%%DFU-E-INVBAKFID, file (%d,%d,%d) %.*s has invalid backlink\n"
            ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol,namelen,name);
...
/* need to check volume set*/
	  if (hdr->fid_overlay.fid_fields.fid_rvn ==
          ...
          { fprintf(fp,"%%DFU-E-SLFBAKFID, file (%d,%d,%d) %.*s backlink points to itself\n"
             ,headers,hdr->fid_overlay.fid_fields.fid_seq,curvol,namelen,name);
	    if (matfix)
	    { if (first == NULL )
	      { list = (struct work *) malloc(sizeof(struct work));
...
/* Phase 1A If we have multiple allocated blocks we will make
   ...
   and the routine new_bitmap will report ALL files involved. */
    ...
    { /* Recreate the new bitmap */
      fprintf(fp,"%%DFU-W-MULTFND, reporting multiple allocated blocks...\n");
...
/* Rewrite the indexf bitmap at this point */
      { bytes = 512 * home[curvol].ibmapsize;
        ...
        if ((status & 1) == 1)
	  fprintf(fp,"%%DFU-S-REBIFMAP, Indexfile bitmap rebuild\n");
	 else
        { printf("%%DFU-E-REBIFMAP, Indexf Bitmap rebuild failed,\n");
          singlemsg(status);
        }
      }
...
/* Check for incorrect blocks in BITMAP.SYS and rebuild on request */
      status = compare_bitmap(rvt[curvol].bchan, rvt[curvol].bmap_addr[0], 
                  page_cnt, rvt[curvol].wlk, home[curvol].cluster, fp, matreb);
      if ((status &1) !=1) 
      { cleanup(); return(status);
      }
...
/* Phase 3 : check lost files */
  if (ctrlc != 1) report_lost_files(fp, matfix);
/* Phase 4 : check diskquota */
  if (ctrlc != 1) check_usage(&usage_table, rvt[curvol].bchan, fp, matreb);
...
/* Phase 5 : fix errors */
  ...
  { for (i=1; i <=maxvol ; i++)
     status = SYS$QIOW(0,rvt[i].channel,IO$_DEACCESS,&iostat[iocnt],
               0,0,0,0,0,0,0,0); 
    if (!ctrlc) printf("%%DFU-I-REPAIR, Fixing errors...\n");
    fib.fib$w_nmctl = FIB$M_NEWVER;
    while (first != NULL)
    { list = first; 
      if (!ctrlc) 
  ...
  if (ctrlc) printf("%%DFU-W-ABORT, Command aborted by CTRL/C\n"); 
...

void report_lost_files(FILE *fp, Boolean matfix)
...
{ struct _da { int bakfid;  char rvn, bitje; } *dyn_array, *temp;
  ...
  fprintf(fp,"%%DFU-I-CHKLOST, Checking for lost files...\n");
...
/* case 1 : Lost file bit clear. Probably this directory is deleted*/
      ...
      { vbn = i - 1 + rvt[j].vbn_file_1; /*Read this header */
        ...
        if (name[19] != ' ') memcpy(&name[20], id->filenamext,66);
        namelen = strindex(&name[0]," ", 86);        
        fprintf(fp,"%%DFU-W-LOSTHDR1, file (%d,%d,%d) %.*s found in nonexistent directory\n"
...
/* case 2 and 3 : directory has invalid backlink, or directory bit not set */
	  ...
	  if ((temp->bitje & 2) == 2)
            fprintf(fp,"%%DFU-W-LOSTHDR2, file (%d,%d,%d) %.*s in directory with bad backlink (%d,%d,%d) %.*s\n"
            ,i ,hdr->fid_overlay.fid_fields.fid_seq,j,namelen,name,
             k ,hdr1->fid_overlay.fid_fields.fid_seq,xrvn,dirlen,dirname);
           else
            fprintf(fp,"%%DFU-W-LOSTHDR3, file (%d,%d,%d) %.*s found in invalid directory (%d,%d,%d) %.*s\n"
            ,i ,hdr->fid_overlay.fid_fields.fid_seq,j,namelen,name,
             k ,hdr1->fid_overlay.fid_fields.fid_seq,xrvn,dirlen,dirname);
        }
      }
...

int build_dir_table(char *dev_str)
/*
    Build directory table needed for DIR/VERSION
	, DIR/ALIAS, DIR/CHECK and DIR/EMPTY
*/

{
  static char dummy_item[80], name[86], outbuf[255];
  struct header_area_struct *hdr;
  Boolean dummy, matoutput, bitset;
  register int i;
  int ivbn, bitblk, bitje, bitval, find, rvn, x, 
    k, size, free_hdr, a_size, r_size, hdrs, namelen, rtvptr;
  unsigned int page_cnt, dir_cnt, bakfid;
  struct _da { short fid[3]; unsigned int bakfid; char rvn, bitje; } *dyn_array;
...
/* Open the volume (set) , read in the home block */
  ...
  { ctx.i = -1;
    if (rvt[curvol].i_open ==1) 
    { if (maxvol == 1)
      {	printf("%%DFU-I-INDSCAN, Building directory table for %.*s (%s) \n",
        device_descr.dsc$w_length, device, &rvt[curvol].fulldevnam[1]); 
      }
      else
      {	printf("%%DFU-I-INDSCAN, Building directory table for %.*s (%s) , RVN = %d\n",
        rvt[curvol].devnam_len, rvt[curvol].devnam, 
        &rvt[curvol].fulldevnam[1], curvol); 
      }
      headers=0; 
/* Phase 1 : Read and parse INDEXF.SYS */
/* Create dynamic array. each entry = 12 bytes so Index file size /6
    + 1 must be enough. Also create space for new bitmap */
      
      page_cnt = 1 + (rvt[curvol].if_size*12)/512;
      rvt[curvol].addr[0] = 0;
      rvt[curvol].bmap_addr[0] = 0;
      status = SYS$EXPREG(page_cnt, &rvt[curvol].addr[0],0,0);
      if ((status &1) != 1)
...
/* Now scan all the directories */
  if (ctrlc !=1) printf("%%DFU-I-DIRSCAN, Scanning %d directories...\n",
     dir_cnt);
...
/* Now scan the directories */
  dir_cnt = 0;
  for (i = 1; i <= maxvol; i++)
   if (ctrlc !=1) dir_cnt += scan_directories(&rvt[0] , i);

  printf("\n");
  if (ctrlc == 1) printf("%%DFU-W-ABORT, Command aborted by CTRL/C\n"); 
  printf("%%DFU-S-DONE, Directories scanned : %d\n",dir_cnt);
  cleanup();  
  return(1);
}

    ...
    { case 0 : /* Get the string */
      ...
      case 1 : /* Convert to integer */
	       ...
	       *convert = atoi(return_descr->dsc$a_pointer); 
	       if (*convert < 0) 
               { printf("%%DFU-E-PARSEERR, Value must be a positive integer\n");
	         *flag = FALSE;
		 return(0);
               }
	       break; 
      ...
      case 6 : /* UIC parsing */
		 ...
		 &uic_descr.dsc$w_length);
	       uic_x[0] = '\0'; x = strindex(&uic_s,",",30);
               if (x == -1)
	       { printf("%%DFU-E-UICERR, Invalid UIC entered\n");
		 return(0);
               }
...
/* invalid backlink */
  ...
  { copy_fid(&fib.fib$w_fid[0], &hdr->fid_overlay.fid_fields.fid_num,
    ...
    { x = strindex(&dirrec.dirnam,"[",198);
      strcpy(ret_dir,&rvt[curvol].fulldevnam[1]);
...
/* Open the device or volume set.
   Outputs :
	...
	Flags : 0 -> process volume set
		...
		2 -> process single disk (only used by INDEXF command)
*/

{
   ...
   if (flag ==2) volcount =1;
   if (devclass != DC$_DISK) 
   { printf("%%DFU-E-NODISK, Device is not a disk device\n");
     return(SS$_NOSUCHDEV);
   }
   if (volcount > 32)
   { printf("%%DFU-F-TOOMANYVOL, DFU does not allow > 32 volumes.\n");
     return(SS$_NOSUCHDEV);
   }
...
/* Lock volume if requested (flag=1) */
   ...
   { stat = SYS$ASSIGN(device_descr, &lock_chan,0,0);
     if ((stat & 1) != 1) 
     { printf("%%DFU-E-ASSIGN, Error calling SYS$ASSIGN,\n");
       singlemsg(stat); 
       cleanup();
       return(stat); 
     }
     ...
     if ((stat & 1) == 1) stat = iostat[0].iosb_1;
     if ((stat & 1) != 1) 
     { printf("%%DFU-E-LOCKERR, Error locking volume,\n");
       singlemsg(stat);
       cleanup();
       return(stat);
     }
     printf("%%DFU-W-LOCKED, Volume now write locked\n");
   }
...
/* Now loop for all members in volume set */
   ...
   while (i <= maxvol) 
   {
     ...
     if ((stat & 1) == 1) stat = iostat[0].iosb_1; 
     if ((stat & 1) != 1) 
     { printf("%%DFU-E-INDEXERR, Error opening Index File,\n");
       singlemsg(stat);
       cleanup();
       return(stat);
     }
     ...
     { /* Not an ODS2 disk */
       printf("%%DFU-NOTODS2, Device is not an ODS2 disk\n");
       cleanup();
       return(1);
     }
...
/* Get actual volume number and maximum blocks */
       ...
       if ((stat & 1) == 1) stat = iostat[0].iosb_1; 
       if ((stat & 1) != 1) 
       { printf("%%DFU-E-NOVOLSET, One ore more volume set members missing\n");
         cleanup();
         return(stat);
       }
       i=volnum; 
...
*/
{
  ...
  for (i=1; i<=iocnt; i++)	    /* Fire off 2 QIO's */
    { stat = lib$get_ef(&efn[i-1]);
      if ((stat & 1) != 1) 
      { printf("%%DFU-E-GETEF, Error getting event flag,\n");
        singlemsg(stat);
        return(stat);
      }
/* Read 500 file headers */
      ...
      if ((stat & 1) != 1) 
      { printf("%%DFU-E-IOERR, Error reading INDEXF.SYS,\n");
        singlemsg(stat);
        return(stat);
      }
...
{ static struct fibdef i_fib;	/* File information block */
  ...
  { stat = SYS$QIOW(0,lock_chan,IO$_ACPCONTROL,&iostat[0],
      0,0,&fibdescr,0,0,0,0,0);
    printf("%%DFU-I-UNLOCK, Volume unlocked\n");
