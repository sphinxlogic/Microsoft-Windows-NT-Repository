...
else if (sz == sizeof (short)) PUT_RVAL (short, rptr, val, mask);
else if (sz == sizeof (int)) PUT_RVAL (int, rptr, val, mask);
else if (sz == sizeof (long)) PUT_RVAL (long, rptr, val, mask);
return;
}

...
int scp_yn (char *ques, int deflt)
{
