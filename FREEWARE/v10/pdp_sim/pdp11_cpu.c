...
#define UNIT_18B	(1 << UNIT_V_18B)

/* Global state */

unsigned short M[MEMSIZE >> 1] = { 0 };			/* memory */
...
int int_mask[8] = { INT_IPL0, INT_IPL1, INT_IPL2,	/* interrupt masks */
...
void WriteB (int data, int addr);
void PWriteW (int data, int addr);
void PWriteB (int data, int addr);
int iopageR (int *data, int addr, int access);
int iopageW (int data, int addr, int access);

int CPU_rd (int *data, int addr, int access);
int CPU_wr (int data, int addr, int access);
int APR_rd (int *data, int addr, int access);
int APR_wr (int data, int addr, int access);
int SR_MMR012_rd (int *data, int addr, int access);
int SR_MMR012_wr (int data, int addr, int access);
int MMR3_rd (int *data, int addr, int access);
int MMR3_wr (int data, int addr, int access);
...
int int_vec[32] = {					/* int req to vector */
	...
	VEC_RK, VEC_RL, VEC_RX, 0, 0, 0, 0, VEC_PIRQ,
...
/* Other CPU routines
	...
	cpu_svc		re-enable breakpoint after SCP service
*/
