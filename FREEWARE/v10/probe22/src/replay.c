 ...
 * NOTES
 *
 * 1. Conditions are signaled according to VAX/VMS standards.
...
# module  REPLAY  "V2.2"
# define  VERSION 0x0202		/* highest known PROBE data format */
# define  eth$envelop	18		/* LLC=destination(6)+source(6)+ptcl:length(2)+fcs(4) */
...
# include <sys_probe:v22$logdef.h>	/* Probe 2.2-x log file layout */
# include <sys_probe:v22$macros.h>	/* Probe 2.2-x macros etc... */
 ...
 ******************************************************************************/
 ...
 char	  s_file[81];			/* sample file name */
...
# define  known		20		/* statically defined known manufacturer address array size */
 unsigned char	node[known][3] =
		{
		  0x08, 0x00, 0x2b,	/* DEC */
		  0x08, 0x00, 0x20,	/* SUN */
		  0x08, 0x00, 0x09,	/* HP */
		  ...
		  0x00, 0x00, 0x0f,	/* NEXT */
		  0x08, 0x00, 0x0e,	/* NCR-AT&T */
		  0x00, 0x00, 0xa7,	/* NCD */
		  0x08, 0x00, 0x11,	/* TEKTRONIX */
		  0x00, 0x00, 0x1d,	/* CABLETRON */
		  0x08, 0x00, 0x8f,	/* CHIPCOM */
		  0x00, 0x00, 0x81,	/* SYNOPTICS */
 ...
 unsigned char	name[known][8+1] =
		{ "DIGITAL", "SUN", "HP", "IBM", "SGI", "MIPS", "APPLE", "NEXT", "NCR/AT&T", "NCD", "TEK",
		  "CABLTRON", "CHIPCOM", "SYNOPTIC", "CISCO", "WELLFLT", "XYPLEX", "NETGNRL", "QMS", "XEROX"
		} ;
...
open_files()
{
 ...
 D_CONSTANT(logicals,"LNM$FILE_DEV");

/* sample file must be accessible */

if ((sample = fopen(s_file,"r","dna=RECORDED.PRB")) == NULL)
   lib$stop(&RPL_NOSAMPLE,1,s_file);

/* analysis file must be created if specified */

if (command.analysis_specified)
   if ((analysis = ((command.analysis_redirected) ? fopen(a_file,"w","dna=.PRB_ANALYSIS")
...
if (command.process_external_table)
   if ((table = fopen(t_file,"r","dna=.TBL")) == NULL)
...
read_sample_header()
{
 globalref RPL_NEWLVL, RPL_BADREC, RPL_DATALESS, RPL_EMPTY, RPL_NOTAVAX,
...
/* versions prior to V2.2 have different primary layout... extract correct elements */

if (revision.level < 0x0202)
   {
   command.sample_was_extracted = log.prb$intro.specific.v20.flags.is_extracted;
   command.sample_from_a_vax = log.prb$intro.specific.v20.flags.from_a_vax;
   ...
   {
   command.sample_was_extracted = log.prb$intro.specific.v22.data.flags.is_extracted;
   command.sample_from_a_vax = log.prb$intro.specific.v22.data.flags.from_a_vax;
   log_record_size = log.prb$intro.specific.v22.size;
   if ((command.is_format_data) && ((log_payload_size = log.prb$intro.specific.v22.data.payload) == 0))
      lib$signal(&RPL_EMPTY), command.is_format_data = 0;
   }
if (!command.sample_from_a_vax)
   lib$stop(&RPL_NOTAVAX);
...
/* prepare master node, protocol lists */
...
/* skip sample filler & load non-conflicting imbedded protocol and node definitions */
...
if (command.is_format_header)
   {
   fprintf(analysis,"PROBE/PLAYBACK/ANALYZE 2.2\n\n");             
   ...
   fprintf(analysis,"\n\nMade:       %s\nOn Node:    %-8s\t(a VAX)\nAdapter:    %-8s\n\n",
	   display_time(master_cycle->block.sample.creation_time),log_node,adapter);
	 ...
	 if (state.do_this_cycle)
	    ...
	    if (command.is_format_frame)
		  ...
		  build_node(target,log.prb$frame.destination);
	       ...
	       fprintf(analysis," Frame %010d %-10s @ %08.2f  From: %-17s  To: %-17s  Size: %4d/%-4d  %s ",
		       frame_count,(log.prb$frame.tag.is_ieee ? "(IEEE)" : "(Ethernet)"),log.prb$frame.time,
			...
			eth$min_frame : log.prb$frame.size+log.prb$frame.tag.overhead+eth$envelop),
		       (log.prb$frame.tag.is_ieee ? "D-S SAPs:" : "Protocol:"));
		  ...
		  fprintf(analysis,"%s ",protocol->object.name);
	       else
		  fprintf(analysis,"%02X-%02X ",log.prb$frame.protocol[0],log.prb$frame.protocol[1]);
	       fprintf(analysis,"%s\n",(log.prb$frame.tag.warning ? "<warning>" : ""));
		  ...
		  if (log.prb$frame.tag.is_more)	/* store contents until end-of-data block 2 */
			...
			max_index = log_payload_size;
		     for (index = 0; index < prb$log_capacity; ++index)
			payload[index] = log.prb$frame.data[index];
		     }
			...
			max_index = log_payload_size;
		     ...
		     for (i = 0; i < 25; ++i)
			if (i < max_index)
			   fprintf(analysis,"%02X%s",log.prb$frame.data[i],(i < max_index-1 ? "," : " "));
			else
			   fprintf(analysis,"   ");
		     fprintf(analysis," ASCII> ");
		     for (i = 0; i < max_index; ++i)
			if ((log.prb$frame.data[i] < 0x20) || (log.prb$frame.data[i] >= 0x7F))
			   fprintf(analysis,".");
			else
			   fputc(log.prb$frame.data[i],analysis);
		     fprintf(analysis,"\n");
		     }
	       }
	    }
 ...
 * form. This is used during protocol value and node address processing.      *
 ******************************************************************************/
...
if (odd(address[0]))			/* multicast */
   ...
   if ((address[0] == 0xAA) && (address[1] == 0x00) && (address[2] == 0x04) && (address[3] == 0x00))
      ...
      {
      found = 0;
      for (i = 0; (i < known) && !found; ++i)
	 for (j = 0; j < 3; ++j)
	    if (!(found = (address[2-j] == node[i][2-j])))	/* test most variant byte first */
	       break;
      if (found)
	 sprintf(new->object.name,"%s:%02X-%02X-%02X",name[--i],address[3],address[4],address[5]);
      else
