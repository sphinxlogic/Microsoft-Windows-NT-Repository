/********************************************************************
 Changes from Alan Coopersmith's patches:
    - moved Gcatd to Locale.c/Locale.h
 ********************************************************************/
  ...
  Requestitem **items;
{
     ...
     while (1) {
          ...
          waddstr(tempwin, Gtxt("[Help: ^-]  [Cancel: ^G]",198));
          ...
          if (numprompts == 1 && items[0]->thing == CUR_PROMPT) {
	       ...
	       wstandend(tempwin);
		    ...
		    case CUR_ASKL:
			 ...
			 for (j = WinWidth - strlen(items[k]->stowage) - 
...
int
CURChoice(cur, WinTitle, Choices, Prompt, DefaultChoice)
  CursesObj *cur;
  char      *WinTitle;
  char      **Choices;
  char      *Prompt;
  int       DefaultChoice;
{
     ...
     if (ChoiceHelp == NULL) {
	  ...
	  ChoiceHelp[0] = Gtxt("^G        : Cancel",202);
