/****************************************************************************
 * F.Macrides	27-Jul-1994 Restore "Garbled = TRUE;" following the GetMenu()
 *			     call in the main command loop for VMS.
 * F.Macrides	14-Jul-1994 Break if the user enters 'L' when already in
 *			     setlocale_LangDir.
 *			    Distinguish VMS versus Unix locatations of help
 *			     files in internationalization code.
 * JL Wilkinson 12-Jul-1994 Extended Gripe() to strip trailing blank lines
 *			     from gripe messages and to use the following 
 *			     defines from CONF.H:
 *			DESCRIBE_GOPHER_GRIPE - if defined, Gripe() will do a
 *				describe_gopher() to include the link info in
 *				the gripe message.
 *			MODIFIABLE_GRIPE_TO - if defined, Gripe() will present
 *				a To: header which the user can change.
 *			NOGRIPE_SECURE - if defined, Gripe() cannot be called
 *				in secure mode.
 *			LOCAL_GRIPE_ADMINISTRATOR - if defined, should be the
 *				email address of the local Gopher server
 *				admninistrator, including local host.  Use 
 *				syntax "Real Name <id@host.domain>".  When
 *				server being griped is not a G+ server, and/or
 *				the administrator string for the item is null,
 *				and the server being griped is at the same
 *				host as the host in the email address, this
 *				address will be substituted.
 * JL Wilkinson 12-Jul-1994 If GOPHERHELP_SECURE defined in CONF.H, use it
 *			        in SecureMode instead of GOPHERHELP.  Also added
 *				use of these strictly as defaults if Gtxt() can
 *				not supply a language-specific version of the 
 *				help files.
 * JL Wilkinson 12-Jul-1994 If TELNET_TRACE defined in CONF.H, test for
 *			        presense of GOPHER_TELNET_LOG_* logicals and
 *			        trace attempts to connect to telnet hosts.
/*****************************************************************************
 * F.Macrides 13-Jul-1994  Added more patches from Alan Coopersmith:
 *	- The international message routines are now supported on non-X/Open
 *	  systems (but can cause infinite loop on Unix).
 *	- I've tried to solve the problems caused by assuming the current menu
 *	  came from the selected item on the previous menu (it may not have if
 *	  the user used 'o', 'w', etc.) with the simple addition of having
 *	  GopherDir's remember where they came from.
 *	- Control-C now redraws the screen just once instead of twice if not
 *	  quitting.
 * F.Macrides 02-Jul-1994  Moved declaration and initialization of
 *			    setlocale_LangDir to globals.h.
 *			   Note that setlocale() doesn't yet work with VMS 
 *			    Alphas, so it presently just returns without doing
 *			    anything and the "hardcoded" English text is used.
 *			    It all works fine with VAXen.
 * F.Macrides 01-Jul-1994  Added code to delete the temporary file when gopher
 *			    is called with the -p or the new -u switch.
 *			   Added J. Lance Wilkinson's (JLW@psulias.psu.edu)
 *			    internationalization port to VMS and:
 *				'L' operation for changing language on the fly.
 *				Pause after telnet/tn3270 to permit reading of
 *				any error messages.
 * F.Macrides 21-Jun-1994  Added telnet and tn3270 to prompt for a URL via
 *			    the 'w' command.
/*****************************************************************************
 * F.Macrides 20-Jun-1994  Modified Alan's dialog box title and prompt for
 *			    the 'w' command.
 *			   NEVERSETOPTIONS compilation symbol disables ability
 *			    to set options via SetOptions() and issues an
 *			    appropriate message.
 *			   NEVERSPAWN compilation symbol disables the spawn
 *			    command and issues an appropriate message.
 *			   On VMS, the NEVERSETOPTIONS and NEVERSPAWN symbols
 *			    can be used to create a gopher image for CAPTIVE
 *			    accounts which still permits functions that would
 *			    be disabled by the -s or -S switch, but issues an
 *			    appropriate message (rather than a DCL error
 *			    message) on spawn attempt and precludes "creative"
 *			    modifications of the display software and print
 *			    command mappings.
 *			   Added missing ! for NoShellMode under binary and
 *			    and encoded save to disk if().
 ****************************************************************************/

/********************************************************************
 Changes from Alan Coopersmith's patches:
    - changes for new Locale.c/Locale.h
    - change Gtxtopen argument from "gopher.cat" to just "gopher"
      (X/Open systems should add the ".cat" if necessary in NLSPATH)
    - check if LC_MESSAGES is defined before using it
        (not all Std. C or X/Open systems define it)
    - if second char of '-T' command line argument is '+', assume
      we're going to connect to a gopher+ server
    - add 'w' key and '-u' command line option to connect to a URL
    - if we don't have a "pretty" name for the Language, display the
      lang code in showinfo()
    - add display of Server Info (when present) in showinfo()
    - optimize out some repeated GSgetView calls in showinfo()
    - add support for 'home' entry in rc files
 ********************************************************************/
 ...
 GopherObj *gs;
{
     ...
     if (!GSisGplus(gs) || GSgetAdmin(gs) == NULL) {
	  ...
	  if (strstr(LOCAL_GRIPE_ADMINISTRATOR,(GSgetHost(gs))) != NULL ) {
  ...
  char *argv[];
{
	 ...
	 setlocale(LC_MESSAGES, getenv("LC_MESSAGES"));
#endif /* LC_MESSAGES */
     ...
     while ((c = getopt(argc, argv, "DsSbrp:t:T:i:u:")) != -1)
	  switch (c) {
	  case 's':
	       SecureMode = TRUE;
	       break;
	  ...
	  case 'u':
	       {
		    ...
		    GSfromURL(tempGopher, optarg, AFTP_HOST, AFTP_PORT);
...
#define	DESCRIBE_GOPHER(a,b)	describe_gopher((a),(b))
	  ...
	  case 'w': /** Open a new session from a URL **/
	  {
		...
		response[1] = NULL;
	       ...
	       if ((CURRequest(CursesScreen, "Connect to a new server",
		    ...
		    i = GSfromURL(tempgs, response[0], AFTP_HOST, AFTP_PORT);
		    if (i & G_HOST) {
			 if (!(i & G_PATH))
			      GSsetPath(tempgs, "");
			 if (!(i & G_TYPE))
	  ...
	  case 'r':  /** Go to root menu of current item **/
	  {
	       ...
	       GSType = GSgetType(GDgetEntry(CurrentDir, 
