...
int main(int argc, char *argv[])
{
    ...
    int SHELL$TO_VMS(), net_link_open(), net_link_query();
    char *cp, *port;
    /*
    ...
    */
    ...
    result.select = NULL;
    count = SHELL$TO_VMS ( argv[2], query_index, 1 );
 /* printf("file found count: %d, result count: %d\n", count, result.count);*/
    select_result(result);
    net_link_close();
}
/****************************************************************************/
/* Action routine for SHELL$TO_VMS.
 */
...
void select_result(Result result)
{
    ...
    char *ptr1, *ptr2, *ptr3, file_name[SPEC_SIZE], selector[SPEC_SIZE];
    char inputline[SPEC_SIZE], *SHELL$TRANSLATE_VMS();
    FILE *fp;
    ...
    for (ind = 0; ind < result.count; ind++) {
        ...
        ptr2 = strchr(ptr1, '\t');
        *ptr2 = '\0';
        strcpy(selector, ptr1);
	ptr1 = ptr2+1;
	ptr2 - strchr(ptr1, '\t');
	ptr3 = URLescape(ptr1);
	strcpy(inputline, ptr3);
	free(ptr3);
	sscanf(inputline + 1, "%d-%d-%s", &start, &end, file_name);
	fname_len = strlen(file_name);
	net_unescape_string ( file_name, &fname_len );
	file_name[fname_len] = '\0';
        cgi_printf (
		"<LI><A HREF=\"http://%s%s\"\n>%s</A>\n",
		Host,
		SHELL$TRANSLATE_VMS(file_name), selector);
    }
