...
static int send_http_header ( char *stsline, char *content )
{
    int status;
    /*
     * Enter RAW mode, set rundown to terminate mode on exit.
     */
    status = net_link_write ( "<DNETRAW>", 9 );
    if ( 0 == (status&1) ) exit ( status );
    status = net_link_set_rundown ( "</DNETRAW>" );
    /*
     * Send back standard header.
     */
    status = net_link_printf ( "%s%s\r\nMIME-version: 1.0\r\n", 
		http_protocol_version, stsline );
...
static int do_include ( int opcode,	/* 1 - include, 2-fsize, 3-flastmod */
	...
	char *tag,
	char *name,		/* command arguments */
	int *outlen, 
	char **outbuf )
{
    char *ibuf, fname[512], translation[512];
...
drctv[2],drctv[3],drctv[4] );
	 ...
	 * Include file.
	 */
	return do_include ( 1, path, drctv[1], drctv[2], outlen, outbuf );

    } else if ( 0 == strcmp ( command, "ECHO" ) ) {
	/*
	 * Echo will insert values of special variables into stream.
	 */
	return do_echo ( path, drctv[1], drctv[2], outlen, outbuf );
    } else if ( 0 == strcmp(command,"FSIZE") ) {
	/*
	 * File attributes.
	 */
	return do_include ( 2, path, drctv[1], drctv[2], outlen, outbuf );
    } else if ( 0 == strcmp(command,"FLASTMOD") ) {
	return do_include ( 3, path, drctv[1], drctv[2], outlen, outbuf );
