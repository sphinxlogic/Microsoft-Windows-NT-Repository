...
$!				with cache disabled.
$!
$! To start the server under another username (i.e. the WWW server account),
$! from system startup, submit this procedure as a batch job with the
$! appropriate /USERNAME and /PARAMETER qualifiers.
$!
...
$! and sys$share:cma$rtl.exe.  Invoking this procedure with a P1 parameter of
$! "INSTALL" will install these images.
$!
...
$!
$ HTTP_CACHE_SIZE = 100			! Entries in small document cache.
...
$ endif
$!
$! Define www_root if not defined.
$!
...
$!
$ httpd = "$" + f$parse(".EXE;",proc) - ";" - "]["
$ p_dir = f$locate(".000000]",httpd)
$ if p_dir .lt. f$length(httpd) then httpd = f$extract(0,p_dir,httpd) + -
		f$extract(p_dir+7,250,httpd)
