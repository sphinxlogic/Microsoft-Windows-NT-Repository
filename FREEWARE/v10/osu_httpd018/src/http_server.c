...
int http_init_doc_cache(), http_max_server_clients(), tlog_init();
int http_parse_elements(), http_read_rules (), http_start_authenticator();
...
char http_server_version[] = "1.8/alpha";
char http_server_date[] = "27-FEB-1995";
...
int main ( int argc, char **argv )
{
	 ...
	 */
	int limit;
	limit = client_limit/3;  if ( limit <= 0 ) limit = 1;
        tlog_putlog ( 0, "Reserving !SL client slot!%S for nocache port !SL!/",
		limit, http_ports[1] );
	client_limit = client_limit - limit;
        status = ts_declare_tcp_port 
	    ( http_ports[1], limit, &client_attr, &http_thread2, http_client );
    }
    status = ts_declare_tcp_port 
...
{
    int length, status, seg_len, tlog_flush(), SYS$GETTIM();
...
/*    LOCK_C_RTL
    sprintf ( log_prefix, "TCP-%d/%d", (unsigned) port, ndx );
    UNLOCK_C_RTL */
	 ...
	 * Read lines until null line read.
         */
	for ( lines = seg_len = 3; 
		(seg_len > 0) && (lines < 126) && (length < sizeof(reqbuf)); 
		lines++ ) {
	...
	request[3].l = -2;
	request[3].s = "500 Server overload, request aborted";
    }
    if ( http_log_level > 0 ) 
