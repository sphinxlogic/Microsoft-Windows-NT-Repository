...
$ Endif
$!----------------------------------------------------------------------------
$ DTquery = "$" + f$search(f$parse("DTquery.EXE;",f$environment("PROCEDURE")))
$ If DTquery .eqs. "$"			! Got foreign command?
$ Then
$   call SEND_STATUS "404 script not found" -
	"Search script (DTquery.exe) not found in htbin directory."
$   goto done
$ Endif
$!----------------------------------------------------------------------------
$ INDEX_DIR = "WWW_Root:[Index]"	! Location of indexes for DTquery.exe.
$!----------------------------------------------------------------------------
$!
$! P2 has the ISINDEX URL.
$!
$ ISINDEX_URL = f$edit(P2,"COLLAPSE,UPCASE")
