...
int main(int argc, char *argv[])
{
    ...
    */
    if (argc < 3) {
    /* Usage: prog index_spec query-string */
    ...
    */
    if (strlen(argv[1]) >= SPEC_SIZE) {
	too_bad(SPEC_TOO_BIG);
	exit(1);
    }
    strcpy(orig_idx, argv[1]);
    ...
    */
    ...
    if (strlen(argv[2]) >= MAX_QUERY) {
	too_bad(QUERY_TOO_BIG);
	exit(1);
    }
    strcpy(orig_qstr, argv[2]);  /* make our copy of the query */
...
#define	YY	cdate+22
    ...
    char file_name[SPEC_SIZE], size[128], DateAndOrSize[256];
    char gtype;
    char host[SPEC_SIZE];
    FILE *fp;
    ...
    printf("</HEAD>\n<BODY>\n");
    printf("<H1>%s</H1>\n", orig_qstr);
    printf("There are %d items found:\n<p>\n", result.count);
    printf("<OL>\n");
	...
	if (*ptr1 != 'R' || send_raw == TRUE) {
	    ...
	    /* Hex escaped URL with range. */
            printf ("<LI>%s<A HREF=\"%s%s%s?TEXT=%s\"\n>%s</A>%s\n",
	    	    FileType(gtype),
	    	    strlen(host) ? "http://" : "",
		    strlen(host) ? host : "",
