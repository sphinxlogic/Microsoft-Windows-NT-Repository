...
int http_max_server_clients(), tlog_init(), tlog_initlog();
...
int main ( int argc, char **argv )
{
    ...
    tlog_putlog ( 0,
	"DECThreads network object, version 1.1, 17-DEC-94, compiled !AZ !AZ!/", 
	__DATE__, __TIME__ );
    /*
...
{
    int length, status, tlog_flush(), ts_decnet_info(), i, tf_len, bd_len;
    string prologue[4];
    string request[128];
	...
	status = send_catalogue ( ctx, tarf, prologue[3].s,
	...
	char *protocol, char *log_prefix )
{
    ...
    if ( !status ) {
        ts_decnet_write ( ctx, "HTTP/1.0 404 module not found\r\n\r\n", 33 );
	return status;
    }
    /*
...
static int send_catalogue ( void *ctx, void *tarf, char *archive_name, 
	char *protocol, char *log_prefix )
{
    int size, status, length, ts_tcp_stack_used();
    unsigned mtime;
    struct out_stream stream;
...
/*
 * Fixup for read_rules reference to ts_set_local_addr()
 */
int ts_set_local_addr ( char *addr ) { return 1; }
