...
*/
    ...
    void Define_Symbol(char *, char *, int);
    void Define_CmdLine_Symbol(char *, char *, int);
    ...
    static void define_local(char *, char *);
    static void apply_subst_rule(char *, char *, char **, int *);

#define min(a,b) (((a) < (b)) ? (a) : (b))

    static struct SYMBOL dcl_symbols = {&dcl_symbols, &dcl_symbols, "", 0};
...
**
**  IMPLICIT INPUTS:	local_symbols, global_symbols
**
...
struct SYMBOL *Lookup_Symbol(char *name) {
    ...
    for (sym = local_symbols.flink; sym != &local_symbols; sym = sym->flink) {
    	if (strcmp(name, sym->name) == 0) return sym;
    }
    for (sym = global_symbols.flink; sym != &global_symbols; sym = sym->flink) {
    	if (strcmp(name, sym->name) == 0) return sym;
    }
    for (sym = dcl_symbols.flink; sym != &dcl_symbols; sym = sym ->flink) {
    	if (strcmp(name, sym->name) == 0) return sym;
    }

    sym = mem_get_symbol();
    strcpy(sym->name, name);
    INIT_SDESC(namdsc, strlen(sym->name), sym->name);
    INIT_DYNDESC(valdsc);
    ...
    if (OK(status)) {
    	...
    	queue_insert(sym, dcl_symbols.blink);
    	str$free1_dx(&valdsc);
    	return sym;
    }
    mem_free_symbol(sym);
...
**  PROTOTYPE:
**
**  	Define_Symbol(char *name, char *val, int vallen)
**
...
**--
*/
void Define_Symbol(char *name, char *val, int vallen) {
    ...
    if (sym == &global_symbols) {
    	...
    	queue_insert(sym, global_symbols.blink);
    } else {
    	free(sym->value);
    }
...
/*
**++
**  ROUTINE:	Define_CmdLine_Symbol
**
**  FUNCTIONAL DESCRIPTION:
**
**  	Creates or re-defines a symbol in the command-line symbol
**  	table.
**
**  RETURNS:	void
**
**  PROTOTYPE:
**
**  	Define_CmdLine_Symbol(char *name, char *val, int vallen)
**
**  IMPLICIT INPUTS:	None.
**
**  IMPLICIT OUTPUTS:	None.
...
**--
*/
void Define_CmdLine_Symbol(char *name, char *val, int vallen) {
    ...
    for (sym = cmdline_symbols.flink; sym != &cmdline_symbols; sym = sym->flink) {
    	if (strcmp(name, sym->name) == 0) break;
    }

    if (sym == &cmdline_symbols) {
    	sym = malloc(sizeof(struct SYMBOL));
    	strcpy(sym->name, upname);
    	queue_insert(sym, cmdline_symbols.blink);
    } else {
    	free(sym->value);
    }
...
int Resolve_Symbols(char *in, int inlen, char **out, int *outlen,
    ...
    static char *non_resolvables[] = {"MMS","MMSQUALIFIERS",
    	...
    	"MMK_DELETE_SOURCE","MMS$SOURCE","MMS$SOURCE_LIST","MMS$CHANGED_LIST",
...
void Clear_Local_Symbols() {
    ...
    while (queue_remove(local_symbols.flink, &sym)) {
    	mem_free_symbol(sym);
    }
} /* Clear_Local_Symbols */

...
void Create_Local_Symbols(struct DEPEND *dep, struct OBJREF *srcref, struct QUE *chgque) {
    ...
    define_local("MMS$TARGET_NAME", nam);
    extract_filename(nam, dep->target->name);
    define_local("MMS$TARGET_FNAME", nam);

    if (dep->target->type == MMK_K_OBJ_LIBMOD) {
    	define_local("MMS$TARGET", dep->target->libfile->name);
    	define_local("MMS$TARGET_MODULE", dep->target->name);
    	define_local("MMS$LIB_ELEMENT", dep->target->name);
    } else {
    	define_local("MMS$TARGET", dep->target->name);
    }
/*
...
*/
    ...
    if (obj != &dep->sources) {
    	define_local("MMS$SOURCE", obj->obj->name);
    	if (obj->obj->type == MMK_K_OBJ_CMSFILE) {
    	    ...
    	    status = cms_parse_name(obj->obj->name, lib, sizeof(lib), 0,
    	    	    	    	nam, sizeof(nam), 0, 0);
    	    if (OK(status)) {
    	    	define_local("MMS$CMS_ELEMENT", nam);
    	    	if (lib[0] != '\0') define_local("MMS$CMS_LIBRARY", lib);
    	    }
    	    if (obj->obj->cms_gen[0] != '\0') {
    	    	define_local("MMS$CMS_GEN", obj->obj->cms_gen);
    	    } else if (cms_default_generation[0] != '\0') {
    	    	define_local("MMS$CMS_GEN", cms_default_generation);
    	    } else define_local("MMS$CMS_GEN", "1+");
    	}
    	extract_name(nam, obj->obj->name);
    	define_local("MMS$SOURCE_NAME", nam);
    	...
    	define_local("MMS$SOURCE_LIST", src);
    	for (cp = src; *cp; cp++) if (*cp == ',') *cp = ' ';
    	define_local("MMS$SOURCE_LIST_SPACES", src);
    	free(src);
    }
...
*/
    ...
    src[srclen] = '\0';
    define_local("MMS$CHANGED_LIST", src);
    for (cp = src; *cp; cp++) if (*cp == ',') *cp = ' ';
    define_local("MMS$CHANGED_LIST_SPACES", src);
    free(src);
...
/*
**++
**  ROUTINE:	define_local
**
**  FUNCTIONAL DESCRIPTION:
**
**  	Inserts an entry in the local symbol table.
**
**  RETURNS:	void
**
**  PROTOTYPE:
**
**  	define_local(char *symname, char *value)
**
**  IMPLICIT INPUTS:	None.
**
**  IMPLICIT OUTPUTS:	None.
...
**--
*/
static void define_local(char *name, char *value) {
    ...
    queue_insert(sym, local_symbols.blink);
