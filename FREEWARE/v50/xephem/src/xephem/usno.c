...
#else
        sprintf (tstname, "%szone1275.acc", cdp);
#endif
	fp = fopenh (tstname, "r");
	if (fp == NULL) {
	    sprintf (msg, "%s: %s", tstname, syserrstr());
	    return (-1);
	}
	fclose (fp);
...
#else
        sprintf (fn, "%szone%04d.acc", cdpath, zone);
#endif
	fp = fopenh (fn, "r");
	if (fp == NULL) {
...
#else
        sprintf (fn, "%szone%04d.cat", cdpath, zone);
#endif
	fp = fopenh (fn, "r");
	if (fp == NULL) {
	    sprintf (msg, "%s: %s", fn, syserrstr());
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: usno.c,v $ $Date: 1999/10/26 04:44:33 $ $Revision: 1.6 $ $Name:  $"};
