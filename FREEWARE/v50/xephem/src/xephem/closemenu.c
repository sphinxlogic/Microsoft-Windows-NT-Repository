...
static void c_unmap_cb P_((Widget w, XtPointer client, XtPointer call));
...
Cursor c;
{
	...
	if (cform_w && (win = XtWindow(cform_w)) != 0) {
...
c_create_form ()
{
	...
	set_something (cform_w, XmNcolormap, (XtArgVal)xe_cm);
	XtAddCallback (cform_w, XmNmapCallback, prompt_map_cb, NULL);
	XtAddCallback (cform_w, XmNunmapCallback, c_unmap_cb, NULL);
	...
	wtip (sep_w, "Enter desired minimum separation");
	XtManageChild (sep_w);
...
/* called when the main dialog is unmapped */
/* ARGSUSED */
static void
c_unmap_cb (w, client, call)
Widget w;
XtPointer client;
XtPointer call;
{
...
XtPointer call;
{
	/* let the unmap do the rest of the work */
	XtUnmanageChild (cform_w);
}
...
#ifndef VMS
	(void) sprintf (fname,"%s/closelist.txt", getXRes("PrivateDir","work"));
#else
	(void) sprintf (fname,"%scloselist.txt", getXRes("PrivateDir","work"));
#endif
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: closemenu.c,v $ $Date: 1998/04/27 21:51:49 $ $Revision: 1.1 $ $Name:  $"};
