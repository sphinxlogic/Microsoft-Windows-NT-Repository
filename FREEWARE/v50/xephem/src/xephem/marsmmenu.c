...
static void marsm_create_form_w P_((void));
...
static void marsm_set_a_button P_((Widget pbw, int whether));
...
static void marsm_unmap_cb P_((Widget w, XtPointer client, XtPointer call));
...
static Widget mmform_w;		/* main form */
...
/* called when the mars menu is activated via the main mars pulldown.
...
marsm_manage ()
{
	...
	if (XtIsManaged(mmform_w))
	    XtUnmanageChild (mmform_w);
	else {
...
marsm_ison()
{
	return (mmform_w && XtIsManaged(mmform_w));
}
...
Cursor c;
{
	...
	if (mmform_w && (win = XtWindow(mmform_w)) != 0) {
	    Display *dsp = XtDisplay(mmform_w);
	    if (c)
...
marsm_create_form_w()
{
	...
	mmform_w = XmCreateFormDialog (toplevel_w, "MarsMoons", args, n);
	set_something (mmform_w, XmNcolormap, (XtArgVal)xe_cm);
	XtAddCallback (mmform_w, XmNhelpCallback, marsm_help_cb, 0);
	XtAddCallback (mmform_w, XmNunmapCallback, marsm_unmap_cb, 0);
	...
	XtSetValues (XtParent(mmform_w), args, n);
...
marsm_create_msform_w()
{
	...
	XtSetArg (args[n], XmNcolormap, xe_cm); n++;
	msform_w = XmCreateFormDialog (toplevel_w, "MarsMoonStats", args, n);
	...
	XtAddCallback (w, XmNactivateCallback, marsm_sstats_cb, 0);
...
marsm_create_tvform_w()
{
	...
	XtSetArg (args[n], XmNdefaultPosition, False); n++;
	XtSetArg (args[n], XmNcolormap, xe_cm); n++;
	mtform_w = XmCreateFormDialog (toplevel_w, "MarsMoonsTV", args, n);
...
int whether;	/* whether setting up for plotting or for not plotting */
{
	...
	for (i = 0; i < NM; i++) {
	    for (j = 0; j < CNum; j++)
		marsm_set_a_button (marsm_w[i][j], whether);
	}
}
 ...
 */
static void
marsm_set_a_button(pbw, whether)
Widget pbw;
int whether;
{
	...
	if (!called) {
	    ...
	    n = 0;
	    tmpw = XmCreatePushButton (mmform_w, "tmp", args, n);
...
XtPointer call;
{
	if ((uswasman = XtIsManaged(msform_w)) != 0)
...
XtPointer call;
{
	...
	if (!mt_pm || nx != last_nx || ny != last_ny) {
	    if (mt_pm)
...
/* ARGSUSED */
static void
marsm_unmap_cb (w, client, call)
Widget w;
XtPointer client;
XtPointer call;
{
	if ((uswasman = (msform_w && XtIsManaged(msform_w))) != 0)
	    XtUnmanageChild (msform_w);
        if (marsm_pm) {
	...
	/* close top view */
	if (mtform_w && XtIsManaged(mtform_w))
	    XtUnmanageChild (mtform_w);
...
XtPointer call;
{
	...
	XGetGeometry(dsp, win, &root, &x, &y, &nx, &ny, &bw, &d);
	if (!marsm_pm || nx != last_nx || ny != last_ny) {
	    if (marsm_pm)
...
int how_much;
{
	...
	if (!XtIsManaged(mmform_w) && !wantstats)
	    return;
...
#define ZCORD(z)        ((int)(((int)ny)/2.0 +    MAPSCALE(z) + 0.5))
	...
	/* draw Mars in center with unit radius */
	i = ew*64*raddeg(asin(sin(degrad(mop->s_elong))/mop->s_sdist));
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: marsmmenu.c,v $ $Date: 1999/03/03 19:06:34 $ $Revision: 1.4 $ $Name:  $"};
