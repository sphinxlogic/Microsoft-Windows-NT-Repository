...
# full path to directory containing *.edb files, ppm.xe and gsc.
$dbdir = '/usr/local/telescope/archive/catalogs';

# generally no need to change anything from here on.
# ==================================================

# set $trace to 1 to see steps in $tracefile
$tracefile = 'xephemdbd.pl.tr';
$trace = 0;
...
# set to xephemdbd stderr log file output
$logfn = "xephemdbd.log";
...
!$trace || &afon (TR);
!$trace || print TR "xephemdbd.pl $$ started\n";
...
if (! -p $infifo && system ("mknod $infifo p")) {
...
if (system ("./xephemdbd -vlic $lkfile $infifo $dbdir >> $logfn 2>&1")) {
...
# @(#) $RCSfile: xephemdbd.pl,v $ $Date: 1998/03/20 01:05:58 $ $Revision: 1.1 $ $Name:  $
