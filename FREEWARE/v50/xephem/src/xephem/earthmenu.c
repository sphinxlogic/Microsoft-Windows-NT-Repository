...
static void e_unmap_cb P_((Widget w, XtPointer client, XtPointer call));
...
static Widget objX_w;	/* pulldown menu widgets for ObjX */
static Widget objY_w;	/* pulldown menu widgets for ObjY */
static Widget objZ_w;	/* pulldown menu widgets for ObjZ */
...
typedef enum {		/* client codes for e_tb_cb and indices into wants[] */
    TRAIL, TRACK, GRID, OBJ, SITES, SUNLIGHT, LOWPREC, LIVEDRAG,
...
static Site *esp;		/* Site info, else NULL */
...
static ESatStatTable estattable[] = {
    {"ESat.SubLat",   "Sub Latitude:", ES_SUBLAT, "Latitude of object now"},
    {"ESat.SubLong",  "Sub Longitude:",ES_SUBLONG,"Longitude of object now"},
    {"ESat.Elevation",NULL,      ES_ELEV,  "Height of object above sea level"},
    {"ESat.Range",    NULL,	 ES_RANGE, "Distance from Main location"},
    {"ESat.RangeRate",NULL,	 ES_RANGER, "Range distance rate of change"},
    {"ESat.Eclipsed", "In Sun Shadow:",ES_ECLIPSED, "Whether object is now in Sun shadow"}
};
...
static unsigned char wxgifr[256], wxgifg[256], wxgifb[256]; /* gif colors */
static unsigned long wxxpix[256];	/* X pixels, indexed by wxgifpix */
...
e_manage()
{
	...
	if (XtIsManaged(eform_w)) {
	    ...
	    if ((eswasman = XtIsManaged(esform_w)) != 0)
...
Cursor c;
{
	...
	if (eform_w && (win = XtWindow(eform_w)) != 0) {
	...
	if (esform_w && (win = XtWindow(esform_w)) != 0) {
...
e_create_form()
{
	...
	static BtnInfo ctl[] = {
	    ...
	    {"Movie",   "Movie Demo",    e_movie_cb,   0,             NULL,
	    	"Start/Stop a simple time-lapse animation"},
	...
	static BtnInfo view[] = {
	    ...
	    {"lowprec",  "Coarse+Faster",e_tb_cb, LOWPREC,       NULL,
	    	"Draw map using lower resolution, but faster"},
	    {"Sep2",     NULL,          NULL,     0,             NULL},
	};
	static BtnInfo obj[] = {
	    {"Sep2",   NULL,           NULL,         0,           NULL},
	    {"track",  "Track Object", e_tb_cb,      TRACK,       &track_w,
	    	"Keep map centered on object after each Main Update"},
	    {"object", "Show Object",  e_tb_cb,      OBJ,         &showobj_w,
	    	"Show 0, 30, 60 and 90 degree local altitude rings for object"},
	    {"trail",  "Show Trail",   e_tb_cb,      TRAIL,       NULL,
	    	"Whether to display an existing time track for the object"},
	...
	set_something (eform_w, XmNcolormap, (XtArgVal)xe_cm);
	XtAddCallback (eform_w, XmNhelpCallback, e_help_cb, 0);
	XtAddCallback (eform_w, XmNunmapCallback, e_unmap_cb, 0);
	...
	pd_w = XmCreatePulldownMenu (mb_w, "ViewPD", args, n);
	    ...
	    i = 0;
	    i += XmToggleButtonGetState(cyl_w) ? 1 : 0;
	    i += XmToggleButtonGetState(sph_w) ? 1 : 0;
	    i += XmToggleButtonGetState(wxm_w) ? 1 : 0;
	    if (i != 1) {
		...
		XmToggleButtonSetState (wxm_w, False, False); 
	    }
	    if (XmToggleButtonGetState(cyl_w)) {
		projection = CYLINDRICAL;
		XtUnmanageChild (wxreload_w);
	    } else if (XmToggleButtonGetState(sph_w)) {
	    ...
	    wtip (w, "Display a dialog with additional object statistics");
	    XtManageChild (w);
	...
	pd_w = XmCreatePulldownMenu (mb_w, "ObjectPD", args, n);
	    ...
	    wtip (w, "Open dialog with which to define a time track");
	    XtManageChild (w);
		...
		wtip(objcb_w,"Name of current object -- use cascade to change");
...
e_create_esform()
{
	...
	for (i = 0; i < XtNumber (estattable); i++) {
	    ...
	    /* create the labels.
	     * defer setting strings for the ones that depend upon preferences.
	     */
	    n = 0;
	    str = 0;
	    if (stp->label) {
	...
	wtip (e_sdt_w, "Date and Time for which data are computed");
...
/* callback when the main form in unmapped */
/* ARGSUSED */
static void
e_unmap_cb (w, client, call)
Widget w;
XtPointer client;
XtPointer call;
{
	if ((eswasman = (esform_w && XtIsManaged(esform_w))) != 0)
	    XtUnmanageChild(esform_w);
...
e_print ()
{
	...
	if (projection == SPHERICAL) {
	    XPSXBegin (win, wb, hb, 2*r, 0, 1*72, 10*72, (int)(6.5*72));
	} else {
	    if (w >= h)
		XPSXBegin (win, 0, 0, w, h, 1*72, 10*72, (int)(6.5*72));
	    else {
		int pw = (int)(72*6.5*w/h+.5);	/* width on paper when 6.5 hi */
		XPSXBegin (win, 0, 0, w, h, (int)((8.5*72-pw)/2), 10*72, pw);
	    }
	}
...
Boolean *continue_to_dispatch;
{
	...
	if ((b2p || m2) && wants[LIVEDRAG]) {
	    ...
	    lasty = wy;
	    e_setelatlng (lt, lg);
	    esp = NULL; /* site code is certainly voided */
...
XtPointer call;
{
	...
	op = db_basic (OBJZ);
	if (op->o_type != UNDEFOBJ) {
	    set_xmstring (objZ_w, XmNlabelString, op->o_name);
	    XtManageChild (objZ_w);
	} else
	    XtUnmanageChild (objZ_w);
}
...
XtPointer call;
{
	...
	case WXMAP:
		...
		XmToggleButtonSetState (cyl_w, False, False);
		XmToggleButtonSetState (sph_w, False, False);
		XmToggleButtonSetState (wxm_w, True, False);
		XtSetSensitive (lat_w, False);
...
e_set_dasize()
{
	int dw = 9*DisplayWidth(XtD, DefaultScreen(XtD))/10;
	...
	case SPHERICAL:
	    ...
	    nwid = nhei = (Dimension)floor(sqrt(a) + 0.5);
	    break;

	case CYLINDRICAL:
	    /* same area but with CYLASPECT aspect */
	    nwid = (Dimension)floor(sqrt(a*CYLASPECT) + 0.5);
	    if ((int)nwid > dw) {
		nwid = (Dimension)dw;
		nhei = (Dimension)floor((double)nwid/CYLASPECT + 0.5);
	    } else
		nhei = (Dimension)floor((double)nwid/CYLASPECT + 0.5);
	    break;
...
XtPointer call;
{
	...
	XmToggleButtonSetState (track_w, False, True);
	esp = pu_info.pu_sp;	/* might be NULL */
	e_all (mm_get_now());
}
...
{
	static int dirty;
	int go = ev->type != ButtonRelease;
	double lt, lg;

	if (go && e_uncoord (projection, r, xb, yb, x, y, &lt, &lg)) {
...
Now *np;
{
	...
	XmScaleSetValue (long_w, (int)floor(raddeg(-elng))); /* stay <= +179 */
...
/* fetch the weather map, and fill wxgifpix, wxgifr/g/b[] and wxxpix[].
 * return 0 if ok, else -1
 */
static int
e_getwxgif()
{
...
#else
	fd = openh ("Sys$Scratch:latest_cmoll.gif", O_RDONLY);
#endif
	if (fd >= 0) {
	...
	if (wxgifpix) {
	    free (wxgifpix);
	    wxgifpix = NULL;
	    XFreeColors (dsp, xe_cm, wxxpix, 256, 0);
	}
	i = explodeGIF(rawgif,nrawgif,&w,&h,&wxgifpix,wxgifr,wxgifg,wxgifb,buf);
	if (i < 0) {
	    if (wxgifpix) {
		free (wxgifpix);
		wxgifpix = NULL;
	...
	for (i = 0; i < 256; i++) {
	    xc.red   = ((wxgifr[i] & 0xf8) << 8) | 0x7ff;
	    xc.green = ((wxgifg[i] & 0xf8) << 8) | 0x7ff;
	    xc.blue  = ((wxgifb[i] & 0xf8) << 8) | 0x7ff;
	    if (XAllocColor (dsp, xe_cm, &xc))
		wxxpix[i] = xc.pixel;
	    else {
		xe_msg ("Can not get weather map colors", 1);
		free (wxgifpix);
		wxgifpix = NULL;
		if (i > 0)
		    XFreeColors (dsp, xe_cm, wxxpix, i, 0);
		return (-1);
	    }
	}
...
short *xp, *yp;		/* gif location */
{
	...
	*yp = (WXM_TY+WXM_BY)/2 - tmp*(WXM_BY-WXM_TY)/2;
	range (&L, 2*PI);
	if (L > PI)
	    L -= 2*PI;
	tmp = L/PI*sqrt(1. - tmp*tmp);
	*xp = (WXM_LX+WXM_RX)/2 + tmp*(WXM_RX-WXM_LX)/2;
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: earthmenu.c,v $ $Date: 1999/10/25 21:00:10 $ $Revision: 1.14 $ $Name:  $"};
