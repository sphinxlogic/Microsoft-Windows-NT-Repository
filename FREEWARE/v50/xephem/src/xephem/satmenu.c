...
static void sm_unmap_cb P_((Widget w, XtPointer client, XtPointer call));
...
sm_manage ()
{
	...
	if (XtIsManaged(satform_w)) {
	    ...
	    if ((sswasman = XtIsManaged(ssform_w)) != 0)
...
Cursor c;
{
	...
	if (satform_w && (win = XtWindow(satform_w)) != 0) {
...
sm_create_form_w()
{
	...
	set_something (satform_w, XmNcolormap, (XtArgVal)xe_cm);
	XtAddCallback (satform_w, XmNhelpCallback, sm_help_cb, 0);
	XtAddCallback (satform_w, XmNunmapCallback, sm_unmap_cb, 0);
...
sm_create_tvform_w()
{
	...
	XtSetArg (args[n], XmNdefaultPosition, False); n++;
	XtSetArg (args[n], XmNcolormap, xe_cm); n++;
	stform_w = XmCreateFormDialog (toplevel_w, "SaturnTV", args, n);
...
XtPointer call;
{
	...
	if (!st_pm || nx != last_nx || ny != last_ny) {
	    if (st_pm)
...
/* ARGSUSED */
static void
sm_unmap_cb (w, client, call)
Widget w;
XtPointer client;
XtPointer call;
{
	if ((sswasman = (ssform_w && XtIsManaged(ssform_w))) != 0)
	    XtUnmanageChild (ssform_w);
        if (sm_pm) {
...
XtPointer call;
{
	...
	default:
        ...
        XGetGeometry(dsp, win, &root, &x, &y, &nx, &ny, &bw, &d);
	if (!sm_pm || nx != last_nx || ny != last_ny) {
	    if (sm_pm)
...
#define	ZCORD(z)	((int)(((int)ny)/2.0 +    MAPSCALE(z) + 0.5))
	...
	/* draw saturn in center with unit radius */
	i = ew*64*raddeg(asin(sin(degrad(sop->s_elong))/sop->s_sdist));
	...
	orw = (int)MAPSCALE(2*ORR);
	irx = nx/2 - MAPSCALE(IRR);
	iry = ny/2 - MAPSCALE(IRR);
	orx = nx/2 - MAPSCALE(ORR);
	ory = ny/2 - MAPSCALE(ORR);
	b = 64*raddeg(acos(1./IRR));
	XDrawArc (dsp, st_pm, s_xgc, irx, iry, irw, irw, b-i, -(180*64+2*b));
	b = 64*raddeg(acos(1./ORR));
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: satmenu.c,v $ $Date: 1999/03/03 19:06:34 $ $Revision: 1.9 $ $Name:  $"};
