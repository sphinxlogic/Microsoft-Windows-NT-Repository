...
*  pseudo-terminal.							*
*									*
*  JUMP_ALPHA contains Alpha architecture-specific code for JUMP.	*
*.......................................................................*
...
{ Use the logical UCB address in the CCB to get to the physical UCB. }
  ...
  VAR Inx : UNSIGNED := 0 ;
      Ccb_Chan	   : [VOLATILE] UNSIGNED := 0 ;
      Ccb_Chan_Ptr : [VOLATILE] Unsigned_Ptr := NIL ;

  BEGIN
  REPEAT
    Ccb_Chan_Ptr::UNSIGNED := CTL$GA_CCB_TABLE + Inx + CCB$L_CHAN ;
    Getmem (Ccb_Chan,Ccb_Chan_Ptr) ;
    Inx := Inx + CCB$K_LENGTH ;
  UNTIL Ccb_Chan = Chan_Num ;

  Ucb::UNSIGNED := CTL$GA_CCB_TABLE + Inx - CCB$K_LENGTH +
		   CCB$L_UCB ;			{ Logical UCB address }
