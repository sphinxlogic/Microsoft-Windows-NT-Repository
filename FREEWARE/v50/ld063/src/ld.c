/*
  ...
  Commands:
	...
	- LD TRACE [/ACCURATE] [/FDT] [/SIZE=xxx] [/RESET] LDan:
	...
	- LD SHOW/TRACE [/STATUS] [/RESET] [/OUTPUT=Filespec] [/INPUT=filespec]
		...
		[/FUNCTION[=TEXT,HEX]] [/ACCURATE] [/FDT] LDan:
...
void trace()
{
	...
	if (getqual(&sw_fdt) > 0)	/* /FDT ? */
	    func |= LDIO_M_FDTTRACE;
	if (getqual(&sw_accurate) > 0)	/* /ACCURATE ? */
	    func |= LDIO_M_ACCURATE;
	stat = sys$assign(&sdev, &chan, 0, 0, 0);	/* Assign channel */
	signal_error(stat, 0);
...
/* Get number of tracebuffers */
	...
	signal_error(stat, iosb.status);
	fdt_active = (iosb.status == SS$_WASSET);
	...
	do {
		...
		if (fdt_active)
		    lib$signal(&ld_fdtactive);	/* FDT tracing active */
		stat = sys$qiow(0, chan, IO$_LD_CONTROL, &iosb, 0, 0,
				0, 0, 0, 0, 0, LDIO_GET_CONNECTION);
		signal_error(stat, iosb.status);
		if (iosb_flags.accurate)
		    lib$signal(&ld_accurate);	/* Accurate timing active */
		break;
	    } else {
...
int overrun;
{
    ...
    static int pid, lbn, bytecount, fdt, accurate;
...
*/
	...
	*record = '\0';		/* Initially nothing */
	...
	if ((p->iosb[0] != SS$_FDT_COMPL) || ((p->iosb[0] == SS$_FDT_COMPL) && fdt > 0)) {
	    *record = '\0';
	    ...
	    if (iosb > 0) {		/* I/O status block */
		    ...
		    sprintf(tmp, "%08.8X %08.8X ", p->iosb[0], p->iosb[1]);
	            ...
	            if ((p->iosb[0] == SS$_FDT_COMPL) && (p->iosb[1] == 0) && (p->lbn == 0) && (p->bcnt == 0))
		        sprintf(tmp,"%-8s","FDT");
		    else
...
void process_input()
{
    ...
    if (((infilei = open(infilename, O_RDONLY, 0, "dna=LD_TRACE.DAT", "ctx=bin")) < 0) ||
	((infile = fdopen(infilei, "rb")) == NULL))
...
int what;
{
	...
	*str = '\0';;
	...
	case IO$_DELETE:
	case IO$_ACPCONTROL:
	    chk_mod(&modifier, IO$M_ACCESS, "ACCESS", str);
	    chk_mod(&modifier, IO$M_CREATE, "CREATE", str);
	    chk_mod(&modifier, IO$M_DELETE, "DELETE", str);
	    chk_mod(&modifier, IO$M_DMOUNT, "DMOUNT", str);
	    chk_mod(&modifier, IO$M_EXFUNC, "EXFUNC", str);
	    break;
	default:
	    break;
...
int single;
{
	...
	if (getqual(&sw_log) > 0)	/* /LOG ? */
	    lib$signal(&ld_nowdisconn, 2, devstr, ldfile);	/* Show what we did */
    }
    return (1);
}
...
int *size;
{
    ...
    if (getqual(&sw_contiguous) > 0) /* /CONTIGUOUS ? */
        ld_fab.fab$l_fop |= FAB$M_CTG;
