...
#include <stdio.h> /*###jp### inserted 22.10.94*/
...
#ifdef MESSAGES
    if (horizontal) {INFMESSAGE(!!!!!!!!!!! horizontal created) }
    else            {INFMESSAGE(!!!!!!!!!!! vertical created) }
#endif
    if (horizontal) {
    ...
    Boolean destroy_scrollbars;	/* destroy un-needed scrollbars? */
{
    ...
    BEGINMESSAGE(ComputeLayout)
    if (child == (Widget) NULL) {INFMESSAGE(no child)ENDMESSAGE(ComputeLayout) return;}
...
/*enddef*/
	...
	if (!needshoriz) {
		...
		XtDestroyWidget( bar ); INFMESSAGE(detroyed horizontal scrollbar)
	...
	if (!needsvert) {
	    ...
	    if (destroy_scrollbars) {
		XtDestroyWidget( bar ); INFMESSAGE(detroyed verical scrollbar)
    ...
    XtPointer call_data;
{
    ...
    BEGINMESSAGE(ThumbProc)
    if (child == NULL) {
       INFMESSAGE(no child to scroll)
       ENDMESSAGE(ThumbProc)
       return;
    }
    SMESSAGE(XtName((Widget)widget))

    if (widget == w->viewport.horiz_bar) {
       IMESSAGE(child->core.width)
     ...
     XtWidgetGeometry *request, *reply_return;
{
  ...
  XtDestroyWidget(sb); INFMESSAGE(destroyed scrollbar)
    ...
    XtWidgetGeometry *request, *reply;
{
    ...
    if (request->request_mode & XtCWQueryOnly) {
      INFMESSAGE (query only)
      result = QueryGeometry(w, request, reply);
...
#   ifdef MESSAGES
       ...
       if (rWidth)  {INFIMESSAGE(width request:,request->width) }
       if (rHeight) {INFIMESSAGE(height request:,request->height) }
#   endif
	...
	if (w->viewport.allowhoriz && request->width > w->core.width) {
	    ...
	    if ((bar = w->viewport.horiz_bar) == (Widget)NULL)
		bar = CreateScrollbar( w, True );
            INFIIMESSAGE(need horiz. scrollbar:,bar->core.height,bar->core.border_width)
	...
	*reply = allowed;
	result = XtGeometryAlmost;
        INFMESSAGE(intermediately agree XtGeometryAlmost) 
    }
    else {
	...
	ComputeLayout( (Widget)w,
       ...
       /*#remark# ###jp### 1/95*/
       /* ComputeLayout moves and resizes the child as needed,
          therefore XtGeometryDone should be returned to tell
          XtMakeResizeRequest to not configure the widget's window again.
          johannes plass, 1/95
       */
       INFMESSAGE(returning XtGeometryYes) 
       result = XtGeometryYes;
    }
    INFIIMESSAGE(reply to child,reply->width,reply->height)
    ...
    Dimension width, height;
{
    ...
    XtGeometryResult result;
    BEGINMESSAGE(GetGeometry)

    INFIIMESSAGE(current size:,w->core.width,w->core.height)
    if (width == w->core.width && height == w->core.height) {
        INFMESSAGE(no need for resize attempt) ENDMESSAGE(GetGeometry)
	return False;
    }
	...
	if (w->core.width != 0) {
	    if (w->core.height != 0) {
               INFMESSAGE(realize call; will use current size) ENDMESSAGE(GetGeometry)
               return False;
            }
	    geometry.width = w->core.width;
	}
    ...
    INFIIMESSAGE(requesting:,geometry.width,geometry.height)
    result = XtMakeGeometryRequest(w, &geometry, &return_geom);
    if (result == XtGeometryAlmost) {
        INFMESSAGE(parent agreed almost)
        INFIIMESSAGE(requesting now:,return_geom.width,return_geom.height)
	result = XtMakeGeometryRequest(w, &return_geom, (XtWidgetGeometry *)NULL);
    }
#   ifdef MESSAGES
       if (result == XtGeometryYes) {INFIIMESSAGE(acknowledged:,w->core.width,w->core.height)}
       else                         {INFIIMESSAGE(denied; use size:,w->core.width,w->core.height)}
#   endif
    ENDMESSAGE(GetGeometry)
