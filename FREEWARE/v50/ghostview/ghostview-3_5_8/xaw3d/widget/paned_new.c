...
/*
 * this code was modified in 1/94 by Johannes Plass,
...
#   include <XAW_DIRECTORY/PanedP.h>
#else
...
static XtResource resources[] = {
    {XtNinternalBorderColor, XtCBorderColor, XtRPixel, sizeof(Pixel),
	 offset(internal_bp), XtRString, (XtPointer) XtDefaultForeground},
    {XtNinternalShadowColor, XtCBorderColor, XtRPixel, sizeof(Pixel),
	 offset(internal_shadow), XtRString, (XtPointer) XtDefaultForeground},
...
Boolean shrink;
{
    ...
    BEGINMESSAGE(ChoosePaneToResize)
...
Direction dir;
{
    ...
    BEGINMESSAGE(LoopAndRefigureChildren)
 ...
 */
	...
	if (pane == NULL) {
	    ...
	    if (pane == NULL) {
               ENDMESSAGE(LoopAndRefigureChildren)
...
Direction dir;
{
    ...
    BEGINMESSAGE(RefigureLocations)

    if (pw->paned.num_panes == 0 || !pw->paned.refiguremode) {
       ENDMESSAGE(RefigureLocations)
       return;
    }
/*
...
PanedWidget pw;
{
    ...
    BEGINMESSAGE(CommitNewLocations)
    ForAllPanes(pw, childP) {
	register Pane pane = PaneInfo(*childP);
...
#define _DrawLine(pw, gc, x1, y1, x2, y2)\
    XDrawLine(XtDisplay(pw), XtWindow(pw), gc, x1, y1, x2, y2)
#define DrawInternalBorders(pw) _DrawInternalBorders((pw),  (pw)->paned.normgc,(pw)->paned.shadowgc);
#define EraseInternalBorders(pw) _DrawInternalBorders((pw), (pw)->paned.invgc, (pw)->paned.invgc);

static void
_DrawInternalBorders(pw, gc,shadowgc)
   PanedWidget	pw;
   GC		gc;
   GC		shadowgc;
{
   ...
   unsigned int	offset;
    ...
    BEGINMESSAGE(_DrawInternalBorders)

    if ((pw->core.background_pixel == pw->paned.internal_bp) ||
        ((unsigned int)(pw->paned.internal_bw < 1))) {
       ENDMESSAGE(_DrawInternalBorders)
       return;
    }

    offset = (unsigned int) ((pw->paned.internal_bw+1)/2);
    ForAllPanes(pw, childP) {
        if (IsVert(pw)) {
           x1 = 0; x2 = (int) pw->core.width;
           y1 =    y2 = (*childP)->core.y - offset;
           xoff = 0; yoff = 1;
        }
        else {
           x1 =    x2 = (*childP)->core.x - offset;
           y1 = 0; y2 = (int) pw->core.height;
           xoff = 1; yoff = 0;
        }
	_DrawLine( pw, gc, x1, y1, x2, y2);
        if (pw->paned.internal_bw > 1)
	   _DrawLine( pw, shadowgc, x1+xoff, y1+yoff, x2+xoff, y2+yoff);
    }
    ENDMESSAGE(_DrawInternalBorders)
}
   ...
   Boolean	erase;
{
   ...
   ForAllPanes(pw, childP) {
      ...
      if ( erase || (pane->olddelta != pane->delta) ) {
         on_size = (unsigned int) ((pw->paned.internal_bw+1)/2);
	 if (!erase) {
	    on_loc = PaneInfo(*childP)->olddelta - (int) on_size;
            _DrawRect( pw, pw->paned.flipgc,on_loc, off_loc, 1, off_size);
         }
...
Direction dir;
{
    ...
    BEGINMESSAGE(StartGripAdjustment)
 ...
 */
    ...
    ENDMESSAGE(StartGripAdjustment)
}
...
int loc;
{
    ...
    BEGINMESSAGE(MoveGripAdjustment)
...
PanedWidget pw;
{
    BEGINMESSAGE(CommitGripAdjustment)
    EraseTrackLines(pw);
    CommitNewLocations(pw);
 ...
 */
    ...
    ENDMESSAGE(CommitGripAdjustment)
}
...
PanedWidget pw;
{
   ...
   BEGINMESSAGE(ManageAndUnmanageGrips)
   ...
   ENDMESSAGE(ManageAndUnmanageGrips)
}
   ...
   Widget	w;
{
   ...
   /* Attach a shadow to the internal border using shadow color. */
   values.foreground = pw->paned.internal_shadow;	
   pw->paned.shadowgc = XtGetGC(w, valuemask, &values);
   ...
   Dimension	off_size;
{
   ...
   BEGINMESSAGE(SetChildrenPrefSizes)
   ForAllPanes(pw, childP)
   ...
   ENDMESSAGE(SetChildrenPrefSizes)
}
...
PanedWidget pw;
{
    ...
    BEGINMESSAGE(ChangeAllGripCursors)
    ForAllPanes(pw, childP) {
	Arg arglist[1];
	Cursor cursor;
    ...
    ENDMESSAGE(ChangeAllGripCursors)
}
   ...
   ForAllChildren(pw, childP) 
      if ( IsPane(*childP) )
         if ( XtIsManaged(*childP) ) {
            ...
            if (HasGrip(*childP)) PaneInfo(pane->grip)->position = pw->paned.num_panes;
   ...
   Cardinal	*num_args;
{
   ...
   if ( (old_pw->paned.internal_bp     != new_pw->paned.internal_bp) ||
        (old_pw->paned.internal_shadow != new_pw->paned.internal_shadow) ||
   ...
   if ((old_pw->paned.grip_indent != new_pw->paned.grip_indent) && (XtIsRealized(new))) {
...
#endif
{
    ...
    BEGINMESSAGE(XawPanedSetMinMax)
    pane->min = min;
    pane->max = max;
    RefigureLocationsAndCommit( widget->core.parent );
    ENDMESSAGE(XawPanedSetMinMax)
}
...
{
    Pane pane = PaneInfo(widget);
    BEGINMESSAGE(XawPanedGetMinMax)
    *min = pane->min;
    *max = pane->max;
    ENDMESSAGE(XawPanedGetMinMax)
}
...
#endif
{
    BEGINMESSAGE(XawPanedSetRefigureMode)
    ((PanedWidget) w)->paned.refiguremode = mode;
    RefigureLocationsAndCommit( w );
    ENDMESSAGE(XawPanedSetRefigureMode)
}
...
#endif
{
    BEGINMESSAGE(XawPanedGetNumSub)
    ENDMESSAGE(XawPanedGetNumSub)
...
#endif
{
    BEGINMESSAGE(XawPanedAllowResize)
    PaneInfo(widget)->allow_resize = allow_resize;
    ENDMESSAGE(XawPanedAllowResize)
}
