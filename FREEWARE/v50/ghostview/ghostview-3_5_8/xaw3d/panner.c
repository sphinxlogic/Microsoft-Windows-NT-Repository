...
#ifdef VMS
#   include <X11_DIRECTORY/IntrinsicP.h>	/* for toolkit routines */
...
#   endif
#   include <XMU_DIRECTORY/CharSet.h>		/* for XmuCompareISOLatin1() */
#   include <XAW_DIRECTORY/XawInit.h>		/* for XawInitializeWidgetSet */
#   include <XAW_DIRECTORY/PannerP.h>		/* us */
#   include <XMU_DIRECTORY/Misc.h>		/* for Min */
    ...
    Dimension *wp, *hp;
{
    ...
    BEGINMESSAGE(get_default_size)
    IIMESSAGE(pw->panner.canvas_width,pw->panner.canvas_height)
    *wp = PANNER_DSCALE (pw, pw->panner.canvas_width) + pad;
    *hp = PANNER_DSCALE (pw, pw->panner.canvas_height) + pad;
    IIMESSAGE(*wp,*hp)
    ENDMESSAGE(get_default_size)
}
