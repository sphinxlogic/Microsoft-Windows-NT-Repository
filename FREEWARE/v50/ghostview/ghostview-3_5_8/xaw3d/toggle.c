...
#   include <XAW_DIRECTORY/ToggleP.h>
#else
...
#if defined(VMS) && (XlibSpecificationRelease<5)
#   include <XAW_DIRECTORY/Xaw3D_VMS_PRAGMA_S.H>
       _EXTERN_ XtActionList VMS_command_actions_list;
...
#if defined(VMS) && (XlibSpecificationRelease<5)
  actions = VMS_command_actions_list;
  for (i = 0 ; i < SuperClass->core_class.num_actions ; i++) {
    ...
    if ( (class->toggle_class.Set != NULL) &&
	 (class->toggle_class.Unset != NULL) ) {
        INFMESSAGE(found actions)
        ENDMESSAGE(ClassInit)
	return;
    }
  }
#else  
...
/* We should never get here. */
  INFMESSAGE(did not find actions)
