...
#define MESSAGE_GETINDENT(indent,len) {					\
	char *result=getenv("MESSAGE_INDENT");				\
	if (result)	{ len=strlen(result); strcpy(indent,result); }	\
...
#define MESSAGE_PRINTF(format,value) {	\
   char indent[100]; unsigned short len;\
   MESSAGE_GETINDENT(indent,len);	\
   printf(indent);			\
   printf(format,value);		\
}
...
#define BEGIN_MESSAGE(txt)	{			\
   ...
   if (len <= 56) strcat(indent,"    ");		\
   MESSAGE_SAVEINDENT(indent)				\
}

#define END_MESSAGE(txt)	{			\
   char indent[100]; unsigned short len;		\
   MESSAGE_GETINDENT(indent,len)			\
   if (len>= 4) indent[len-4] = '\0';			\
