...
#   include <XAW_DIRECTORY/SmeThreeDP.h>
...
static XtResource resources[] = {
	...
	offset(sme_threeD.shadow_width), XtRImmediate, (XtPointer) 3},
	...
	offset(sme_threeD.top_shadow_contrast), XtRImmediate, (XtPointer) 25},
    {XtNbottomShadowContrast, XtCBottomShadowContrast, XtRInt, sizeof(int),
	offset(sme_threeD.bot_shadow_contrast), XtRImmediate, (XtPointer) 45},
    ...
    Widget new;
{
    ...
    extern void _Xaw3dComputeTopShadowRGB();

    _Xaw3dComputeTopShadowRGB (scn, 
			       tdo->object.parent->core.background_pixel,
			       tdo->sme_threeD.top_shadow_contrast,
			       &set_c);
    ...
    Widget new;
{
    ...
    Screen *scn = XtScreenOfObject (new);
    Colormap cmap = DefaultColormapOfScreen (scn);
    extern _Xaw3dComputeBotShadowRGB();

    _Xaw3dComputeBotShadowRGB (scn,
			       tdo->object.parent->core.background_pixel,
			       tdo->sme_threeD.bot_shadow_contrast,
			       &set_c);
    (void) XAllocColor (dpy, cmap, &set_c);
    ...
    Cardinal *num_args;
{
	...
	w->sme_threeD.shadow_width = 0;
	w->sme_threeD.top_shadow_pxmap = w->sme_threeD.bot_shadow_pxmap = 0;
	w->sme_threeD.top_shadow_GC = w->sme_threeD.bot_shadow_GC =
	    w->sme_threeD.erase_GC = (GC) 0;
    } else {

	if (w->sme_threeD.be_nice_to_cmap) {
	    AllocTopShadowPixmap (new);
	    AllocBotShadowPixmap (new);
	} else {
	    if (w->sme_threeD.top_shadow_pixel == 
		w->sme_threeD.bot_shadow_pixel) {
     ...
     Widget gw;
{
    ...
    if (w->sme_threeD.top_shadow_GC)
	XtReleaseGC (gw, w->sme_threeD.top_shadow_GC);
    if (w->sme_threeD.bot_shadow_GC)
	XtReleaseGC (gw, w->sme_threeD.bot_shadow_GC);
    if (w->sme_threeD.erase_GC)
    ...
    Widget gw;
{
     ...
     *
     *	don't do anything if the shadow_width is 0 or the
