...
#   include <XMU_DIRECTORY/CvtCache.h>
#   include <XMU_DIRECTORY/Drawing.h>
#   include <stdlib.h>                 /*###jp### inserted 20.10.94 */
#   include <stdio.h>                  /*###jp### inserted 20.10.94 */
...
#endif
{
    ...
    /*
     * Search order:
     *    1.  name if it begins with / or ./ (or, or for VMS, contains : or [)
     *    2.  "each prefix in file_paths"/name
     *    3.  BITMAPDIR/name
     *    4.  name if didn't begin with / or .
     * for VMS, both name and name.XBM are searched for each of these.
     */


#ifndef BITMAPDIR
#   ifdef VMS /*###jp###*/
#      define BITMAPDIR "DECW$BITMAPS:"    /* Can include private ones in */
                                           /* search list                 */
#   else
...
#           else
	       if (!(  (name[0] == '/') || ((name[0] == '.') && (name[1] == '/')) )) continue;
#	    endif
	    fn = (char *) name;
...
#ifdef VMS
	/*
	 * First try raw name, and then (if there is no filetype already)
	 * with .XBM on the end.
	 * NB. Despite the indentation, this loop includes most of the
	 * rest of this routine.
	 */
	if          (tpos= strrchr (name, '.'))
	   if      ((dpos= strrchr (name, ']')) && tpos<dpos) tpos= NULL;
	   else if ((dpos= strrchr (name, ':')) && tpos<dpos) tpos= NULL;
	   else if ((dpos= strrchr (name, '/')) && tpos<dpos) tpos= NULL;
	addxbm= (tpos==NULL);
	do {
#endif
...
#ifdef VMS
	if (addxbm) {
	    if (fn != filename) fn= strcpy (filename, fn);
	    strcat (filename, ".XBM");
	    addxbm= 0;
	} else break;
	} while (1);
