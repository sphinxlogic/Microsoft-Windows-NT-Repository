...
/*$Id: gdevdjet.c,v 1.1 2000/03/09 08:40:41 lpd Exp $ */
...
#define prev_row ((byte *)prev_row_words)
	...
	for (lnum = 0; lnum < num_rows; lnum++) {
	    ...
	    if (num_blank_lines == lnum) {	/* We're at the top of a page. */
		if (cmodes == mode_2p || cmodes == mode_0ns) {	/* Start raster graphics. */
		    fputs("\033*r1A", prn_stream);
		    for (; num_blank_lines; num_blank_lines--)
			fputs("\033*bW", prn_stream);
		} else if (cmodes == mode_3ns) {	/* Start raster graphics. */
		    fputs("\033*r1A", prn_stream);
	    ...
	    else if (num_blank_lines != 0) {	/* For Canon LBP4i and some others: */
		...
		if ((num_blank_lines < MIN_SKIP_LINES &&
		    ...
		    if (cmodes == mode_3ns && compression != 2) {	/* Switch to mode 2 */
			fputs(from3to2, prn_stream);
			compression = 2;
		    }
		    if (cmodes == mode_3 || cmodes == mode_3ns) {	/* Must clear the seed row. */
