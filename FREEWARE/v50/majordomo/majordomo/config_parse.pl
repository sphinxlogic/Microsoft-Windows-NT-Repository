...
# $Header: /sources/cvsrepos/majordomo/config_parse.pl,v 1.34.4.3 1995/01/07 17:30:56 rouilj Exp $
...
%known_keys = (
	'private_get',		'yes', # must be on list to use get
        ...
        'private_which',	'no', # must be on list to use which
        'private_info',		'no', # must be on list to use info
...
be on the mailing list in order to get files.",

'private_index',
"If set to yes, then the requestor must
be on the mailing list in order to get a file index.",

'private_who',
"If set to yes, then the requestor must
 be on mailing the list in order to use the who command.",

'private_which',
"If set to yes, then the requestor must
 be on the mailing list in order to get which info from that list.",
...
'admin_passwd',
"(Default is specified in the file <listname>.passwd)
...
%subsystem = ( 
	'private_get',		'majordomo',
        'private_index',	'majordomo',
...
# the keyword
%parse_function = (
	'private_get',		'grab_bool',
        'private_index',	'grab_bool',
...
&main'open_temp(OUT, "$listdir/$list\_config.out") ||
	 &main'abort("Can't create new config file $listdir/$list\_config.out");
...
open(IN, "< $listdir/$list\_config.out") ||
	 die "Can't read new config file $listdir/$list\_config.out";
...
close(MCONFIG);
close(IN);
unlink("$listdir/$list\_config.out");
}
...
sub handle_flag_files {
  ...
  if ( -e "$listdir/$list.private") {
      ...
      $main'config_opts{$list,"private_who"} = 1;
      $main'config_opts{$list,"private_which"} = 1; }
...
sub main'get_config {
  ...
  $main'config_opts{$list,''} = '1'; # set a flag to indicate that we 
				     ...
				     # this list
  print "adding site-wide defaults\n" if $debug > 1; 
  ...
  print "Overriding with existing config files\n" if $debug > 1;
  &handle_flag_files($listdir, $list); # this looks for files of 
                                       # the form listname.function

  #
  # Lock the config file. 
  $lockno = &main'shlock_with_retry("$listdir/$list\_config.LOCK");

#  &main'shlock("$listdir/$list\_config.LOCK",CONFIG_CONFIG);
  ...
  print "parsing config get_config($listdir, $list)\n" if $debug > 1;
  &main'lopen(CONFIG, "", "$listdir/$list.config")
			 || die "can't open $listdir/$list.config";
...
#
#  grab_regexp -- validates a regexp only a limited subset of a regexp
#                 (word characters, `\', `?' , `<', `>', `$', `*', `+' and
#		   `^' are allowed in the pattern). Must have leading and
#                  trailing /.
#
...
sub grab_restrict_post {
	...
	foreach $i (@files) {
	   push(@errors,
	   "Can't find restrict_post file $main'listdir/$i at line $.\n" )
			 if ( ! -e "$main'listdir/$i" );
       }
