static char  VERSION_STRING[]	= "@(#)pcal v4.7 - generate Postscript calendars";
/*
...
#endif
{
	...
	*strchr(p, ' ') = '\0';		/* terminate after version */
	 ...
	 */
	if (map_default == TRUE)
		mapfonts = lang_mapping[lang_active];
...
#endif
{
	char *parg, *opt, *p, *pass, tmpbuf[STRSIZ], c;
	FLAG_USAGE *pflag, *pf;
	int i, flag, sv_debug;
	...
	/* Walk argument list, ignoring first element (program name) */
 	...
 	while ((opt = *++argv) != NULL) {
		...
		case F_SETLANG:		/* select output lang for month/days */
			...
			do_undef(gen_lang_sym(lang_active));

			lang_active = LANG_DEFAULT;
			if (parg) {
			...
			do_define(gen_lang_sym(lang_active));
			break;
...
#endif
{
	...
	sprintf(Xtval, "%d", XTVAL);
	sprintf(Ytval, "%d", YTVAL);
	sprintf(Xsval, "%.3f", XSVAL);
	sprintf(Ysval, "%.3f", YSVAL);
