...
#endif
{
	...
	if (numeric_ok && isdigit(*cp))
		...
		/* accept month names in the default or active language */
		for (mm = JAN;
		     mm <= DEC &&
			ci_strncmp(cp, months_ml[LANG_DEFAULT][mm-1], MIN_MONTH_LEN) &&
			ci_strncmp(cp, months_ml[lang_active][mm-1], MIN_MONTH_LEN);
		     mm++)
			;
...
#endif
{
	...
	/* accept day names in the default or active language */
	for (w = SUN; w <= SAT; w++)
		if (ci_strncmp(cp, days_ml[LANG_DEFAULT][w], MIN_DAY_LEN) == 0 ||
		    ci_strncmp(cp, days_ml[lang_active][w], MIN_DAY_LEN) == 0)
			return w;
...
#endif
{
	...
	if ((pmonth = pyear->month[m]) == NULL) {	/* find/create month record */
		...
		for (i = 0; i < ARRAYSIZE(pmonth->day); i++)
			pmonth->day[i] = NULL;
	}
