...
#define DT_HOLIDAY	 9	/* predefined holiday */
...
#define NOT_HOLIDAY	-1	/* flag for end of holiday list */
...
/* if MAPFONTS is defined, generate PS code to re-map text fonts (cf.
 * fontmaps.ps) according to either the Roman8, ISO Latin1, or Esperanto
 * definitions of the 8-bit character set.  Note that the overhead to do
 ...
 * Roman8, ISO Latin1, or Esperanto definitions by defining MAPFONTS as
 * ROMAN8, LATIN1, or ESPERANTO (respectively) in the Makefile.
 */
...
#ifdef MAPFONTS
#if (MAPFONTS != ROMAN8 && MAPFONTS != LATIN1 && MAPFONTS != ESPERANTO)
#undef MAPFONTS	
...
/* scaling/translation factors based on A4 (595 x 842 point) page */

#define PAGEWIDTH	595		/* page dimensions (not rotated) */
#define PAGEHEIGHT	842

#define DAYWIDTH	108		/* width/height of date boxes */
#define DAYHEIGHT_L	 77		/* landscape or whole-year mode */

#define PAPERSIZE	"A4"
#else

/* scaling/translation factors based on Letter (612 x 792 point) page */

#define PAGEWIDTH	612		/* page dimensions (not rotated) */
#define PAGEHEIGHT	792

#define DAYWIDTH	100		/* width/height of date boxes */
#define DAYHEIGHT_L	 80		/* landscape or whole-year mode */

#define PAPERSIZE	"Letter"
#endif

#define XSVAL_L		1.0		/* scale/translate values (landscape) */
#define YSVAL_L		1.0
#define XTVAL_L		((PAGEHEIGHT - (7 * DAYWIDTH)) / 2)
#define YTVAL_L		(-85)		/* top of calendar boxes */

#define XSVAL_P		((double)PAGEWIDTH/(double)PAGEHEIGHT)	/* portrait */
#define YSVAL_P		((double)PAGEWIDTH/(double)PAGEHEIGHT)
#define XTVAL_P		((PAGEHEIGHT - (7 * DAYWIDTH)) / 2)
#define YTVAL_P		(PAGEWIDTH + YTVAL_L)

#ifdef ORIG_BOX_HEIGHT
			/* portrait mode (same scale as landscape) */
#define DAYHEIGHT_P	DAYHEIGHT_L
#else
			/* portrait mode (stretched vertically) */
#define DAYHEIGHT_P	(((DAYWIDTH * 7) + 3) / 6)
#endif
