...
#endif
{
	 ...
	 * with desired character remapping (Roman8, Latin1, or Esperanto,
	 * as requested)
	 */
	...
	for (i = JAN; i <= DEC; i++) {
		printf(i % 6 == 1 ? "\n\t" : " ");
		(void) print_word(months_ml[lang_active][i-1]);
	}
	printf(" ] def\n");
	...
	for (i = SUN; i <= SAT; i++) {
		...
		strcpy(tmp,
		   days_ml[lang_active][(i + first_day_of_week) % 7]);
		if (do_whole_year)
			tmp[ABBR_DAY_LEN] = '\0';
		(void) print_word(tmp);
		}
	...
	/* page width and height (unrotated) */
	...
	printf("/pageheight %d def\n", PAGEHEIGHT);
 	...
 	printf("/xsval %.3f def\n/ysval %.3f def\n", xsval, ysval);
 	printf("/xtval %d def\n/ytval %d def\n", xtval,
		(rotate == PORTRAIT && !do_whole_year) ?
			ytval + 6 * (DAYHEIGHT_P - DAYHEIGHT_L) :
			ytval);
	printf("/ytop %d def\n", -YTVAL_L);
...
#endif
{
	...
	print_html(months_ml[lang_active][this_month-1]);
	...
	for (day = 0; day < 7; day++) {
		char buf[40];
		strcpy(buf, days_ml[lang_active][(day + first_day_of_week) % 7]);
#if 0
...
#endif
{
	...
	for (i = 0, m = first_month, y = first_year;
		...
		print_html(months_ml[lang_active][m-1]);
		printf(" %d%s</th>\n", y, MONTHYEAR_W_POST);
	}
	printf("</tr>\n");
...
#endif
{
	...
	do {			/* loop until format character found */
		...
		case 'A':	/* %A : full weekday */
			...
			strcpy(buf, days_ml[lang_active][wkday]);
			if (c == 'a')
				buf[ABBR_DAY_LEN] = '\0';
			break;
		...
		case 'B':	/* %B : full month name */
			strcpy(buf, months_ml[lang_active][(work_month + adjust + 11) % 12]);
			if (c == 'b')
				buf[ABBR_MONTH_LEN] = '\0';
			break;
