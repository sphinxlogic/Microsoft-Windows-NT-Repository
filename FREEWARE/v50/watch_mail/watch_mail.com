...
$!
$	VERSION = "V04.01-001"		!update this when edits are made
$!
...
$!      send replies to addresses that do not already contain a transport
$!      prefix (e.g., SMTP%).  Otherwise, mail that cannot be replied to due
$!      to a network failure will remain in the NEWMAIL folder until the
$!      next time the batch job executes, at which time another attempt will
$!      be made to reply to it.
$!
...
$!
$!	Modifications:
$!        04.01 30SEP97 DAC     Fixed so that Nmail prefix is not used with an
$!                              address that includes a transport (e.g., SMTP%).
...
$	IF FOUND THEN GOTO DO_NOT_REPLY
$!
$! Now attempt to reply to the sender.  If the sender's address does not
$! include a transport specification (e.g., SMTP%), use Nmail (if available).
$!
$       STRING = SENDER
$       IF F$ELEMENT(1,"%",F$ELEMENT(0,"""",SENDER)) .EQS. "%" -
          THEN STRING = NMAIL_PREFIX + STRING
$       IF STRING - """" .NES. STRING THEN GOSUB DOUBLE_QUOTES
$	SET NOON
