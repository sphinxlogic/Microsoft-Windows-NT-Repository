...
#   include <file.h> /*###jp### inserted 21.10.94 */
#else
...
#ifdef VMS
#   include <X11_DIRECTORY/Xos.h> /* for O_RDONLY */
#   include <XAW_DIRECTORY/AsciiText.h>
#else
...
#     ifdef __DECC /* sys_errlist[errno] crashes on OpenVMS AXP ###jp### 18-10-94 */
         (void) sprintf( msg, "*** Error: %s ***",
	         (errno>0 && errno<__ERRNO_MAX) ?
	         strerror(errno) : "Can't open file" );
#     else
         (void) sprintf( msg, "*** Error: %s ***",
...
#ifdef VMS /*###jp### changed 24.10.94*/
       ((fid = open(str, O_RDONLY,0)) <= 0))
#else
...
# ifdef VMS  /*###jp### changed 21.10.94*/
  dir = (XawTextScanDirection)(int) ((caddr_t)((int)XawToggleGetCurrent(search->left_toggle) -
				R_OFFSET));
# else
