...
static XtResource resources[] = {
  ...
  {XtNthumbTopIndent, XtCThumbTopIndent, XtRDimension, sizeof(Dimension),
       Offset(thumb_top_indent), XtRImmediate, (XtPointer) 2},
  {XtNthumbSideIndent, XtCThumbSideIndent, XtRDimension, sizeof(Dimension),
       Offset(thumb_side_indent), XtRImmediate, (XtPointer) 1},
    ...
    Widget w;
{
    ...
    gcValues.foreground = FOREGROUND; gcValues.background = BACKGROUND_PIXEL;
    ...
    Cardinal *num_params;
{
    ...
    if ((position>=tsoar) && (position <= teoar)) {
	   ...
	   SCROLLMODE = ARROW_UP;
           CALLscrollProc(w,-ARROW_SCROLL_AMOUNT);
           DYNAMIC = INTERVAL;
	...
	SCROLLMODE = PAGE_UP;
        CALLscrollProc(w,-PAGE_SCROLL_AMOUNT);
        DYNAMIC = INTERVAL;
        SET_TIMER(w,DELAY);
