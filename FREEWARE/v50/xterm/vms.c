...
#define	BUFFERS		        6
#define	PAGE			512
/* now in DATA.H too!  Change in both places if need be */
#define VMS_TERM_BUFFER_SIZE	500

typedef struct tt_buffer TT_BUF_STRUCT;
...
struct mem_region
{
  struct tt_buffer *start;
  struct tt_buffer *end;
} ret_addr;  

int read_stopped = FALSE;
int write_stopped = FALSE;

/* DRM, these have to be NOT extern here
extern int tt_width;
...
static void tt_echo_ast(buff_addr)
     struct tt_buffer *buff_addr;
...
int tt_write(tt_write_buf,size)
     char *tt_write_buf;
     int size;
{
  int status;
  struct tt_buffer *echoBuff;
...
static void tt_read_ast(buff_addr)
     struct tt_buffer  *buff_addr;
{
  int status;
...
static void freeBuff (buff_addr)
     struct tt_buffer *buff_addr;
{
  int ast_stat;
  int status;
...
static void CloseDown(exit_status)
     int exit_status;
{
  int status;
