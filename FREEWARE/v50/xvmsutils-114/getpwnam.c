...
char *name;
{
   ...
   struct itmlist3 {
	...
	unsigned long  int retaddr;
   	...
   	{12, UAI$_USERNAME, (unsigned long)&UserName, 0},
	{8,  UAI$_PWD, 0, 0},
	{4,  UAI$_UIC, 0, 0},
	{32, UAI$_OWNER, (unsigned long)&UserOwner, 0},
	{32, UAI$_DEFDEV, (unsigned long)&UserDevice, 0},
	{64, UAI$_DEFDIR, (unsigned long)&UserDir, 0},
	{2,  UAI$_SALT, 0, 0},
	{4,  UAI$_ENCRYPT, 0, 0},
	{0, 0, 0, 0}
   };

   VMSNAME.dsc$w_length = strlen(name);
   VMSNAME.dsc$a_pointer = name;
   ItemList[0].retaddr = (unsigned long)&UserNameLen;
   ItemList[1].addr =  (unsigned long)&UserPwd;
   ItemList[2].addr = (unsigned long)&UicValue;
   ItemList[3].retaddr = (unsigned long)&UserOwnerLen;
   ItemList[4].retaddr = (unsigned long)&UserDeviceLen;
   ItemList[5].retaddr = (unsigned long)&UserDirLen;
   ItemList[6].addr = (unsigned long)&UserSalt;
   ItemList[7].addr = (unsigned long)&UserEncrypt;
   UserNameLen = 0;
