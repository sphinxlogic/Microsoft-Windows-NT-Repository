...
/** poviso: Sep 27 '96 R.S. **/
#ifdef POVISO
extern DBL EP_Best_Intersection;
#endif
/** --- **/
...
#ifdef MotionBlurPatch
DBL Trace_Primary_Ray_With_Blur(RAY *Ray, COLOUR Colour, DBL Weight, int x, INTERSECTION *Inter)
{
  COLOUR Colour2;
  DBL Weight2=Weight/opts.motionBlurCount;
  DBL depth;

  if(TimeStamp || opts.motionBlurCount<=1)
  {
    return Trace_Primary_Ray(Ray,Colour,Weight,x,Inter);
  }
  else
  {
    RAY BackupRay = *Ray; /* this is in case Trace_... messes with Ray */

    depth = Trace_Primary_Ray(Ray,Colour,Weight,x,Inter);

    if(/*!backtraceFlag &&*/ TimeStamp) /* we never call this with photons */
    {
      ...
      for (TimeStamp=1; TimeStamp<=opts.motionBlurCount; TimeStamp++)
      {
        ...
        {
          *Ray = BackupRay; /* restore in case Trace_... messed with it before */
          depth+=Trace_Primary_Ray(Ray, Colour2, Weight2, x, NULL);
          Add_Colour(Colour,Colour,Colour2);
        }
      }

      Scale_Colour(Colour,Colour,1.0/opts.motionBlurCount);
      depth /= opts.motionBlurCount;
    }
    TimeStamp = 0;
  }
  return depth;
...
DBL Trace_Primary_Ray (RAY *Ray, COLOUR Colour, DBL Weight, int x, INTERSECTION *Inter)
{
...
/** poviso: Sep 27 '96 R.S. **/
#ifdef POVISO
   EP_Best_Intersection=Best_Intersection.Depth;
#endif
/** --- **/
...
/** poviso: Sep 27 '96 R.S. **/
#ifdef POVISO
   EP_Best_Intersection=BOUND_HUGE;
#endif
/** --- **/
  ...
  if (Inter) *Inter = Best_Intersection;

#ifdef PostProcessPatch2
  refractDepth = savedRefractDepth;
  savedRefractDepth = 0;
  return (refractDepth+Best_Intersection.Depth);
#else
...
static int intersect_vista_tree(RAY *Ray, PROJECT_TREE_NODE *Tree, int x, INTERSECTION *Best_Intersection)
{
  ...
  /* Add Object-Ray options [ENB 9/97] */
    if (!Test_Flag(New_Intersection.Object, NO_IMAGE_FLAG) || In_Reflection_Ray == TRUE || backtraceFlag)
    {
      if (!Test_Flag(New_Intersection.Object, NO_REFLECTION_FLAG) || In_Reflection_Ray == FALSE || backtraceFlag) 
      {
#endif
