...
*  -------------------------------------------------------
*  ATTENTION:
*  This is an unofficial version of povray.c modified by
...
#ifdef POVISO
#include "isosrf.h"  /* R.S. July14,96 */
#endif
/** --- **/
#ifdef IsoBlobPatch
#include "isoblob.h" /* Lummox JR, July 99 */
#endif
#include "lathe.h"
#include "lighting.h"
#include "mem.h"
...
#ifdef GetTicksPatch
/*YS enabling time mesurement */
  extern DBL ChronoStart;
#endif
...
DBL tphoton, tphoton_total;
#ifdef PostProcessPatch
DBL tpost, tpost_total;
#endif
...
static void init_frame_vars(void);
...
#endif            /* ...would be a lot less hassle!! :-) AAC */
{
...
#ifdef df3OuputPatch
  /* disable continue trace if df3 output selected (PoD)*/
  ...
  /* do some cleanup */
  DestroyLightGroupTable();

  /* NK persist - Nov 1999 */
  /* make sure all sym tables are deleted */
  Terminate_Tokenizer(TRUE);
  /* NK ---- */
...
static void FrameRender()
{
  ...
  /* Create isoblob queue. */
  Init_Isoblob_Queue();
  /* Added by Lummox JR, July 1999 */
#endif

#ifdef TracePatch
   /* must initialize mesh code before parsing (for the trace function) */
   Initialize_Mesh_Code();
#endif

  Parse();

  /* NK opt 1999 - added this line, since we can change
    rendering window settings in the POV script now */
  fix_up_rendering_window();
  /* NK ---- */
  ...
  {
    Warning(0,"Warning: This rendering uses the following experimental features:\n");
    if (Experimental_Flag & EF_RADIOS) 
    {
       Warning(0," radiosity");
    }
    Warning(0,".\nThe design and implementation of these features is likely to\n");
    Warning(0,"change in future versions of POV-Ray.  Full backward compatibility\n");
    Warning(0,"with the current implementation is NOT guaranteed.\n");
    Warning(0,"!!! This MegaPov version of radiosity differs from the official version !!!\n");
  }
  ...
  /* 
    NK persist - must do this here, since
    init_vars() does not get called for every frame
  */
  Root_Object=NULL;
  /* NK ---- */
  ...
  open_output_file();
#ifdef PostProcessPatch
  if (opts.postProcess) Open_Post_File();
#endif
...
#ifdef PostProcessPatch
  if (opts.postProcess) Close_Post_File();
#endif
  ...
  Destroy_Blob_Queue();
#ifdef IsoBlobPatch
  Destroy_Isoblob_Queue();      /* Lummox JR, July 1999 */
#endif
  Destroy_Light_Buffers();
  Destroy_Vista_Buffer();
  Destroy_Bounding_Slabs();

  /* NK persist - terminate_pov will take care of destroying all of the
  objects.  for now we only destroy the unnamed ones if persistentAnimation
  is true */
  if (opts.persistentAnimation)
    Destroy_Frame(FALSE);
  else
    Destroy_Frame(TRUE);

  /* if not persistent, then destroy and re-create the light-group table */
  if (!opts.persistentAnimation)
  {
    DestroyLightGroupTable();
    InitLightGroupTable();
  ...
  if (opts.postProcess) 
  {
    Stage = STAGE_POST_PROCESS;

    START_TIME

    DoPostProcess(Actual_Output_Name);
    Deinitialize_PostProcessing();
    Delete_Post_File();

    /* Get the post-processing time. */
    STOP_TIME
    tpost = TIME_ELAPSED
...
static void fix_up_rendering_window()
{
  ...
  {
     Warning(0,"Preview_Start_Size must be a power of 2. Changing to %d.\n",temp);
     opts.PreviewGridSize_Start=temp;
  }

  if ((temp=closest_power_of_2((unsigned)opts.PreviewGridSize_End))!=opts.PreviewGridSize_End)
  {
     Warning(0,"Preview_End_Size must be a power of 2. Changing to %d.\n",temp);
...
static void fix_up_animation_values()
{
...
#ifdef df3OuputPatch
  opts.FrameSeq.ActualInitialFrame = opts.FrameSeq.InitialFrame;
  opts.FrameSeq.ActualFinalFrame = opts.FrameSeq.FinalFrame; /* PoD: needed for df3 output */
#endif
  ...
  else
  {
    ...
    ClockDiff = (float)(opts.FrameSeq.FinalClock-opts.FrameSeq.InitialClock);
...
#ifdef df3OuputPatch
      case 'd':
      case 'D' : Output_File_Handle = Get_DF3_File_Handle(); def_ext=".df3"; break;
#else
      case 'd' :
...
static void setup_output_file_name()
{
  ...
  if(opts.FrameSeq.FrameType!=FT_MULTIPLE_FRAME ||
#ifdef df3OuputPatch
     opts.Options & TO_STDOUT ||   opts.OutputFormat=='d' ||    opts.OutputFormat=='D')
#else
  ...
  if (strlen (opts.Output_Path) == 0)
  {
    ...
    /*
     * Modification By: Steven M. Schweda.
     *                  sms@antinode.org
     *
     * This fixes/cleans up some glitches for Unix style direcory paths
     * with OpenVMS.
     */
    #if defined(VMS) && defined(DECC)
      getcwd (opts.Output_Path, sizeof (opts.Output_Path) - 1, 0) ;
    #else
  ...
  if (opts.Options & DISKWRITE)
  {
    ...
    if (opts.Options & CONTINUE_TRACE)
    {
            ...
            READ_MODE) != 1)
      {
        ...
        Warning (0,"Error opening continue trace output file.\n");
...
******************************************************************************/
/* NK persist - Nov 1999  */
extern int alreadyInitializedSymTables;
...
static void init_vars()
{
  ...
  /*
  bug fix
  Mark Wagner <mark.wagner17@gte.net>
  reset number of waves between renders
  */
  Number_Of_Waves = 10;
  Number_Of_Files = 0;
  Number_of_istacks = 0;
  ...
  tphoton = tphoton_total = 0.0;
#ifdef PostProcessPatch
  tpost   = tpost_total   = 0.0;
#endif
...
static void init_frame_vars()
{
  ...
  */
  /* NK rad - changed rad brightess default to 1 
  opts.Radiosity_Brightness = 3.3;*/
  opts.Radiosity_Brightness = 1.0;
  /* NK rad - decreased default radiosity count
  opts.Radiosity_Count = 100;*/
  opts.Radiosity_Count = 35;
  /* NK rad - dist_max is always computed on the fly now - FYI */
  opts.Radiosity_Dist_Max = 0.0;   /* default calculated in Radiosity_Initialize */
  /* NK rad - default error bound is 2.23 - actually, it should ALWAYS be this
  opts.Radiosity_Error_Bound = .4;*/
  opts.Radiosity_Error_Bound = 1.8;
  /* NK rad - I don't like radiosity_gray - not realistic
  opts.Radiosity_Gray = .5;*/       /* degree to which gathered light is grayed */
  opts.Radiosity_Gray = 0.0;
  /* NK rad - this low_error_factor is fine */
  opts.Radiosity_Low_Error_Factor = 0.5;
  /* NK rad - default min_reuse is good */
  opts.Radiosity_Min_Reuse = 0.015;
  /* NK rad - nearest_count default to 4
  opts.Radiosity_Nearest_Count = 6;*/
  opts.Radiosity_Nearest_Count = 5;
  /* NK rad = set default recursion limit to 2
  ...
  opts.Reflection_Blur_Max = Max_Trace_Level;  /* Generally 5 */
  opts.Reflection_Blur_Max_ADC = 0.0;

  closed_flag = FALSE;
  Stop_Flag = FALSE;

#ifdef MotionBlurPatch
  opts.motionBlurCount = 1; /* turn off motion blur */
  opts.Radiosity_Use_Motion_Blur = FALSE;
#endif

#ifdef PostProcessPatch
  opts.postProcess = NULL;
  opts.postProcessFlags = PP_FLAG_NONE;
#ifdef PostProcessKeepDataPatch
  opts.keep_post_process_data_file = FALSE;
#endif
  opts.postProcessOverwriteOriginal = FALSE;
#endif
  ...
  photonOptions.photonMap.rangeSelector = 0.0;
  photonOptions.globalPhotonMap.rangeSelector = 0.0;
  photonOptions.mediaPhotonMap.rangeSelector = 0.0;
  ...
  alreadyBlurred = FALSE; /* for reflection blur */
...
void close_all()
{
  ...
  Deinitialize_Mesh_Code();
  Deinitialize_VLBuffer_Code();
  /* NK persist - destroy all objects, including named objects */
  Destroy_Frame(TRUE);
  /* NK ---- */
  Destroy_IStacks();
  FreeFontInfo();
