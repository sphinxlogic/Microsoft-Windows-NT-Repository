...
void UNIX_Process_Povray_Ini(void)
{
  ...
  /*
   * So we can keep POV-Ray and MegaPOV-Ray On The Same Machine Without
   * Conflicts We Will Look For "megapovray.ini" instead of "povray.ini"
   *
   * Robert Alan Byer <byer@mail.ourservers.net>
   * May 17, 2000
   */
# if defined(VMS) && defined(DECC)
    if (parse_ini_file("./megapovray.ini"))
# else
...
# endif
  ...
  if ((Option_String_Ptr = getenv("HOME")) != NULL)
  {
    ...
    /*
     * So we can keep POV-Ray and MegaPOV-Ray On The Same Machine Without
     * Conflicts We Will Look For ".megapovrayrc" instead of ".povrayrc"
     *
     * Robert Alan Byer <byer@mail.ourservers.net>
     * May 17, 2000
     */
#   if defined(VMS) && defined(DECC)
      strncat(ini_name, "/.megapovrayrc", FILE_NAME_MAX);
#   else
...
#   endif
   ...
   */
# if defined(VMS) && defined(DECC)
    strncat(ini_name, "/megapovray.ini", FILE_NAME_MAX);
# else
