...
/*
 * OpenVMS with DEC C will probably need <math.h> included.
 */
...
/*
 * OpenVMS with DEC C dosen't need the following prototypes.
 */
#if !defined(VMS) && !defined(DECC)
  int unlink(char *);
  char *getenv(char *);
 ...
 * OpenVMS users will define the library path at compile time.
 */
#if !defined(POV_LIB_DIR)
#  if defined(VMS) && defined(DECC)
#    include "vms_povlib_dir.h"
#  else
...
/*
 * OpenVMS with DEC C has "gettimeofday()" so we will keep the
 * definition for "PRECISION_TIMER_AVAILABLE"
 */
