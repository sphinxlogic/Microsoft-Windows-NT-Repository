	%TITLE "DECTALK-BANK_DEMO.BAS  --  BASIC APPLICATION GUIDELINE PROGRAM"
 !
 !			  BANK_DEMO.BAS
 ! 
 ...
 !		statement where the code determines which of 5 menu items
 !		are chosen.  The menu choices and menu message must be
 !		modified to reflect the menu items being implemented.
 !		The menu choices are:
 !
 !			1) RETAIL BANKING
 !			2) MORTGAGE RATE INFORMATION
 !			3) ATM LOCATIONS
 !			4) CREDIT-CARD INFORMATION
 !
 !		        ************ NOTE ************
 !		ERROR MESSAGES WILL NOT BE LOGGED TO THE OPERATOR
 !		TERMINAL (AS DESCRIBED BELOW) FOR THE NOAC BANKING
 !		DEMO BECAUSE NO TERMINAL WILL BE AVAILABLE (I.E. A
 !		CONSOLE OR OTHER SPARE TERMINAL) TO RECEIVE OPERATOR
 !		MESSAGES.  THEREFORE, IGNORE THE TEXT BELOW THAT
 !		DESCRIBES HOW TO ENABLE THE OPERATOR TERMINAL TO 
 !		RECEIVE ERROR MESSAGES FOR THIS DEMO.  IF AN ERROR
 !		OCCURS, THE DEMO WILL BE TERMINATED AND THE APPROPRIATE
 !		MESSAGE ENTERED IN THE TXAx.ERR file WHERE x IS THE 
 !		NUMBER OF THE TERMINAL LINE THE FAILED DEMO PROGRAM
 !		IS CONNECTED TO.
 !		        *********** END NOTE ***********
 !
 ...
 !
  ...
  dispatch_desk = "In an actual application, DECtalk would now transfer "&
	+ "your call to the dispatch-desk. This demonstration "&
	+ "just returns to the selection menu. "
  ...
  COMMON (application_specific)	LONG		voice_id, &
   ...
   msg_welcome$  = "Welcome to the Digital Telephone Banking Demonstration.  "
...
out:
 ...
 %INCLUDE "DTKDEF"
  ...
  main_menu_msg$ = "Please enter a command.  For help, press 0 followed by "&
		+ "the pound-key. To exit, press the star-key."

  main_menu$	= "For retail banking information, press 1, followed by the "&
	+ "pound-key. For Mortgage-rate information, press 2, followed by the "&
	+ "pound-key.  For ['eyt`iy'ehm] locations, press 3, followed by the "&
	+ "pound-key. For credit-card information, press 4, followed by the "&
	+ "pound-key. To repeat this message, press 0, followed by the "&
	+ "pound-key. To exit this section, press the star-key. "

  mortgage$  =    "Welcome to [hx'aemdaxn] National Bank's Mortgage-Line.  "&
                + "All of the following rates are subject to change. The "&
		+ "application fee is $250. 30 year rates with 10% [']down "&
		+ "are 8.9% with 3 [aen ax] half [p`oynts], 9.2% with 3 "&
		+ "[p`oynts], 9.5% with 2 [aen ax] half [p`oynts], 9.8% with "&
		+ "no [p`oynts]. 15 year rates with 10% [']down are 8.9% "&
		+ "with 2 [aen ax] half [p`oynts], 9.1% with 2 [p`oynts], and "&
		+ " 9.5% with no [p`oynts]. Adjustable rates are at 8.5% "&
		+ "fixed for 3 years, with 2% a year and 6% lifetime caps "&
		+ "thereafter. "
 ...
 !
	...
	ELSE
		...
		CASE = 	DTK$K_TRM_ONE			! banking demo
			stat% = banking
			IF (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
		...
		CASE = 	DTK$K_TRM_TWO			! mortgage rate info
		...
		CASE = 	DTK$K_TRM_THREE			! atm locations
			stat% = atm
			IF (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
		...
		CASE = 	DTK$K_TRM_FOUR			! credit card info
			stat% = credit_card
			IF (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
...
HERE:
 ...
 SUB end_call ()
  ...
  msg_goodbye$  = "Thank you for calling the DECtalk-demonstration.  "&
	...
	WHILE 1% = 1%
		...
		!  detected before DECtalk hungup the telephone.
		!
	        CALL set_timer(Time_45_sec)
	        stat% = DTK$READ_KEYSTROKE(voice_id, xx%, , timeout%)
	        CALL cancel_timer
 ...
 !		        ************ NOTE ************
 !		ERROR MESSAGES WILL NOT BE LOGGED TO THE OPERATOR
 !		TERMINAL FOR THE NOAC BANKING DEMO BECAUSE NO TERMINAL 
 ! 	        WILL BE AVAILABLE (I.E. A CONSOLE OR OTHER SPARE TERMINAL)
 !		TO RECEIVE OPERATOR MESSAGES.  THEREFORE, ALL THE CODE
 !		IN THIS SUBROUTINE THAT DEALS WITH LOGGING A MESSAGE
 !		TO AN OPERATOR TERMINAL HAS BEEN COMMENTED OUT.  IF AN
 !		ERROR OCCURS, THE DEMO WILL BE TERMINATED AND THE 
 !		APPROPRIATE MESSAGE ENTERED IN THE TXAx.ERR FILE WHERE
 !		x IS THE NUMBER OF THE TERMINAL LINE THE FAILED DEMO
