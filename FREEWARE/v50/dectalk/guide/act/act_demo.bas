	%TITLE "DECTALK-ACT-DEMO.BAS  --  BASIC APPLICATION GUIDELINE PROGRAM"
 !
 !			  ACT_DEMO.BAS
 ! 
 ...
 !			C) DISTRIBUTION
 !			D) LABOR REPORTING
 !			E) DECTALK PRODUCT INFORMATION
 !			F) FINANCIAL INFORMATION
 !			G) INFORMATION SERVICES
 !			H) SOFTWARE SUPPORT
 !
 !	NOTE:	The first five menu items have been chosen as the default.
 !
 ...
 !
  ...
  COMMON (application_specific)	LONG		voice_id, &
   ...
   msg_welcome$  = "Welcome to DECtalk.  Digital Voice Solutions can increase "&
 		+ "your business [prowdaxkt'ihvixtiy]. "
...
out:
 ...
 %INCLUDE "DTKDEF"
  ...
  main_menu$	= "To hear about DECtalk in banking, Press 1, followed by the "&
	+ "pound-key. To hear a field-service application, Press 2, followed "&
	+ "by the pound-key.  To hear a distribution-demonstration, Press 3, "&
	+ "followed by the pound-key.  To hear a time-reporting demonstration,"&
	+ " Press 4, followed by the pound-key.  To hear DECtalk "&
	+ "product-information, press 5, followed by the pound-key.  "&
	+ "To repeat this message, Press 0, followed by the pound-key.  "&
  ...
  soft_support$	= "There are three options for software-development. Digital "&
	+ "Software Services, Third party vendors, or customer internal "&
	+ "development.  For a list of third party software vendors, call the "&
	+ "Sales Support-Team, at 1, 800, 8 3 2, 6 2 7 7. [+]If you choose to "&
	+ "write your own application, we recommend the [`]DECtalk "&
	+ "[`]Application Development-Guide. The [`]DECtalk [`]Application "&
	+ "Development-Guide provides a comprehensive understanding of Voice "&
	+ "application issues.  Sample application software, intended to "&
	+ "simplify development, is provided. The [`]DECtalk [`]Application "&
	+ "Development-Guide may be ordered by calling the Educational "&
	+ "Services Sales Support-Team, at 1, 800, 3 3 2, 5 6, 5 6.  "
 ...
 GET_SHORT_READ:
	...
	ELSE	IF (terminator_value = DTK$K_TRM_NUMBER_SIGN)
		...
		ELSE IF ((terminator_value <> DTK$K_TRM_TIMEOUT) OR	&
			...
			!
		     THEN
				CALL speak_text(DTK$K_WAIT, main_menu_msg$)
		     END IF
	         END IF
	     END IF
	!
 ...
 !
	...
	ELSE
		...
		CASE = 	DTK$K_TRM_TWO			! field service demo
			stat% = field_service
			IF (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
		...
		CASE = 	DTK$K_TRM_THREE			! distribution demo	
			stat% = distribution
			IF (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
		...
		CASE = 	DTK$K_TRM_FOUR			! labor entry
			stat% = labor_reporting			
			IF (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
			END IF
 ...
 ! ---------------------------------------------------------------------------
 !   options available:
 !	this code can be put into the select code to give different menu items.
 !
 ...
 !						period_pause_duration)
 !			CALL speak_text(DTK$K_WAIT, soft_support$)
 !			illegal_entry_count = 0%
 !
 !	---------------------------------------------------------------------
