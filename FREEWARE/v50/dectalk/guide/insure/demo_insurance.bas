	%TITLE "DECTALK-DEMO_INSURANCE.BAS"
 !
 !				DEMO_INSURANCE.BAS
 ! 
 ...
 !  shows how DECtalk can be used in the insurance industry.  The 
 ...
 !
 !			A) Insurance Policy Information
 !			B) Participating Physician Location
 !			C) Health Insurance Eligibility Verification
 !			D) Auto Insurance Claims
 ...
 !
  ...
  COMMON (application_specific)	LONG		voice_id, &
   ...
   msg_welcome$  = "Welcome to the DECtalk insurance demonstration.  "
...
out:
 ...
 %INCLUDE "DTKDEF"
  ...
  main_menu$	= "For insurance policy information, press 1, followed by "&
	+ "the pound-key.  For participating physician locations, press 2, "&
 	+ "followed by the pound-key.  For health insurance eligibility "&
	+ "verification, press 3, followed by the pound-key.  For auto "&
	+ "insurance claims, press 4, followed by the pound-key.  For "&
	+ "DECtalk product information, press 5, followed by the pound-key.  "&
	+ "To repeat this message, press 0, followed by the pound-key.  To "&
	+ "exit, press the star key.  "
 ...
 !
	...
	ELSE
		...
		CASE = 	DTK$K_TRM_ONE			! policy info demo
			stat% =  policy_info
			if (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
		...
		CASE = 	DTK$K_TRM_TWO			! doctor location demo
			stat% = physician_location
			if (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
			END IF
		...
		CASE = 	DTK$K_TRM_THREE			! insurance verification
			stat% = insurance_verification
			if (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
			END IF
