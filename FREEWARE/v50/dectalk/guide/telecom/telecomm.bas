	%TITLE "DECTALK-TELECOMM.BAS  --  BASIC APPLICATION GUIDELINE PROGRAM"
!
!			  	TELECOMM.BAS
! 
!	NOTE:  This program was written with VAX BASIC V3.0.
!
!  The following file contain a "modifiable" demonstratoin program
!  for the telecommunications industry.  This demonstration program
!  typifies most telepone application programs written for DECtalk.
!  It is written in VAX BASIC and user Version 4.5 or later of the
!  VMS/MicroVMS Run-Time Library Support provided for DECtalk.
!
!  Program description:  In the "MENU" subroutine, there is a SELECT
!			 statement where the code determines which of
!			 5 menu items are chosen.  The menu choices and
!			 menu message must be modified to reflect the 
!			 menu items being implemented.  
!	 		 The menu choices are:
!
!				1) Selecting Long Distance Vendor
!				2) Ordering Custom Calling Service
!				3) Field Service Dispatch
!				4) Order Entry/Status
!				5) DECtalk Product Information
!
!  NOTE:  For this demonstration, logging to an operator terminal WILL NOT
!	  be done since not all systems (especially MicroVMS systems) have
!	  OPCOM running.
!
!	  All error messages are logged only to the operator terminals that 
!	  have specified the "OPER11" type of message will be handled.
!	  These messages give the error message, and the terminal line 
!	  connected to that process.  To have error messages logged to the
!	  console terminal, or any other terminal, at the DCL prompt ($),
!	  enter the REPLY/ENABLE command:
!
!				$ reply/enable=oper11
!
!	  To enter this command, you must have the OPER user privelege.
!	  Also, OPCOM must be running on your system.
!
...
COMMON (MESSAGE_TEXT) STRING    msg_invalid  = 40, &
    ...
    msg_invalid       = "Invalid entry.  Please reenter.  "
    msg_timeout       = "No key pressed in the timeout period specified."
    msg_bad_command   = "Invalid command.  Please reenter.  "
    msg_no_terminator = "Please remember to terminate your entry with the "&
		      + "pound-key.  Command accepted.  "
    customer_service  = "In an actual application, DECtalk would now "&
		      + "transfer your call to a customer service "&
		      + "representative. This demonstration just returns "&
		      + "to the selection menu. "
    dispatch_desk     = "In an actual application, DECtalk would now "&
		      + "transfer your call to the dispatch-desk. This "&
		      + "demonstration just returns to the selection menu. "
    msg_access        = "Please enter your access code followed by the "&
		      + "pound-key.  You may enter any number as a test."

    COMMON (constants)		    STRING time_45_sec, time_6_min, &
					   time_16_min, &
				    LONG   MAX_ENTRY, T1SECOND, &
					   T20SECOND, T30SECOND, T15MINUTE

    COMMON (dt_features)	    LONG   voice, speaking_rate, &
					   comma_pause_duration, &
					   period_pause_duration, speech_on, &
					   new_mode, number_of_rings, &
					   keypad_mode
    ...
    msg_welcome$  = "Welcome to DECtalk.  Digital Voice Solutions can "&
		  + "increase your business [prowdaxkt'ihvixtiy].  "
...
! main
 ...
 WHILE_LOOP:
	...
	!  CAN BE LARGER THAN 16 MINUTES.  Next, wait for the phone connected
	!  to the DECtalk to ring.  If the phone does not ring in "timeout"
	!  seconds (or the watchdog timer times out), normally, the status
	!  of the DECtalk unit would be checked.  This is not possible in
	!  the current version (v4.5) of the VMS Run-Time Library Support 
	!  for DECtalk.  Therefore, instead of reinitializing DECtalk 
	!  every time the unit power cycled, it is reinitialized every time
        !  no phone calls are received in "timeout" seconds.  Then, a 
	!  greeting message is spoken to the user.
        !
	IF init_dtk = need_to_init
	THEN  
	    stat% = initialize_dectalk		! Reinitialize DECtalk params
	    IF (STAT% AND STS$M_SUCCESS) = 0%
	    THEN
		!
		!  Initialization failed.  It appears that the DECtalk
		!  module may be dead.  For now, I will exit.
		!
	...
	!
	! The greeting message DECtalk speaks upon answering the 
	! the telephone can be modified by changing the text in 
	! the character string "msg_welcome".  The number of rings 
	! DECtalk waits to answer the telephone ("number_of_rings"),
	! can be changed, but it is recommended that the phone is 
	...
	THEN
	    ...
	    !  NOTE: DTK$ANSWER_PHONE automatically enables
	    !	     autostop keypad mode and wink detection.
	    !
	    CALL menu
	    CALL end_call
...
! for device name, and dictionary_name for the name of the dictionary file.
!
...
!  are enabled as phonemic delimiters. To specify other modes, the 
!  bit masks for the modes to be set should be OR'd together with the
!  DTK$M_SQUARE bit mask and assigned to the variable "NEW_MODE".  Next,
!  the default speaking voice and speaking rate are selected for the 
!  application.  The comma pause and period pause are set to DECtalk defaults.
!  Other voice can be selected for the default speaking voice by modifying
!  the variable "voice".  Likewise, a different speaking rate can be 
!  specified by changing the static variable "speaking_rate".  Finally, the 
!  user dictionary is loaded (by invoking "load_dictionary").  Note, other
...
FUNCTION LONG initialize_dectalk()
    ...
    COMMON (constants)		    STRING time_45_sec
    ...
    END IF
    !
    !  Indicate the type of voice and speaking rate
    !
    CALL set_timer(Time_45_sec)
...
%INCLUDE "DTKDEF"
	...
	WHILE hold_key_buf(hold_buf_ptr) <> DTK$K_TRM_NUMBER_SIGN AND &
		...
		GO TO read_key
	    END IF
	    input_key_buf(num_input_keys) = hold_key_buf(hold_buf_ptr)
	...
	num_hold_keys = 0%
	!
	!  Recopy the remaining keys in the typeahead buffer to the
	!  beginning of the buffer.  If the terminating keycode was a "#"
    ...
    THEN
	!
	!  This test assumes no 1 character prompts are sent to
...
%INCLUDE "DTKDEF"
    ...
    main_menu$  = "To select your primary long distance vendor, press 1, "&
		+ "followed by the pound-key.  To order a custom-calling "&
		+ "service, press 2, followed by the pound-key.  To hear "&
		+ "a field-service application, press 3, followed by the "&
		+ "pound-key.  To hear a distribution-demostration, "&
		+ "press 4, followed by the pound-key.  To hear DECtalk "&
		+ "product-information, press 5, followed by the pound-key.  "&
		+ "To repeat this message, Press 0, followed by the "&
		+ "pound-key.  To exit, Press the star-key. "

    dtcfamily$	= "[+] The [`]DECtalk [`]Voice Response-System is the latest "&
		+ "addition to Digital's Voice solutions. It features the "&
		+ "performance of the MicroVAX 2, plus the flexibility of "&
		+ "the foremost voice synthesis product on the market today. "&
		+ "The [`]DECtalk [`]Voice Response-System is sized to "&
		+ "support from 2 to 32 DEctalk lines.  It is ideally "&
		+ "suited to front end large data-bases. Packaged with "&
		+ "the [`]DECtalk [`]Voice Response-System is the DECtalk "&
		+ "Application Development-Guide.  This guide consists of a "&
		+ "user's manual and sample application-software. It is "&
		+ "designed to significantly reduce the time required to "&
		+ "develop a voice response application for any of the "&
		+ "DECtalk products. DECtalk component products include "&
		+ "Dual-Line DECtalk, a two-line subsystem, and Multi-line "&
		+ "DECtalk, an eight-line subsystem, both of which are rack "&
		+ "mountable. In addition, the DECtalk board can be used to "&
		+ "expand the number of lines needed.  [+] Single line "&
		+ "DECtalk is a table-top unit with a built-in loud-speaker. "&
		+ "It is best for [st'aend#axl`own] applications, such as "&
		+ "aids for the handicapped. It is also commonly used in "&
		+ "manufacturing, for [m'aonaxtaxrixnx] status on the "&
		+ "factory floor. [+] All DECtalk products appropriately "&
		+ "pronounce commonly occurring forms, such as numbers, "&
		+ "money, abbreviations, dates, times, and "&
		+ "[paxnxkchuw'eyshaxn]. "
...
GET_SHORT_READ:
	...
	IF (stat% AND STS$M_SUCCESS) = 0%
	THEN
	    EXIT SUB				    ! Fatal error so exit
	END IF
	IF terminator_value = DTK$K_TRM_ASTERISK
	THEN
	    EXIT SUB				    ! User said hangup
	ELSE IF (terminator_value = DTK$K_TRM_NUMBER_SIGN)
	    THEN
		GOSUB process_menu_entry	    ! Valid command so process
...
!
    ...
    ELSE
	    ...
	    CASE = DTK$K_TRM_ONE		    ! Select long dist vendor
		stat% = vendor_change
		IF (stat% AND STS$M_SUCCESS) = 0%
		THEN
		    EXIT SUB
	    ...
	    CASE = DTK$K_TRM_TWO		    ! Order custom service
		stat% = custom_service
		IF (stat% AND STS$M_SUCCESS) = 0%
		THEN
		    EXIT SUB
	    ...
	    CASE = DTK$K_TRM_THREE		    ! Field service demo
		stat% = field_service
		IF (stat% AND STS$M_SUCCESS) = 0%
		THEN
		    EXIT SUB
	    ...
	    CASE = DTK$K_TRM_FOUR		    ! distribution demo	
		stat% = distribution
		IF (stat% AND STS$M_SUCCESS) = 0%
		THEN
		    EXIT SUB
	    ...
	    CASE = DTK$K_TRM_FIVE		    ! dectalk product info
    ...
    END IF
    !
    !  check status from menu fuctions -- If bad status (wink) exit menu.
    !
    IF (stat% AND STS$M_SUCCESS) = 0%
    THEN
	EXIT SUB
...
%INCLUDE "DTKDEF"
	...
	IF word_count = 0%
	THEN		
	    word_count = POS(dictionary_entry, tab$, 1)
	END IF
...
SUB end_call ()
    ...
    msg_goodbye$  = "Thank you for calling the DECtalk-demonstration.  "&
		  + "For more information, please call 1, 800, 8 3 2, 6 2 7 7. "
    ...
    END IF
    !
    !  Workaround to clear the internal WINK flag
    !
    timeout% = 1%
    WHILE 1% = 1%
        ...
        THEN
	    CALL set_timer(time_45_sec)		! Set system (watchdog)
	    stat% = DTK$READ_KEYSTROKE(voice_id, xx%, , timeout%)
	    CALL cancel_timer			! Cancel system timer
	    IF (stat% AND STS$M_SUCCESS) = 0%
	    THEN
...
%INCLUDE "DTKDEF"
    ...
    COMMON (dt_features)	  LONG   voice, speaking_rate, &
				         comma_pause_duration, &
					 period_pause_duration, &
					 speech_on, new_mode, number_of_rings,&
					 keypad_mode
    COMMON (application_specific) LONG	 voice_id
...
!	  releases.  Also, for this demo, error message WILL NOT be written
!	  to the operator terminal since OPCOM is not always running on 
!	  all systems, especially MicroVMS systems.
!
