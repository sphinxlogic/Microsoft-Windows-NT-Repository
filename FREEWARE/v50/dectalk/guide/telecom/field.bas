!  --------------------------  FIELD.BAS  ---------------------------
!
!  This subroutine contains the field service section of the 
!  telecommunication demo.  
...
%INCLUDE "DTKDEF"
    ...
    service_menu$  = "To hear your [f'own#m`ehsixjhixz], press 1. "&
		   ...
		   + "To exit the field-service demonstration, "&
		   + "press the star-key. "

    phone$	   = "You have 2 [f'own#m`ehsixjhixz]. "
    service$	   = "You have 1 [s'rrvixs#k`aol]. "

    badge_no$	   = "Enter your badge number followed by the pound-key. "&
...
%INCLUDE "DTKDEF"
    ...
    phone_msg(1)   = "Message 1 is from your manager.  It reads, Training "&
		   + "for new hardware installation is scheduled for "&
		   + "next Wednesday. "

    phone_msg(2)   = "Message 2 is from the warehouse. The subject is Parts. "&
		   + "It reads: the parts you ordered last [fr'aydxey] for "&
		   + "the air-conditioning system have arrived. "

    service_msg(1) = "[s'rrvixs#k`aol] 1. The customer is Carter Labs. "&
		   + "The system type is [axl'ehvixn thrrdxiy*f'or], "&
		   + "Option is ['aar*p`iy*`ow*s'ihks].  Contact is "&
		   + "Warren Foster.  His [f'own#n`ahmbrr] is "&
		   + "4 1 3 -- 5 5 5,   1 2 1 2.  The problem is the "&
		   + "[dr'ayv<150>] won't spin up.  Please call customer. "
    ...
    message_menu$  = "To hear the message repeated, press 1. "&
		   ...
		   + "To exit, press the star-key.  [:nh +] In a "&
		   + "real system, the caller could file or delete messages. "
