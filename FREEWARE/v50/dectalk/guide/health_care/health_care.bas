 ...
 !  shows how DECtalk can be used in the heath care industry.  The 
 ...
 !		statement where the code determines which of 4 menu items
 !		are chosen.  The menu choices and menu message must be
 !		modified to reflect the menu items being implemented.
 !		The menu choices are:
 !
 !			A) Health Insurance Eligibility Verification
 !			B) Claim Status Information
 !			C) Participating Physician Location
 !			D) DECtalk Product Information
 !
...
out:
 ...
 %INCLUDE "DTKDEF"
  ...
  main_menu$	= "For health insurance eligibility verification, press 1, "&
	...
	+ "followed by the pound-key.  For participating physician "&
	+ "[lowk'eyshaxnz] press 3, followed by the pound-key.  For DECtalk "&
	+ "product information, press 4, followed by the pound-key.  To "&
	+ "repeat this message, press 0, followed by the pound-key.  To exit, "&
	+ "press the star key.  "
 ...
 !
	...
	ELSE
		...
		CASE = 	DTK$K_TRM_ONE			! insurance verification
		...
		CASE = 	DTK$K_TRM_TWO			! claim status
			stat% = claim_status
			if (stat% AND STS$M_SUCCESS) = 0%
			THEN
				EXIT SUB
		...
		CASE = 	DTK$K_TRM_THREE			! doctor location demo
		...
		CASE = 	DTK$K_TRM_FOUR			! dectalk product info
...
HERE:
 ...
 SUB end_call ()
  ...
  msg_goodbye$  = "Thank you for calling Digital's Health-care Voice "&
	+ "Information Demonstration.  For more information, please call "&
	+ "1, 800, 8 3 2, 6 2 7 7.  "
