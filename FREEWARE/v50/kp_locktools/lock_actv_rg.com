...
$! LOCK_ACTV_RG.COM
$!
$! Summarize lock activity in the cluster.  Generate report from data
$! collected earlier by LOCK_ACTV_DC.COM.
$!							K.Parris 12/99
$!	P1: Input filespec (.e.g. LOCK_ACTV_19991215_0719.DAT)
$!	P2: Output filespec (default is same as input but with .RPT extension)
$!
$! Modification history:
$!	Check for the case of a data collection failing.   K.Parris 05/24/2000
$!
...
$	pid = f$getjpi("","PID")
$	if p2 .eqs. "" then -
		p2 = f$element(0,";",p1) - f$parse(p1,,,"TYPE") + ".RPT"
$	temp_file := lock_actv_rg_'pid'_temp.temp_file
$ 'if_debug' type/page 'p1'
...
$!               1         2         3
$!
$! First, make one quick pass through the SYSMAN output to pick up the
$! nodenames and CSIDs of all nodes.  We can't use F$GETSYI to map between
$! these at this point because CSIDs change across reboots, and a node may
$! have rebooted since this data was collected.
$!
$	close/nolog ls1
$	open/read ls1 'p1'
$!
...
$ loop_ls1:
$	read/end=eof_ls1 ls1 rec
$	if f$extract(0,44,rec) .eqs. "%SYSMAN-I-OUTPUT, command execution on node "
$	then
...
$		endif
$		node = f$extract(44,6,rec)	!Heading identifies nodename
...
$		csid_'csid'_nodename = node	!Remember CSID-->Node mapping
$ 'if_debug' write sys$output "Node ",node," has CSID ",csid	!!!
$		goto loop_ls1
$	endif
$	goto loop_ls1
$ eof_ls1:
...
$!!! show symbol CSID_*_*	!!!
$!
$! Now make the main pass through the data file
$!
$	close/nolog lis
$	open/read lis 'p1'
$	close/nolog ls2
...
$	then	!CSID record
$		csid = f$element(1," ",rec)
$ 'if_debug' if csid_'csid'_nodename .nes. node then write sys$output "*** Nodename mismatch"	!!!
$	else
$		oact = f$element(0," ",rec)
$		mstcsid = f$element(1," ",rec)
$		mstnode = csid_'mstcsid'_nodename
...
$		write rpt "    File specification: ",filespec
$	endif
...
$	write rpt f$fao("        Total:!7UL",total_act)
$!
...
$	write rpt f$fao("       !AS!6AS!7UL",mstflag,node,oact)
$	goto node_loop
$ node_done:
$	goto loop_rpt
$!
...
$	write rpt ""
$	write rpt "       * Lock Master Node for the resource"
$!	write rpt ""
$!	write rpt ""
$	close rpt
$	show time
