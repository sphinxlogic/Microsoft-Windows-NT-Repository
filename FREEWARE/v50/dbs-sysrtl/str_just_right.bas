	...
	sub str_just_right (string target, string pad_char, string source)
...
!
! Abstract:	Right justify and pad a string.
!
...
!	This routine right justifies a string and left fills the target
!	with the pad character supplied.
!
! Calling Sequence:
!
!	call str_just_right (target, pad_char, source)
!
! Formal Argument(s):
...
mainline:
	...
	filler_len = target_len - source_len

	rset target = left(source, source_len)

	loop = 1

	while (loop <= filler_len)
