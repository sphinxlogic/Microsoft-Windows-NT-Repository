/* modified by CEL to prevent DECC accvio!!! maybe only DECC V5.5-002
    problem occurs with the _CMA_VOLATILE_ flag applied to one member
    in:
      typedef struct EXC_CONTEXT_T {
         ...
         _CMA_VOLATILE_ struct EXC_CONTEXT_T
        ...
    removing the _CMA_VOLATILE_ fixes it.

    This problem is not present on at least some later versions of
    DECC and/or include files
*/
...
# endif
# define cma__exc_setjmp(__env) cma_save_exc_context((__env))
#else
...
#   define cma__exc_setjmp(__env)               _setjmp ((exc_int_t *)(__env))
#   define cma__exc_longjmp(__env,__val)        _longjmp((exc_int_t *)(__env),(__val))
#  else
    extern int _setjmp _CMA_PROTOTYPE_ ((_CMA_VOLATILE_ int *));
    extern void _longjmp _CMA_PROTOTYPE_ ((_CMA_VOLATILE_ int *, int));
#   define cma__exc_setjmp(__env)               _setjmp ((__env))
#   define cma__exc_longjmp(__env,__val)        _longjmp((__env),(__val))
#  endif
# else
#  define cma__exc_setjmp(__env)        setjmp ((__env))
#  define cma__exc_longjmp(__env,__val) longjmp((__env),(__val))
# endif
#endif
 ...
 *
 * Note that this also modifies the "extern" routine definitions immediately
 * below.
 */
...
# define exc_get_message        cma$exc_get_message
#endif
 ...
 * DECthreads exceptions; thus we need a condition handler. Due to
 * differences in behavior, the conditions for deciding whether to use an
 * external (client) or internal (within DECthreads) name are slightly
 * different
 */
# if _CMA_HARDWARE_ == _CMA__VAX
...
#define TRY \
    { \
        ...
        if (!cma__exc_setjmp (exc_ctx.jmp)) {
