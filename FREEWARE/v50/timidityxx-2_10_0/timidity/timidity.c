...
#else
#define OPTCOMMANDS "4A:aB:b:C:c:D:d:E:eFfg:hI:i:jk:L:M:m:n:O:o:P:p:Q:q:R:rS:s:t:T:UW:w:x:YZ:"   /* Y for Motif VMS */
#endif
...
{
#if !defined(__MACOS__) && !defined(VMS) && defined(HAVE_POPEN) && defined(HAVE_ISATTY) && !defined(IA_W32GUI)
    char *pager;
...
#else
"            m:Maxmum buffer, n:Filled to start   (default is 1.0/100%%)",
#endif
...
#ifdef VMS
      case 'Y':  /* Motif interface with Mailbox comm. */
        {
            mbx_exists ();
        }
      break;
...
#else
    if(!opt_aq_max_buff)
	opt_aq_max_buff = safe_strdup("1.0");
#endif
    if(!opt_aq_fill_buff)
