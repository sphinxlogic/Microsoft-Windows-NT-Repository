...
static void File_ListCB(Widget widget, int data, XtPointer call_data)
{
    ...
    XtFree(text);
		/* AB restart asynchronous read. */
    mbx_read_async(); /* Start asynchronus read for next message. */
    ...
    XtInputId *id;
{
	...
	case FILENAME_MESSAGE : {
	    ...
	    pc = strrchr(filename,']');
	    pe = strrchr(filename,';');
	    if (pe) *pe = '\0';		/* Omit the version number */
    ...
    mbx_read_async(); /* AB Start asynchronus read for next message. */
...
/*    XtSetArg(al[ac], XmNrightAttachment, XmATTACH_FORM); ac++; */
    ...
    /*
      ...
      */
    XtSetArg(al[ac], XmNtitle, "TiMidity 2.10.0"); ac++;
...
#ifdef VMS
    mbx_read_async();               /* Start an asynchronus read . */
#endif
