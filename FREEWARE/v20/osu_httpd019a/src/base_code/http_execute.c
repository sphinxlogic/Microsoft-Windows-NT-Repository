 ...
 * Revised:	11-MAY-1995	Remove stub routine added for OSF version.
 * Revised:	15-JUL-1995	Support multihome extensions.
 */
#include <stdio.h>
...
int http_multihomed;		/* If true, multiple rulefile domains. */
	...
	tu_stream inbound )		/* Input character stream */
{
	 ...
	 */
	int local_port, http_multihomed_hostname(); unsigned int remote_address;
	ts_tcp_info ( &local_port, (int *) 0, &remote_address );
	http_multihomed_hostname ( remote_address, scb.acc );
	if ( http_log_level > 2 ) tlog_putlog (3,
		"!AZ, IP interface used: !XL -> '!AZ'!/", log_prefix, 
		remote_address, acc.local_address );
    } else {
