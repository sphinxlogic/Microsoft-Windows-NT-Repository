	...
	string *iobuf )				/* I/O buffer */
{
	...
	status = http_send_error ( scb, "501 illegal filename",
	...
	if ( of ) {
		...
		if ( scb->acc->local_address && (scb->request[2].l > 0) ) {
		    ...
		    tu_strcpy ( &stsmsg[39], scb->acc->local_address );
	...
	of = tf_open ( ident, "rd", buffer );
	    ...
	    tu_strcpy ( &stsmsg[43], scb->acc->local_address );
...
{
    int i, state, length, j, req_len, line_matched;
    char *p, *q, c;
    /*
