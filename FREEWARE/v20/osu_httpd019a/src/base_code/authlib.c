 ...
 * Revised:	22-JUN-1995		Support unix.
 */
#include <stdarg.h>
#include <stdlib.h>
#include <stdio.h>
...
#endif
#include <ssdef.h>
int SYS$CREMBX(), SYS$QIOW(), SYS$SETPRN(), LIB$GET_EF();
#endif
...
{
    va_list callback;
    int status, req_mbx, rsp_mbx, auth_main_loop();
    int i;
	...
	int code_count, auth_callback **cb_list )
{
...
#else
    status = write ( rsp_mbx, &length, sizeof(length) );
    if ( status == sizeof(length) ) {
	iosb.count = write ( rsp_mbx, &rsp_msg, length );
	if ( iosb.count <= 0 ) status = 0; else status = 1;
    } else status = 0;
#endif
    /*
	 ...
	 */
        ...
        if ( (status&1) == 0 ) break;
#else
	status = read ( req_mbx, &length, sizeof(length) );
	if ( status == sizeof(length) ) {
	    status = read ( req_mbx, &req_msg, length );
	}
	if ( status <= 0 ) {
	    if ( status < 0 && errno == EPIPE ) status = SS$_NOWRITER;
	    break;
	}
#endif
        /*
	...
	status = write ( rsp_mbx, &length, sizeof(length) );
	if ( status == sizeof(length) )
	    status = write ( rsp_mbx, &rsp_msg, length );
	if ( status <= 0 ) {
	    if ( status < 0 && errno == EPIPE ) status = SS$_NOREADER;
	    break;
	}
#endif
    }
