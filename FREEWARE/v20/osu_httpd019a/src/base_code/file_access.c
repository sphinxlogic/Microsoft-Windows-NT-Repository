...
int tf_initialize ( char *prefix )
{
    ...
    status = pthread_keycreate ( &file_key, (pthread_destructor_t) tf_rundown );
...
void *tf_open ( char *ident, char *mode, char errmsg[256] )
{
	...
	CONDITIONAL_YIELD
        LOCK_C_RTL
	if ( new->binary_mode ) {
            new->fptr = fopen ( ident, mode, "mbc=64", "ctx=stm",
		"dna=[000000]" );
	} else {
            new->fptr = fopen ( ident, new->dir_mode ? "r" : mode, "mbc=64", 
		new->dir_mode ? "dna=[000000].dir" : "dna=[000000]" );
	}
        new->cerrno = errno;
	 ...
	 */
	for ( scnt = j = i = 0; j < sizeof(new->fspec)-15; i++ ) {
	    if ( ident[i] == '\0' ) break;
	    if ( ident[i] == '/' ) {
		if ( scnt == 0 ) {
		} else if ( scnt == 1 ) {
	...
	if ( j > 1 ) if ( new->fspec[j-1] != ']' ) new->fspec[j++] = ']';
