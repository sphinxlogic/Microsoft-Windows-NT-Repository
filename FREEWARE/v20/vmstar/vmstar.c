 ...
 *	n - don't preserve create date on files
 ...
 * Nashua, NH 03062
 *
 * Modified a bit more to preserve dates by
 * David Pasterchik
 * OpenVMS Engineering
 * Digital Equipment Corporation
 * 110 Spit Brook Road
 ...
 *
 * 1.5-3.2 Added modification date preservation for file extract operations
 *         (date in tar header becomes create and modify date for extracted
 *         file, but old behavior can be retained by using -n flag with -x 
 *         flag) and did ANSI clean-up, adding "standard" prototypes for
 *         functions and so on.  DEC C compile on Alpha can now be done using
 *         CC/PREFIX=ALL VMSTAR
 *         (/PREFIX=ALL needed to resolve references to the non-ANSI-standard 
 *         functions close, creat, getgid, getuid, mkdir, stat and write).
...
#define MMBBSIZE 32768          /* Maximum multi-block buffer size */
...
int extract=0,          /* x option, extract */
    ...
    waitp=0,            /* w option, prompt */
    dot=0,              /* d option, suppress dots */
    create=0,           /* c option, create */
    binmode=0,          /* z option, binary mode */
    nopresdat=0,        /* n option, don't preserve date on extract */
...
struct XABDAT xdtblock;  /* Date and time extended attribute block */
struct XABRDT xrdtblock; /* Revision date and time extended attribute block */
struct RAB rblock;	/*  Record attribute block */
...
FILE *vmsfile;
int vmsfd, outfd;
unsigned int vmsmrs;		    /* maximum record size */
time_t vmstime;			    /* modification time */
...
*/	  
void usage(void);
void tar2vms(int _ac,char **_av);
void vms2tar(int _ac,char **_av);
int hdr_read(char *_bf);
int decode_header(void);
int scan_title(char *_l,char *_d,char *_f);
int cleanup_dire(char *_s);
int make_new(char *_w);
int copyfile(char *_f,int _nb);
int tarskip(int _nb);
int vms_cleanup(char *_s);
int initsearch(char *_s);
int search(char *_bf,int _ml);
int cleanup_dire(char *_s);
int scan_name(char *_l,char *_d,char *_f,int _a);
int get_attributes(char *_f);
int fill_header(char *_s);
int write_header(int _fd);
int addfile(char *_v,char *_u);
int write_trailer(int _fd);
int out_file(char *_fn,int _nb,int _fd);
int fill_header(char *_fn);
void flushout(int _fd);
int lowercase(char *_s);
int convtovmstim ( char * _tcdt , int * _avmstm);
...
char *make_directory(void), *argp, *ptr;
...
/* open the file for reading */
    ...
    if (extract==1 && nopresdat==0)
	{
	printf ("Modify dates on non-directory files will be preserved\n");
	printf ("as create/revise dates; -n overrides.\n");
        }

/* Now keep reading headers from this file, and decode the names, etc. */

    while((status=hdr_read((char *)&header))==DSIZE)    /* 0 on end of file */
    {
        process = 0;
...
/* This function simply copies the file to the output, no conversion */
  /*  for OpenVMS, it defaults to using RMS routines directly :-( to	    */
  /*  preserve the date recorded in the header, since no safe provision	    */
  /*  exists in the OpenVMS version of creat to specify use of the	    */
  /*  date-related XABs (or any other XABs, for that matter).  However,	    */
  /*  specifying the -n flag forces the "original" behavior using creat,    */
  /*  write & close.  In order not to lose a lot of performance compared to */
  /*  the write routine, the function employs a multiple buffering scheme, 							    */
int copyfile(char *outfile,int nbytes)

{
int inbytes, fil, s, status;
int vmsabstim[2];
char *mblbuf;  /*  multiblock buffer */
size_t mblbsize, tsize;  /*  and its size and internal transfer size */
int oimbb;  /*  offset of next transfer into multiblock buffer */

    /*  Pick between C functions and RMS to create file  */
    if (nopresdat)
    {
        /*	  
	**  Not preserving create/modify date in header.  Use standard create
	**  and write and close C functions.
	*/	  
    ...
    {
        /*	  
	** Preserving create/modify date in header.  Use RMS functions
	** directly to mimic what's done above with create and write and
	** close.
	*/	  
    /*  Open the output file */

	/*  First, initialize all the various fields */
	fblock = cc$rms_fab;	/* FAB default */
	nblock = cc$rms_nam;	/* NAM default */
	rblock = cc$rms_rab;	/* RAB default */
	xdtblock = cc$rms_xabdat;  /* date XAB default */
	xrdtblock = cc$rms_xabrdt;  /* revision date XAB default */
	
	if (binmode)  /*  for binary, fake "rfm=fix","mrs=512" */
	{
	    fblock.fab$b_rfm = FAB$C_FIX;
	    fblock.fab$w_mrs = 512;
	}
	else  /*  otherwise, fake "rfm=stmlf","rat=cr" */
	{
	    fblock.fab$b_rfm = FAB$C_STMLF;
	    fblock.fab$b_rat = FAB$M_CR;
	}
        /*	  
	**  Fill in fields not dependent on whether or not extract is binary.
	*/	  
	
/*	    fil = creat(outfile,0,"rfm=stmlf","rat=cr");*/
	fblock.fab$b_fac = FAB$M_PUT|FAB$M_BIO;    /*  Block output access */
	fblock.fab$l_alq = (nbytes + 511) / 512;  /*  File size */
	fblock.fab$l_fna = outfile;  /*  file name */
	fblock.fab$b_fns = strlen (outfile);  /*  and length */
	fblock.fab$l_xab = (char *) &xdtblock;  /*  1st XAB = date block */
	fblock.fab$l_fop = FAB$M_MXV | /*  create new version if file exists, */
			    FAB$M_SQO;  /*  and access sequentially only. */
	xdtblock.xab$l_nxt = (char *) &xrdtblock;   /*  next XAB = rev date block */
	if ((status = convtovmstim (creation,vmsabstim)) != SS$_NORMAL)
	{
            /*	  
	    **  Date in header was in error; don't use.
	    */	  
            printf("tar: ignoring erroneous mod date for file %s \n",outfile);
	    fblock.fab$l_xab = 0;
	}
	else
	{
	    memcpy (&xdtblock.xab$q_cdt,vmsabstim,8);   /*  set create date */
	}
	
        /*	  
	**  Do the actual sys$create here
	*/	  
        status = sys$create (
                     &fblock, 
                     0, 
	...
	/*	  
	**  Make sure revision date will be forced on close.
	*/	  
	memcpy (&xrdtblock.xab$q_rdt,vmsabstim,8);
	/*	  
	**	Allocate a multi-block buffer, point RAB to FAB and
	**	connect it in.
	*/	  
	mblbsize = (nbytes > MMBBSIZE)? MMBBSIZE:nbytes;
	mblbuf = malloc (mblbsize);
	if (mblbuf == NULL)
	{
	    printf("tar: error writing file %s\n",outfile);
	    if (nbytes) tarskip(nbytes);
	...
	oimbb = 0;
        rblock.rab$l_fab = &fblock;  /*  address of FAB */
	status = sys$connect (
		     &rblock, 
		     0, 
		     0);
	...
	{
	    sprintf(buffer,"*** This file is a link to %s\n",linkname);
	    rblock.rab$l_rbf = buffer;   /*  address of write buffer */
	    rblock.rab$w_rsz = strlen(temp);
	    status = sys$write (
                     	 &rblock, 
                     	 0, 
                     	 0);
	...
	{
	    rblock.rab$l_rbf = mblbuf;   /*  address of write buffer */
	    while(nbytes > 0 && status == RMS$_NORMAL)
	    {
		if((inbytes=fread(buffer,1,DSIZE,tarfp)) > 0)
		{
		    tsize = (nbytes > DSIZE)? DSIZE:nbytes;
                    /*	  
		    **  {@tbs@}
		    */	  
		    
		    memcpy (mblbuf+oimbb,buffer,tsize);
		    oimbb += tsize;
                    /*	  
		    **	Fill the multi-block buffer a block at a time,
		    **	and flush it when it's too full or if there
		    **	won't be any more additions to it.
		    */	  
                    if ((oimbb > mblbsize - tsize) || (nbytes - inbytes <= 0))
		    {
			rblock.rab$w_rsz = oimbb;
			oimbb = 0;
			status = sys$write (
				     &rblock, 
				     0, 
...
/* Verify checksum */
    ...
    for(value = 0, ptr = (char *)&header; ptr < &header.chksum[0]; ptr++)
             value += *ptr;                /* make the checksum */
    for(ptr = (char *)&header.linkcount; ptr <= &header.dummy[255]; ptr++)
             value += *ptr;
...
/* Open the output file */
    ...
    if (create==1 && nopresdat==0)
	{
	printf ("Create date will written to archive on non-directory\n");
	printf ("files instead of modify date; -n overrides.\n");
        }
...
int out_file(char *filename,int nbytes,int fdout)
...
/* now get the file attributes, we don't use them all */
    ...
    if (nopresdat)  /*  Use mod time only if -n flag specified */
    	vmstime = sblock.st_mtime;
    else
    	vmstime = sblock.st_ctime;  /*  otherwise, use create time */
...
int scan_name(char *line,char *dire,char *fname,int absolute)
...
int search(char *buff,int maxlen)
...
/* I know that the next two are already zero, but do them */
        ...
        for(chksum=0, ptr = (char *) &header;ptr < &header.linkcount;ptr++)
...
/*	  
**  convtovmstim -- converts ctime-style time (1st argument) to
**  OpenVMS absolute time (2nd argument).  Returns whatever code the
**  $BINTIM system service returns to it.
*/	  
int convtovmstim ( char * tfcdt , int * avmstm)
    {
    static char vat[26];  /*  OpenVMS Standard ASCII format absolute time  */
		   /*  with null terminator */
    $DESCRIPTOR (vatd, vat);
    /*	  
    **  "Re-shape" ctime-style ASCII time string into OpenVMS-style ASCII time
    **	string.  This first part could be done with strptime and strftime but
    **	these aren't available for some older versions of C.
    */	  
    sprintf (vat, "%2.2s-%3.3s-%4.4s %9.9s.00",
		    tfcdt+8, tfcdt+4, tfcdt+20, tfcdt+10);
    vat[4] = toupper (vat[4]);  /*  fix lower-case */
    vat[5] = toupper (vat[5]);  /*  2nd and 3rd letters of month */
    vatd.dsc$w_length = 23;  /*  and ignore nulls which make time invalid */
    return sys$bintim (  /*  now do conversion to 64-bit time format */
               &vatd, 
               avmstm);
    }
...
void usage(void)
{
    printf("usage: tar x[n]|c|t[vbd][f tarfile] [file [file...]]\n");
    exit(1);
}
