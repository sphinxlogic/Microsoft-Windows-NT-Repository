...
static char rcsid[] = "$Id: display.c,v 4.16 1993/11/19 04:01:12 mikes Exp $";
#endif
/*
...
#include "hebrew.h"
    ...
    int vtpute(CELL);
    int vteeol(void);
    int updateline(int, CELL *, CELL *, short *,short *);
    int updext(void);
    int mlputi(int, int);
...
out:
...
#ifdef HEBREW
		if(lp->l_type==L_HEB)vscreen[i]->v_flag |= VHEBR;
		else vscreen[i]->v_flag &= ~VHEBR;
#endif
	...
	j = vp1->v_flag;
	    ...
	    if (typahead())
            ...
            updateline(i, &vp1->v_text[0], &vp2->v_text[0], &vp1->v_flag, &vp2->v_flag);
	...
	if(sgarbk != FALSE){
	    ...
	    if(lastflag&CFFILL){
#ifndef HEBREW
		wkeyhelp("GORYKCXJWVU^",
		         "Get Help,WriteOut,Read File,Prev Pg,Cut Text,Cur Pos,Exit,Justify,Where is,Next Pg,UnJustify,Set Mark");
#else
if(compose_heb){
		wkeyhelp("GORYKtX^WVUT",
                         "Get Help,WriteOut,Read File,Prev Pg,Cut Text,Direction,Exit,Set Mark,Where is,Next Pg,UnCut Text,  Heb/Eng");
	      }
else{
		wkeyhelp("GORYKCXJWVU^",
		         "Get Help,WriteOut,Read File,Prev Pg,Cut Text,Cur Pos,Exit,Justify,Where is,Next Pg,UnJustify,Set Mark");
...
#ifndef HEBREW
		wkeyhelp("GORYKCXJWVU^",
		         "Get Help,WriteOut,Read File,Prev Pg,Cut Text,Cur Pos,Exit,Justify,Where is,Next Pg,UnCut Text,Set Mark");
#else
if(compose_heb){
		wkeyhelp("GORYKtX^WVUT",
                         "Get Help,WriteOut,Read File,Prev Pg,Cut Text,Direction,Exit,Set Mark,Where is,Next Pg,UnCut Text,  Heb/Eng");
