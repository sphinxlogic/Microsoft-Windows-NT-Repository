...
register int tab;
CELL temp[HEB_LEN];
register CELL   *cp1,*cp2;

if(nbytes > padlen)return 1;
cp1=line;
cp2=temp;
tab=8;
k=0;

for(i=0;i<nbytes && cp2 < temp+padlen;i++){
  if(cp1->c==0x09){
    for(j=0;j<tab && cp2 < temp+padlen;j++,cp2++,k++){
      cp2->c=' ';
      cp2->a=0;
    }
    tab=1;
...
bcopy(&temp[0],&line[0],k*sizeof(CELL));
for(i=k;i<padlen;i++){
  line[i].c=' ';
  line[i].a=0;
}
...
reverse_pad_buf()
{
	...
	int new_len;
        ...
        lp1 = lforw(curbp->b_linep);             /* First line.          */
        while (lp1 != curbp->b_linep) {
	  if(lp1->l_type==L_HEB){
	    tab=8;
	    len=0;
	    new_len = (HEB_LEN>lp1->l_used)?HEB_LEN:lp1->l_used;
	    if ((lp2=lalloc(new_len)) == NULL)return (FALSE);
	    cp1 = &lp1->l_text[0];
	    cp2 = &lp2->l_text[0];	         
	    ...
	    free(lp1);
	    lp1=lp2;
	    if(lp1->l_used < HEB_LEN)lp1->l_used=HEB_LEN;	    
	    reverse_line(&lp1->l_text[0], llength(lp1));
	  }
          lp1 = lforw(lp1);
        }
	return TRUE;  
