...
dnl ------------------------
dnl $Header: /usr2/foxharp/src/pgf/vile/RCS/aclocal.m4,v 1.17 1995/10/19 20:02:25 pgf Exp $
dnl
dnl ---------------------------------------------------------------------------
dnl
define([VC_ERRNO],
...
dnl ---------------------------------------------------------------------------
dnl Test if the compiler recognizes prototypes w/o arguments
define([VC_ANSI_PROTOS],
[
AC_MSG_CHECKING([if compiler recognizes prototypes])
AC_CACHE_VAL(vc_cv_ansi_protos,[
	AC_TRY_COMPILE([
	extern int func (char *, int, ...); ],
	[ func("", 0); func("", 0, 1, 2) ],
	[vc_cv_ansi_protos=yes],
	[vc_cv_ansi_protos=no])
	])
AC_MSG_RESULT($vc_cv_ansi_protos)
if test $vc_cv_ansi_protos = yes
then
	AC_DEFINE(ANSI_PROTOS,1)
else
	AC_DEFINE(ANSI_PROTOS,0)
fi
])dnl
dnl ---------------------------------------------------------------------------
...
dnl ---------------------------------------------------------------------------
dnl Test if we should use ANSI or K&R style variable-argument lists.
define([VC_ANSI_VARARGS],
[
AC_MSG_CHECKING([for standard varargs])
AC_CACHE_VAL(vc_cv_ansi_varargs,[
	AC_TRY_COMPILE([
#if HAVE_STDARG_H
#include <stdarg.h>
#else
...
dofmt( char *fmt, ...)
{
	...
	func(fmt, &ap);
	va_end(ap) ],
	[vc_cv_ansi_varargs=yes],
	[vc_cv_ansi_varargs=no])
	])
AC_MSG_RESULT($vc_cv_ansi_varargs)
if test $vc_cv_ansi_varargs = yes
then
	AC_DEFINE(ANSI_VARARGS,1)
else
	AC_DEFINE(ANSI_VARARGS,0)
fi
])dnl
