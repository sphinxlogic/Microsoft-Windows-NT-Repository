 ...
 *
 * $Header: /usr2/foxharp/src/pgf/vile/RCS/vms2unix.c,v 1.13 1995/08/18 12:32:58 pgf Exp $
 *
 */
#include	"estruct.h"
#include	"edef.h"
...
static	int	is_version P((char *s));
	...
	const char *src;
{
	...
	&&	   (base[1] != '-')
	&&	   (base[1] != PERIOD)
	&&	   (base[1] != R_BRACK))) {	/* must supply a device */
	...
	if ((s = strchr(base, L_BRACK)) != NULL) {
		if (s[1] == R_BRACK) {
			if (*dst != SLASHC && dst != output)
				*dst++ = SLASHC;
			*dst++ = PERIOD;
			if (s[2] != EOS)
				*dst++ = SLASHC;
