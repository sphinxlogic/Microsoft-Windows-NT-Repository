 ...
 * framing, are hard.
 *
 * $Header: /usr2/foxharp/src/pgf/vile/RCS/basic.c,v 1.87 1995/10/08 16:05:53 pgf Exp $
 *
 */
...
register int	n;
{
	...
	if ((status = !same_ptr(lFORW(DOT.l), buf_head(curbp))) == TRUE) {
		...
		while (!same_ptr(lp, buf_head(curbp))) {
			lp = lFORW(lp);
...
register int	n;
{
	...
	lp = curwp->w_line.l;
	if (!same_ptr(lBACK(lp), buf_head(curbp))) {
...
register int	n;
{
	...
	if ((status = !same_ptr(lFORW(dlp), buf_head(curbp))) == TRUE) {
		...
		while (!same_ptr(lFORW(dlp), buf_head(curbp))) {
			llp = lFORW(llp);
			dlp = lFORW(dlp);
...
register int	n;
{
	...
	if ((status = !same_ptr(lBACK(dlp), buf_head(curbp))) == TRUE) {
		n -= line_height(curwp,dlp);
		while (!same_ptr(lBACK(dlp), buf_head(curbp))) {
			llp = lBACK(llp);
			dlp = lBACK(dlp);
			if ((n -= line_height(curwp,dlp)) < 0)
				break;
