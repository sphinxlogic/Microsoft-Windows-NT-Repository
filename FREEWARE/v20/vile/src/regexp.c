 ...
 *		pgf, 11/91
 *
 * $Header: /usr2/foxharp/src/pgf/vile/RCS/regexp.c,v 1.54 1995/10/01 14:44:59 pgf Exp $
 *
 */
...
static	char *	RegStrChr2 P(( char *, int, char * ));
...
char *prog;
{
	...
	while (scan != NULL) {
		...
		case EXACTLY: {
			...
			break;
		case ANYOF:
			if (reginput == regnomore || RegStrChr2(OPERAND(scan),
...
char *p;
{
	...
	case EXACTLY:
		...
		break;
	case ANYOF:
		while (scan != regnomore && RegStrChr2(opnd, *scan,
						(char *)0) != NULL) {
			count++;
			scan++;
		}
