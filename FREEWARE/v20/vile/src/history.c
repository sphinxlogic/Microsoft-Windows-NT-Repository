 ...
 *
 * $Header: /usr2/foxharp/src/pgf/vile/RCS/history.c,v 1.27 1995/09/09 14:14:17 pgf Exp $
 *
 */
	 ...
	 && ((bp = makeMyBuff()) != 0)) {
		...
		for_each_window(wp) {
			if (wp->w_bufp == bp) {
				...
				wp->w_force = -1;
				if (l_ref(wp->w_dot.l) != lBack(buf_head(bp))) {
