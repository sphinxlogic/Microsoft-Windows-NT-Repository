...
long			devclass;
struct dsc$descriptor	search_devnam_desc;
struct dsc$descriptor	return_devnam_desc;
char			search_devnam[1];
char			return_devnam[64];
long			status;
short			retlen;
#endif
...
void LoadCurrentDirectory()
{
...
#ifdef VMS
  /* fake being in non-existant VMS 'root' directory for dirMBlist */
  if (VMSroot)
    strcpy (path,"/");
#endif
  ...
  i=0;
	...
	{
	/*
	Since VMS has no 'root' directory, we will be providing a list of
	all available devices on the system and allow the user to select
	the drive they want to use. Picking a drive will turn off VMSroot
	mode and restore normal directory movement.

	VMSroot mode is turned on in changedDirMB() when the 'root'
	directory / is selected, and is turned off in FNameCdable()
	when we select a valid drive.

	The device list may be semi-scrambled since we get it in the order VMS
	wants to give it to us, and the default qsort() routine called later
	seems to mangle the list in some cases. If you use someone else's code,
	you get what you pay for...
	*/

	/* initialize data required for device scan calls */
	search_devnam_desc.dsc$w_length  = sizeof(search_devnam);
	search_devnam_desc.dsc$b_dtype   = DSC$K_DTYPE_T;
	search_devnam_desc.dsc$b_class   = DSC$K_CLASS_S;
	search_devnam_desc.dsc$a_pointer = search_devnam;
	search_devnam[0] = '*';

	return_devnam_desc.dsc$w_length  = sizeof(return_devnam);
	return_devnam_desc.dsc$b_dtype   = DSC$K_DTYPE_T;
	return_devnam_desc.dsc$b_class   = DSC$K_CLASS_S;
	return_devnam_desc.dsc$a_pointer = return_devnam;

	device_scan_itmlst.item[0].buflen = 4;
	device_scan_itmlst.item[0].itmcod = DVS$_DEVCLASS;
	device_scan_itmlst.item[0].bufadr = &devclass;
	device_scan_itmlst.item[0].retadr = 0;
	device_scan_itmlst.eol = 0;
	...
	/* load next device name until there are no more devices */
	while ((status = sys$device_scan (&return_devnam_desc,&retlen,&search_devnam_desc,&device_scan_itmlst,context)) != SS$_NOMOREDEV)
	    {
	    return_devnam[retlen] = '\0';

	    /*
	    Only list devices we are allowed to use. Reasons devices may not
	    appear in this list are: protection violation, device allocated by
	    another process, device is not mounted, etc. If you can't change
	    your default directory to it, you can't use it...
	    */
	    strcpy (path,return_devnam);
	    strcat (path,"[000000]");
	    if (chdir(path) == 0)
		{
		if ((i&31)==0) WaitCursor();

		fnames[i] = (char *) malloc(strlen(return_devnam)+2); /* +2=ftype + '\0' */

		if (!fnames[i]) FatalError("malloc error while reading directory");
		strcpy(fnames[i]+1, return_devnam);

		/* force type of file to 'directory' (like a un*x device..) */
		fnames[i][0] = C_DIR;

		i++;
		}
	    }
	strcpy (path,"/");	/* reset path to 'root' */
	}
    else
	/* not VMSroot mode, do the normal while loop below */
#endif
...
static int FNameCdable()
{
  ...
  if (VMSroot)
    ...
    strcat (newpath,"[000000]");
    if (chdir(newpath) == 0)
	/*
	Success - went to top-level directory on device.
	Reset VMSroot mode, and load top-level directory of selected device.
	*/
	{
	VMSroot = 0;
	loadCWD();
	...
	/*
	We failed to set the default directory to the designated drive. They
	either hand-typed in a bad device name, or the device they selected has
 ...
 */
    sprintf(cmd, "%s /Delete %s", filename+1, outFName);
#endif
