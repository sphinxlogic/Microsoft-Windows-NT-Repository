...
#define RK_MAXMEM	(MEMSIZE / sizeof (short))	/* words/memory */
#define RK_CTLI		1				/* controller int */
#define RK_SCPI(x)	(2 << (x))			/* drive int */
...
int rkcs = 0;						/* control/status */
int rkds = 0;						/* drive status */
int rkba = 0;						/* memory address */
int rkda = 0;						/* disk address */
int rker = 0;						/* error status */
int rkwc = 0;						/* word count */
int rkintq = 0;						/* interrupt queue */
int last_drv = 0;					/* last r/w drive */
int rk_stopioe = 1;					/* stop on error */
int rk_swait = 10;					/* seek time */
int rk_rwait = 10;					/* rotate time */
int rk_svc (UNIT *uptr);
int rk_reset (DEVICE *dptr);
void rk_go (void);
void rk_set_done (int error);
...
REG rk_reg[] = {
	...
	{ GRDATA (FLG0, rk_unit[0].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG1, rk_unit[1].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG2, rk_unit[2].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG3, rk_unit[3].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG4, rk_unit[4].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG5, rk_unit[5].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG6, rk_unit[6].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
	{ GRDATA (FLG7, rk_unit[7].flags, 8, UNIT_W_UF, UNIT_V_UF), REG_RO },
...
int rk_rd (int *data, int PA, int access)
{
UNIT *uptr;
...

int rk_wr (int data, int PA, int access)
{
int i, t;
UNIT *uptr;
...
if ((uptr -> flags & UNIT_ONLINE) == 0) {		/* not present? */
...
static unsigned short fill[RK_NUMWD] = { 0 };
int rk_svc (UNIT *uptr)
{
int comp, drv, err, awc, twc, wc;
int pa, da, fillc, track, sect;

drv = (uptr - rk_dev.units) / sizeof (UNIT);		/* get drv number */
...
for (i = 0; i <= RK_NUMDR; i++) {			/* loop thru intq */
	if (rkintq & (1 << i)) {			/* bit i set? */
		rkintq = rkintq & ~(1 << i);		/* clear bit i */
