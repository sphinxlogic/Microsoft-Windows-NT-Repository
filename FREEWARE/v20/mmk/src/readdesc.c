...
*/
#ifdef __DECC
#pragma module READDESC "V1.6-7"
#else
#ifndef __GNUC__
#module READDESC "V1.6-7"
#endif
#endif
#include <ctype.h>
#include "mmk.h"
...
*/
    ...
    extern int extract_nametype(char *, char *);
    ...
    extern unsigned int cms_fetch_file(char *, char *);
...
*/
    	if (*fspec == '\0') {
    	    status = cms_fetch_file("DESCRIP.MMS", 0);
    	    if (!OK(status)) status = cms_fetch_file("MAKEFILE.", 0);
    	} else {
    	    char element[256];
...
/*
**  We only need to fill in sources when the target is a file _and_
**  there are no explicit action lines for the dependency.
*/
    for (dep = dependencies.flink; dep != &dependencies; dep = dep->flink) {
    	if (dep->target->type != MMK_K_OBJ_FILE) continue;
    	if (dep->cmdqptr != 0 && dep->cmdqptr->flink != dep->cmdqptr) continue;
