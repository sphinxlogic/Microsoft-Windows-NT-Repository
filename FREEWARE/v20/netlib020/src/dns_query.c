...
**  back, or we run out of nameservers.
*/
...
**  run through all the search domains, return the ENDOFFILE error (no
**  such host or domain).
*/
    	    if (!OK(status)) {
...
*/
    ...
    if (OK(status) && ior->iosb.iosb_w_count >= sizeof(struct NETLIB_DNS_HEADER)) {
    	...
    	hp2 = (struct NETLIB_DNS_HEADER *) dnsreq->query_rbuf;
    	if (hp1->dns_w_queryid == hp2->dns_w_queryid) {
    	    if (ior->ctx != 0) netlib_close(&ior->ctx);
    	    ior->ctx = 0;
    	    if (ior->iosbp != 0)
