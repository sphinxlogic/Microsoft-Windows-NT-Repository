 ...
 *
 *		Doug Tody, NOAO (Hacked to make it run on VMS)
 */
...
#ifdef DEBUG
fprintf (stderr,
    "Usage: compress [-dDVf] [-b maxbits] [-c outfile] [file ...]\n");
}
int debug = 0;
#else
fprintf (stderr,
 ...
 *
 * Usage: compress [-dfv] [-b bits] [-c outfile] [file ...]
 * Inputs:
 *	-d:	    If given, decompression is done instead.
 *
 *      -c:         Concatenate output to named file.
 *
 ...
 *	(_z suffix) only if the file decreases in size.
 * Algorithm:
...
register int argc; char **argv;
{
    ...
    *filelist = NULL;
     ...
     *	    given also.
     * -c outfile => cat all output to named file
...
#ifdef vms
			if (!ARGVAL()) {
			    fprintf (stderr, "Missing file arg to -c <file>\n");
			    Usage();
			    exit(1);
			}
			strcpy (ofname, *argv);
	...
	for (fileptr = filelist; *fileptr; fileptr++) {
		...
		/* Check for _z suffix */
		if (strcmp(*fileptr + strlen(*fileptr) - 2, "_z") != 0) {
		    /* No _z: tack one on */
		...
		if (!zcat_flg) {
		    ...
		    ofname[strlen(*fileptr) - 2] = '\0';  /* Strip off _z */
#ifdef vms
		}
#endif
	    } else {					/* COMPRESSION */
		if (strcmp(*fileptr + strlen(*fileptr) - 2, "_z") == 0) {
		    	fprintf(stderr, "%s: already has _z suffix -- no change\n",
			    *fileptr);
		    continue;
		}
		/* Open input file */
		...
		if (!zcat_flg) {
		    ...
		    if (strlen(cp) > 12) {
			fprintf (stderr,
			    "%s: filename too long to tack on _z\n",cp);
			continue;
		    }
...
#else
		if (freopen (ofname, zcat_flg ? "a" : "w", stdout) == NULL) {
#endif
		    perror (ofname);
		    continue;
		}
...
char *ifname, *ofname;
{
	...
	fprintf(stderr, " -- has %d other links: unchanged",
		statbuf.st_nlink - 1);
	exit_stat = 1;
    } else if (exit_stat == 2 && (!force)) { /* No compression: remove file_z */
	if(!quiet)
	...
	{ exit_stat = 0;
#endif
	if (zfdele(ifname))	/* Remove input file */
	    perror(ifname);
	if(!quiet)
