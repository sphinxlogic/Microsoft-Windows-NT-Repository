...
#ifdef VMS
#define X_DEFAULT_SEARCH_PATH "DECW$SYSTEM_DEFAULTS:%N%C%S,DECW$SYSTEM_DEFAULTS:%N%S"
     ...
     char *class;
{
  ...
  path = search_magic_path (path, class, 0, X_DEFAULT_SUFFIX);
  if (path)
    {
...
#ifdef VMS
  path = "DECW$USER_DEFAULTS:%N%S";
  if (file = search_magic_path (path, class, 0, X_DEFAULT_SUFFIX))
#else
