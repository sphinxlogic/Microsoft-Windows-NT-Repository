...
/* First some internal ones */
static unsigned long __quotient, __remainder;

long __vms_time_secs (vms_time time)
{
  unsigned long divisor = 10000000; /* Hundreds of nanoseconds in a second */
  int status = LIB$EDIV (&divisor, time.t, &__quotient, &__remainder);
  return __quotient;
}

long __vms_time_usecs (vms_time time)
{
  unsigned long divisor = 10; /* Hundreds of nanoseconds
				 in a microsecond */
  __vms_time_secs (time);
  return __remainder / divisor;
}

void __vms_set_secs (vms_time * time, int secs)
{
  unsigned long multiplier = 10000000; /* Hundreds of nanoseconds
					  in a second */
  int status;

  __vms_time_secs (*time);
  status = LIB$EMUL (&multiplier, &secs, &__remainder, time->t);
}

void __vms_set_usecs (vms_time * time, int usecs)
{
  unsigned long multiplier1 = 10000000; /* Hundreds of nanoseconds
					   in a second */
  unsigned long multiplier2 = 10; /* Hundreds of nanoseconds
				     in a microsecond */
  unsigned long zero = 0;
  long secs = __vms_time_secs (*time);
  vms_time tmp1, tmp2;

  int status = LIB$EMUL (&multiplier1, &secs, &zero, tmp1.t);
  status = LIB$EMUL (&multiplier2, &usecs, &zero, tmp2.t);
  __vms_add_time (time, tmp1, tmp2);
}

void __vms_add_time (vms_time * dest, vms_time time1, vms_time time2)
{
  int status = LIB$ADDX (&time1.t[0], &time2.t[0], dest->t);
}

void __vms_sub_time (vms_time * dest, vms_time time1, vms_time time2)
{
  int status = LIB$SUBX (&time1.t[0], &time2.t[0], dest->t);
}

void __vms_negate_time (vms_time * dest, vms_time time)
{
  dest->t[0] = -time.t[0];
  dest->t[1] = -time.t[1] - (time.t[0] != 0);
}

/* Now, the more official ones... */
...
struct tm *sys_gmtime(const time_t *clock)
{
  ...
  if (tmp_offset != 0)
    ...
    {
      vms_internal_time[2][0] = vms_internal_time[0][0];
      vms_internal_time[2][1] = vms_internal_time[0][1];
    }
