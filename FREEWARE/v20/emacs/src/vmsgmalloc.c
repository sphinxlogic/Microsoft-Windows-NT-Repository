...
#ifdef VMS
	  /* We have to do it this way, or we might get something
	     from another heap, and that will trash the _fraghead
	     list */
	  result = _internal_malloc (BLOCKSIZE);
#else
...
#define VMS_ALLOCATION_SIZE (512*(512+256))
#endif
...
/* This holds the discovered pagesize.  */
  ...
  if (vms_brk_info_fetched < 0)
    {
	 ...
	 So the code below effectivelly gives us one page.  */
      vms_brk_current = vms_brk_start = (__ptr_t) __sbrk (1);
      pagesize = (__ptr_t) __sbrk (0) - vms_brk_start;
	...
	vms_brk_info_fetched = 0;
	 ...
	 Knowledge about this is also stored in vms_brk_info_fetched.  */
      vms_brk_end = __brk (vms_brk_start);
     ...
     ptrdiff_t size;
{
  ...
  if (vms_brk_end != 0)
	...
	vms_brk_current = vms_brk_start;
      ...
      fprintf (stderr, "end of memory moved from 0x%p to ", sbrk (0));
#endif
...
/* This is needed, so mapin_data can clear some inappropriate data */
void vms_clear_data (int index)
{
  __vms_malloc_data[index].__malloc_initialized = 0;
}
...
void malloc_clear_hooks ()
{
  ...
  /* Do not worry, these get set right by the malloc and free routines
     below */
  __morecore = __vms_morecore;
  __lesscore = __vms_lesscore;
}
...
static void use_vms_data (int i)
{
  ...
  _heapbase = __vms_malloc_data[i]._heapbase;
  _heapinfo = __vms_malloc_data[i]._heapinfo;
  heapsize = __vms_malloc_data[i].heapsize;
  _heapindex = __vms_malloc_data[i]._heapindex;
  _heaplimit = __vms_malloc_data[i]._heaplimit;
  {
    int j;
    for (j = 0; j < BLOCKLOG; j++)
      _fraghead[j] = __vms_malloc_data[i]._fraghead[j];
  }
  _chunks_used = __vms_malloc_data[i]._chunks_used;
  _bytes_used = __vms_malloc_data[i]._bytes_used;
  _chunks_free = __vms_malloc_data[i]._chunks_free;
  _bytes_free = __vms_malloc_data[i]._bytes_free;
  __malloc_initialized = __vms_malloc_data[i].__malloc_initialized;
}
  
static void store_vms_data (int i)
{
  __vms_malloc_data[i]._heapbase = _heapbase;
  __vms_malloc_data[i]._heapinfo = _heapinfo;
  __vms_malloc_data[i].heapsize = heapsize;
  __vms_malloc_data[i]._heapindex = _heapindex;
  __vms_malloc_data[i]._heaplimit = _heaplimit;
  {
    int j;
    for (j = 0; j < BLOCKLOG; j++)
      __vms_malloc_data[i]._fraghead[j] = _fraghead[j];
  }
  __vms_malloc_data[i]._chunks_used = _chunks_used;
  __vms_malloc_data[i]._bytes_used = _bytes_used;
  __vms_malloc_data[i]._chunks_free = _chunks_free;
  __vms_malloc_data[i]._bytes_free = _bytes_free;
  __vms_malloc_data[i].__malloc_initialized = __malloc_initialized;
}
...
{
  return
    (*__vms_core_routines[current_vms_core_routines].__morecore) (increment);
}
...
{
  return
    (*__vms_core_routines[current_vms_core_routines].__lesscore) (ptr,increment);
}
     ...
     size_t size;
{
  ...
  if (old_current_vms_data != -1)
    use_vms_data (old_current_vms_data);
  return result;
}
     ...
     size_t size;
{
  ...
  if (ptr >= vms_initial_buffer && ptr <= vms_end_brk)
    {
      use_vms_data (0);
      result = _internal_realloc (ptr, size);
      store_vms_data (0);
    }
  else
    {
...
/* An extra debugging routine to follow _fraghead lists */
void follow_fraghead (int i)
{
  struct list *next;

  printf ("_fraghead[%d] = { n:%p, p:%p } -->\n",
	  i, _fraghead[i].next, _fraghead[i].prev);
...
void follow_free_list (void)
{
  size_t start, block;
  start = block = _heapindex;
  do
    {
      printf ("Free block, p:%p, size:%d\n",
	      ADDRESS (block), _heapinfo[block].free.size);
      block = _heapinfo[block].free.next;
    }
  while (block != start);
  fflush (stdout);
