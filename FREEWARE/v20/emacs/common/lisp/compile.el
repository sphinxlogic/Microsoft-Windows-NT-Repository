	  ...
	  (if (eq system-type 'vax-vms)
		...
		(let ((process-connection-type nil))
		  (setq proc (start-process-shell-command (downcase mode-name)
							  outbuf
							  "@home_root:[000000]foo")))
		(process-send-string proc "SHOW DEFAULT")
		;;(process-send-string proc "SHOW LOG sys$sysdevicev")
		(process-send-string proc (concat "write sys$output \"thisdir = " thisdir "\""))
		(process-send-string proc (concat "set default " thisdir))
		(sleep-for 1)
		(process-send-string proc "SHOW DEFAULT")
		(process-send-string proc "write foo f$environment(\"DEFAULT\")")
