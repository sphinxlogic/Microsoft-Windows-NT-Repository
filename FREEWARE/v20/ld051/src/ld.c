...
int single;
{
	...
	if (single)
	    lib$signal(&ld_nosupport, 1, which);	/* Show it to the world */
	else
	    return (1);
    }
...
/* No maxblocks specified, check if geometry specified. If true set maxblocks
   according to that */
	...
	if (unit == 0)		/* Don't use unit 0 */
	    lib$signal(&ld_nosupport, 1, fulldevspec(s_device));
	set_seed(unit);
	stat = sys$assign(&sdev, &chan, 0, 0, 0);	/* Assign channel */
	signal_error(stat, 0);
	getdevnam(chan, s_device);
...
char *s_device;
{
    ...
    stat = lib$getdvi(&DVI$_DEVNAM, &chan, 0, 0, &dev, &rlen);
    signal_error(stat, 0);
...
void trace()
{
	...
	if (get_unit(s_device, 1) == 0)	/* Don't use unit 0 */
	    lib$signal(&ld_nosupport, 1, fulldevspec(s_device));	/* Show it to the world */
	sdev.addr = s_device;
...
void watch()
{
    ...
    int *blkpnt, wptcnt;
    int function_read, function_write, function_code, action_suspend,
     action_crash, action_error, action_opcom;
...
/* Get switches */
	...
	getdevnam(chan, devspc);	/* Check if on other device */
        devspc[strlen(devspc)-1] = '\0';/* Zap trailing ':' */
	if (strcmp(devspc,ld_nam.nam$t_dvi+1))
		lib$signal(&ld_fileonother, 0); /* File not allowed to be on other device */
    }
...
/*
 If we enable a watchpoint on a file we have to use the same
 channel through which we accessed the file. In that way the
 driver can validate that the file is opened with cathedral
 windows.
*/
    if (file)
	qchan = fchan;
    ...
    stat = sys$qiow(0, qchan, IO$_LD_CONTROL, &iosb, 0, 0,
		    wpt, cnt, 0, 0, 0, func);
    signal_error(stat, iosb.status);
    stat = sys$dassgn(chan);	/* Deassign channel */
    signal_error(stat, 0);
...
void show_trace()
{
	...
	if (get_unit(s_device, 1) == 0)	/* Don't use unit 0 */
	    lib$signal(&ld_nosupport, 1, s_device);	/* Show it to the world */
	sdev.addr = s_device;
	sdev.len = strlen(s_device);
...
int single;
{
	...
	if (single)
	    lib$signal(&ld_nosupport, 1, devstr);	/* Show it to the world */
	else
	    return (1);
    }
    sdev.addr = dev;
...
int *size;
{
    ...
    ld_fab.fab$l_dna = ".DSK";	/* Default extension */
    ld_fab.fab$b_dns = 4;	/* and length */
    ld_fab.fab$l_fop = FAB$M_NAM | FAB$M_CTG | FAB$M_OFP;	/* NAM + contiguous */
...
int *newstatus;
{
    ...
    else if (oldstatus == SS$_INCSHAMEM)
	code = &ld_nesting;	/* 'Nesting not allowed' */
