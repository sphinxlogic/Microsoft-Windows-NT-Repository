...
**  	19-JUN-1995 V1.5    Madison 	Moved Yes_Answer to this module so
**  	    	    	    	    	    it can use SMG routines.
**--
*/
#define MODULE_PAGER
#include "newsrdr.h"
...
*/
    ...
    static unsigned int pbid = 0, vdid = 0, kbid = 0, ktid = 0;
...
int Yes_Answer(char *prompt, int defans) {
    ...
    while (1) {
    	if (kbid != 0) {
    	    status = smg$read_composed_line(&kbid, 0, &adsc, &pdsc, &len);
    	} else {
    	    status = lib$get_input(&adsc, &pdsc, &len);
    	}
    	if (!OK(status)) len = 0;
