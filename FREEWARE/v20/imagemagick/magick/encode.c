  ...
  *image;
{
  ...
  */
  ...
  if ((image->class == PseudoClass) && IsGrayImage(image))
    {
  ...
  *image;
{
  ...
  if (!image->matte)
    {
      Warning("Image does not have a matte channel",image->filename);
      return(False);
    }
  /*
    Open output image file.
...
%
*/
static unsigned int WriteMPEGImage(image_info,image)
ImageInfo
  *image_info;
  ...
  *image;
{
  ...
  Warning("Cannot write MPEG images",image->filename);
  status=WriteMIFFImage(image_info,image);
  return(status);
}

  ...
  */
  if (strcmp(image->magick,"PPM") == 0)
    ...
    if (strcmp(image->magick,"PGM") == 0)
      {
        QuantizeImage(image,256,8,image_info->dither,GRAYColorspace);
        SyncImage(image);
      }
    else
  ...
  */
  ...
  if (!IsPseudoClass(image))
  ...
  else
    {
      ...
      */
      format='6';
      if (!IsGrayImage(image) || (strcmp(image->magick,"PPM") == 0))
        packets*=3;
      else
        {
          /*
  ...
  *image;
{
  ...
  if (flipped_image == (Image *) NULL)
    ...
    {
      status=WriteSUNImage(image_info,flipped_image);
  ...
  if (status)
    {
      ...
      (void) sprintf(command,"rastorle -o %s %s\n",image_info->filename,
        image->filename);
    ...
    if (photometric == PHOTOMETRIC_PALETTE)
      {
        ...
        */
        ...
        if ((blue == (unsigned short *) NULL) ||
          ...
          {
            Warning("Unable to read TIFF image","Memory allocation failed");
            return(False);
          }
        /*
  ...
  *image;
{
  ...
  */
  yuv_pixels=(unsigned char *)
    malloc((3*(image->columns*image->rows)/2)*sizeof(unsigned char));
  ...
  */
  ...
  for (i=0; i < (reduced_image->rows*reduced_image->columns); i++)
  {
    *q=p->green;
    p++;
    q++;
  ...
  */
  ...
  (void) fwrite((char *) yuv_pixels,1,(int) (3*(image->columns*image->rows)/2),
    image->file);
  ...
  *image;
{
  ...
  */
  ...
  OpenImage(reduced_image,"wb");
  if (reduced_image->file == (FILE *) NULL)
    {
      Warning("Unable to open file",reduced_image->filename);
      DestroyImage(reduced_image);
      return(False);
    }
  p=reduced_image->pixels;
  ...
  (void) fwrite((char *) yuv_pixels,1,(int)
    (reduced_image->columns*reduced_image->rows),reduced_image->file);
