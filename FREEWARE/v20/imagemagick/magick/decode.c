    ...
    if (jpeg_info.data_precision > 8)
      {
        if (jpeg_info.out_color_space == JCS_GRAYSCALE)
          ...
          {
            gray_value=GETJSAMPLE(*p) >> 4;
            q->red=(unsigned char) gray_value;
            q->green=(unsigned char) gray_value;
            q->blue=(unsigned char) gray_value;
            q->index=(unsigned short) gray_value;
            q->length=0;
            p++;
            q++;
          }
          ...
          for (j=0; j < (count*image->columns); j++)
          {
            q->red=(unsigned char) (GETJSAMPLE(*p++) >> 4);
            q->green=(unsigned char) (GETJSAMPLE(*p++) >> 4);
            q->blue=(unsigned char) (GETJSAMPLE(*p++) >> 4);
            q->index=0;
            q->length=0;
            q++;
          }
  ...
  *image_info;
{
  ...
  */
  ...
  if (status == False)
    {
      Warning("Corrupt MPEG image",image->filename);
      DestroyImage(image);
      return((Image *) NULL);
    }
  while (status == True)
...
%
*/
static Image *ReadNULLImage(image_info)
ImageInfo
  *image_info;
{
  Image
...
static Image *OverviewImage(image_info,image,number_images)
ImageInfo
  *image_info;
...
#define ClientName  "montage"
  ...
  */
  ...
  XrmInitialize();
  XGetDefault(display,ClientName,"dummy");
  ...
  resource_value=XGetResourceClass(resource_database,ClientName,"borderWidth",
  ...
  resource_info.matte_color=XGetResourceInstance(resource_database,ClientName,
    "mattecolor",DefaultTileMatte);
  XGetMontageInfo(&montage_info);
  montage_info.number_tiles=number_images;
  /*
  ...
  */
  if (montage_info.number_tiles != 0)
    image=XMontageImage(display,&resource_info,&montage_info,
      image_info->filename,images);
  XCloseDisplay(display);
  (void) free((char *) images);
  if (image == (Image *) NULL)
    {
      Warning("PCD Overview translation failed",image_info->filename);
      return((Image *) NULL);
    }
  return(image);
}
  ...
  *image_info;
{
  ...
  */
  ...
  if ((status == False) ||
    ...
    {
      Warning("Not a PCD image file",(char *) NULL);
      DestroyImages(image);
      return((Image *) NULL);
    }
  ...
  */
  image->packets=image->columns*image->rows;
  chroma1=(unsigned char *) malloc(image->packets*sizeof(unsigned char));
  chroma2=(unsigned char *) malloc(image->packets*sizeof(unsigned char));
  luma=(unsigned char *) malloc(image->packets*sizeof(unsigned char));
      ...
      if ((subimage >= 5) && status)
        {
          ...
          */
          ...
          (void) PCDDecodeImage(image,luma,chroma1,chroma2);
        }
    }
  Upsample(image->columns >> 1,image->rows >> 1,image->columns,chroma1);
  Upsample(image->columns >> 1,image->rows >> 1,image->columns,chroma2);
  ...
  if ((rotate == 1) || (rotate == 3))
    {
      ...
      */
      ...
      image->orphan=True;
      rotated_image=RotateImage(image,degrees,(ColorPacket *) NULL,False);
      image->orphan=False;
            ...
            */
            ...
            for (red=0; red < 8; red++)
              for (green=0; green < 8; green++)
                ...
                {
                  image->colormap[i].red=(red*MaxRGB)/7;
                  image->colormap[i].green=(green*MaxRGB)/7;
                  image->colormap[i].blue=(blue*MaxRGB)/3;
                  i++;
                }
          }
      }
    ...
    switch (format)
    {
      ...
      case '7':
      {
        ...
        */
        if (max_value > 255)
          {
            ...
            */
            for (i=0; i < image->packets; i++)
            {
              q->index=scale[LSBFirstReadShort(image->file)];
              q->length=0;
              q++;
            }
            SyncImage(image);
        ...
        pixels=(unsigned char *) malloc(image->packets*sizeof(unsigned char));
      ...
      case '6':
      {
        ...
        */
        if (max_value > 255)
          {
            ...
            */
            for (i=0; i < image->packets; i++)
            {
              q->red=scale[LSBFirstReadShort(image->file)];
              q->green=scale[LSBFirstReadShort(image->file)];
              q->blue=scale[LSBFirstReadShort(image->file)];
              q->index=0;
              q->length=0;
              q++;
            }
        ...
        pixels=(unsigned char *) malloc(image->packets*3*sizeof(unsigned char));
        if (pixels == (unsigned char *) NULL)
          {
            Warning("Memory allocation error",(char *) NULL);
            DestroyImages(image);
...
#define PageBoundingBox  "%%PageBoundingBox:"
  ...
  */
  ...
  if (file == (FILE *) NULL)
    {
      Warning("Unable to write file",postscript_filename);
      return((Image *) NULL);
    }
  /*
        ...
        if (image_info->page == (char *) NULL)
          {
            ...
            */
            width=((lower_x+upper_x)*x_resolution+(dx_resolution >> 1))/
              dx_resolution;
            height=((lower_x+upper_y)*y_resolution+(dy_resolution >> 1))/
              dy_resolution;
        ...
        */
        ...
        for (i=0; i < number_colormaps; i++)
          for (j=0; j < map_length; j++)
            *p++=ColorShift(LSBFirstReadShort(image->file));
      }
    if (flags & 0x08)
      {
        unsigned int
        ...
        */
        ...
        (void) ReadData((char *) image->comments,1,(int) length-1,image->file);
        ...
        */
        ...
        if (number_colormaps == 0)
          ...
          if (number_colormaps == 1)
            ...
            {
              image->colormap[i].red=(*p);
              image->colormap[i].green=(*(p+map_length));
              image->colormap[i].blue=(*(p+map_length*2));
              p++;
            }
        p=rle_pixels;
        if (!image->matte)
  ...
  *image_info;
{
  ...
  */
  ...
  background_color.red=ColorShift(pixel_info.background_color.red);
  background_color.green=ColorShift(pixel_info.background_color.red);
  background_color.blue=ColorShift(pixel_info.background_color.red);
  ...
  */
  ...
  /*
    Initialize colormap.
  */
  image->colormap[0].red=ColorShift(pixel_info.background_color.red);
  image->colormap[0].green=ColorShift(pixel_info.background_color.green);
  image->colormap[0].blue=ColorShift(pixel_info.background_color.blue);
  image->colormap[1].red=ColorShift(pixel_info.foreground_color.red);
  image->colormap[1].green=ColorShift(pixel_info.foreground_color.green);
  image->colormap[1].blue=ColorShift(pixel_info.foreground_color.blue);
  /*
    ...
    if ((bits_per_sample > 8) || (samples_per_pixel > 1) || TIFFIsTiled(tiff))
      {
        ...
        */
        ...
        pixels=(uint32 *) malloc(image->packets*sizeof(uint32));
...
%
*/
static Image *ReadVIDImage(image_info)
ImageInfo
  *image_info;
{
#define ClientName  "montage"
  ...
  */
  ...
  number_files=1;
  filelist[0]=image_info->filename;
  ...
  *image_info;
{
  ...
  */
  while (fgets(data,MaxTextLength-1,image->file) != (char *) NULL)
  ...
  switch (image->class)
  {
      ...
      if (image->colors != 0)
        for (y=0; y < image->rows; y++)
        {
          for (x=0; x < image->columns; x++)
          {
            ...
            q->red=ColorShift(colors[index].red);
            index=(unsigned short) ((pixel >> green_shift) & green_mask);
            q->green=ColorShift(colors[index].green);
            index=(unsigned short) ((pixel >> blue_shift) & blue_mask);
            q->blue=ColorShift(colors[index].blue);
            q->index=0;
            q->length=0;
            q++;
          }
        ...
        for (y=0; y < image->rows; y++)
          for (x=0; x < image->columns; x++)
          {
            pixel=XGetPixel(ximage,x,y);
            color=(pixel >> red_shift) & red_mask;
            q->red=ColorShift(((unsigned long) color*65535)/red_mask);
            color=(pixel >> green_shift) & green_mask;
            q->green=ColorShift(((unsigned long) color*65535)/green_mask);
            color=(pixel >> blue_shift) & blue_mask;
            q->blue=ColorShift(((unsigned long) color*65535)/blue_mask);
            q->index=0;
            q->length=0;
            q++;
          }
    ...
    case PseudoClass:
    {
      ...
      */
      ...
      for (i=0; i < image->colors; i++)
      {
        image->colormap[i].red=ColorShift(colors[i].red);
        image->colormap[i].green=ColorShift(colors[i].green);
        image->colormap[i].blue=ColorShift(colors[i].blue);
      }
      for (y=0; y < image->rows; y++)
        for (x=0; x < image->columns; x++)
        {
          pixel=XGetPixel(ximage,x,y);
          q->red=ColorShift(colors[pixel].red);
          q->green=ColorShift(colors[pixel].green);
          q->blue=ColorShift(colors[pixel].blue);
  ...
  *image_info;
{
  ...
  */
  ...
  OpenImage(zoomed_image,"rb");
  if (zoomed_image->file == (FILE *) NULL)
    {
      Warning("Unable to open file",zoomed_image->filename);
  ...
  */
  ...
  (void) ReadData((char *) yuv_pixels,1,(int) (3*(width*height)/2),
    zoomed_image->file);
  /*
  ...
  *image_info;
{
  ...
  */
  ...
  (void) ReadData((char *) y_pixels,4,(int) zoomed_image->packets,
    zoomed_image->file);
  ...
  */
  ...
  (void) ReadData((char *) uv_pixels,1,(int) zoomed_image->packets,
    zoomed_image->file);
  p=uv_pixels;
  q=zoomed_image->pixels;
  for (i=0; i < zoomed_image->packets; i++)
  ...
  *image_info;
{
  ...
  if (!decode_info.assert || (strncmp(decode_info.magick,"IRIS",4) == 0) ||
      (strncmp(decode_info.magick,"PCD",3) == 0))
    {
        ...
        if ((decode_image.file == stdin) || decode_image.pipe)
          {
            ...
            */
            ...
            file=fopen(decode_image.filename,"wb");
            if (file == (FILE *) NULL)
              {
          ...
          */
          ...
          if (((unsigned char) magic_number[0] == 0xff) &&
              ((unsigned char) magic_number[1] == 0xff))
            {
              ...
              */
              for (i=0; i < (0x800-sizeof(magic_number)); i++)
      ...
      */
      ...
      if (strncmp(magic_number,"\001\332",2) == 0)
        (void) strcpy(decode_info.magick,"IRIS");
      ...
      if (strncmp(magic_number,"PCD",3) == 0)
  ...
  */
  switch (*decode_info.magick)
  {
    ...
    case 'G':
    {
      if (strcmp(decode_info.magick,"GIF") == 0)
        image=ReadGIFImage(&decode_info);
      else
        if (strcmp(decode_info.magick,"GIF87") == 0)
          image=ReadGIFImage(&decode_info);
        else
    ...
    case 'I':
    {
      image=ReadIRISImage(&decode_info);
      break;
    }
    case 'J':
    {
    ...
    case 'M':
    {
      if (strcmp(decode_info.magick,"MAP") == 0)
        ...
        if (strcmp(decode_info.magick,"MATTE") == 0)
          ...
          if (strcmp(decode_info.magick,"MIFF") == 0)
        ...
        if (strcmp(decode_info.magick,"PCX") == 0)
          ...
          if (strcmp(decode_info.magick,"PICT") == 0)
            ...
            if (strcmp(decode_info.magick,"PM") == 0)
              ...
              if ((strcmp(decode_info.magick,"PS") == 0) ||
                  (strcmp(decode_info.magick,"PS2") == 0))
                image=ReadPSImage(&decode_info);
              else
    ...
    case 'R':
    {
      if (strcmp(decode_info.magick,"RAD") == 0)
        ...
        if (strcmp(decode_info.magick,"RAS") == 0)
          ...
          if (strcmp(decode_info.magick,"RGB") == 0)
    ...
    case 'T':
    {
      if (strcmp(decode_info.magick,"TGA") == 0)
        ...
        if ((strcmp(decode_info.magick,"TIF") == 0) ||
            (strcmp(decode_info.magick,"TIFF") == 0))
    ...
    case 'X':
    {
      if (strcmp(decode_info.magick,"X") == 0)
        image=ReadXImage(&decode_info,False,False,False,True);
      else
