...
*/
  ...
  if (Options & VERBOSE) Print_Options();

  Initialize_Tokenizer(Input_File_Name);
  if (Options & VERBOSE) fprintf (stderr,"Parsing...");
  ...
  if (Use_Slabs) 
    {
    if (Options & VERBOSE) fprintf (stderr, "Preprocessing...\n");  /* Added */
    BuildBoundingSlabs(&Root_Object);        /* Added */
    }

  if (Options & DISPLAY)
    {
    if (Options & VERBOSE) printf ("Displaying...\n");
