...
void SetPixel (Display *display, XImage *ximage, Window window, GC gc, unsigned x, unsigned y, unsigned index)
{
  ...
  if (y != lastY)
  {
    ...
    XPutImage (display, window, gc, ximage, 0, lastY, 0, lastY, gWidth, 1) ;
    lastY = y ;
  }
  ...
  if (gDepth == 24)
    {
      ...
      if (y != lastY)
        {
          XPutImage (gDisplay, gWindow, gGc, gXimage, 0, lastY, 0, lastY, gWidth, gHeight) ;
          lastY = y ;
        }
      offset  = gXimage->data + ( (gXimage->bytes_per_line * y) + (4 * x) );
      pixel  =  ((Blue  << 16) & 0xFF0000) |
		((Green << 8)  & 0x00ff00) |
		(Red          & 0x0000FF);
      ptr = (int *)offset;
      *ptr++ = pixel;
      
/*    XPutPixel (gXimage, x, y, pixel); */
