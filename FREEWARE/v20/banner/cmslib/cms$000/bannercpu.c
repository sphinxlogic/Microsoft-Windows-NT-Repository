...
*5D
 	XSetFillStyle (XtDisplay(Bnr.cpu_widget), BannerGC, FillOpaqueStippled);
 	XFillRectangle (XtDisplay(Bnr.cpu_widget), XtWindow (Bnr.cpu_widget),
 	    BannerGC, n_start, 
...
*5D
 	XSetFillStyle (XtDisplay(Bnr.cpu_widget), BannerGC, FillSolid);
 	...
 	XFillRectangle (XtDisplay(Bnr.cpu_widget), XtWindow (Bnr.cpu_widget),
 		...
 		(Bnr.font_height * 2), 
 		Bnr.font_width, 
 		Bnr.cpu_height - 3*Bnr.font_height - modes_height);
...
*8E
     {
*8D
     int width, x_pos, f_height;
 
     x_pos = bar_x + (Bnr.cpu_count - ticks);
     width = bar_x1 - x_pos;
 
     XFillRectangle (XtDisplay(Bnr.cpu_widget), XtWindow (Bnr.cpu_widget),
 	BannerGC, x_pos, seg_height*j + height[j], width, bar_height - height[j]);
*8E
*8I
 	cpu_hist[j][0].x = bar_x;
 	cpu_hist[j][0].y = seg_height * j;
...
*8D
     XFillRectangle (XtDisplay(Bnr.cpu_widget), XtWindow (Bnr.cpu_widget),
 	BannerGC, x_pos, seg_height*j, width, height[j]);
*8E
*8I
 	XFillPolygon (XtDisplay(Bnr.cpu_widget), XtWindow(Bnr.cpu_widget),
 	    BannerGC, &cpu_hist[j][0], p_size, Complex, CoordModeOrigin);
