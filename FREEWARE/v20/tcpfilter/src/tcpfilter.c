...
/*	2A	24-may-1995 gg: added "port" missing keyword */
#include <assert.h>
#include <ctype.h>
#include <file.h>
#include <setjmp.h>
...
/*	lecture d'une ligne; */
/* curchar contient le premier caractere non blanc de cette ligne */
/* linidx est l'index de ce caractere dans le buffer ligne ligne */
/* linsiz est le nombre de caracteres dans cette ligne */
/* la ligne est terminée par un NUL */
...
static	const	struct dsc$descriptor accessnames[32] = 
	...
	skipchar ('(');
	aclbuf[0]= '(';
	for (cp=aclbuf+1; (parcnt>0) && (curchar!='\0') ; get_char()) {
		switch (curchar) {
		case '(': parcnt++; break;
		case ')': parcnt--; break;
		}
