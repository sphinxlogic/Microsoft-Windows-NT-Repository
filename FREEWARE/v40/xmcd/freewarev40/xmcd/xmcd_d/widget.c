...
{
	register_focus_cb(m->options.form, cd_shell_focus_chg, m->options.form);
...
register_dbprog_callbacks(widgets_t *m, curstat_t *s)
{
	...
	register_activate_cb(m->dbprog.wwwwarp_btn, dbprog_wwwwarp, s);
	...
	register_activate_cb(m->dbprog.send_btn, dbprog_send, s);
...
{
	register_focus_cb(m->wwwwarp.form, cd_shell_focus_chg, m->wwwwarp.form);
	register_activate_cb(m->wwwwarp.go_btn, dbprog_wwwwarp_go, s);
	register_activate_cb(m->wwwwarp.cancel_btn, dbprog_wwwwarp_cancel, s);

	/* Install WM_DELETE_WINDOW handler */
	add_delw_callback(
		XtParent(m->wwwwarp.form),
		(XtCallbackProc) dbprog_wwwwarp_cancel,
		(XtPointer) s
	);
}
...
register_help_callbacks(widgets_t *m, curstat_t *s)
{
	...
	register_activate_cb(m->help.cancel_btn, cd_help_popdown, s);
...
register_dialog_callbacks(widgets_t *m, curstat_t *s)
{
	...
	register_activate_cb(btn, cd_warning_popdown, s);

	btn = XmMessageBoxGetChild(
		m->dialog.fatal,
		XmDIALOG_OK_BUTTON
	);
	register_activate_cb(btn, cd_fatal_popdown, s);
