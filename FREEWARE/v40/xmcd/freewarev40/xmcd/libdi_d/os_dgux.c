 ...
 *   The name "Data General" and DG/UX are used here for
 *   identification purposes only.
 */
#ifndef LINT
static char *_os_dgux_c_ident_ = "@(#)os_dgux.c	6.20 98/05/10";
#endif
...
)
{
	...
	(void) memset(&param_blk, 0, sizeof(param_blk));

	param_blk.cdb_ptr = (union user_scsi_cdb *)(void *) cmdpt;
	param_blk.cdb_size = cmdlen;
	param_blk.data_ptr = datapt;
	param_blk.data_size = datalen;
	param_blk.flags.data_write = (rw == OP_DATAOUT);
	...
	if (app_data.debug) {
        ...
        if (ioctl(fd, USER_SCSI_CMD, &param_blk) < 0) {
