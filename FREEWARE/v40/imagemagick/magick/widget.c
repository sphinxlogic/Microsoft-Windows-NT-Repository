...
#define MinTextWidth  (25*XTextWidth(font_info,"_",1))
...
%
*/
static void XDrawTriangleEast(display,window_info,triangle_info)
Display
  *display;
...
#define ResetButtonText  "Reset"
      ...
      if (colorlist == (char **) NULL)
        {
          XNoticeWidget(display,windows,"Unable to obtain colors names:",
            glob_pattern);
          XDialogWidget(display,windows,action,"Enter color name:",reply);
          return;
        }
    }
  /*
  ...
  */
  ...
  height=font_info->ascent+font_info->descent;
  windows->popup.width=
    width+Min(text_width,MaxTextWidth)+6*font_info->max_bounds.width;
  windows->popup.height=
    ((81*height) >> 2)+((13*font_info->max_bounds.width) >> 1)+4;
  windows->popup.min_width=width+MinTextWidth+4*font_info->max_bounds.width;
                  ...
                  if (status != False)
                    {
                      (void) sprintf(reply_info.text,"#%02x%02x%02x",
                        ColorShift(color.red),ColorShift(color.green),
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        ...
        submenu_info.raised=False;
        submenu_info.bevel_width--;
        submenu_info.width=((5*(font_info->ascent+font_info->descent)) >> 3)-
          (submenu_info.bevel_width << 1);
        submenu_info.height=submenu_info.width;
        ...
        */
        ...
        for (id=0; id < number_selections; id++)
        {
          ...
          if ((id >= 0) && (id < NumberMenus))
            {
              submenu_info.y=
                selection_info.y+(windows->command.font_info->ascent >> 1);
              XDrawTriangleEast(display,&windows->command,&submenu_info);
            }
          selection_info.y+=(int) selection_info.height;
        }
        state&=(~RedrawWidgetState);
            ...
            if (id == selection_info.id)
              {
                ...
                */
                windows->command.x_origin=windows->command.width;
                windows->command.y_origin=title_height+
                  id*selection_info.height+(selection_info.height >> 1);
                entry=
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  Function XConfirmWidget displays a popup window with a notice to the user.
%  The function returns True if the user presses Yes otherwise False is
%  returned.
%
...
%
*/
unsigned int XConfirmWidget(display,windows,message,qualifier)
Display
  *display;
...
#define YesButtonText  "Yes"
  ...
  */
  ...
  if (qualifier != (char *) NULL)
    if (XTextWidth(font_info,qualifier,strlen(qualifier)) > width)
      width=XTextWidth(font_info,qualifier,strlen(qualifier));
  height=(font_info->ascent+font_info->descent);
  windows->popup.width=width+7*font_info->max_bounds.width;
  windows->popup.height=12*height;
  windows->popup.min_width=width+(font_info->max_bounds.width << 2);
  ...
  */
  ...
  /*
    Map popup window.
  */
  (void) sprintf(windows->popup.name,"Confirm");
  (void) XStringListToTextProperty(&windows->popup.name,1,&window_name);
  XSetWMName(display,windows->popup.id,&window_name);
  window_changes.width=windows->popup.width;
  window_changes.height=windows->popup.height;
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        XGetWidgetInfo(CancelButtonText,&cancel_info);
        cancel_info.width=font_info->max_bounds.width+
          XTextWidth(font_info,CancelButtonText,strlen(CancelButtonText));
        cancel_info.height=(3*height) >> 1;
        cancel_info.x=windows->popup.width-cancel_info.width-
          font_info->max_bounds.width;
        cancel_info.y=windows->popup.height-(cancel_info.height << 1);
        yes_info=cancel_info;
        yes_info.text=YesButtonText;
        yes_info.x=font_info->max_bounds.width;
        state&=(~UpdateConfigurationState);
      }
    if (state & RedrawWidgetState)
      {
        ...
        */
        ...
        XDrawBeveledButton(display,&windows->popup,&cancel_info);
        XDrawBeveledButton(display,&windows->popup,&yes_info);
        XHighlightWidget(display,&windows->popup,4,4);
        state&=(~RedrawWidgetState);
      }
    /*
  ...
  if (filelist == (char **) NULL)
    {
      ...
      */
      XNoticeWidget(display,windows,"Unable to read directory:",
        working_directory);
      XDialogWidget(display,windows,action,"Enter filename:",reply);
      return;
    }
  /*
    Determine popup window attributes.
                ...
                if (*reply_info.text == '~')
                  {
                    ...
                    */
                    ...
                    if (p != (char *) NULL)
                      {
                        (void) strcpy(working_directory,p);
                        (void) strcat(working_directory,reply_info.text+1);
                      }
                  }
              }
            state|=UpdateListState;
...
#define ResetButtonText  "Reset"
  ...
  */
  ...
  (void) qsort((void *) fontlist,fonts,sizeof(char **),
    (int (*) _Declare((const void *, const void *))) FontCompare);
  /*
    Determine popup window attributes.
  */
  font_info=windows->popup.font_info;
...
#define CancelButtonText  "Cancel"
  ...
  */
  ...
  height=font_info->ascent+font_info->descent;
  windows->popup.width=
    Min(text_width,MaxTextWidth)+((9*font_info->max_bounds.width) >> 1);
  windows->popup.height=
    ((81*height) >> 2)+((13*font_info->max_bounds.width) >> 1)+4;
  windows->popup.min_width=MinTextWidth+4*font_info->max_bounds.width;
  windows->popup.min_height=
    ((23*height) >> 1)+((13*font_info->max_bounds.width) >> 1)+4;
  /*
    Position popup window.
  ...
  *item;
{
  ...
  */
  ...
  if (title == (char *) NULL)
    {
      x=windows->command.x+windows->command.x_origin;
      y=windows->command.y+windows->command.y_origin;
    }
  ...
  */
  ...
  do
  {
    ...
    if ((title != (char *) NULL) && (number_selections > 3))
      {
        /*
          Redraw Menu line.
        */
...
*/
      ...
      case MotionNotify:
      {
        ...
        */
        ...
        if (event.xmotion.window == windows->command.id)
          {
            y=AbsoluteValue(windows->command.y_origin-event.xmotion.y);
            if (y > (selection_info.height >> 1))
              state|=ExitState;
            break;
          }
        if (event.xmotion.window != windows->popup.id)
          break;
        if (state & InactiveWidgetState)
          break;
        id=(event.xmotion.y-title_height)/(int) selection_info.height;
...
%
*/
void XTextViewWidget(display,resource_info,windows,mono,title,textlist)
Display
  *display;
...
#define DismissButtonText  "Dismiss"
  ...
  */
  ...
  if (text_info == (XFontStruct *) NULL)
    text_info=windows->popup.font_info;
  text_width=0;
  for (i=0; textlist[i] != (char *) NULL; i++)
    if (XTextWidth(text_info,textlist[i],strlen(textlist[i])) > text_width)
      text_width=XTextWidth(text_info,textlist[i],strlen(textlist[i]));
  lines=i;
  width=XTextWidth(font_info,DismissButtonText,strlen(DismissButtonText));
  width+=font_info->max_bounds.width;
  height=text_info->ascent+text_info->descent;
  windows->popup.width=
    Min(text_width,MaxTextWidth)+5*font_info->max_bounds.width;
  ...
  */
    ...
    if (state & UpdateConfigurationState)
      {
        ...
        */
        ...
        slider_info.height=
          scroll_info.height-((slider_info.min_y-scroll_info.y+1) << 1)+2;
        visible_lines=(scroll_info.height-(height >> 3)-4)/
        ...
        if (slider_info.id != selection_info.id)
          {
            /*
              Redraw scroll bar and file names.
            */
            windows->popup.font_info=text_info;
            XSetFont(display,windows->popup.annotate_context,text_info->fid);
            XSetFont(display,windows->popup.highlight_context,text_info->fid);
            selection_info.id=slider_info.id;
            selection_info.y=list_info.y+(height >> 3)+2;
            ...
            windows->popup.font_info=font_info;
            XSetFont(display,windows->popup.annotate_context,font_info->fid);
            XSetFont(display,windows->popup.highlight_context,font_info->fid);
            /*
              Update slider.
            */
            if (slider_info.y > expose_info.y)
  ...
  if (text_info != windows->popup.font_info)
