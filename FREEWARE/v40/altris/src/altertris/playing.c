    ...
    Bool            falldown;
{
	...
	while (flag) {
		...
		case '>':
		case '.':
		    if (!paused && LEGAL(triangles + 1, diagonal, mixed, bonus)) {
			triangles++;
			redoNext();
			if (showNext) {
			    clearNext();
		...
		case 'g':
		case 'G':
		    if (!paused && LEGAL(triangles, !diagonal, mixed, bonus)) {
			diagonal = !diagonal;
			redoNext();
			if (showNext) {
			    clearNext();
    ...
    move_t          move;
{
	...
	putTri();
	lines = checkLines();
	score += prefilled * prefilled + lines + !gradualAppear + !grid;
	score += (showNext ? 3 : 5) * level * level * lines * lines *
          triangles * triangles * (bonus + diagonal + 1) / (16 * (1 + mixed));
	rows += lines;
    ...
    int             num;
{
    ...
    (void) fprintf(stderr, "            GENERIC TERTRIS  HALL OF FAME\n\n");
    (void) fprintf(stderr,
