...
Bool            showNext = False, beep = False, bonusNow = False,
		diagonal = False, mixed = False, bonus = False, cw = True,
...
static XrmOptionDescRec opTable[] = {
    ...
    {"-n", "*numberTriangles", XrmoptionSepArg, (caddr_t) NULL},
    ...
    char           *argv[];
{
    ...
    XrmParseCommand(&cmmDB, opTable, opTableEntries, "tertris", argc, argv);
    ...
    if (XrmGetResource(cmmDB, "tertris.scoresOnly", "Tertris.ScoresOnly",
		       str_type, &value) == True) {
        if (XrmGetResource(cmmDB, "tertris.scoreFile", "Tertris.ScoreFile",
                       str_type, &value) == True) {
    ...
    /* get display now */
    if (XrmGetResource(cmmDB, "tertris.display", "Tertris.Display",
		       str_type, &value) == True)
...
getDefaults()
{
    ...
    if (XrmGetResource(rDB, "tertris.startLevel", "Tertris.StartLevel",
		       str_type, &value) == True) {
	if ((sscanf(value.addr, "%d", &level) <= 0) ||
		(level < 0) || (level >= NUM_LEVELS)) {
	    (void) fprintf(stderr, "Tertris: Invalid level.\n");
	    exit(1);
	}
    }
    ...
    if (XrmGetResource(rDB, "tertris.preFilledLines", "Tertris.PreFilledLines",
		       str_type, &value) == True) {
	if ((sscanf(value.addr, "%d", &prefilled) <= 0) ||
		(prefilled < 0) || (prefilled >= ROWS - MAX_TRIANGLES)) {
	    (void) fprintf(stderr, "Tertris: Invalid prefilled lines.\n");
	    exit(1);
	}
    }
    ...
    if (XrmGetResource(rDB, "tertris.diagonal", "Tertris.Diagonal",
			str_type, &value) == True) {
	diagonal = True;
    }
    ...
    if (XrmGetResource(rDB, "tertris.mixed", "Tertris.Mixed",
			str_type, &value) == True) {
	mixed = True;
    }
    ...
    if (XrmGetResource(rDB, "tertris.bonus", "Tertris.Bonus",
			str_type, &value) == True) {
	bonus = True;
    }
    ...
    if (XrmGetResource(rDB,
		       "tertris.numberTriangles", "Tertris.NumberTriangles",
		       str_type, &value) == True) {
	if (sscanf(value.addr, "%d", &triangles) <= 0) {
	    (void) fprintf(stderr,
                "Tertris: Invalid number of triangles for current modes.\n");
	    exit(1);
	}
    }
    if (!LEGAL(triangles, diagonal, mixed, bonus)) { 
	    (void) fprintf(stderr,
                "Tertris: Invalid number of triangles for current modes.\n");
	    exit(1);
    }
    ...
    if (XrmGetResource(rDB, "tertris.showNext", "Tertris.ShowNext",
			str_type, &value) == True) {
	showNext = True;
    }
    ...
    if (XrmGetResource(rDB, "tertris.beep", "Tertris.Beep",
			str_type, &value) == True) {
	beep = True;
    }
    ...
    if (XrmGetResource(rDB, "tertris.ccw", "Tertris.Ccw",
                        str_type, &value) == True) {
        cw = False;
    }
    ...
    Fixed fill randomly places TRIANGLES_PER_ROW on each line */

    if (XrmGetResource(rDB, "tertris.fixedFill", "Tertris.FixedFill",
                        str_type, &value) == True) {
        randomFill = False;
    }

    /* sudden appearance of polyiamond */

    if (XrmGetResource(rDB, "tertris.suddenAppear", "Tertris.SuddenAppear",
                       str_type, &value) == True) {
        gradualAppear = False;
    }
    ...
    if (XrmGetResource(rDB, "tertris.noGrid", "Tertris.NoGrid",
                       str_type, &value) == True) {
        grid = False;
    }
    ...
    if (XrmGetResource(rDB, "tertris.scoreFile", "Tertris.ScoreFile",
                       str_type, &value) == True) {
        (void) strncpy(scorefile, value.addr, ZLIM(value.size, FILENAMELEN));
        scorefile[FILENAMELEN - 1] = '\0';
    } else
    ...
    if (XrmGetResource(rDB, "tertris.playerName", "Tertris.PlayerName",
		       str_type, &value) == True) {
    ...
    if (XrmGetResource(rDB, "tertris.foreground", "Tertris.Foreground",
		       str_type, &value) == True) {
    ...
    if (XrmGetResource(rDB, "tertris.background", "Tertris.Background",
		       str_type, &value) == True) {
	(void) strncpy(name, value.addr, ZLIM(value.size, 255));
	name[254] = '\0';
	bg = getGround(name);
    ...
    if (XrmGetResource(rDB, "tertris.geometry", "Tertris.Geometry",
		       str_type, &value) == True) {
    ...
    if (XrmGetResource(rDB, "tertris.iconGeometry", "Tertris.IconGeometry",
		       str_type, &value) == True) {
    ...
    if (XrmGetResource(rDB, "tertris.bigFont", "Tertris.BigFont",
		       str_type, &value) == True) {
	(void) strncpy(name, value.addr, ZLIM(value.size, 255));
	name[254] = '\0';
    } else {
	...
	(void) fprintf(stderr, "Tertris: can't open font '%s'.\n", name);
	exit(1);
    }
    if (XrmGetResource(rDB, "tertris.tinyFont", "Tertris.TinyFont",
		       str_type, &value) == True) {
	(void) strncpy(name, value.addr, ZLIM(value.size, 255));
	name[254] = '\0';
    } else {
