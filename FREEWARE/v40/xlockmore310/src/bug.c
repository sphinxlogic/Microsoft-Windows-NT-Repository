...
#ifndef lint
static char sccsid[] = "@(#)bug.c	3.10 96/07/20 xlockmore";
...
init_bug(ModeInfo * mi)
{
	...
	bp->nccols = min(bp->width, 2 * gridsize);
	bp->ncrows = min(bp->height, 2 * gridsize);
	...
	bp->edenwidth = bp->nhrows / 4;
	bp->edenheight = bp->nhcols / 4;
	if (bp->edenheight)
		bp->edenheight -= !(bp->edenheight && 1);	/* Make sure its odd */
	bp->eden = (bp->edenwidth && bp->edenheight);
	if (bp->eden) {
