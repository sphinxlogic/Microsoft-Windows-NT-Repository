...
$nolink:
$ open/read opt [.src]laser.opt
$ tmpfile:= laser_opt$'f$getjpi (0,"PID")'.tmp
$ open/write tmp 'tmpfile'
$ read opt line1
$ line1= f$element (0,"""", line1) + """" + - 
	 f$element (0,"-",f$element(1,"""",line1)) + -
	 "-" + f$parse (f$search("laser.opt"),,,"VERSION") - ";" + """"
$ write tmp line1
...
$	close tmp
$purge 'tmpfile'
$rename 'tmpfile' [.src]laser.opt
$ set noon
$stop/reset dbg$queue
$on control_y then continue
$sav_prv= f$setprv ("SYSPRV")
$copy 'exedir'laser.exe sys$common:[sysexe]laserdbgsmb
