...
static next_token P_((void));
static chk_funcs P_((void));
static void skip_double P_((void));
static compile P_((int prec));
static execute P_((double *result));
static parse_fieldname P_((char name[], int len));
...
chk_funcs()
{
	static struct {
	    ...
	    int st_tok;
	     ...
	     {"atan2", ATAN2},	 {"atan", ATAN},   {"cos", COS},
	     {"degrad", DEGRAD}, {"exp", EXP},	   {"log10", LOG10},
	     {"log", LOG},	 {"pi", PITOK},	   {"pow", POW},
	     {"raddeg", RADDEG}, {"sin", SIN},	   {"sqrt", SQRT},
	     {"tan", TAN}
	};
	int i;
...
int prec;
{
	...
	for (;;) {
		...
		*pc++ = NEG;
		...
		continue;
	    /* one-arg functions */
            case ABS: case SIN: case COS: case TAN: case ASIN: case ACOS:
	    case ATAN: case DEGRAD: case RADDEG: case LOG: case LOG10:
	    case EXP: case SQRT:
