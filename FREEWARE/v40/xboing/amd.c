...
static struct dsc$descriptor_s devname; /* device we are using */
static short int so_chan;	/* Channel to SODRIVER */
static char ReadBuffer[512];		/* Asynchronous read buffer */
static unsigned short ReadIOsb[4] = { 0, 0, 0, 0};
static int ReadPending = 0;
static int ReadCompleted = 0;


/*
 *	Initialize access to the AMD chip
 */
unsigned long int AmdInitialize(device, volume)
char *device;
int volume;
{
 ...
 */
unsigned long int AmdRead(callback)
int (*callback)();
{
	unsigned long int status;
 ...
 */
unsigned long int AmdWrite(buffer, len)
char *buffer;
int len;
{
	unsigned long int status;
