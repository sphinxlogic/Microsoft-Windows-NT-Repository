...
#define YSIZ (aa_imgheight(context)-6)
#define MAXTABLE (256*5)
aa_context *context;
aa_renderparams *params;
aa_palette palette;
static unsigned char *bitmap;
static unsigned int table[MAXTABLE][4];
static int pal[] =
{
...
static void gentable()
{
	...
	if (i > 6) {
	    p2 = (i - 6) / 5;
	    table[i][3] = p2 * 256 * 256 * 256;
	    table[i][2] = p2 * 256 * 256;
	    table[i][1] = p2 * 256;
	    table[i][0] = p2;
	} else
	    table[i][0] = 0,
...
{
    register unsigned int last3, put, last2;
...
#define END (bitmap + XSIZ * YSIZ)
	...
	put = table[(*(p + XSIZ - 1) + *(p + XSIZ + 1) + *(p + XSIZ))][0];
#if 0
	i -= *(p - 2 + XSIZ * 2);
	i += *(p + 1 + XSIZ * 2);
	put = table[(i + *(p + 3 * XSIZ - 1) + *(p + 4 * XSIZ + 1))][0];
	i -= *(p - 1 + XSIZ * 2);
	i += *(p + 2 + XSIZ * 2);
	put += table[(i + *(p + 3 * XSIZ + 0) + *(p + 4 * XSIZ + 2))][1];


	i -= *(p - 0 + XSIZ * 2);
	i += *(p + 3 + XSIZ * 2);
	put += table[(i + *(p + 3 * XSIZ + 1) + *(p + 4 * XSIZ + 3))][2];

	i -= *(p + 1 + XSIZ * 2);
	i += *(p + 4 + XSIZ * 2);
	put += table[(i + *(p + 3 * XSIZ + 2) + *(p + 4 * XSIZ + 4))][3];
	*((unsigned long *) (p)) = put;
#endif
    }
}
