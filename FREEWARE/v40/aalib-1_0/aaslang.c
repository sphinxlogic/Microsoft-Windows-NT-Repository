...
static int slang_init(struct aa_hardware_params *p, void *none)
{
    fflush(stdout);
    if (!__slang_is_up) {
	SLtt_get_terminfo();
	...
	slang_d.params.supported &= ~AA_BOLDFONT_MASK;
    }
    SLsmg_Display_Eight_Bit = 128;
    slang_d.params.supported |= AA_EIGHT;
...
struct aa_driver slang_d =
{
    ...
    {NULL, AA_NORMAL_MASK | AA_BOLD_MASK | AA_REVERSE_MASK | AA_BOLDFONT_MASK | AA_DIM_MASK},
    slang_init,
    slang_uninit,
    slang_getsize,
    slang_setattr,
