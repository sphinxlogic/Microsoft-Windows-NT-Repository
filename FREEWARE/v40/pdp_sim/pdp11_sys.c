...
int specf (unsigned int addr, int spec, int nval, int flag, int iflag)
{
int reg;
...
{
int i, j, c1, c2, inst, fac, srcm, srcr, dstm, dstr, l8b, brdisp, wd1, wd2;
extern int FPS;
...
/* Instruction decode */
	...
	case I_V_ASOP: case I_V_ASMD:			/* asop, asmd */
		printf ("%s %s,", opcode[i], fname[fac]);
		return specf (addr, dstm, val[2], cflag, TRUE);
	case I_V_DOP:					/* dop */
		printf ("%s ", opcode[i]);
...
int get_reg (char *cptr, const char *strings[], char mchar)
{
int i;
...
char *get_addr (char *cptr, int *dptr, int *pflag)
{
int val, minus;
char *tptr;
...
int get_spec (char *cptr, int addr, int n1, int *sptr, unsigned int *dptr,
	int cflag, int iflag)
{
int reg, indir, pflag, disp;
...
{
int i, j, reg, spec, n1, n2, disp, pflag;
unsigned int r;
char gbuf[CBUFSIZE];
...
if (gbuf[0] == '\'') {					/* single char? */
	...
	val[0] = (unsigned int) gbuf[1];
	return SCPE_OK;  }
if (gbuf[0] == '"') {					/* string? */
	if (gbuf[1] == 0) return SCPE_ARG;
	val[0] = ((unsigned int) gbuf[2] << 8) + (unsigned int) gbuf[1];
	return SCPE_OK;  }
...
case I_V_3B: case I_V_6B: case I_V_8B:			/* xb literal */
	...
	if (get_unum (gbuf, 8, (1 << j) - 1, &r) != SCPE_OK) return SCPE_ARG;
	val[0] = val[0] | r;				/* put in place */
	break;
case I_V_BR:						/* cond br */
