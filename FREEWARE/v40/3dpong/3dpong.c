...
#define M_PI       3.1415926535897932385E0      /* pi         */
#endif
#endif
...
void eventloop(void)
{
  ...
  do
      ...
      for (pln = 0; pln < num_screens; pln++)
	{
	  ...
	  XCopyArea(display[pln], backbuffer[pln], window[pln], whitegc[pln],
      ...
      do
	{
      ...
      while (now.time == then.time && now.millitm < then.millitm + FRAMERATE);
    }
  while (done == False);
}
...
void setup(int argc, char * argv[])
{
  ...
  if (argc == 2)
	  ...
	  strcmp(argv[1], "-v") == 0)
	{
	  ...
	  printf("http://zippy.sonoma.edu/kendrick/nbs/unix/x/3dpong/\n\n");
	  exit(0);
	}
    }
  ...
  if (argc == 2)
  ...
  else
    {
      ...
      fprintf(stderr, "server1 [server2 | -computer] [-gravity value]\n");
      fprintf(stderr, "       [-net value] [-sound] [-nockick1] ");
      fprintf(stderr, "[-noclick2]\n");
      fprintf(stderr, "\n");
      fprintf(stderr, "Controls:\n");
      fprintf(stderr, "  [V] - Change view\n");
      fprintf(stderr, "  [3] - Toggle 3D glasses mode\n");
      fprintf(stderr, "  [C] - Toggle \"noclick\" mode\n");
...
void Xsetup(int pln)
{
  ...
  backbuffer[pln] = XCreatePixmap(display[pln], root[pln], WIDTH, HEIGHT, 8);
