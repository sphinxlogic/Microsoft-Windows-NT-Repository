...
;; VMS modifications by Hal Brand (BRAND@addvax.llnl.gov)
;;                  and Richard Levitte <levitte@e.kth.se>
  ...
  (and (null rmail-inbox-list)
       ...
       (setq rmail-inbox-list
	     (or rmail-primary-inbox-list
 		 (if (member system-type '(vax-vms axp-vms))
 		     '("+VMS+")
		   (list (or (getenv "MAIL")
...
(defun rmail-expunge-and-save ()
  ...
  (save-buffer)
  (if (eq system-type 'vax-vms)
      (call-process "purge" nil nil nil
		    (concat "/keep=" 
			    (if (and (boundp 'rmail-vms-versions-kept)
				     (numberp rmail-vms-versions-kept))
				rmail-vms-versions-kept
			      2))
  ...
  (unwind-protect
	...
	(save-excursion
	  (save-restriction
	    ...
	    ;; On VMS, get new messages from MAIL if a file wasn't specified
	    ;; and the rmail-inbox-list is empty.  NOT ANY MORE!
	    ;; (if (and (eq system-type 'vax-vms)
	    ;;          (not file-name)
	    ;;          (not rmail-inbox-list))
	    ;;     (call-process "vmsgetmail" nil t nil
	    ;;                   " --newmail" "--delete"))
	    ;; Scan the new text and convert each message to babyl format.
	    (goto-char (point-min))
	    (save-excursion
	      (setq new-messages (rmail-convert-to-babyl-format)))
	    ;; On VMS getmail spits errors into the buffer if there is no new mail
	    ;; This hack make rmail think nothing happened.
	    ;; (if (and (eq system-type 'vax-vms)
	    ;;          (zerop new-messages))
	    ;;     (progn
...
(defun rmail-insert-inbox-text (files renamep)
  (let (file tofile delete-files movemail popmail)
    (while files
      (setq vmsmovemail (and (member system-type '(vax-vms axp-vms))
 			     (member (car files) '("+VMS+" "+vms+")))
 	    file (file-truename
	    ...
	    ;; (as happened to me!).
	    tofile (concat file (if (eq system-type 'vax-vms) "$" "+")))
	...
	(if (or (and (file-exists-p tofile)
      ...
      (cond ((not renamep)
	    ...
	    ((or (file-exists-p tofile) (and (not popmail)
	     ...
	     nil)
	    ((and (not vmsmovemail) (not movemail) (not popmail))
	    ...
	    (t
	     (let ((errors nil)
		   (error-prefix (if (member system-type '(vax-vms axp-vms))
				     "vmsgetmail: "
				   "movemail: ")))
	       (unwind-protect
		   (save-excursion
		     ...
		     (buffer-disable-undo errors)
 		     (if (member system-type '(vax-vms axp-vms))
 			 (call-process "vmsgetmail" nil errors nil
 				       "--newmail" "--file" tofile
 				       "--append" "--delete")
		       (call-process
...
(defun rmail-convert-to-babyl-format ()
  (let ((count 0) start
	...
	(cond ((looking-at "BABYL OPTIONS:");Babyl header
		 ...
		 (delete-region (point-min) (point))))
	      ;;*** VMS MAIL format.  This has the be placed before the
	      ;; Babyl message test, since that one also checks for ^L!
	      ((and ; is this needed? (memq system-type '(axp-vms vax-vms))
  ...
  (let (from reply-to cc subject date to message-id resent-reply-to)
	...
	(let* ((cc-list (rmail-dont-reply-to
	  ...
	  (if (string= cc-list "") nil
	    (if (eq system-type 'vax-vms)
		(rmail-format-vms-cc-list cc-list reply-to)
	      cc-list))))
      (current-buffer)
...
(defun rmail-format-vms-cc-list (cc-list to-field)
  (let (protocol
	end
	pos
	(new-cc-list nil)
	cc-addr)
    (if (setq end (string-match "%" to-field))
	(progn
	  (setq pos (1- end))
	  (while (and (>= pos 0)
		      (or (/= (aref to-field pos) ? )
			  (/= (aref to-field pos) ?,)))
	    (setq pos (1- pos)))
	  (setq protocol (substring to-field (1+ pos) (1+ end)))
	  (while cc-list
	    (if (setq pos (string-match ", " cc-list))
		(setq cc-addr (substring cc-list 0 pos)
		      cc-list (substring cc-list (match-end 0)))
	      (setq cc-addr cc-list
		    cc-list nil))
	    (if (and (string-match "@" cc-addr)
		     (not (string-match protocol cc-addr)))
		(setq cc-addr (concat protocol "\"" cc-addr "\"")))
	    (setq new-cc-list
		  (concat new-cc-list (if new-cc-list ", ") cc-addr)))
	  new-cc-list)
      cc-list)))

(defun rmail-make-in-reply-to-field (from date message-id)
  ;; Remove the possible FOO%"..." wrapping on VMS.
  ;; vms-mail-remove-transport is defined in mail-utils.el
  (if (and (eq system-type 'vax-vms)
	   (string-match "^[a-zA-Z0-9]+::[^@\\.]+$" from))
      from
    (and (eq system-type 'vax-vms)
	 (setq from (vms-mail-remove-transport from)))
    (cond ((not from)
	   (if message-id
	       message-id
             nil))
