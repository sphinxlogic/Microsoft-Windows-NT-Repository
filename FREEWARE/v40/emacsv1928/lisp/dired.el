...
;;;###autoload
(defvar dired-listing-switches (if (eq system-type 'vax-vms)
				   "/SIZE/DATE/OWNER/PROTECTION"
				 "-al")
...
;;;###autoload
(defvar dired-trivial-filenames (if (eq system-type 'vax-vms)
				    nil
				  "^\\.\\.?$\\|^#")
...
;;;###autoload
(defvar dired-copy-preserve-time (not (eq system-type 'vax-vms))
...
(defvar dired-re-dir
  (if (member system-type '(vax-vms axp-vms))
      (concat dired-re-maybe-mark
	      "[_0-9A-Z$][_0-9A-Z$---]*.DIR\\(;[0-9]*\\)?[ \t\n]")
    (concat dired-re-maybe-mark dired-re-inode-size "d")))
...
(if dired-mode-map
  ...
  (if (eq system-type 'vax-vms)
      (define-key dired-mode-map ";" 'dired-version-toggle))
  )

;; Make menu bar items.
