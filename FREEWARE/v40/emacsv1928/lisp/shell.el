...
(defvar shell-cd-regexp ;(if (eq system-type 'vax-vms)
			    ;"\\(cd\\|set  *de\\(f\\|fa\\|fau\\|faul\\|fault\\)\\)"
			  "cd";)
...
\(Type \\[describe-mode] in the shell buffer for a list of commands.)"
  ...
  (if (not (comint-check-proc "*shell*"))
      (let* ((prog (or explicit-shell-file-name
	     ...
	     (startfile (concat "~/.emacs_" (make-legal-file-name name)))
...
Environment variables are expanded, see function `substitute-in-file-name'."
  (if shell-dirtrackp
	  ...
	  (let ((start (progn
			 (string-match "^\\(\\s\\|[; ]\\)*" ; skip whitespace
				       str)
			 (match-end 0)))
		end cmd arg1 arg2)
	      ...
	      (cond ((eq (string-match shell-popd-regexp cmd) 0)
		    ...
		    ((eq (string-match shell-cd-regexp cmd) 0)
 		    ...
 		    ((and (memq system-type '(vax-vms axp-vms))
 			  (string-match "^set$" cmd)
 			  (= (string-match "^def\\(a\\(u\\(lt?\\)?\\)?\\)?$"
 					    arg1) 0))
		     (setq arg2 (comint-arguments (substring str start end) 2 2))
 		     (shell-process-cd (substitute-in-file-name arg2))))
 	      (setq start (progn
 			    (string-match "\\(\\s\\|[; ]\\)*" ; skip again
 					  str end)
 			    (match-end 0)))
...
(defun shell-prefixed-directory-name (dir)
  (if (memq system-type '(vax-vms axp-vms))
      ;; It may be a logical the user's moving to, and in that case,
      ;; it's probably not concealed...
      (let ((old-dir dir))
	(while (and dir
		    (string-match "^\\([A-Za-z$_][A-Za-z$_]*\\):?$" dir)
		    (setq old-dir dir
			  dir
			  (vms-system-info "LOGICAL"
					   (upcase
					    (substring dir
						       (match-beginning 1)
						       (match-end 1)))))))
...
(defun shell-dirstack-message ()
  (let* ((msg "")
	 ...
	 (home (expand-file-name (concat (if (memq system-type
						   '(vax-vms axp-vms))
					     ""
    ...
    (while ds
      (let ((dir (car ds)))
	(and (not (memq system-type '(vax-vms axp-vms)))
	     (>= (length dir) homelen)
