...
#ifdef MULTINET
#ifndef __GNUC__
#define __TYPES 1		/* Otherwise, the Multinet types.h is
				   defining time_t, even though it's already
				   defined */
#include "multinet_root:[multinet.include.sys]types.h"
#undef FD_SET    /* we need types for the rest of the multinet includes */
#undef FD_CLR    /* to work with vaxc.  but types makes bogus defns */
#undef FD_ISSET  /* of the FD_* macros... */
#undef FD_ZERO
#endif
...
ALMOST_STATIC Lisp_Object Vprocess_connection_type;
...
ALMOST_STATIC int process_tick;
...
ALMOST_STATIC int max_process_desc;
...
/* On VMS, nonzero means split the status code returned by a subprocess
   into a list of numbers, one for each field in the status code:

     (SEVERITY MESSAGE FACILITY INTERNAL)

   Where those fields correspond to these bits in the status code:

     SEVERITY:  Bits 0-2 contain the severity level of the message.
     MESSAGE:   Bits 3-15 contain the number of the corresponding
                message.
     FACILITY:  Bits 16-27 contain a number for the software component,
                or facility, that generated the message.
     INTERNAL:  Bits 28-31 contain internal control flags.

   There is a routine, do_vms_split_status_code(), which does the split
   for you.  */
int vms_split_status_code;
...
/* Used with VMS only */
Lisp_Object
do_vms_split_status_code (status)
     unsigned long status;
{
  return Fcons (make_number (status & 7), /* Bits 0-2 */
		Fcons (make_number ((status >> 3)
				    & ((1<<13) - 1)), /* Bits 3-15 */
		       Fcons (make_number ((status >> 16)
					   & ((1<<12) -1)), /* Bits 16-27 */
			      Fcons (make_number ((status >> 28)
						  & ((1<<4) - 1)), /* Bits 28-31 */
				     Qnil))));
}
     ...
     Lisp_Object name;
{
  ...
  register int i;
#ifdef VMS
  VMS_PROC_STUFF *get_vms_process_stuff (), *ps;

  ps = get_vms_process_stuff ();
  if (ps == 0)
     ...
     register Lisp_Object proc;
{
  ...
  sys$setast (0);
  vs = get_vms_process_pointer (XPROCESS (proc));
  if (vs != 0)
    vs->active = 0;
  sys$setast (1);
#endif /* VMS */
...
#ifdef VMS
	  if (XINT (tem) < NSIG)
	    write_string (strerror (XINT (tem)), -1);
	  else
#endif
	    Fprinc (symbol, Qnil);
	}
...
#define START 4
  ...
  {
    int start = 3;
    CHECK_STRING (args[2], 0);
    if (strcmp (XSTRING (args[2])->data, "*dcl*") == 0
	&& nargs > start + 1)
      {
	CHECK_STRING (args[start], 1);
	if (strcmp (XSTRING (args[start])->data, "-c") == 0)
	  {
	    new_argv[0] = XSTRING (args[2])->data;
      ...
      {
	tem = alloca (XSTRING (args[2])->size + 256);
	strcpy (tem, XSTRING (args[2])->data);
      ...
      Lisp_Object name, buffer, host, service;
{
...
#ifdef VMS
      /* So far, getservbyname exists as an entry, but not in reality.
	 As long as it stays this way, it returns -1. */
      if (svc_info == (struct servent *)(-1))
	error ("VMS can't handle named services (\"%s\"). Please use a numeric one.", XSTRING (service)->data);
#endif
 ...
 loop:
  ...
  XSET (XPROCESS (proc)->infd, Lisp_Int, inch);
     ...
     Lisp_Object proc;
{
  ...
  if (inchannel >= 0)
	...
	extern give_back_vms_process_stuff ();

	vms_close_fd (inchannel);
	if (outchannel != inchannel)
	  vms_close_fd (outchannel);
	vs = get_vms_process_pointer (p);
	if (vs == 0)
	  error ("Could not deactivate process: %x",
		 (getpid () & 0xff000000) | p->pid);
...
#endif /* DEBUG */
	rv = vms_write_fd (XFASTINT (XPROCESS (proc)->outfd), buf, this,
			   vs->translate_p);
#else
...
#ifdef VMS
  gid = - ((getpid () & 0xff000000) | (XFASTINT (p->pid)));
#endif
...
#ifdef VMS
      {
	unsigned long tem_pid = (getpid () & 0xff000000) | (XFASTINT (p->pid));
	status = sys$forcex (&tem_pid, 0, SS$_FORCEDEXIT);
	goto whoosh;
      }
#endif
    case SIGKILL:
      ...
      {
	unsigned long tem_pid = (getpid () & 0xff000000) | (XFASTINT (p->pid));
	int status = sys$delprc (&tem_pid, 0);
      }
      whoosh:
#endif
     ...
     int desc;
{
  ...
  vs->chan = desc;
  desc = get_vms_channel_handle (vs);
#endif
...
The value takes effect when `start-process' is called.");
  ...
  DEFVAR_BOOL ("vms-split-status-code", &vms_split_status_code,
    "Non-nil means split the status code returned by subprocesses.\n\
nil means return the status code as is.  This variable is meaningful\n\
only on VMS.\n\
On VMS, the status code is really a 32-bit array of bit fields, with\n\
...
\n\
  ...
  INTERNAL:  Bits 28-31 contain internal control flags.\n\
\n\
Normally, it is just returned as one integer, but when\n\
vms-split-status-code is t, the status code will instead be returned as\n\
(SEVERITY MESSAGE FACILITY INTERNAL).");
  vms_split_status_code = 0;
