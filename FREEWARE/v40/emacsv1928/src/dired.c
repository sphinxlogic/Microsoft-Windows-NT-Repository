...
#ifdef VMS
#define Ffile_name_all_completions_with_versions	Ffile_name_all_compls_w_vers
#define Qfile_name_all_completions_with_versions	Qfile_name_all_compls_w_vers
#define Sfile_name_all_completions_with_versions	Sfile_name_all_compls_w_vers
Lisp_Object Qfile_name_all_completions_with_versions;
#endif
  ...
  if (!NILP (match))
    {
	 ...
	 compile_pattern to do the work for us.  */
      ...
      compile_pattern (match, &searchbuf, 0,
		       XSTRING (buffer_defaults.downcase_table)->data);
#else
...
#ifdef VMS
  d = opendir (XSTRING (dirname)->data, "*.*");
#else
...
#ifdef VMS
DEFUN ("file-name-all-completions-with-versions",
       Ffile_name_all_completions_with_versions,
       Sfile_name_all_completions_with_versions, 2, 2, 0,
  "Return a list of all completions of file name FILE in directory DIR.\n\
These are all file names in directory DIR which begin with FILE.\n\
All versions of each file is included in the list.")
  (file, dirname)
     Lisp_Object file, dirname;
{
  Lisp_Object handler;
  ...
  if (!NILP (handler))
    return call3 (handler, Qfile_name_all_completions_with_versions, file, dirname);
     ...
     int all_flag, ver_flag;
{
  ...
  char *file_pattern = "*.*";
  int search_directories_p = 0;
  int can_take_zeroes = 0;
  char *dir_delimiter;
  Lisp_Object save_file = Qnil, save_dirname = Qnil;
  ...
  bestmatch = Qnil;
#ifdef VMS
  GCPRO4 (save_file, file, dirname, bestmatch);
#else
...
#ifdef VMS
  {
    int i = XSTRING (file)->size;
    /* There is a case when an extra space is added at the end... */
    while (XSTRING (file)->data[i - 1] == ' ')
      i--;
    if (i < XSTRING (file)->size)
      file = make_string (XSTRING (file)->data, i);
  }
  file = Fupcase (file);
  {
    char *fstart = XSTRING (file)->data, *fptr = fstart;

    if (fstart[0] == '[' || fstart[0] == '<')
      {				/* AHA! Completing a directory! */
	char *tmp;
	search_directories_p = 1;
	can_take_zeroes = XSTRING (file)->size < 8
	  && strncmp(fstart + 1, "000000", XSTRING(file)->size - 1) == 0;
	dir_delimiter = build_string (fstart[0] == '[' ? "]" : ">");
	save_dirname = dirname;
	for (tmp = fstart; tmp > (char *)1; tmp++)
	  {
	    fptr = tmp;
	    tmp = strchr (tmp, '.');
	  }
	/* fptr should either be at the start of the filename, or after
	   the last dot */
	if (fptr == fstart)
	  fptr++;
	if (fptr <= fstart + 2)
	  { /* This "file" start with "[" or "[.", so we add the root
	       specification, so opendir knows where we want to search.  */
/*	    if (XSTRING (dirname) ->data[XSTRING (dirname)->size - 1] != ']'
		&& XSTRING (dirname) ->data[XSTRING (dirname)->size - 1] != '>') */
	    if (fptr == fstart + 1)
	      if (*fptr != '\0')
		dirname = concat2 (dirname, build_string ("[000000]"));
	      else
		dirname = concat2 (dirname,
				   concat2 (file, build_string ("000000]")));
	  }
	else
	  { /* Otherwise, we have a dir spec in which we want to search.
	       We just pick the right part of the file name and concatenate
	       it with the dirname.  */
	    dirname = concat2 (dirname,
			       concat2 (make_string (XSTRING (file)->data,
						     fptr - fstart - 1),
					build_string ("]")));
	  }
	save_file = make_string (XSTRING (file)->data, fptr - fstart);
	file = make_string (fptr, XSTRING (file)->size - (fptr - fstart));
	ver_flag = 0;
      }
  }
  {
    static char fp_buf[256];
    char *fstart = XSTRING(file)->data;
    int sepnbr = 0;
    char *ftmp = strchr (fstart, '.'), *fptr;

    if (ftmp == 0)
      {
	if ((ftmp = strchr (fstart, ';')) != 0)
	  sepnbr = 2;
      }
    else
      {
	...
	if (ftmp != 0)
	  {
	    *ftmp = ';';	/* If it was a dot, it won't match... */
	    sepnbr = 2;
	  }
      }
    /* The following is deep dark C magic (ok, almost...).  It should
       make a file pattern with the right amount of wildcards added to
       it.  */
    if (search_directories_p)
      {
	strcpy (fp_buf, fstart);
	strcat (fp_buf, "*.DIR");
	file_pattern = fp_buf;
      }
    else
      {
	if (ver_flag)
	  { /* I want to be able to get the value of that expression
	       when I'm debugging.  -- Richard Levitte  */
	    int dummy = (ver_flag == -1) + sepnbr*2;
	    fptr = "*.*;*" + dummy;
	  }
	else
	  { /* same goes here... */
  ...
  for (passcount = !!all_flag; NILP (bestmatch) && passcount < 2; passcount++)
    {
#ifdef VMS
      if (!(d = opendir (XSTRING (dirname)->data, file_pattern)))
#if 0				/* For the sake of c-mode */
	blah ();
#endif
#else
...
#ifdef VMS
	  if (can_take_zeroes && bestmatch == Qnil)
	    {
	      static DIRENTRY __d;
	      dp = &__d;
	      strcpy (dp->d_name, "000000.DIR");
	      dp->d_namlen = len = strlen(dp->d_name);
	      directoryp = 1;
...
#ifdef VMS
  if (search_directories_p)
    if (XSTRING (bestmatch)->size == bestmatchsize)
      return concat2 (save_file, Fsubstring (bestmatch,
					     make_number (0),
					     make_number (bestmatchsize - 5)));
    else
      return concat2 (save_file, Fsubstring (bestmatch,
					     make_number (0),
					     make_number (bestmatchsize)));
  if (XSTRING (bestmatch)->size == bestmatchsize
      && XSTRING (bestmatch)->data[XSTRING (bestmatch)->size - 1] == '/')
    {
      return Ffile_name_as_directory
	(make_string (XSTRING (bestmatch)->data,
		      XSTRING (bestmatch)->size - 1));
    }
#endif
...
10. inode number. On VMS, it is a list of three numbers.\n\
11. Device number.\n\
\n\
...
#endif
#ifdef VMS
  values[10] = Fcons (make_number (s.st_ino[0]),
		      Fcons (make_number (s.st_ino[1]),
			     Fcons (make_number (s.st_ino[2]), Qnil)));
#else
...
#ifdef VMS
  Qfile_name_all_completions_with_versions = intern ("file-name-all-completions-with-versions");
#endif
...
#ifdef VMS
  defsubr (&Sfile_name_all_completions_with_versions);
