...
These are the possibilities for the first arg (upper or lower case ok):\n\
    ...
    logical     Translates VMS logical name (second argument)\n\
                an optional third argument is the index in a search list\n\
...
{
  char buf[256];
  extern char *expand_search_paths ();

  CHECK_STRING (filename, 0);
  if(NILP (filename)) return (Qnil);
  /* Really, I'd like to send the `filename' as a char *,
     not as a Lisp_Object. -- Richard Levitte   */
  return build_string (expand_search_paths (XSTRING (filename)->data,
					    !NILP (expand_completely)));
}
...
{
  Lisp_Object tem, dev, result = build_string (file), temp_result;
  static char buf[256];
  int i,n,new;
  char *cp, *bp;
  int skip_0_dir = 0;
  ...
  while(1)
      ...
      if (!cp)
	{
      ...
      *cp = 0;
      ...
      n = vms_trnlnm (XSTRING (dev)->data, 0, buf);
      if (n <= 0)
	{
	  UNGCPRO;
	  return XSTRING (result)->data; /* no more translations or error */
	}
      ...
      for(i = 0 ; i < n ; i++)
	{
	  ...
	  tem = Fexpand_file_name (build_string (buf), Qnil);
	  ...
	  if (i < n-1)
	    vms_trnlnm (XSTRING (dev)->data, i + 1, buf);
	}
    }
}
...
{
  int level, attr = LNM$M_CASE_BLIND;
  short length, levlen;
  struct itemlist {
    short buffer_length;
    short item_code;
    ...
    255, LNM$_STRING   , (char *) output, (int *) &length,
    4,   LNM$_MAX_INDEX, (char *) &level, (int *) &levlen,
    0,   0,   0,  0 };
  struct dsc$descriptor_s lognam =
    { strlen (name), DSC$K_DTYPE_T, DSC$K_CLASS_S, name };
  $DESCRIPTOR (tabnam, "LNM$DCL_LOGICAL");

  trnlnm_status = sys$trnlnm (&attr, &tabnam, &lognam, 0, itmlst);

  if (trnlnm_status != SS$_NORMAL)
    return -1;
     ...
     Lisp_Object arg1, arg2;
{
  ...
  char *name;
  int status;
  int index, save_index, level;

  CHECK_STRING (arg1, 0);
  if (!NILP (arg2))
    {
      CHECK_NUMBER (arg2, 0);
      save_index = XINT (arg2);
    }
  else
    save_index = 0;
  
  name = XSTRING (arg1)->data;

  index = save_index >= 0 ? save_index : 0;

  level = vms_trnlnm (name, index, str);
  if (level < 0)
    {
      if (trnlnm_status == SS$_NOLOGNAM)
	return Qnil;
      error ("Unable to translate logical name: %s", 
	     vmserrstr (trnlnm_status));
    }
  if (save_index < 0)
    return make_number (level);
     ...
     ()
{
  ...
  static int chan = 0;
  static int buf = 0x9d + ('2'<<8) + ('2'<<16) + (0x9cU<<24);
  int status;
