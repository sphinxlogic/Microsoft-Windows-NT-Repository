...
$ node_p = f$locate("::", default)
$ if node_p .lt. len
$  then
$   default_node = f$extract(0,node_p,default)+"::"
$   default = f$extract(node_p+2,len,default)
$   len = len - node_p
$  endif
...
$   node = ""
$   dev = ""
$   node_p = f$locate("::", file_e)
$   if node_p .lt. len
$    then
$     node = f$extract(0,node_p,file_e)+"::"
$     file_e = f$extract(node_p+2,len,file_e)
$     len = len - node_p
$    endif
$   dev_p = f$locate(":", file_e)
$   if dev_p .lt. len
$    then
$     dev = f$extract(0,dev_p,file_e)
$     file_e = f$extract(dev_p+1,len,file_e)
$     len = len - node_p
$    endif
$
$   if node .eqs. "" then node = default_node
$   if dev .eqs. "" then dev = default_dev
$   c1 = f$extract(0,1,file_e)
$   c2 = f$extract(0,2,file_e)
$!DEBUG
$	dummy := 'c1'		!c1
$	dummy := 'c2'		!c2
$	dummy := 'file_e'	!file_e
$!NODEBUG
$   if c2 .eqs. "[." .or. c2 .eqs. "<." then -
	file_e = (default_dir - "]" - ">") + (file_e - "[" - "<")
$   if c1 .nes. "[" .and. c1 .nes. "<" then file_e = default_dir + file_e
$
$   max_index = ""
$   if dev .nes. "" then max_index = f$trnlnm(dev,,,,,"MAX_INDEX")
$!DEBUG
$	dummy := 'node'		!node
$	dummy := 'dev'		!dev
...
$	dummy := 'file_e'	!file_e
$!NODEBUG
$   if node .eqs. "" .and. f$string(max_index) .nes. ""
$    then
$     dev_i = max_index
$     __result = ""
...
$     __result = ""
$     delete/symbol/local __result
$     @'proc_dir'canonicaldir 'node''dev':'file_e' __result
$    endif
$   if f$search(__result) .nes. ""
$    then
$     copy 'node''dev':'file_e' 'postfix' 'prefix'
$    else
$     write sys$error "%GNUCOPY-W-NOFILE, file ",node,dev,":",file_e," does not exist"
$    endif
$   ! 'f$verify(0)
$   goto loop_files
$  endif
