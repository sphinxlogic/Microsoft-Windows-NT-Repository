     ...
     char *argv[];
{
...
#ifdef USE_HACKARGV
  extern hackargv (int *pargc, char ***pargv, ... /* int start */);
#else
  extern char *gfnames ();
...
#if defined(VMS) && defined(USE_HACKARGV)
  hackargv (&argc, &argv);
#endif
  progname = argv[0];
  ...
  cwd = concat (etags_getcwd (), "", ""); /* the current working directory */
#ifndef VMS
  strcat (cwd, "/");
#endif
  if (streq (tagfile, "-"))
  ...
  argc -= optind;
  argv += optind;
  while ((this_file = gfnames (&argc, &argv, &got_err)) != NULL)
    {
      if (got_err)
	{
  ...
  for (; optind < argc; optind++)
    {
      ...
      {				/* solely to balance with the VMS code above */
#endif
...
#define MAXPATHLEN 1024
  char buf[MAXPATHLEN];
  char *temp = getcwd (buf, MAXPATHLEN);
  if (temp == 0)
    return 0;
#endif
  return buf;
     ...
     char *file, *dir;
{
  char *fp, *dp, *res;
#ifdef VMS
  char *outtadir = 0, *outtadirdp, *savefp, *savedp;
#endif

#ifdef VMS /* DEBUG */
#ifdef DEBUG
  fprintf (stderr, "DEBUG: start of relative_filename (%s, %s)\n",
	   file, dir);
#endif
#endif
  /* Find the common root of file and dir. */
  savefp = fp = absolute_filename (file, cwd);
  savedp = dp = dir;
#ifndef VMS
  while (*fp++ == *dp++)
    continue;
...
#define DIRPARENT "-"
#ifdef DEBUG
fprintf (stderr, "DEBUG: fp = \"%s\", dp =  \"%s\", outtadir =  \"%s\"\n",
	 fp, dp, outtadir);
#endif
#endif
  /* Build a sequence of "../" strings for the resulting relative filename. */
  for (dp = etags_strchr (dp + 1, DIRDELIM), res = "";
       dp != NULL;
       dp = etags_strchr (dp + 1, DIRDELIM))
    {
#ifdef VMS
#ifdef DEBUG
      fprintf (stderr, "DEBUG: fp = \"%s\", dp =  \"%s\", outtadir =  \"%s\"\n",
...
#ifdef VMS /* DEBUG */
#ifdef DEBUG
  fprintf (stderr, "DEBUG: end of relative_filename (%s, %s) -> %s\n",
	   file, dir, res);
#endif
#endif
...
absolute_filename (file, cwd)
     char *file, *cwd;
{
  char *slashp, *cp, *res, *eol;

#ifdef VMS /* DEBUG */
#ifdef DEBUG
  fprintf (stderr, "DEBUG: start of absolute_filename (%s, %s)\n",
	   file, cwd);
#endif
#endif
#ifndef VMS
  ...
  for (cp = etags_strchr (res, '<'); cp != 0; cp = etags_strchr (cp + 1, '<'))
    *cp = '[';
  for (cp = etags_strchr (res, '>'); cp != 0; cp = etags_strchr (cp + 1, '>'))
    *cp = ']';
  eol = etags_strrchr (res, ']');
  cp = etags_strrchr (res, ':');
  if (cp > eol)
    eol = cp;
#endif
  ...
  while (slashp != 0 && slashp < eol)
      ...
      while (slashp[1] == '-')
	{
	  ...
	  if (cp <= res)
	    {
	      cp = savecp;	/* Whooops, much too far... */
	      break;
	    }
	  slashp++;
	}
...
#ifdef VMS /* DEBUG */
#ifdef DEBUG
  fprintf (stderr, "DEBUG: end of absolute_filename (%s, %s) -> %s\n",
	   file, cwd, res);
#endif
#endif
  return res;
...
#ifdef VMS /* DEBUG */
#ifdef DEBUG
  fprintf (stderr, "DEBUG: start of absolute_dirname (%s, %s)\n",
	   file, cwd);
#endif
#endif
#ifndef VMS
...
#ifdef VMS /* DEBUG */
#ifdef DEBUG
      fprintf (stderr, "DEBUG: end of absolute_dirname (%s, %s) -> %s\n",
	       file, cwd, cwd);
#endif
#endif
      return cwd;
