 ...
 *
 * $Header: /usr/build/vile/vile/RCS/buffer.c,v 1.177 1998/09/30 01:30:11 tom Exp $
 *
 */
...
namebuffer(int f GCC_UNUSED, int n GCC_UNUSED)	/*	Rename the current buffer	*/
{
	...
	if (is_scratchname(curbp->b_bname)) {
		char procname[NBUFN];
		(void) strip_brackets(procname, curbp->b_bname);
		if (rename_namebst(procname, bufn) != TRUE)
			return FALSE;
	}
#endif
