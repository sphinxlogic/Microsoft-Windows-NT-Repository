 ...
 *
 * Revisions:
 *	?/93:  Fixed several major bugs..., internal 3.9
 ...
 *	2/93:  Added LosingFocusCB for text-field to commit edit.
 ...
 *	9/94:  Added oddRowBackground & evenRowBackground
...
static char sccsid[] = "@(#)Matrix.c	4.1 3/19/95";
#endif
...
#else
#	define DRAW_SHADOW(dpy, draw, tgc, bgc, sz, x, y, w, h)	\
		_XmDrawShadows(dpy, draw, tgc, bgc, x, y, w, h, sz, XmSHADOW_OUT)
#endif

#define CLIENT(t,v)     t v=(t)client
#define CALL(t,v)       t v=(t)call

#if (__STDC__ > 0) || defined(__cplusplus) || defined(c_plusplus) || \
    defined(FUNCPROTO) || (NeedFunctionPrototypes != 0)
/* ANSI stype function args and prototypes */
#  define _			,
#  define F_ARGS(fal, fat)	( fat )
#  define P_ARGS(a)		a
#else
/* Original K&R stype function args and prototypes */
#  define _			;
#  define __			,
#  define F_ARGS(fal, fat)	( fal ) fat ;
#  define P_ARGS(a)
#endif

#if !defined(XlibSpecificationRelease) && !defined(bcopy) && !defined(XBAE_NEED_BCOPY)
...
void CopyBackground P_ARGS((Widget widget, int offset, XrmValue *value));
void CopyForeground P_ARGS((Widget widget, int offset, XrmValue *value));
...
#define ROW_LABEL_WIDTH(mw)	(mw->matrix.row_labels \
				 ? (mw->matrix.row_label_width * \
				    FONT_WIDTH(mw)) + (int)TEXT_WIDTH_OFFSET(mw) * 2 \
				 : 0)
 ...
 * New Matrix methods
 */
static void SetCell(XbaeMatrixWidget, int, int, _Xconst String, Boolean);
...
static Boolean CommitEdit(XbaeMatrixWidget, Boolean);
static void CancelEdit(XbaeMatrixWidget, Boolean);
static void AddRows();
 ...
 */
static void CreateDrawGC P_ARGS((XbaeMatrixWidget mw));
static void GetInverseGC P_ARGS((XbaeMatrixWidget mw));
static void CreateDrawClipGC P_ARGS((XbaeMatrixWidget mw));
static void CreateInverseClipGC P_ARGS((XbaeMatrixWidget mw));
static void CreateTopShadowClipGC P_ARGS((XbaeMatrixWidget mw));
static void CreateBottomShadowClipGC P_ARGS((XbaeMatrixWidget mw));
static void SetClipMask P_ARGS((XbaeMatrixWidget mw));
static void NewFont P_ARGS((XbaeMatrixWidget mw));
static void GetCellTotalWidth P_ARGS((XbaeMatrixWidget mw));
static void GetColumnPositions P_ARGS((XbaeMatrixWidget mw));
static void DrawString P_ARGS((XbaeMatrixWidget mw, Window win, String string, int length, int x, int y, int maxlen, unsigned char alignment, Boolean highlight, Boolean bold, Boolean clip, Pixel color));
static void DrawColumnLabel P_ARGS((XbaeMatrixWidget mw _ int column));
static void DrawRowLabel P_ARGS((XbaeMatrixWidget mw _ int row));

static void DrawCellPixmap P_ARGS((XbaeMatrixWidget mw _ int row _ int column _ Pixmap pixmap));
static void RedrawCells P_ARGS((XbaeMatrixWidget mw _ Rectangle *expose));
static void RedrawLabelsAndFixed P_ARGS((XbaeMatrixWidget mw _ Rectangle *expose));
static void ComputeSize P_ARGS((XbaeMatrixWidget mw _ Boolean compute_width _ Boolean compute_height));
static void FreeCells P_ARGS((XbaeMatrixWidget mw));
static void FreeRowLabels P_ARGS((XbaeMatrixWidget mw));
static void FreeColumnLabels P_ARGS((XbaeMatrixWidget mw));
static void FreeColors P_ARGS((XbaeMatrixWidget mw));
static void FreeSelectedCells P_ARGS((XbaeMatrixWidget mw));
static void CreateColors P_ARGS((XbaeMatrixWidget mw));
static void CopySelectedCells P_ARGS((XbaeMatrixWidget mw));
static void ResizeCells P_ARGS((XbaeMatrixWidget current _ XbaeMatrixWidget new));
static void ResizeSelectedCells P_ARGS((XbaeMatrixWidget current _ XbaeMatrixWidget new));
static void ResizeColors P_ARGS((XbaeMatrixWidget current _ XbaeMatrixWidget new));
static void RowColToXY P_ARGS((XbaeMatrixWidget mw _ int row _ int column _ int *x _ int *y));

static void AddRowsToTable P_ARGS((XbaeMatrixWidget mw _ int position _ String *rows _ String *labels _ Pixel *colors _ int num_rows));
static void DeleteRowsFromTable P_ARGS((XbaeMatrixWidget mw _ int position _ int num_rows));

static void AddColumnsToTable();
static void DeleteColumnsFromTable();
static void ClearCell();
static void GetVisibleRows();
static void GetVisibleColumns();
static void GetVisibleCells();
static void MakeRowVisible();
static void MakeColumnVisible();
static void MakeCellVisible();
static void AdjustTopRow();
static void TraverseIn();

#if XmVersion >= 1002
static void TraverseInTimeOut P_ARGS((XtPointer, XtIntervalId *));
#endif /* XmVersion >= 1002 */

static Boolean IsRowVisible(), IsColumnVisible(), IsCellVisible(), XYToRowCol(), DoCommitEdit(),
  EventToXY();
static short MaxRowLabel();
static int XtoCol();
static void CopyRowLabels(), CopyColumnLabels(), CopyCells(), CopyColumnWidths(), CopyColumnMaxLengths(),
...
static void LosingFocusCB(Widget, XtPointer, XtPointer);
...
static void SmDestroyScrollMgr(), SmAddScroll(), SmRemoveScroll(), SmScrollEvent();
...
static void StringArrayDestructor();
static Boolean CvtStringToWidthArray();
static void WidthArrayDestructor();
static Boolean CvtStringToMaxLengthArray();
static void MaxLengthArrayDestructor();
static Boolean StringsAreEqual();
static Boolean CvtStringToAlignmentArray();
static void AlignmentArrayDestructor();

/*
 * Actions
 */
static void EditCellACT(), CancelEditACT(), CommitEditACT(), SelectCellACT(), TraverseNextACT(),
  TraversePrevACT();
...
void
CopyBackground F_ARGS(widget __ offset __ value , \
		      Widget widget _ int offset _ XrmValue *value)
{
...
void
CopyForeground F_ARGS(widget __ offset __ value ,\
		      Widget widget _ int offset _ XrmValue *value)
{
...
static void
ClassPartInitialize F_ARGS(mwc , XbaeMatrixWidgetClass mwc)
{
...
#ifdef NEED_24BIT_VISUAL
static Widget
get_shell_ancestor F_ARGS(w , Widget w)
{
	Widget sh;
...
static void
CreateDrawGC F_ARGS(mw , XbaeMatrixWidget mw)
{
  XGCValues values;
...
static void
GetInverseGC F_ARGS(mw , XbaeMatrixWidget mw)
{
  XGCValues values;
...
static void
CreateDrawClipGC F_ARGS(mw , XbaeMatrixWidget mw)
{
  XGCValues values;
  unsigned long mask = GCForeground | GCFont;
...
static void
CreateInverseClipGC F_ARGS(mw , XbaeMatrixWidget mw)
{
  XGCValues values;
  XtGCMask mask = GCForeground | GCFont;
...
static void
CreateTopShadowClipGC F_ARGS(mw , XbaeMatrixWidget mw)
{
  XGCValues values;
...
static void
CreateBottomShadowClipGC F_ARGS(mw , XbaeMatrixWidget mw)
{
  XGCValues values;
  XtGCMask mask = GCForeground | GCBackground;
 ...
 */
static void
SetClipMask F_ARGS(mw , XbaeMatrixWidget mw)
{
  XRectangle r;
...
static void
NewFont F_ARGS(mw , XbaeMatrixWidget mw)
{
 ...
 */
static void
GetCellTotalWidth F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i;
 ...
 */
static void
GetColumnPositions F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i, x;
     ...
     Cardinal *num_args;
{
   ...
   */
  new->matrix.horizontal_sb = XtVaCreateWidget("horizScroll",
...
*/
  ...
  new->matrix.vertical_sb = XtVaCreateWidget("vertScroll",
			  xmScrollBarWidgetClass, (Widget) new,
...
*/
   ...
   */
  XtAddCallback(TextChild(new), XmNlosingFocusCallback, LosingFocusCB, (XtPointer) new);
 ...
 */
static void
RowColToXY F_ARGS(mw __ row __ column __ x __ y , XbaeMatrixWidget mw _ int row _ int column _ int *x _ int *y)
{
  /*
	    ...
	    if (INBOX(rect, *x, *y))
	    {
	      ...
	      *column = XtoCol(mw, *x + (mw->matrix.fixed_columns
	      ...
	      /*
               ...
               */
	      if ((mw->matrix.fixed_rows && *row >= (int)mw->matrix.fixed_rows) ||
		  (mw->matrix.fixed_columns &&
                   *column >= (int)mw->matrix.fixed_columns) ||
		  *row < 0 || *column < 0)
		return False;
 ...
 */
static void
DrawString F_ARGS(mw __ win __ string __ length __ x __ y __ maxlen __ alignment __ highlight __ bold __ clip __ color , \
     XbaeMatrixWidget mw _ Window win _ String string _ int length _ int x _ int y _ int maxlen _ unsigned char alignment _ Boolean highlight _ Boolean bold _ Boolean clip _ Pixel color)
{
 ...
 */
static void
DrawColumnLabel F_ARGS(mw __ column , XbaeMatrixWidget mw _ int column)
{
  String label;
  int x, y, i;
 ...
 */
static void
DrawRowLabel F_ARGS(mw __ row , XbaeMatrixWidget mw _ int row)
{
  int y;
 ...
 */
static void
DrawCellPixmap F_ARGS(mw __ row __ column __ pixmap , \
	XbaeMatrixWidget mw _ int row _ int column _ Pixmap pixmap)
{
  int x, y;
 ...
 */
static void
DrawCellString(mw, row, column, string)
     XbaeMatrixWidget mw;
     int row, column;
     String string;
{
  ...
  /* //// set the background */
  /* NEW
   * If this cell is selected, fill the cell with the selected foreground
   * color
   */
  if (mw->matrix.selected_cells[row][column])
  {
    if (mw->matrix.colors)
      color = mw->matrix.colors[row][column]; /* OLD */
    else
      color = mw->manager.foreground;
  }
  else if (XtIsManaged(TextChild(mw)) &&
           row == mw->matrix.current_row &&
	   column == mw->matrix.current_column)
  {
    color = mw->matrix.text_background;
  }
    /* NEW color = mw->matrix.selected_background; */
/* later
...
/* NEW - end */
   ...
   */
  XSetForeground(XtDisplay(mw), gc, color);
  XFillRectangle(XtDisplay(mw), win, gc,
		x, y, COLUMN_WIDTH(mw, column), ROW_HEIGHT(mw));
/*
                 x + (mw->matrix.cell_highlight_thickness + mw->matrix.cell_shadow_thickness),
                 y + (mw->matrix.cell_highlight_thickness + mw->matrix.cell_shadow_thickness),
                 COLUMN_WIDTH(mw, column) -
                 (mw->matrix.cell_highlight_thickness +
...
*/
  ...
  if (*string != '\0')
  {
    DrawString(mw, win, string, strlen(string),
	       x + TEXT_X_OFFSET(mw),
	       y + TEXT_Y_OFFSET(mw),
	       mw->matrix.column_widths[column],
	       mw->matrix.column_alignments
...
static XbaeCellType
get_draw_cell_value F_ARGS(mw __ row __ column __ string __ pixmap , \
		   XbaeMatrixWidget mw _ int row _ int column _ String *string _ Pixmap *pixmap)
{
     ...
     int row, column;
{
  if (mw->matrix.draw_cell_callback == NULL)
    DrawCellString(mw, row, column, mw->matrix.cells[row][column]);
  else
  {
    String string;
    Pixmap pixmap;

    if (get_draw_cell_value(mw, row, column, &string, &pixmap) == XbaeString)
      DrawCellString(mw, row, column, string);
    else
      DrawCellPixmap(mw, row, column, pixmap);
  }
}
 ...
 */
static void
RedrawLabelsAndFixed F_ARGS(mw __ expose , XbaeMatrixWidget mw _ Rectangle *expose)
{
  /*
 ...
 */
static void
RedrawCells F_ARGS(mw __ expose , XbaeMatrixWidget mw _ Rectangle *expose)
{
  int startCol, endCol, startRow, endRow, i, j;
  Rectangle rect;
...
#define EQ(field)	(current->field == new->field)
  ...
  if (EQ(matrix.odd_row_background) &&
      (current->matrix.odd_row_background == current->core.background_pixel))
  {
    new->matrix.odd_row_background = new->core.background_pixel;
    redisplay = True;
  }
  if (EQ(matrix.even_row_background) &&
      (current->matrix.even_row_background == current->core.background_pixel))
  {
    new->matrix.even_row_background = new->core.background_pixel;
    redisplay = True;
  }
...
static void
ComputeSize F_ARGS(mw __ compute_width __ compute_height , XbaeMatrixWidget mw _ Boolean compute_width _ Boolean compute_height)
{
  ...
  if (mw->matrix.visible_columns)
    width = ROW_LABEL_WIDTH(mw) + 2 * mw->manager.shadow_thickness +
      COLUMN_WIDTH(mw, (mw->matrix.visible_columns + mw->matrix.fixed_columns) - 1) +
      mw->matrix.column_positions[mw->matrix.fixed_columns + mw->matrix.visible_columns - 1];
 ...
 */
static void
CopySelectedCells F_ARGS(mw , XbaeMatrixWidget mw)
{
  Boolean **copy;
  int i, j;
...
static void
FreeCells F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i, j;
...
static void
FreeRowLabels F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i;
...
static void
FreeColumnLabels F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i;
...
static void
FreeColors F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i;
...
static void
FreeSelectedCells F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i;
 ...
 */
static void
CreateColors F_ARGS(mw , XbaeMatrixWidget mw)
{
  int i;
 ...
 */
static void
ResizeCells F_ARGS(current __ new , XbaeMatrixWidget current _ XbaeMatrixWidget new)
{
  int i, j;
  int safe_rows;
 ...
 */
static void
ResizeSelectedCells F_ARGS(current __ new , XbaeMatrixWidget current _ XbaeMatrixWidget new)
{
  int i;
  int safe_rows;
 ...
 */
static void
ResizeColors F_ARGS(current __ new , XbaeMatrixWidget current _ XbaeMatrixWidget new)
{
  int i, j;
  int safe_rows;
 ...
 */
static void
AddRowsToTable F_ARGS(mw __ position __ rows __ labels __ colors __ num_rows , XbaeMatrixWidget mw _ int position _ String *rows _ String *labels _ Pixel *colors _ int num_rows)
{
  int i, j;
 ...
 */
static void
DeleteRowsFromTable F_ARGS(mw __ position __ num_rows , XbaeMatrixWidget mw _ int position _ int num_rows)
{
  int i, j;
...
MakeCellVisible(mw, row, column)
     ...
     int row, column;
{
  if (mw->matrix.scroll_select || !IsRowVisible(mw, row ))
    MakeRowVisible(mw, row);
  if (mw->matrix.scroll_select || !IsColumnVisible(mw, column ))
    MakeColumnVisible(mw, column);
 ...
 */
static void
SetCell F_ARGS(mw __ row __ column __ value __ update_text , \
     XbaeMatrixWidget mw _ int row _ int column _ _Xconst String value _ \
     Boolean update_text)
{
 ...
 */
void
XbaeMatrixSetCell F_ARGS(w __ row __ column __ value , \
			 Widget w _ int row _ int column _ _Xconst String value)
{
  /*
     ...
     int row, column;
{
  ...
  {
    if (get_draw_cell_value(mw, row, column, &string, &pixmap) == XbaePixmap)
    {
      /*
     ...
     int row, column;
{
  ...
  if (!mw->matrix.selected_cells[row][column])
  {
    ...
    if (mw->matrix.scroll_select || IsCellVisible(mw, row, column))
      DrawCell(mw, row, column);
  }
}
 ...
 */
Boolean
XbaeMatrixCommitEdit F_ARGS(w __ unmap , Widget w _ Boolean unmap)
{
  /*
   * Make sure w is a Matrix or a subclass
   */
 ...
 */
void
XbaeMatrixCancelEdit F_ARGS(w __ unmap , Widget w _ Boolean unmap)
{
  /*
   * Make sure w is a Matrix or a subclass
   */
 ...
 */
int
XbaeMatrixGetEventRowColumn(Widget w, XEvent * event, int *row, int *column)
{
  XbaeMatrixWidget mw;
  int x, y;
  CellType cell;
...
static void
LosingFocusCB(Widget tf, XtPointer matrix, XtPointer call)
{
 ...
 */
void
XbaeMatrixGetCurrentCell(Widget w, int *row, int *column)
{
  XbaeMatrixWidget mw;
