...
int wxImage::openPic(char *fullname)
{
  ...
  filetype = UNKNOWN;
  if (strncmp(magicno,"GIF8",4)==0) filetype = GIF;
...
//    XFreeColormap(theDisp,LocalCmap);
  ...
  if (epic != cpic && epic != NULL) free((char *)epic);
  if (cpic !=  pic && cpic != NULL) free((char *)cpic);
...
Bool wxLoadIntoBitmap(char *filename, wxBitmap *bitmap, wxColourMap **cmap)
{
  ...
  if (FileExists(filename) && tempImage->Load(filename))
  {
    ...
    GC gc = XCreateGC(dpy, bitmap->x_pixmap, (unsigned long )NULL,
       NULL);
