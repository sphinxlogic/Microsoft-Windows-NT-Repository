...
int wxImage::LoadGIF(char *fname,int nc)
{
  ...
  if (strncmp((const char *)ptr, (const char *)id, 4)) 
  ...
  free((char *)RawGIF);	 RawGIF = NULL; 	/* We're done with the raw data now */
...
int wxImage::GifError(char *st)
{
  ...
  if (RawGIF != NULL) free((char *)RawGIF);
  if (Raster != NULL) free((char *)Raster);
  if (pic    != NULL) free((char *)pic);
