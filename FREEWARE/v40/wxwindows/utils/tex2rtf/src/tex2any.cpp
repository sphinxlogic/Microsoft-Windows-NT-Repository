...
Bool            htmlBrowseButtons = HTML_BUTTONS_TEXT;
...
//      j -= 5;
    ...
    {
      actualFile = copystring(actualFile);
      ...
      {
        char errBuf[300];
        sprintf(errBuf, "Could not open include file %s", actualFile);
        CurrentInputIndex --;
        OnError(errBuf);
      }
...
void TraverseFromChunk(TexChunk *chunk, wxNode *thisNode, Bool childrenOnly)
{
  ...
  switch (chunk->type)
  {
    ...
    case CHUNK_TYPE_STRING:
    {
      extern Bool issuedNewParagraph;
    ...
    if (start)
    {
      ...
      while (bibFile)
      {
        ...
        if (actualFile)
        {
          ...
          {
            char buf[300];
            sprintf(buf, ".bib file %s not found or malformed", actualFile);
            OnError(buf);
          }
        }
        else
