...
{
	png_ptr->fp = fp;
//	png_set_error(png_error_default, png_warning_default);
}
...
void
png_error_default(png_struct *png_ptr, char *message)
{
...
void
png_warning_default(png_struct *png_ptr, char *message)
{
	if (!png_ptr)
      return;
...
void png_set_error(void (*pngerror)(png_struct *, char *),
	void (*pngwarning)(png_struct *, char *))
{
  if (pngwarning) png_warning = pngwarning;
  if (pngerror) png_error = pngerror;
  else 
	fprintf(stderr, "libpng No lo hizo!\n");
