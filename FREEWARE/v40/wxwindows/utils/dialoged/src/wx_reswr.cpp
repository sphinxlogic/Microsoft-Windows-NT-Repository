/*
 * File:	wx_reswr.cc
 * Purpose:	Writing resources to file
 * Author:	Julian Smart
 * Created:	1995
 * Updated:	
 * Copyright:	(c) 1995, AIAI, University of Edinburgh
	Last change:  JS   25 May 97    2:15 pm
 */
...
wxItem *wxResourceTableWithSaving::CreateItem(wxPanel *panel, wxItemResource *childResource)
{
  wxItem *item = wxResourceTable::CreateItem(panel, childResource);
  if (item)
...
Bool wxResourceTableWithSaving::SaveResource(ostream& stream, wxItemResource *item)
{
  ...
  if (itemType == "wxDialogBox" || itemType == "wxPanel")
    {
      if (itemType == "wxDialogBox")
      {
      ...
      {
        stream << ",\\\n  label_font = ";
        OutputFont(stream, item->GetLabelFont());
      }
...
void wxResourceTableWithSaving::GenerateWindowStyleString(long windowStyle, char *buf)
{
  ...
  GenerateStyle(buf, windowStyle, wxVERTICAL_LABEL, "wxVERTICAL_LABEL");
  GenerateStyle(buf, windowStyle, wxHORIZONTAL_LABEL, "wxHORIZONTAL_LABEL");
  ...
  GenerateStyle(buf, windowStyle, wxNATIVE_IMPL, "wxNATIVE_IMPL");
}
