...
extern WXDLLEXPORT proioErrorHandler currentProioErrorHandler;
...
class WXDLLEXPORT PrologExpr
{
 public:
  ...
  inline PrologExpr *GetFirst(void) { return ((type == PrologList) ? value.first : NULL); }

  // Get next expr if this is a node in a list
  inline PrologExpr *GetNext(void) { return next; }

  // Get last expr in list
  inline PrologExpr *GetLast(void) { return ((type == PrologList) ? last : NULL); }
...
class WXDLLEXPORT PrologDatabase: public wxList
{
...
class WXDLLEXPORT ClipsTemplate: public wxObject
{
 public:
  char *name;
  wxList slots;
...
class WXDLLEXPORT ClipsTemplateSlot: public wxObject
{
 public:
  char *slot_name;
  char *default_name;
...
// Make a call
WXDLLEXPORT PrologExpr *wxMakeCall(char *functor ...);
...
extern WXDLLEXPORT PrologDatabase *hyPrologDatabase;
