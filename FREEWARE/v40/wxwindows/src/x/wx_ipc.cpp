...
//
// ntohs etc. are not correctly resolved by the linker, so in
// order to allow non-optimized code (for debugging), make sure
// these functions are registered as C code. Jens P. Brage, 25 Feb 96.
//
...
#if !defined(VMS) && !defined(__GNUG__)
#include <memory.h>
#endif
...
//  bzero ((caddr_t) & s.inaddr, sizeof (s.inaddr));
  memset((caddr_t) & s.inaddr, 0, sizeof (s.inaddr));
  ...
  memcpy ((caddr_t) & s.inaddr.sin_addr, (caddr_t) server->h_addr, server->h_length);
