	...
	long style, char *name)
{
  ...
  Widget textWidget = XmCreateScrolledText (parentWidget, windowName, args, 2);
...
void wxTextWindow::SetSize (int x, int y, int w, int h, int sizeFlags)
{
  ...
  Bool managed = XtIsManaged(parent);
...
#ifdef wx_xview
  if(clipboard)
      textsw_insert((Textsw)handle,clipboard,strlen(clipboard));
  else
      wxMessageBox("Clipbard is empty.",
                               "Warning",wxOK|wxCENTRE,this,-1,-1);
#endif
}
...
void wxTextWindow::Cut(void)
{
  ...
  if(CopySelectionToClipboard())
      Remove(selectionFirst,selectionLast);
#endif
}
...
#ifdef wx_xview
int wxTextWindow::CopySelectionToClipboard()
{
    ...
    server = (Xv_Server)xv_get(xv_get((Textsw)handle, 
                                    XV_SCREEN), SCREEN_SERVER);

    holder = selection_inquire(server, SELN_PRIMARY);


    if (seln_holder_same_client(&holder, (char *)(Textsw) handle)) 
      {
    ...
    else 
      {
          ...
          len = strlen(response->data + sizeof(SELN_REQ_CONTENTS_ASCII));
          if(clipboard)
              delete[] clipboard;
          clipboard = new char[len+1];
          strcpy(clipboard,
    ...
    selectionFirst = (long)first;
    selectionLast = (long)last;
    return 1;
}
#endif
...
static int CalcPixelFromWXColour(wxColour * col)    // see also wx_dc.cpp
{
	 XColor xc;
	 xc.red   = (((long) col->Red ()) << 8);
	 xc.green = (((long) col->Green ()) << 8);
	 xc.blue  = (((long) col->Blue ()) << 8);
...
void wxTextWindow::SetBackgroundColour(wxColour * col)
{
	 XtVaSetValues ( (Widget) handle,
	XmNbackground, CalcPixelFromWXColour(col),
	NULL);
}
#endif