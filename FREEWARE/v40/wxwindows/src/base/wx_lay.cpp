/*
 * File:	wx_lay.cc
 * Purpose:	Constraint layout system implementation
 * Author:	Julian Smart
 * Created:	1995
 * Updated:	August 1995
  ...
  if (other->GetChildren()->Member(thisWin))
  {
    switch (which)
    {
      ...
      case wxHeight:
      {
        ...
        if ((which == wxRight) || (which == wxWidth))
          return w;
        else
          return h;
      }
...
{
  WXSET_TYPE(wxSizer, wxTYPE_SIZER)
...
wxSizer::~wxSizer(void)
{
  ...
  while (node)
  {
    ...
    {
      // Is a true child, so deleting will delete node also
      wxSizer *sz = (wxSizer *)win;
      RemoveSizerChild(sz);
      delete sz;
    }
    else
  ...
  if (sizerParent && !sizerParent->IsKindOf(CLASSINFO(wxSizer)))
  {
    sizerParent->SetSizer(NULL);
  }
...
void wxSizer::GetSize(int *w, int *h)
{
  *w = sizerWidth;
  *h = sizerHeight;
}

void wxSizer::GetPosition(int *x, int *y)
{
  *x = sizerX;
  *y = sizerY;
}
  ...
  *noChanges = 0;
  switch (sizerBehaviour)
  {
    case wxSizerExpand:
    {
      ...
      int pw, ph;
      sizerParent->GetClientSize(&pw, &ph);
      SetSize(0, 0, pw, ph);
      wxLayoutConstraints *constr = GetConstraints();
