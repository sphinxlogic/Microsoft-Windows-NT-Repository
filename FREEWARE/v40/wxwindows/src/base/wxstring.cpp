 ...
 * Updated:
 * Copyright:   (c) 1994
	Last change:  JS   25 May 97   11:09 am
 */
...
wxString wxString::operator +(const wxString& cString) const
{
  const char *cs = cString.GetData();
...
// String comparisons
int wxString::CompareTo(const char* cs2, wxString::caseCompare cmp) const
{
...
int wxString::CompareTo(const wxString& st, wxString::caseCompare cmp) const
{
#if 0
  const char *cs2 = st.GetData() ;
  const char* cs1 = GetData();
  if(cs2==NULL){
    return(cs1==cs2?0:1);
  }
...
Bool wxString::Contains(const char* pat, wxString::caseCompare cmp) const
{
  wxASSERT(pat!=NULL,"Bool wxString::Contains(const char* pat, caseCompare cmp) const: NULL argument");
  return(Index(pat,0,cmp)!=NO_POS);
}

Bool wxString::Contains(const wxString& pat, wxString::caseCompare cmp) const
{
  return(Index(pat,0,cmp)!=NO_POS);
}
...
// Pattern Matching:
size_t wxString::Index(const char* pattern, size_t startIndex, wxString::caseCompare cmp) const
{
...
size_t wxString::Index(const wxString& cs, size_t i, wxString::caseCompare cmp) const
{
...
// Metrowerks doesn't define isascii
#ifndef __MWERKS__
