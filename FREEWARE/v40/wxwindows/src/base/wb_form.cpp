...
wxRealRange::wxRealRange (double the_lo, double the_hi)
{
  lo = the_lo;
  hi = the_hi;
}
	  ...
	  case wxFORM_RADIOBOX:
	      ...
	      if (constraint)
		{
		  ...
		  PanelItem = new wxRadioBox (panel, (wxFunction) NULL, Label,
			      -1, -1, Width, Height, n, (char **)choices, 1, wxFLAT);
		  panel->SetLabelPosition (wxHORIZONTAL);
		}
	      break;
	    }
		...
		if ((range->lo > val) || (range->hi < val))
		  {
		    sprintf (msg_buf, wxSTR_VIOLATION_FLOAT,
			     Label, (float)range->lo, (float)range->hi);
		    ok = FALSE;
		  }
		break;
	      }
...
Bool wxFormItem::CheckDoubleValue (double val)
{
  char msg_buf[_MAX_MESSAGE_LEN];
  if (Constraints)
    {
  ...
  switch (Type)
	...
	switch (ItemType)
	  ...
	  case wxFORM_TEXT:
	    {
	      wxText *text_item = (wxText *) PanelItem;
	      char *svalue = DoubleToString (*(ValuePtr.DoubleValuePtr));
	      if (svalue)
		{
		  text_item->SetValue (svalue);
		}
...
Bool wxFormItem::UpdateValue (void)
{
  switch (Type)
	  ...
	  case wxFORM_TEXT:
	    {
	      wxText *text_item = (wxText *) PanelItem;
	      char *svalue = text_item->GetValue ();
	      if (!StringToDouble (svalue, &double_value))
                return FALSE;
	      break;
	    }
	  default:
	...
	if (CheckDoubleValue (double_value))
	  *(ValuePtr.DoubleValuePtr) = double_value;
	else
	  return FALSE;
	break;
      }
  ...
  if (wx_form_panel)
    {
      ...
      if (wx_form_panel->IsKindOf(CLASSINFO(wxDialogBox)))
        wx_form_panel->Close();
      else
		 ...
		 int width, int height)
{
  ...
  if (label)
    item->Label = copystring (label);
  item->Value.FloatValue = (float)0.0;
  item->Height = height;
  item->Width = width;
  return item;
}
...
wxFormItem *
wxMakeFormDouble (char *label, double *var,
		 int item_type, wxList * constraints,
		 char *help_string, int style,
		 int width, int height)
{
  ...
  wxFormItem *item = new wxFormItem (wxFORM_DOUBLE, item_type);
  item->Constraints = constraints;
  item->style = style;
  if (help_string)
    item->HelpString = copystring (help_string);
  item->ValuePtr.DoubleValuePtr = var;
  if (label)
    item->Label = copystring (label);
  item->Value.DoubleValue = 0.0;
  item->Height = height;
  item->Width = width;
  return item;
}
...
wxFormItemConstraint *
wxMakeConstraintRange (double lo, double hi)
{
  wxFormItemConstraint *constraint =
