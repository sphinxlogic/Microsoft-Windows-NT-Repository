...
//
// The contents of this file are the combined sourcefiles
...
//       therefore we use malloc (yes, i know that this is a memory waste ;-)
// (steve) 03.12.94
...
wxRegex::wxRegex(const char* t, int fast, int bufsize, 
               const char* transtable)
{
  int tlen = (t == 0)? 0 : strlen(t);
  buf = new wxRE_pattern_buffer;
  reg = new wxRE_registers;
  if (fast)
  ...
  char* msg = wxRE_compile_pattern((const char*)t, tlen, buf);
  if (msg != 0)
    (*lib_error_handler)("wxRegex", msg);
  else if (fast)
    wxRE_compile_fastmap(buf);
}

int wxRegex::match_info(int& start, int& length, int nth) const
{
  if ((unsigned)(nth) >= wxRE_NREGS)
    return 0;
  else
  {
    start = reg->start[nth];
...
int wxRegex::Search(const char* s, int len, int& matchlen, int startpos) const
{
  ...
  matchpos = wxRE_search_2(buf, 0, 0, (char*)s, len, pos, range, reg, len);
  if (matchpos >= 0)
...
int wxRegex::Match(const char*s, int len, int p) const
{
  if (p < 0)
  {
    p += len;
    if (p > len)
      return -1;
    return wxRE_match_2(buf, 0, 0, (char*)s, p, 0, reg, p);
  }
  else if (p > len)
    return -1;
  else
    return wxRE_match_2(buf, 0, 0, (char*)s, len, p, reg, len);
}

int wxRegex::OK() const
{
// can't verify much, since we've lost the original string
  int v = buf != 0;             // have a regex buf
  v &= buf->buffer != 0;        // with a pat
  if (!v) (*lib_error_handler)("wxRegex", "invariant failure");
  return v;
}
...
const wxRegex wxRXwhite("[ \n\t\r\v\f]+", 1);
const wxRegex wxRXint("-?[0-9]+", 1);
const wxRegex wxRXdouble("-?\\(\\([0-9]+\\.[0-9]*\\)\\|\\([0-9]+\\)\\|\\(\\.[0-9]+\\)\\)\\([eE][---+]?[0-9]+\\)?", 1, 200);
const wxRegex wxRXalpha("[A-Za-z]+", 1);
const wxRegex wxRXlowercase("[a-z]+", 1);
const wxRegex wxRXuppercase("[A-Z]+", 1);
const wxRegex wxRXalphanum("[0-9A-Za-z]+", 1);
const wxRegex wxRXidentifier("[A-Za-z_][A-Za-z0-9_]*", 1);
...
#else
#ifdef _HPUX_SOURCE
//**df we don't have alloca under HP-UX 9.0X (we do in 10.0), so we do a 
//**df memory waste
//**df Dan Felman (steve), 10.12.95
#define alloca malloc
#else
...
/* This must be nonzero for the wordchar and notwordchar pattern
   commands in wxRE_match_2.  */
#ifndef Sword 
#define Sword 1
#endif

#define SYNTAX(c) wxRE_syntax_table[c]


#ifdef SYNTAX_TABLE

char *wxRE_syntax_table;

#else /* not SYNTAX_TABLE */

static char wxRE_syntax_table[256];
...
init_syntax_once ()
{
   ...
   memset (wxRE_syntax_table, 0, sizeof wxRE_syntax_table);

   for (c = 'a'; c <= 'z'; c++)
     wxRE_syntax_table[c] = Sword;
...
/* These are the command codes that appear in compiled regular
   ...
   So regex.h defines a symbol `wxRE_EXACTN_VALUE' to be 1; the value of
...
enum regexpcode
  {
    ...
    start_memory, /* Start remembering the text that is matched, for
		    storing in a memory register.  Followed by one
                    ...
                    must be in the range 0 through wxRE_NREGS.  */
    stop_memory, /* Stop remembering the text that is matched
		    and store it in a memory register.  Followed by
                    one byte containing the register number. Register
                    numbers must be in the range 0 through wxRE_NREGS.  */
...
int
wxRE_set_syntax (int syntax)
{
  int ret;

  ret = wxObscure_syntax;
  wxObscure_syntax = syntax;
  return ret;
}

/* Set by wxRE_set_syntax to the current regexp syntax to recognize.  */
int wxObscure_syntax = 0;



/* Macros for wxRE_compile_pattern, which is found below these definitions.  */
...
/* Subroutines for wxRE_compile_pattern.  */
...
/* wxRE_compile_pattern takes a regular-expression string
   and converts it into a buffer full of byte commands for matching.

   PATTERN   is the address of the pattern string
   SIZE      is the length of it.
   BUFP	    is a  struct wxRE_pattern_buffer *  which points to the info
	     on where to store the byte commands.
	     This structure contains a  char *  which points to the
	     actual space, which should have been obtained with malloc.
	     wxRE_compile_pattern may use realloc to grow the buffer space.

   The number of bytes of commands can be found out by looking in
   the `struct wxRE_pattern_buffer' that bufp pointed to, after
   wxRE_compile_pattern returns. */

char *
wxRE_compile_pattern (const char *pattern, int size, struct wxRE_pattern_buffer *bufp)
{
  ...
  while (p != pend)
	...
	case '$':
	  {
	    ...
	    if (! (wxObscure_syntax & wxRE_CONTEXT_INDEP_OPS))
	      while (p1 != pend)
		{
		  if (*p1 == '\\' && p1 + 1 != pend
		      && (p1[1] == '<' || p1[1] == '>'
            ...
            if (wxObscure_syntax & wxRE_TIGHT_VBAR)
	      {
		if (! (wxObscure_syntax & wxRE_CONTEXT_INDEP_OPS) && p1 != pend)
		  goto normal_char;
	    ...
	    /* $ means succeed if at end of line, but only in special contexts.
	      If validly in the middle of a pattern, it is a normal character. */
            ...
            if ((wxObscure_syntax & wxRE_CONTEXTUAL_INVALID_OPS) && p1 != pend)
	      goto invalid_pattern;
	    if (p1 == pend || *p1 == '\n'
		|| (wxObscure_syntax & wxRE_CONTEXT_INDEP_OPS)
		|| (wxObscure_syntax & wxRE_NO_BK_PARENS
		    ? *p1 == ')'
		    : *p1 == '\\' && p1[1] == ')')
		|| (wxObscure_syntax & wxRE_NO_BK_VBAR
		    ? *p1 == '|'
		    : *p1 == '\\' && p1[1] == '|'))
	      {
		BUFPUSH (endline);
	...
	case '^':
	  /* ^ means succeed if at beg of line, but only if no preceding 
          ...
          if ((wxObscure_syntax & wxRE_CONTEXTUAL_INVALID_OPS) && laststart)
            goto invalid_pattern;
	...
	case '+':
	case '?':
	  if ((wxObscure_syntax & wxRE_BK_PLUS_QM)
	      || (wxObscure_syntax & wxRE_LIMITED_OPS))
	    goto normal_char;
	handle_plus:
	case '*':
	  ...
	  if (!laststart)
            {
              if (wxObscure_syntax & wxRE_CONTEXTUAL_INVALID_OPS)
                goto invalid_pattern;
              else if (! (wxObscure_syntax & wxRE_CONTEXT_INDEP_OPS))
		goto normal_char;
            }
	  ...
	  while (1)
		...
		break;
	      ...
	      if (c == '*')
		;
	      else if (!(wxObscure_syntax & wxRE_BK_PLUS_QM)
		       && (c == '+' || c == '?'))
		;
	      else if ((wxObscure_syntax & wxRE_BK_PLUS_QM)
		       && c == '\\')
	...
	case '.':
	  ...
	  if ((wxObscure_syntax & wxRE_HAT_NOT_NEWLINE) && b[-2] == charset_not)
            SET_LIST_BIT ('\n');
	  ...
	  while (1)
	    {
		 ...
		 When we set the bit for the character, we translate it.  */
	      ...
	      if ((wxObscure_syntax & wxRE_AWK_CLASS_HACK) && c == '\\')
	        {
	          PATFETCH(c1);
                  SET_LIST_BIT (c1);
	          continue;
		      ...
		      /* If this is an empty bracket expression.  */
                      if ((wxObscure_syntax & wxRE_NO_EMPTY_BRACKETS) 
                          && p == pend)
                        goto invalid_pattern;
                    }
                  else 
              ...
              if (p[0] == '-' && p[1] != ']')
		{
		  ...
		  if ((wxObscure_syntax & wxRE_NO_EMPTY_RANGES) && c > c1)
                    goto invalid_pattern;
                    
		  if ((wxObscure_syntax & wxRE_NO_HYPHEN_RANGE_END) 
                      && c1 == '-' && *p != ']')
                    goto invalid_pattern;
	      ...
	      else if ((wxObscure_syntax & wxRE_CHAR_CLASSES)
			&&  c == '[' && p[0] == ':')
                {
	...
	case '(':
	  if (! (wxObscure_syntax & wxRE_NO_BK_PARENS))
	    goto normal_char;
	  else
	    goto handle_open;
	...
	case ')':
	  if (! (wxObscure_syntax & wxRE_NO_BK_PARENS))
        ...
        case '\n':
	  if (! (wxObscure_syntax & wxRE_NEWLINE_OR))
	    goto normal_char;
	  else
	    goto handle_bar;
	...
	case '|':
	  if ((wxObscure_syntax & wxRE_CONTEXTUAL_INVALID_OPS)
              && (! laststart  ||  p == pend))
	    goto invalid_pattern;
          else if (! (wxObscure_syntax & wxRE_NO_BK_VBAR))
	    goto normal_char;
	  else
	    goto handle_bar;

	case '{':
           if (! ((wxObscure_syntax & wxRE_NO_BK_CURLY_BRACES)
                  && (wxObscure_syntax & wxRE_INTERVALS)))
             goto normal_char;
           else
	  ...
	  switch (c)
	    ...
	    handle_open:
	      if (stackp == stacke) goto nesting_too_deep;
              ...
              /* Laststart should point to the start_memory that we are about
                 to push (unless the pattern has wxRE_NREGS or more ('s).  */
              *stackp++ = b - bufp->buffer;    
	      if (regnum < wxRE_NREGS)
	        {
	    ...
	    case '|':
              if ((wxObscure_syntax & wxRE_LIMITED_OPS)
	          || (wxObscure_syntax & wxRE_NO_BK_VBAR))
		goto normal_backsl;
	    handle_bar:
              if (wxObscure_syntax & wxRE_LIMITED_OPS)
		...
		store_jump (fixup_jump, jump, b);
	      ...
	      break;
            ...
            case '{': 
              if (! (wxObscure_syntax & wxRE_INTERVALS)
		  /* Let \{ be a literal.  */
                  || ((wxObscure_syntax & wxRE_INTERVALS)
                      && (wxObscure_syntax & wxRE_NO_BK_CURLY_BRACES))
		  /* If it's the string "\{".  */
		  || (p - 2 == pattern  &&  p == pend))
                goto normal_backsl;
            handle_interval:
	      ...
	      if (!laststart)
		    ...
		    goto invalid_pattern;
		     ...
		     || (! (wxObscure_syntax & wxRE_NO_BK_REFS)
                         && *laststart == duplicate)))
                {
                  if (wxObscure_syntax & wxRE_NO_BK_CURLY_BRACES)
                    goto normal_char;
		...
		upper_bound = lower_bound;
	      ...
	      if (c != '}' || lower_bound < 0 || upper_bound > wxRE_DUP_MAX
		 ...
		 { /* Set to 5 if only one repetition is allowed and
	              hence no jump_n is inserted at the current end of
                   ...
                   /* Initialize the succeed_n to n, even though it will
                      ...
                      wxRE_compile_fastmap will need to know it.  Jump to
	    ...
	    case '8':
	    case '9':
	      if (wxObscure_syntax & wxRE_NO_BK_REFS)
                goto normal_char;
              c1 = c - '0';
	      if (c1 >= regnum)
		{
  		  if (wxObscure_syntax & wxRE_NO_EMPTY_BK_REF)
                    goto invalid_pattern;
                  else
                    goto normal_char;
                }
	...
	normal_char:		/* Expects the character in `c'.  */
	  if (!pending_exact || pending_exact + *pending_exact + 1 != b
	      ...
	      || ((wxObscure_syntax & wxRE_BK_PLUS_QM)
		  ? *p == '\\' && (p[1] == '+' || p[1] == '?')
		  : (*p == '+' || *p == '?'))
	      || ((wxObscure_syntax & wxRE_INTERVALS) 
                  && ((wxObscure_syntax & wxRE_NO_BK_CURLY_BRACES)
...
/* Given a pattern, compute a fastmap from it.  The fastmap records
   ...
   that matches the pattern.  This fastmap is used by wxRE_search to skip
...
void
wxRE_compile_fastmap (struct wxRE_pattern_buffer *bufp)
{
...

/* Like wxRE_search_2, below, but only one string is specified, and
   doesn't let you say where to stop matching. */

int
wxRE_search (struct wxRE_pattern_buffer *pbufp,
	   char *string,
	   int size,
	   int startpos,
	   int range,
	   struct wxRE_registers *regs)
{
  return wxRE_search_2 (pbufp, (char *) 0, 0, string, size, startpos, range, 
		      regs, size);
}
...
int
wxRE_search_2 (struct wxRE_pattern_buffer *pbufp,
	     ...
	     int mstop)
{
  ...
  while (1)
	...
	return -1;
      ...
      val = wxRE_match_2 (pbufp, string1, size1, string2, size2, startpos,
			regs, mstop);
      if (val >= 0)
	return startpos;
      if (val == -2)
...
int
wxRE_match (struct wxRE_pattern_buffer *pbufp,
	  char *string,
	  int size,
	  int pos,
	  struct wxRE_registers *regs)
{
  return wxRE_match_2 (pbufp, (char *) 0, 0, string, size, pos, regs, size); 
}
#endif /* not emacs */


/* The following are used for wxRE_match_2, defined below:  */

/* Roughly the maximum number of failure points on the stack.  Would be
   exactly that if always pushed MAX_NUM_FAILURE_ITEM each time we failed.  */
   
int wxRE_max_failures = 2000;

/* Routine used by wxRE_match_2.  */
static int bcmp_translate (char *, char *, int, unsigned char *);


/* Structure and accessing macros used in wxRE_match_2:  */
...
#define MAX_NUM_FAILURE_ITEM   (wxRE_NREGS * NUM_REG_ITEMS + 2)
...
#define PUSH_FAILURE_POINT(pattern_place, string_place)			\
  {									\
    ...
    for (last_used_reg = wxRE_NREGS - 1; last_used_reg > 0; last_used_reg--)\
	...
	if (len > wxRE_max_failures * MAX_NUM_FAILURE_ITEM)		\
	  return -2;							\
									\
...
#define POP_FAILURE_POINT()						\
  {									\
    ...
    temp = (size_t) *--stackp;	/* How many regs pushed.  */	        \
...
#define SET_REGS_MATCHED 						\
  { unsigned this_reg; 							\
    for (this_reg = 0; this_reg < wxRE_NREGS; this_reg++) 		\
      { 								\
...
int
wxRE_match_2 (struct wxRE_pattern_buffer *pbufp,
	    ...
	    int mstop)
{
 ...
 /* Failure point stack.  Each place that can handle a failure further
  ...
  unsigned char *initial_stack[MAX_NUM_FAILURE_ITEM * NFAILURES];
  unsigned char **stackb = initial_stack;
  unsigned char **stackp = stackb;
  unsigned char **stacke = &stackb[MAX_NUM_FAILURE_ITEM * NFAILURES];
  ...
  unsigned char *regstart[wxRE_NREGS];
  unsigned char *regend[wxRE_NREGS];
  ...
  struct register_info reg_info[wxRE_NREGS];
  ...
  unsigned char *best_regstart[wxRE_NREGS];
  unsigned char *best_regend[wxRE_NREGS];

  /* Initialize subexpression text positions to -1 to mark ones that no
     \( or ( and \) or ) has been seen for. Also set all registers to
     inactive and mark them as not having matched anything or ever
     failed.  */
  for (mcnt = 0; mcnt < wxRE_NREGS; mcnt++)
    {
  ...
  while (1)
              ...
              if (stackp != stackb)
                {
                  ...
                  if (! best_regs_set
                      ...
                      || (! in_same_string && ! MATCHING_IN_FIRST_STRING))
                    {
                      ...
                      for (mcnt = 1; mcnt < wxRE_NREGS; mcnt++)
                        {
	...
	case anychar:
	  ...
	  if ((translate ? translate[*d] : *d) == '\n'
              || ((wxObscure_syntax & wxRE_DOT_NOT_NULL) 
...
static struct wxRE_pattern_buffer wxRE_comp_buf;
...
wxRE_comp (char *s)
{
  ...
  if (!wxRE_comp_buf.buffer)
    {
      if (!(wxRE_comp_buf.buffer = (char *) malloc (200)))
	return "Memory exhausted";
      wxRE_comp_buf.allocated = 200;
      if (!(wxRE_comp_buf.fastmap = (char *) malloc (1 << BYTEWIDTH)))
	return "Memory exhausted";
    }
  return wxRE_compile_pattern (s, strlen (s), &wxRE_comp_buf);
}
...
{
  int len = strlen (s);
  return 0 <= wxRE_search (&wxRE_comp_buf, s, len, 0, len,
			 (struct wxRE_registers *) 0);
}
#endif /* not emacs */
...
{
  char pat[80];
  struct wxRE_pattern_buffer buf;
  int i;
  char c;
  ...
  while (1)
    {
      ...
      if (*pat)
	{
          wxRE_compile_pattern (pat, strlen(pat), &buf);
      ...
      i = wxRE_match (&buf, pat, strlen (pat), 0, 0);
...
#ifdef NOTDEF
void 
print_buf (struct wxRE_pattern_buffer *bufpbufp)
{
  int i;
