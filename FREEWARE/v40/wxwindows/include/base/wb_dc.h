...
class wxCanvasDC;
class wxDC;
class WXDLLEXPORT wxbDC: public wxObject
{
 protected:
  Bool dcOptimize;
 public:
...
//  wxDC(char *driver, char *device, char *output, Bool interactive = TRUE);
  ...
  virtual void CrossHair(int x, int y) = 0;
  virtual void DrawArc(float x1,float y1,float x2,float y2,float xc,float yc)=0;
  virtual void DrawEllipticArc (float x, float y, float w, float h, float sa, float ea) = 0;
  ...
  virtual void DrawText(const char *text, float x, float y, Bool use16bit = FALSE) = 0;
  ...
  inline  void GetUserScale(float *x, float *y) { *x = user_scale_x; *y = user_scale_y; }
  ...
  virtual inline void GetSizeMM(float *width, float *height) { *width = (float)0.0; *height = (float)0.0; };
  ...
  virtual Bool Blit(float xdest, float ydest, float width, float height,
            wxCanvasDC *source, float xsrc, float ysrc, int rop = wxCOPY, Bool useMask = FALSE) = 0;
