...

/* Parameter files */
/*~~~ The following FOUR were transposed on VMS */
#include <stdio.h>
#include <ctype.h>
#include "osdep.h"
#include "mail.h"
...
#ifdef VMS
#define	printf	our_printf
#define	fputs	our_fputs
#define	fgets	our_fgets
#define	putchar	our_putchar
#endif	/* VMS */
	...
	char *argv[];
{
  ...
  mail_link (&vmsmaildriver);	/* VMS/MAIL driver */
#else /* VMS */
...
#endif /* Multinet */
#ifdef NETLIB
    get_local_host_name(cmdbuf,TMPLEN-1);
    host = cpystr(cmdbuf);
#endif /* NETLIB */
#endif /* !VMS */
...
#ifdef VMS
  dorc ("UTIL$:imapd.conf");	/* run system init */
				/* run user's init */
#else /* VMS */
...
#ifndef VMS	/* Do not allow anonymous access */
	...
	if ((!(anonymous || strcmp (cmd,"SELECT"))) ||
	  ...
	  if (!(s = snarf (&arg))) response = misarg;
...
#ifdef VMS
	    sprintf (tmp,"(%s)%s%s", home, (*cmd == 'B') ? "*" : "",s);	/* Add the user's sepcifications */
#else /* VMS */
		...
		&& ((response == win) || (response == altwin))) {
				/* flush old list */
	      fs_give ((void **) &flags);
              mm_exists (stream,stream->nmsgs);	/*~~~~ Wasn't here... */
...
#define	MAXBUF	512		/* Save up to 512 characters before sending them */
...
our_printf(fmt, A, B, C, D, E, F, G)
char	*fmt;
{
	char	buffer[8192];

	sprintf(buffer, fmt, A, B, C, D, E, F, G);
	___write_terminal(buffer, strlen(buffer));
	return 0;
}
...
char	*string;
{
	___write_terminal(string, strlen(string));
	return 0;
}
...
our_fgets(buffer, buffersize)
char	*buffer;
{
	int	status = read_from_stdin(buffer, buffersize);
	if(status > 0)
		return buffer;
	else	return NULL;
}
...
~~~~~~~ >Do not use timeout at present.
  ...
  ----*/
int
read_from_stdin(buffer, buffersize)
char	*buffer;
int	buffersize;
{
	int size;
	...
	status = sys$qiow((int)(0), TerminalChan,
		(short)(IO$_READLBLK | IO$M_NOFORMAT | IO$M_NOFILTR | IO$M_NOECHO | IO$M_TRMNOECHO),
			iosb, (int)(0), (int)(0),
			buffer, buffersize - 1, NULL, NULL, NULL, NULL);
	if(((status & 0x1) == 0) || ((iosb[0] & 0x1) == 0)) {
		exit(status);
	}

	buffer[size = (int)(iosb[1] & 0xffff)] = '\0';
	if(size > 0) {	/* Something is there */
		buffer[size++] = '\n'; buffer[size++] = '\0';
	}
	return size;
}
...
#ifdef VMS
/* Dummy since we call it at VMS_MAIL */
q_status_message()
{}
#endif	/* VMS */
