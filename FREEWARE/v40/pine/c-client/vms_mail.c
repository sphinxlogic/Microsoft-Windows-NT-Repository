/*
 * Program:	VMS mail routines. Based on the Tenex one.
 *
 * Author:	Mark Crispin
...
#else	/* MULTINET */
  if (!lhostn) {		/* have local host yet? */
	lhostn = cpystr(getenv("SYS$NODE"));
	strncpy(tmp, lhostn, MAILTMPLEN);
  }
#endif	/* Multinet */
  return LONGT;			/* Always succeed */
}
...
void *vms_mail_parameters (function,value)
	...
	void *value;
{
  fatal ("Invalid vms_mail_parameters function");
  return NIL;
}

	...
	long m;
{
  ...
  elt = mail_elt (stream,m);
	...
	if((status & 0x1) == 0) {
		printf(" GetMessage header failed, message=%d, status=%d\n", m, status);
		exit(status);
	}
...
/* Must be second*/ { 0, MAIL$_MESSAGE_FILENAME, GlobalMailDirectory, NULL },
	...
	if(FolderCreated) {	/* New folder */
		char	tmp[512];
		sprintf(tmp, "Folder '%s' was created", folder);
	        q_status_message(0, 1, 1, tmp);
	}
	return 1;	/* All ok */
}
