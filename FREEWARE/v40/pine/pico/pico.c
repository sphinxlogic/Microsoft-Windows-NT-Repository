...
static char rcsid[] = "$Id: pico.c,v 4.19 1993/12/02 16:44:31 mikes Exp $";
#endif
/*
...
int c, f, n;
{
	...
	if(((llength(curwp->w_dotp)+2 >= ((Pmaster && term.t_ncol > 80)
      ...
      {
      if(compose_heb){
	if((curwp->w_doto>llength(curwp->w_dotp)-3) ||
	   (hebmain && ((curwp->w_dotp->l_type==L_ENG &&
		curwp->w_dotp->l_text[0].c!=' ') ||
		(curwp->w_dotp->l_type==L_HEB &&
		 curwp->w_dotp->l_text[llength(curwp->w_dotp)-1].c!=' '))) ||
		(!hebmain && ((curwp->w_dotp->l_type==L_ENG &&
			curwp->w_dotp->l_text[llength(curwp->w_dotp)-1].c!=' ') ||
			(curwp->w_dotp->l_type==L_HEB &&
			curwp->w_dotp->l_text[0].c!=' '))))
...
int f, n;
{
    switch(mlyesno("Cancelling will abandon your mail message.  Cancel", 
      ...
      case TRUE:
#ifdef HEBREW
	if(compose_heb)reverse_pad_buf();
#endif
