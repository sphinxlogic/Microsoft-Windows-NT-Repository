...
/* add more definitions to avoid compiler errors. B. Z. Lederman */
...
extern int cistrcmp();
extern int part_depth();
extern int part_close();
extern int part_fill();
extern int part_ungets();
extern char * part_gets();
extern int part_addboundary();
extern int part_readboundary();
extern long int handleUuencode();
extern int handleText();
extern void os_perror();
extern void os_donewithdir();
extern void os_warnMD5mismatch();
extern void os_closetypedfile();

int handlePartial (struct part *inpart,
		    char *headers,
...
int handleMultipart (struct part *inpart,
			char *contentType,
			params contentParams,
			int extractText);

int saveToFile (struct part *inpart,
		int inAppleDouble,
		char *contentType,
		params contentParams,
...
#endif		/* end of added definitions				*/
...
void from64 (inpart, outfile, digestp, suppressCR)
struct part *inpart;
FILE *outfile;
char **digestp;
int suppressCR;
...
void fromqp(inpart, outfile, digestp)
struct part *inpart;
FILE *outfile;
char **digestp;
{
...
void fromnone(inpart, outfile, digestp) 
struct part *inpart;
FILE *outfile;
char **digestp;
{
