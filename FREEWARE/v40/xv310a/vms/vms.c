...
/*
** Modified by:
** 	Sept. 6, 1995	Brian J. McCarthy		Digital Equipment Corp.
**	- Added several #ifdef's around code that is currently provided or 
**	  will be provided in future releases of OpenVMS.
**	- Remove call to free(uname) to prevent access violation.  With
**	  OpenVMS V7.0 and beyond calling free() with the return from
**	  translate_vms causes an access violation.  The memory allocated in
**	  translate_vms routine has become thread specific and is handled 
**	  by DECC$SHR.
**	- Removed the ability to test dirent code directly.  The need to 
**	  case off of __VMS_VER and TEST was getting too complicated.
*/
#include <stdio.h>
...
rmdir(p)
     char *p;
{printf("rmdir: Sorry, not available on pre-OpenVMS 7.0,'%s' is unchanged!\n",p);}
...
# define SHELL$TRANSLATE_VMS   decc$translate_vms 
 ...
 if ( uname = SHELL$TRANSLATE_VMS ( s ) ) {
    /* printf("vms: '%s' -> unix: '%s'\n", s, uname ); */
    if ( strlen(s) >= strlen(uname) )  { 
	strcpy(s,uname); 

#if 0
/* 
** The free() call for the return of SHELL$TRANSLATE_VMS should have never
** been valid.  With future versions of the DECC$SHR, calling free(uname) 
** tries to free thread-specific memory that is not owned by the calling
** process.  Since that specific version of DECC$SHR may end up being
** released on pre-OpenVMS V7.0 system, I am commenting out the call to 
** free(uname).  
** This may end up, on pre-OpenVMS V7.0 system to cause a small memory leak.
*/
	free(uname); 
#endif
    } else {
	s = uname;  /* will lose s's old allocation */
    }
 } 
 (*pargv)[(*pargc)++] = s;
}
...
#if defined (VMS) && __VMS_VER <= 60100000
...
/* gethostname exits on OpenVMS V6.1 and beyond (when using DECC) */
