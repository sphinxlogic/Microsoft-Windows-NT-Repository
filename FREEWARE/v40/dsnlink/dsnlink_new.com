...
ABSTRACT:
     This program automates checking the DSNlink ITS databases for new
     articles.
     ...
     articles retreived by DSNlink_NEW, and mail the article titles that
...
PARAMETERS:
 ...
 {** Note -- parameters P2 through P6 are used by DSNlink_NEW.COM internally
     ...
     the mail from DSNlink_NEW into a a file called SYS$LOGIN:DSNLINK_GET.DAT 
...
Copyright © 1991-1996 by Dan Wing.  This code may be freely distributed
...
Internet:  wing@tgv.com or wing@eisner.decus.org
 ...
 =----------------------------------------------------------------------=
  ...
  5.1-2   10-MAY-1994  Dan Wing, wing@tgv.com
     ...
     title retreived from the DSNlink host.  Thanks to Larry Stone. 
...
$  ENDIF
$
$  WRITE SYS$OUTPUT F$FAO("!/DSNlink_NEW, version !AS.!/Started by !AS on node !AS on !AS, !AS !8AS.", -
...
$
$  IF F$SEARCH("SYS$SYSTEM:DSN$MAIN.EXE") .EQS. "" THEN GOTO _NO_DSNLINK
$
...
$      OPEN/READ/WRITE/SHARE=WRITE DSN_OUTFILE 'DSN_HISTORY_FILE'
$      CLOSE DSN_OUTFILE
...
$  
$  OPEN/WRITE DSN_OUTFILE 'TMP_DSNFILE'
$
$  COUNT = 0
$  DB_NAME = ""
$_FILE_LOOP_1:
$  IF DB_NAME .NES. "" 
$  THEN 
$    WRITE DSN_OUTFILE "close ", DB_NAME
$    WRITE DSN_OUTFILE ""
$  ENDIF
$  DB_NAME = DB_NAME_'COUNT'
$  DB_DIRFILE = TMP_EXTRACTFILE + "_" + F$STRING(COUNT)
$  DB_DIRFILE_FLASH = DB_DIRFILE + "_FLASH"
$  WRITE DSN_OUTFILE "open ", DB_NAME
$  WRITE DSN_OUTFILE "directory/all/since=", QUOTE, F$CVTIME(DSN_SINCE,"ABSOLUTE"), QUOTE
$  WRITE DSN_OUTFILE "extract/directory ", DB_DIRFILE
$  IF DB_FLASH_'COUNT'
$  THEN
$    WRITE DSN_OUTFILE "directory/all/flash/since=", QUOTE, F$CVTIME(DSN_SINCE,"ABSOLUTE"), QUOTE
$    WRITE DSN_OUTFILE "extract/directory ", DB_DIRFILE_FLASH
$  ENDIF ! symbol is TRUE
$  COUNT = COUNT + 1
$  IF COUNT .LE. DB_NAME_COUNT THEN GOTO _FILE_LOOP_1
$
$  WRITE DSN_OUTFILE "exit"
$  CLOSE DSN_OUTFILE
$
...
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$
$  IF BATCH THEN SET RESTART_VALUE="Processing retreived articles"
...
$  OPEN/WRITE DSN_OUTFILE 'TMP_DSNFILE';       ! new version
$
$  IF PERFORMING_GET
$  THEN
$    IF GET_STYLE .EQS. "EDT"
$    THEN
$      WRITE DSN_OUTFILE "$! GET_STYLE = EDT"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE DSN$ITS_EDIT CALLABLE_EDT"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE EDTSYS NLA0:"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE EDTINI NLA0:"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE SYS$ERROR ", TMP_ERRORFILE
$      ! defining SYS$OUTPUT seems to mess up EDT (gives %SYSTEM-F-IVDEVNAM)
$
...
$      !   This problem doesn't seem to exist with other usernames....
...
$      !   to SYS$LOGIN.
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE DSN$SCRATCH ", F$PARSE("SYS$LOGIN") - ".;"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE SYS$SCRATCH ", F$PARSE("SYS$LOGIN") - ".;"
$    ENDIF
$    IF GET_STYLE .EQS. "EXTRACT"
$    THEN
$      WRITE DSN_OUTFILE "$! GET_STYLE = EXTRACT"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE SYS$ERROR ", TMP_ERRORFILE
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE SYS$OUTPUT ", TMP_OUTFILE
$    ENDIF
$  ELSE
$    IF .NOT. CHECKING_DATABASES
$    THEN
$      WRITE DSN_OUTFILE "$! normal DSNlink_NEW processing"
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE SYS$ERROR ", TMP_ERRORFILE
$      WRITE DSN_OUTFILE "$ DEFINE/USER_MODE SYS$OUTPUT ", TMP_OUTFILE
$    ELSE
$      WRITE DSN_OUTFILE "$! check database DSNlink_NEW processing"
$      WRITE DSN_OUTFILE "$! (cannot use /USER_MODE on the DEFINEs)"
$      WRITE DSN_OUTFILE "$ DEFINE SYS$ERROR ", TMP_ERRORFILE
$      WRITE DSN_OUTFILE "$ DEFINE SYS$OUTPUT ", TMP_OUTFILE
$    ENDIF
$  ENDIF
$
$  WRITE DSN_OUTFILE "$"
$  WRITE DSN_OUTFILE "$ DEFINE/USER_MODE DSN$ITS_INIT NLA0:"
$  WRITE DSN_OUTFILE "$"
$  WRITE DSN_OUTFILE "$ SET noON"
$  WRITE DSN_OUTFILE "$ DSN ITS"
$  CLOSE DSN_OUTFILE
$
$  APPEND 'TMP_DSNFILE';-1 'TMP_DSNFILE'
$
$  OPEN/APPEND DSN_OUTFILE 'TMP_DSNFILE'
$  IF F$EXTRACT(1,-1,DSN$VERSION) .LTS. "1.2" THEN WRITE DSN_OUTFILE "no"  ! in case DSN ITS asks for a survey (V1.2 and up don't ask for surveys if we're running in batch mode)
$  IF CHECKING_DATABASES
$  THEN
$    WRITE DSN_OUTFILE "$ DEASSIGN SYS$OUTPUT"
$    WRITE DSN_OUTFILE "$ DEASSIGN SYS$ERROR"
$  ENDIF
$  WRITE DSN_OUTFILE "$ EXIT '$STATUS'"
$  CLOSE DSN_OUTFILE
$
$  IF DEBUG
$  THEN
...
$      IF BATCH THEN SET RESTART_VALUE="Connection retry count = ''DSN_RETRY_COUNT'"
...
$  OPEN/READ DSN_INFILE 'DB_DIRFILE'
$
$  CREATE 'DB_DIRFILE';               ! create a new version, which contains 'real' stuff
$  OPEN/APPEND DSN_OUTFILE 'DB_DIRFILE'
$
...
$
$_CHK_HIST_LOOP:
$  READ/END_OF_FILE=_EXIT_CHK_HIST_LOOP DSN_INFILE ART_TITLE
$  ARTICLE_TITLE = F$EXTRACT(8,78,ART_TITLE)     ! strip off article numbers
...
$      WRITE DSN_OUTFILE F$FAO("!80AS", "Hist/DBG:" + F$EXTRACT(0,10,HIST_RECORD) + ":" + ARTICLE_TITLE)
...
$
$_INCLUDE_TITLE:
$  WRITE DSN_OUTFILE "  ", ARTICLE_TITLE
...
$    IF DSNLINK_FLASH
$    THEN
$      WRITE DSN_OUTFILE F$FAO("FLASH !AS database, !UL article!%S:", DB_NAME, ACCEPTED_ARTICLE_COUNT - REJECTED_ARTICLE_COUNT)
$      DB_ARTICLES_FLASH_'COUNT' = ACCEPTED_ARTICLE_COUNT - REJECTED_ARTICLE_COUNT
$    ELSE
$      WRITE DSN_OUTFILE F$FAO("!AS database, !UL article!%S:", DB_NAME, ACCEPTED_ARTICLE_COUNT - REJECTED_ARTICLE_COUNT)
...
$
$  RETURN  ! from CHECK_DSNLINK_HISTORY
$
$_HISTORY_DISABLED:
$  NEW_ARTICLE_FOUND = TRUE
...
$    CLOSE/noLOG DSN_OUTFILE
$    OPEN/WRITE DSN_OUTFILE 'TMP_MAILFILE'
$    WRITE DSN_OUTFILE "*** An error caused DSNlink_NEW to fail ***"
$    WRITE DSN_OUTFILE ""
$    WRITE DSN_OUTFILE "The error detected by DSNlink_NEW was:"
$    WRITE DSN_OUTFILE "  ", ERROR_MESSAGE
$    WRITE DSN_OUTFILE ""
$    DISPLAY_MESSAGES_OUTPUT = "DSN_OUTFILE"
$    GOSUB _DISPLAY_MESSAGES 
$    CLOSE DSN_OUTFILE
...
$  RETURN ! Leaving DISPLAY_MESSAGES
$
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$_DELETE_TMP_FILES:
$  IF F$TRNLNM("DSN_REFFILE") .NES. "" THEN CLOSE DSN_REFFILE
$  IF F$TRNLNM("DSN_SEARCHFILE") .NES. "" THEN CLOSE DSN_SEARCHFILE
$  IF F$TRNLNM("DSN_FILE") .NES. "" THEN CLOSE DSN_FILE
$  IF F$TRNLNM("DSN_OUTFILE") .NES. "" THEN CLOSE DSN_OUTFILE
$  IF F$TRNLNM("DSN_INFILE") .NES. "" THEN CLOSE DSN_INFILE
$  IF F$TRNLNM("DSN_CONFIGFILE") .NES. "" THEN CLOSE DSN_CONFIGFILE
...
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$_NO_DSNLINK:
$  WRITE SYS$OUTPUT "%DSNlink_NEW-E-NOTPRESENT, You do not have DSNlink software on this node"
$  WRITE SYS$OUTPUT "-DSNlink_NEW-E-NOTPRESENT, Contact your local Digital office to obtain DSNlink"
$  FINISHED_OK = TRUE
$  GOTO _EXIT
$
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
$  ENDIF
$  EXIT           ! display_databases
$  ENDSUBROUTINE  ! display_databases
$
$!
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
$
$  IF F$TRNLNM("DSN_OUTFILE") .NES. "" THEN CLOSE DSN_OUTFILE
$  OPEN/APPEND DSN_OUTFILE 'P1'
$  WRITE DSN_OUTFILE "_____"
$  WRITE DSN_OUTFILE ""
$
$  WRITE DSN_OUTFILE F$FAO("Message sent by DSNlink_NEW version !AS on node !AS, using:", DSNLINK_NEW_VERSION, F$GETSYI("NODENAME"))
$  WRITE DSN_OUTFILE F$FAO("  !14<Config file:!>!AS", DSN_CONFIG_FILENAME)
$  IF PERFORMING_GET 
$  THEN 
$    WRITE DSN_OUTFILE "  Performing GET processing"
$    IF F$TYPE(GET_FILENAME) .NES. ""
$    THEN
$      WRITE DSN_OUTFILE F$FAO("  !14<Get file:!>!AS", GET_FILENAME)
$    ELSE
$      WRITE DSN_OUTFILE F$FAO("  !14<Get file:!><unknown>"
$    ENDIF
$  ENDIF ! performing_get
$  TMP_GROUP_COUNT = MAIL_GROUP_'GRPNAME'
$  WRITE DSN_OUTFILE F$FAO("  !14<Mail group:!>!AS", MAIL_GROUP_NAME_'TMP_GROUP_COUNT')
$  IF F$TYPE(ORIGINALLY_FROM) .NES. "" THEN -
     IF ORIGINALLY_FROM .NES. USERNAME THEN -
     WRITE DSN_OUTFILE F$FAO("  !14<Sent by:!>!AS", ORIGINALLY_FROM)
$  IF PERFORMING_GET .AND. MAIL_GROUPS .NES. "<unknown>" THEN WRITE DSN_OUTFILE F$FAO("!15* = !AS", MAIL_GROUPS)  ! for RETAIN
$
$  IF MAIL_USERS .EQS. "<none>"
$  THEN
$    MAIL_USERS = USERNAME
$    WRITE DSN_OUTFILE "%DSNlink_NEW-E-MAILUSERS, Undefined mail users for this database; using SELF."
...
$  THEN
$    IF GRPNAME .NES. "$ERROR$" THEN -
       CALL _DISPLAY_DATABASES DSN_OUTFILE 2 "Databases:    " "''GRPNAME'" 2
$  
$    WRITE DSN_OUTFILE F$FAO("  !14<Since:!>!AS (!AS, !AS)", -
       DSN_SINCE, -
       F$CVTIME(DSN_SINCE,,"WEEKDAY"), -
       F$CVTIME(DSN_SINCE,"ABSOLUTE","DATE"))
$    IF DSN_HISTORY_FILE .NES. "NONE"
$    THEN
$      WRITE DSN_OUTFILE F$EXTRACT(0,79,F$FAO("  !14<History:!>!AS  (!UL blocks)", -
          DSN_HISTORY_FILE, F$FILE_ATTRIBUTES(DSN_HISTORY_FILE,"EOF")))
$    ELSE
$      WRITE DSN_OUTFILE F$FAO("  !14<History:!>NONE")
$    ENDIF ! history enabled/disabled
$  ENDIF ! performing GET
$
$  IF BATCH .AND. GRPNAME .EQS. "$ERROR$" 
$  THEN 
$    WRITE DSN_OUTFILE F$FAO("  !14<Log file:!>!AS", F$PARSE(F$GETQUI("DISPLAY_JOB","LOG_SPECIFICATION",,"THIS_JOB"),LOG_FILE))
$    IF "''ORIGINAL_GET_FILE'" .NES. "" THEN -
     WRITE DSN_OUTFILE F$FAO("  !14<Orig GET file:!>!AS", ORIGINAL_GET_FILE)
$  ENDIF
$
$  IF EXCEEDED_RETRY_LIMIT .AND. (.NOT. RECEIVED_FATAL_ERROR)
$  THEN 
$    WRITE DSN_OUTFILE ""
$    WRITE DSN_OUTFILE "%DSNlink_NEW-E-EXRETRY, Exceeded retry limit"
$    WRITE DSN_OUTFILE ""
$  ENDIF
$  IF RECEIVED_FATAL_ERROR
$  THEN
$    WRITE DSN_OUTFILE ""
$    WRITE DSN_OUTFILE "%DSNlink_NEW-E-NORETRY, Received fatal error - aborting retry attempts."
$    WRITE DSN_OUTFILE ""
$  ENDIF
$
...
$    THEN
$      WRITE DSN_OUTFILE F$FAO("Total connect time: !8AS, !UL reconnection!%S.  !UL total articles excluded.", -
       TOTAL_ELAPSED_CONN_TIME, DSN_RETRY_COUNT, TOTAL_EXCLUDED_ARTICLES)
$    ELSE
$      WRITE DSN_OUTFILE F$FAO("Total connect time: !8AS, !UL reconnection!%S.", -
       TOTAL_ELAPSED_CONN_TIME, DSN_RETRY_COUNT)
$    ENDIF
$  ENDIF
$  IF PERFORM_AUTO_COMPRESS THEN WRITE DSN_OUTFILE "History file auto-compression was triggered (history file > ", DSN_HISTORY_AUTO_COMPRESS, " blocks)."
$
$  CLOSE DSN_OUTFILE
$
$  IF DEBUG 
...
$    WRITE SYS$OUTPUT "%DSNlink_NEW-I-DEBUG, +-+ begin mail message +-+"
$    TYPE 'P1'
$    WRITE SYS$OUTPUT "%DSNlink_NEW-I-DEBUG, -+- end mail message -+-"
$  ENDIF
$
...
$    OPEN/WRITE DSN_FILE 'TMP_YESFILE'
$    WRITE DSN_FILE "Y"
$    CLOSE DSN_FILE
$  ENDIF
$  
...
$  
$  
$  EXIT           ! SEND_MAIL_MESSAGE
$  ENDSUBROUTINE  ! SEND_MAIL_MESSAGE
$
$!
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$_COMPRESS_HISTORY:
...
$
$  OPEN/WRITE DSN_FILE 'TMP_SPEC_FILE'
$  WRITE DSN_FILE "/FIELD=(NAME=HISTORY_DATE,POSITION:1,SIZE:10)"
$  WRITE DSN_FILE "/FIELD=(NAME=DATABASE,POSITION:10,SIZE:32)"
$  WRITE DSN_FILE "/FIELD=(NAME=TITLE,POSITION:42,SIZE:78)"
$  WRITE DSN_FILE "/FIELD=(NAME=DATABASE_KEY,POSITION:10,SIZE:110)"
$  WRITE DSN_FILE "/KEY=(DATABASE_KEY)"
$  WRITE DSN_FILE "/CONDITION=(NAME=KEEP_RECORD,TEST=(HISTORY_DATE GE ", QUOTE, ALLOW_IF_OLDER, QUOTE, "))"
$  WRITE DSN_FILE "/INCLUDE=(CONDITION=KEEP_RECORD)"
$  CLOSE DSN_FILE
$
...
$
$! undocumented symbol DSN$VERSION (only works with DSNlink V1.2 and above)
...
$! in case DSN SHOW VERSION doesn't work (it is apparently undocumented)
$  SET noON         
...
$! V1.2A also has the problem
$  GET_STYLE = "EDT"
$  IF F$EXTRACT(1,-1,DSN$VERSION) .LTS. "1.2" THEN GET_STYLE = "EXTRACT"
$  IF F$EXTRACT(1,-1,DSN$VERSION) .EQS. "1.2A" THEN GET_STYLE = "EXTRACT"
...
$  OPEN/READ/ERROR=_ERROR DSN_CONFIGFILE 'DSN_CONFIG_FILENAME'
$  DB_NAME_COUNT = 0
$  MAIL_GROUP_COUNT = 0
$  EXC_COUNT = 0
$  CONFIG_ERROR = FALSE
...
$  READ/ERROR=_ERROR/END_OF_FILE=_END_PARSE_CONFIG DSN_CONFIGFILE ORIG_RECORD
...
$  THEN
$    IF DEBUG THEN WRITE SYS$OUTPUT "%DSNlink_NEW-I-CONFIG, Mail emptry set to \", PARM2, "\"
...
$  THEN
$    IF DEBUG THEN WRITE SYS$OUTPUT "%DSNlink_NEW-I-CONFIG, Retry wait time set to \", PARM2, "\"
$    DSN_RETRY_WAIT_TIME = PARM2
$    VALID_RECORD = TRUE
$  ENDIF
$  
...
$    ENDIF
$    IF DEBUG THEN WRITE SYS$OUTPUT "%DSNlink_NEW-E-CONFIG, Excluding article \", EXCLUDE_ARTICLE_'EXC_COUNT', "\"
$    EXC_COUNT = EXC_COUNT + 1
$  ENDIF
$
...
$  ENDIF
$
$! the MX SITE agent's subprocess doesn't have SYS$LOGIN defined, whcih
...
$
$  IF BATCH THEN SET RESTART_VALUE
$  RETURN       !  leaving INIT_PARSE_CONFIG
$
$!
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
$  CREATE 'GET_TEMPFILE'
$  OPEN/APPEND DSN_OUTFILE 'GET_TEMPFILE'
$  WRITE DSN_OUTFILE "! ", F$SEARCH(GET_TEMPFILE)
$  WRITE DSN_OUTFILE "! Created by DSNlink_NEW, version ", DSNLINK_NEW_VERSION
$  WRITE DSN_OUTFILE "! file used for GET processing."
$  WRITE DSN_OUTFILE F$FAO("!! Created at !%D by user !AS::!AS", 0, F$GETSYI("NODENAME"), USERNAME)
$  WRITE DSN_OUTFILE "!"
$  CLOSE DSN_OUTFILE
...
$  ENDIF
$
$  FILENAME = TMP_EXTRACTFILE + "_" + F$STRING(CURRENT_COUNT)
...
$
$! get title (second record) and count the lines
$    OPEN/READ FILE 'FILENAME'
$    LINE_COUNT = 2
$    READ FILE JUNK/END_OF_FILE=_LINE_COUNT_EXIT
$    READ FILE SECOND_RECORD/END_OF_FILE=_LINE_COUNT_EXIT
$
$    _LINE_COUNT_LOOP:
...
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$_PROCESS_GET_FILE:
$! process contents of GET file and (optionally) create ITS command file
$
$  IF DEBUG
$  THEN
...
$  ENDIF  ! debug
$
$  OPEN/READ DSN_INFILE 'GET_FILENAME'
$
$  IF CREATE_CMD 
$  THEN 
$    OPEN/WRITE DSN_OUTFILE 'TMP_DSNFILE'
...
$
$_GET_VALID_LOOP:
$  READ/END_OF_FILE=_DONE_GET_VALID_LOOP DSN_INFILE RECORD
...
$      ! (we hold off on doing the OPEN until we've got an artilce title, 
$      !  to prevent OPENing a database and retrieving 0 articles)
...
$        THEN 
$          IF CREATE_CMD THEN WRITE DSN_OUTFILE "close ", LAST_OPENED_DB
...
$        THEN
$          ! perform the search
$          WRITE DSN_OUTFILE "search/english ", RECORD
$          IF DEBUG THEN WRITE DSN_OUTFILE "directory  ! this line only present during debugging"
$          IF GET_STYLE .EQS. "EDT"
$          THEN
$            WRITE DSN_OUTFILE "read/edit 1"
$            ! now we're talking to EDT
$            WRITE DSN_OUTFILE "EXIT " + TMP_EXTRACTFILE + "_" + F$STRING(TOTAL_ARTICLE_COUNT)
$            ! now we're back talking to DSN ITS
$          ENDIF ! get_style
$          IF GET_STYLE .EQS. "EXTRACT"
$          THEN
$            WRITE DSN_OUTFILE "read 1"
$            WRITE DSN_OUTFILE "extract " + TMP_EXTRACTFILE + "_" + F$STRING(TOTAL_ARTICLE_COUNT)
$          ENDIF ! get_style
$        ENDIF ! create_cmd 
...
$    OPEN/WRITE DSN_FILE 'TMP_DSNFILE'
$    WRITE DSN_FILE "SHOW DATABASE"
$    WRITE DSN_FILE "EXIT"
$    CLOSE DSN_FILE
$
$    WRITE SYS$OUTPUT "%DSNlink_NEW-I-CHECKDB, Performing database check"
$    GOSUB _PHONE_DCL_HOST
$    OPEN/READ DSN_FILE 'TMP_OUTFILE'
...
KEY 0
  ...
  SEG0_LENGTH        32
  SEG0_POSITION      0
$    OPEN/READ/WRITE DSN_OUTFILE 'TMP_CHECKFILE'
$
$!
...
$!
$    _CHK_DB_LOOP_1:
$    READ/END_OF_FILE=_CHK_DB_LOOP_1_END/ERROR=_ERROR DSN_FILE RECORD
$    IF DEBUG THEN WRITE SYS$OUTPUT "%DSNlink_NEW-I-DEBUG, Record: \", RECORD, "\"
$    IF RECORD .NES. "The currently available databases are:" THEN GOTO _CHK_DB_LOOP_1
$    _CHK_DB_LOOP_2:
$    READ/END_OF_FILE=_CHK_DB_LOOP_2_END/ERROR=_ERROR DSN_FILE RECORD
$! all databases start with database name in first position
$! end of database list is the string "No databases are currently open"
$    IF F$EXTRACT(1,1,RECORD) .NES. " " .AND. -
        RECORD .NES. "No databases are currently open" .AND. -
        F$EDIT(F$ELEMENT(1," ",RECORD),"COLLAPSE") .NES. ""
$    THEN
$      IF DEBUG THEN WRITE SYS$OUTPUT "%DSNlink_NEW-I-DEBUG, Found database \", F$ELEMENT(1," ",RECORD), "\"
$      WRITE DSN_OUTFILE F$FAO("!32AS!11%D", F$ELEMENT(1," ",RECORD), 0)
$    ENDIF
$    GOTO _CHK_DB_LOOP_2
$
$    _CHK_DB_LOOP_1_END:
...
$    CLOSE DSN_FILE
$    
$    IF DEBUG THEN WRITE SYS$OUTPUT "%DSNlink_NEW-I-DEBUG, Database file:"
$    IF DEBUG THEN TYPE 'TMP_CHECKFILE'
$
$    OPEN/WRITE DSN_OUTFILE 'TMP_MAILFILE'       ! for the generated mail message
$
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
$    OPEN/READ DSN_REFFILE 'DSN_CHECK_DATABASES_FILE'
$    ! open the new database
$    OPEN/READ DSN_SEARCHFILE 'TMP_CHECKFILE'
$
$    _CHK_DB_LOOP_3:
$    ! read record from reference ("old") file
$    READ/END_OF_FILE=_CHK_DB_LOOP_3_END/ERROR=_ERROR DSN_REFFILE RECORD
$    REF_DATE   = F$EXTRACT(32,11,RECORD)
$    REF_RECORD = F$EXTRACT(0,32,RECORD)
$
$    ! now try to find the record in the new file
$    READ/KEY=&REF_RECORD/ERROR=_CHK_DB_MISSING_NF DSN_SEARCHFILE JUNK
$    GOTO _CHK_DB_LOOP_3
$
$    _CHK_DB_MISSING_NF:
$    ! we didn't find it
$    IF MISSING_COUNT .EQ. 0 THEN WRITE DSN_OUTFILE "Missing database(s):"
$    MISSING_COUNT = 1
$    WRITE DSN_OUTFILE F$FAO("  Database !32AS last seen !AS, !AS", REF_RECORD, -
     F$CVTIME(REF_DATE,,"WEEKDAY"), REF_DATE)
$    GOTO _CHK_DB_LOOP_3
$    _CHK_DB_LOOP_3_END:
$    CLOSE DSN_SEARCHFILE
$    CLOSE DSN_REFFILE
$
$
$!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
$    READ/END_OF_FILE=_CHK_DB_LOOP_4_END/ERROR=_ERROR DSN_SEARCHFILE RECORD
$    NEW_RECORD = F$FAO("!32AS",F$EXTRACT(0,32,RECORD))
$
$    ! see if record is in reference ("old") file
$    READ/KEY=&NEW_RECORD/ERROR=_CHK_DB_NEW_NF DSN_REFFILE JUNK
$    GOTO _CHK_DB_LOOP_4
$
$    _CHK_DB_NEW_NF:
$xx = $status
$write sys$output "--debug ", f$message(xx)
$    ! we didn't find it
$    IF NEW_COUNT .EQ. 0 THEN WRITE DSN_OUTFILE "New database(s):"
$    NEW_COUNT = NEW_COUNT + 1
$    WRITE DSN_OUTFILE "  ", NEW_RECORD
$    GOTO _CHK_DB_LOOP_4
$
$    _CHK_DB_LOOP_4_END:
$    CLOSE DSN_REFFILE
...
$  WRITE SYS$OUTPUT "-DSNlink_NEW-I-HISTORY, File ", F$SEARCH(DSN_HISTORY_FILE)
$
$  OPEN/READ  DSN_FILE    'DSN_HISTORY_FILE'
...
KEY 0
  ...
  SEG0_POSITION      10
$
$  OPEN/READ/WRITE DSN_OUTFILE 'TMP_DSNFILE'
$  JUNK = 0
$
$  _NEW_HIST_LOOP:
$  READ/END_OF_FILE=_END_NEW_HIST_LOOP DSN_FILE RECORD
$  RECORD = F$EXTRACT(0,30,RECORD) + F$FAO("!#* ",12) + F$EXTRACT(30,-1,RECORD)
$  WRITE/SYMBOL DSN_OUTFILE RECORD
$  JUNK = JUNK + 1
