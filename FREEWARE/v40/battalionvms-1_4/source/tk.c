...
/*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-*/
/* remember to put this back into battalion.c once it is retrieved
 * from the mac. This funxtion should be useful for the mac tk
 * library as well as the unix one ... if the window coordinates
 * match the code written for the mouse down below should be the
 * same for the mac else it should work with some slight tweaking
...
static GLenum DoNextEvent(void)
{
	...
	lastEventType = ButtonPress;
	    ...
	    /* here we are going to do a little kludge to allow the user to pick the
	    monsters using the mouse rather than the 6-9 keys. If the user clicks the
	    mouse within a specified area then the appropriate digit will be sent to
	    simulate that number being pressed */
	    
	    #define col_1_left	    0.17
	    #define col_1_right	    0.386
	    #define col_2_left	    0.657
	    #define col_2_right	    0.814
	    
	    #define row_1_top	    0.327
	    #define row_1_bottom    0.49
	    #define row_2_top	    0.655
	    #define row_2_bottom    0.836
	    
	    winClickX = (float) current.xbutton.x / windInfo.width;
	    winClickY = (float) current.xbutton.y / windInfo.height;
	    
	    if ((winClickX > col_1_left) && (winClickX < col_1_right) &&
		(winClickY > row_1_top) && (winClickY < row_1_bottom))
		 mouseKey(TK_6, 0);
	    
	    if ((winClickX > col_1_left) && (winClickX < col_1_right) &&
		(winClickY > row_2_top) && (winClickY < row_2_bottom))
		 mouseKey(TK_8, 0);
	    
	    if ((winClickX > col_2_left) && (winClickX < col_2_right) &&
		(winClickY > row_1_top) && (winClickY < row_1_bottom))
		 mouseKey(TK_7, 0);
