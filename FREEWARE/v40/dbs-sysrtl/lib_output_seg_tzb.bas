	%title	"Formatted output in ascii and hex"
	%ident	"V1-002"

	sub lib_output_seg_tzb (string input_buffer			  &
...
!		buffer as ascii and hex byte values.
!
! Environment:	User mode.
!
! History:
!
!	15-Jun-1990, DBS, Version V1-001
...
!	Take the buffer as byte values and format them into ascii and hex.
!	Displays are generated from SYS$FAO and directed to sys$output.
!	Output is generated in segments of the specified size and a byte
!	offset from the start of the buffer is included with each segment.
!
! Calling Sequence:
!	call lib_output_seg_tzb (buffer, buffer_size, segment_size)
!		-or-
!	pushal	segment_size
!	pushal	buffer_size
!	pushaq	buffer
...
!
! Formal Argument(s):
!	buffer.rt.ds	Address of a string descriptor that points to the
...
! Type declarations for constants
	...
	declare long constant	default_segment_size = 16
	declare long constant	max_segment_size = 32
	declare long constant	max_array_size = 38	! 32 + 6
	declare string constant	fao_data = "  [!#<!AF!>]-!5UL-[!XB!#(3XB)]"
...
mainline:
	...
	while (loop < number_of_segments)
		...
		b_byte(1) = segment_size
		b_byte(2) = bytes_this_segment
		b_byte(3) = string_address + first - 1
		b_byte(4) = first - 1
		b_byte(5) = ascii(mid(input_buffer, first, 1))
		b_byte(6) = bytes_this_segment - 1
