...
xss_create_menu_buttons(title, menu, menulist, nitems, auxdata)
     char           *title;
     Widget          menu;
     xs_menu_struct *menulist;
     int             nitems;
     caddr_t         auxdata;
{
  ...
  for(i=0;i<nitems;i++){
	...
	XtCreateWidget(menulist[i].name,
		       xmCascadeButtonWidgetClass,
		       menu, wargs, 1);
      xss_create_menu_buttons(menulist[i].sub_menu_title,
			      sub_menu, menulist[i].sub_menu,
			      menulist[i].n_sub_items,auxdata);
    }
  } 
  /*
   * Manage all button widgets. Menu panes are not managed.
