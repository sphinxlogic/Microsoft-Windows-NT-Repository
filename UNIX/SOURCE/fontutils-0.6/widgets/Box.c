/* Box.c: implementation of the Athena Box widget.  The DoLayout
   procedure has been modified from the distributed version.
...
static void
DoLayout(bbw, width, height, reply_width, reply_height, position)
    BoxWidget	bbw;
    ...
    Boolean	position;	/* actually reposition the windows? */
{
	...
	if (widget->core.managed) {
	    ...
	    if (position && ((Position) lw != widget->core.x
                             || (Position) h != widget->core.y)) {
	...
	if (bbw->composite.num_children == num_mapped_children)
		...
		if (XtIsManaged(*childP) &&
		    (*childP)->core.mapped_when_managed) {
                    /* If we do not ensure the child is realized,
                       XtMapWidget can fail; e.g., kern items in xbfe.  */
