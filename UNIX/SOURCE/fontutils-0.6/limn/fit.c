...
real reparameterize_threshold = 30.0;
...
#define APPEND_CORNER(index, angle, c)					\
  do									\
    {									\
      ...
      if (wants_display)						\
        display_corner (O_COORDINATE (pixel_outline, index));		\
      LOG4 (" (%d,%d)%c%.3f",						\
...
find_tangent (curve_type curve, boolean to_start_point, boolean cross_curve)
{
  ...
  if (*curve_tangent == NULL)
    {
      *curve_tangent = xmalloc (sizeof (vector_type));
