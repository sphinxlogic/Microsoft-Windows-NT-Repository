  ...
  *argv[];
{
  ...
  status=WriteAlienImage(stereo_image);
  /*
    Display detailed info about the image.
  */
  (void) fprintf(stderr,"[%d] %s %s=>%s %dx%d",stereo_image->scene,
    left_image->filename,right_image->filename,stereo_image->filename,
    stereo_image->columns,stereo_image->rows);
  if (stereo_image->class == DirectClass)
