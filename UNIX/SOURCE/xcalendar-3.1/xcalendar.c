 ...
 *	$Header: /p/p4/X11R5/contrib/purdue/xcalendar/RCS/xcalendar.c,v 1.4 1993/04/01 15:00:50 bingle Exp $
 *      $Author: bingle $
 *      $Locker:  $
 *
...
#ifndef lint
static char *rcsid_calendar_c = "$Header: /p/p4/X11R5/contrib/purdue/xcalendar/RCS/xcalendar.c,v 1.4 1993/04/01 15:00:50 bingle Exp $";
#endif /* lint */ */
...
XtResource appResourceList[] = {
   ...
   {"firstDay","FirstDay",XtRInt,sizeof(int),
      0,XtRInt,(XtPointer)&defFirstDay},
...
/*  some compilers don't like XtOffset  */
    ...
    SetResOffset(resNo++,calendarDir);
    SetResOffset(resNo++,oldStyle);
    SetResOffset(resNo++,markCurrent);
    SetResOffset(resNo++,updateCurrent);
    SetResOffset(resNo++,currentForeground);

    XtGetApplicationResources(toplevel,(XtPointer)&appResources,appResourceList,
...
/** mark the entries **/
    ...
    marked_day = -1;
    if (appResources.markCurrent == True) {
   ...
   if(!(initialized || InitEditors()))
     ...
     if (allDaysList)
	   ...
	   if (is_today((Widget)listp->element) && appResources.markCurrent) {
	      XtSetArg(colors[0],XtNforeground,appResources.currentForeground);
     ...
     Boolean   doFlag;
{
   ...
   if(doFlag == True){
	...
	return; /* it's not on the list */
	 ...
	 if (is_today(w) && appResources.markCurrent)
	    XtSetArg(colors[1],XtNforeground,appResources.currentForeground);
	 XtSetValues(w,colors,2);
      }
...
void CurrentTic(client_data, tid)
...
void MakeDayList() 
{
	...
	XtSetArg(args[0],XtNbackground,&DayBackground(allDaysList));
	XtSetArg(args[1],XtNforeground,&(DayForeground(allDaysList)));
