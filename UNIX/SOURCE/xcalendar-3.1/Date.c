...
static char *getmonthday (num, startptr)
...
char *startptr; /* String to pick it from */
{
        ...
        for (i = 0; i < num; i++) {
                ...
                if (end == (char *) NULL) return returnval;
                start = end + 1;
        }
        end = index (start, '/');
        if (end == (char *) NULL)  strcpy (returnval, start);
        else {
                char *op;
        ...
        XtIntervalId *id;
{
	...
	if (id || !w->date.interval_id)
		...
		XtAppAddTimeOut( XtWidgetToApplicationContext( (Widget) w),
				w->date.update*1000, clock_tic, (XtPointer)w );
	(void) time(&time_value);
	tm = *localtime(&time_value);
	/*
	...
	}
	{
		...
		if (*p != '%')
           	    *datep++ = *p;
            	    ...
            	    case 'M':
                	    datep += strlen(strcpy(datep, 
				getmonthday(tm.tm_mon,
				(char *) w->date.monthnames)));
			    break;
            	    case 'm':
                	    datep += strlen(strcpy(datep,
				getmonthday(tm.tm_mon,
				(char *) w->date.monthnms)));
			    break;
            	    case 'W':
                	    datep += strlen(strcpy(datep,
				getmonthday(tm.tm_wday,
				(char *) w->date.daynames)));
			    break;
            	    case 'w':
                	    datep += strlen(strcpy(datep,
				getmonthday(tm.tm_wday,
