     ...
     Widget          w;
{
  ...
  tptr->state = qres.zmodemAutoDownload; tptr++;
     ...
     XtPointer       client_data;
{
  ...
  DestroyParentPopup(NULL, dialog, NULL);
...
ToggleZmodemAutoDownload()
{
  toggle_flag(&(qres.zmodemAutoDownload));
  return 0;
}
     ...
     String          port;
{
  ...
  mport(port);
  if (open_modem() < 0) {
    SeErrorF("/SP/ Could not open new modem device %s", port, "", "");
	mport(oldPort);
	if (open_modem() < 0)
	  SeErrorF("/SP/ Could not re-open old modem device %s", oldPort, "", "");
    return False;
  }
...
s_set_autozm()
{
  set_onoff(&(qres.zmodemAutoDownload));
}
...
put_parameters()
{
  ...
  param.zmodemAutoDownload = qres.zmodemAutoDownload;
  param.newlineTrMode = newlineTrMode;

  if (write(param_pipe[1], (char *)&param, sizeof(param)) == -1)
    SeError("/pp/ Could not write to pipe");
}
...
get_parameters()
{
  ...
  read(param_pipe[0], (char *)&param, sizeof(param));
  ...
  qres.zmodemAutoDownload = param.zmodemAutoDownload;
