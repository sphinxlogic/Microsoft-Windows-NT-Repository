 ...
 *
 * DIALSTR is a sprintf format string that assumes a HAYES-compatible modem.
 *
 * HAYSATT is the HAYES "attention" signal.
 *
 * HAYSHUP is the HAYES "hangup" command.
 *
	 ...
	 int             newModem;
{
  ...
  if (useModemControl && (retStat = IoGetModemStat(mfd)) < 0) {
	SeErrorF("/GMS/ Could not get control line status for device %s", 
			  modem_port, "", "");
	SeNotice("/GMS/ Disabling status toggles for that device");
...
#ifndef __SVR4_2__
/* modem control not supported as far as I can see! */
...
open_modem()
{
  ...
  if (modem_port[0] == '\0') {
    SeError("No modem device is specified");
    return (-1);
  }

  if (lock_tty()) {
    SeError("Modem device is locked");
    return -1;
  }

  /* Need O_NDELAY to get the file open before we have carrier */
  if ((mfd = open(modem_port, O_RDWR | O_NDELAY)) < 0) {
    SeError("Could not open modem device");
    SePError(modem_port);
    unlock_tty();
    return -1;
  }
  ...
  if (((oldflags = fcntl(mfd, F_GETFL, 0)) == -1) ||
    ...
    SePError("open_modem could not reset O_NDELAY");
    unlock_tty();
    return -1;
  }
