	 ...
	 int             top;
{
  ...
  if (top)
	XoAppAddSignal(app_con, SIGCHLD, exec_cmd_handler, (XtPointer) & fio);
  else
  ...
  if (forkRes == 0) {
    ...
    if (setuid(getuid()) < 0)
      SeError("/ESC/ Failed to set effective uid");

    if (*scmd == '\0') {
      SeNoticeF("Executing the shell \"%s\"", shell, "", "");
      execl(shell, shell, (char *)NULL);
      SeErrorF("/ESC/ Execution of the shell %s failed", shell, "", "");
      exit(1);
    }

    SeNoticeF("Executing the command \"%s\"", scmd, "", "");
    execl(shell, shell, "-c", scmd, (char *)NULL);
    SeErrorF("/ESC/ Execution of the command \"%s\" failed", scmd, "", "");
    exit(1);
  }
  else if (forkRes > 0) {
