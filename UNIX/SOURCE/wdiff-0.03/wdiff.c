...
/* One may also, optionnaly, define a default PAGER_PROGRAM.  This is
   normally done from the Makefile.  If PAGER_PROGRAM is undefined and
   the PAGER environment variable is not set, none will be used.  */
...
char *tmpnam ();
void *xmalloc ();
#ifndef _STRING_H					    /* this comes from string.h, which also defines
							     * this stuff (Greg Lehey, LEMIS, 16 May 93)  */
const char *strstr ();
#endif
const char *xstrdup ();
...
extern char copyright[];	/* short copyright */
...
int show_statistics;		/* for printing summary statistics */
int is_printer;			/* use printer modes */
int is_display;			/* use termcap strings */
int pager_is_less;		/* if paging through the "less" program */
...
struct side
{
  ...
  const char *temp_name;	/* temporary file name */
...
int output_is_less;		/* flag indicating that pager is less */
...
initialize_strings (void)
{
  ...
  /* Insure some default strings.  */

  if (!term_delete_start && !is_printer && user_delete_start == NULL)
    user_delete_start = "[-";
  if (!term_delete_end && !is_printer && user_delete_end == NULL)
    user_delete_end = "-]";
  if (!term_insert_start && !is_printer && user_insert_start == NULL)
    user_insert_start = "{+";
  if (!term_insert_end && !is_printer && user_insert_end == NULL)
    user_insert_end = "+}";
}
  ...
  while (isspace (side->character))
    {
      ...
      /* While changing lines, insure we stop any special display prior
...
reformat_diff_output (void)
{
  ...
  while (1)
      ...
      if (decode_directive_line ())
	{
	  ...
	  /* Show any inserted code, or insure skipping over it in case the
...
launch_output_program (void)
{
  ...
  /* By default, use stdout for output_file.  Insure the termcap
     initialization string is sent to stdout, not the pager.  */
...
complete_output_program (void)
{
  ...
  /* Insure the termcap termination string is sent to stdout, not the
     pager, and this only once the pager has terminated.  */
...
main (int argc, char *argv[])
{
  ...
  show_statistics = 0;
  is_printer = 0;
  is_display = isatty (fileno (stdout));
  pager_is_less = 0;
  ...
  while ((option_char
	  = getopt_long (argc, argv, "123Chdpstvw:x:y:z:", longopts, NULL)
	  ) != EOF)
    switch (option_char)
      {
      case '1':
  ...
  left_side->temp_name = xstrdup (tmpnam (NULL));
  right_side->temp_name = xstrdup (tmpnam (NULL));
