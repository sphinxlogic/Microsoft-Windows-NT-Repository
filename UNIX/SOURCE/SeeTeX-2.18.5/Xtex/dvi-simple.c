...
double ScaleUnitsNum[]
  = {1,   1,  100, 7200,  254, 2540, 1157, 1157,  65536};
...
struct PostAmbleInfo *p;
{
  ...
  CSetConversion(&PrivateConv,
		 DviDpi, DviUserMag, Numerator, Denominator, Magnification);
  
  DviTotalPages = p -> pai_NumberOfPages;
  DviWidestPage = cfromSP(&PrivateConv, p -> pai_WidestPageWidth);
  DviTallestPage = cfromSP(&PrivateConv, p -> pai_TallestPageHeight);
...
char *name;
{
  ...
  if ( EnvFontPath == 0 ) {
    ...
    if (EnvFontPath != 0) {
	len = strlen(EnvFontPath);
	if (EnvFontPath[0] == ':') {	/* prepend system path */
	    char *tmp = malloc(len + strlen(xtexResources.tfmPath) + 1);
	    strcpy(tmp,xtexResources.tfmPath);
	    strcat(tmp,EnvFontPath);
	    EnvFontPath = tmp;
	} else if (EnvFontPath[len-1] == ':') {	/* append system path */
	    char *tmp = malloc(len + strlen(xtexResources.tfmPath) + 1);
	    strcpy(tmp,EnvFontPath);
	    strcat(tmp,xtexResources.tfmPath);
	    EnvFontPath = tmp;
	}
    }
  }
