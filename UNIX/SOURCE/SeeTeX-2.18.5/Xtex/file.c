...
#include <sys/types.h>
#ifdef __STDC__
extern FILE *XsraSelFile (Widget, char *, char *, char *, char *, char *,
  char *, int * (char *, char **, struct stat *), char **);
static int isDviFileName (char *, char **, struct stat *);
#else
extern FILE *XsraSelFile ();
static int isDviFileName ();
#endif
#endif
     ...
     Cardinal *num_params;
{
  ...
  if ( *num_params < 1 ) {
    FileOpenShouldCopy = 1;		/* default to copy */
  }
...
#ifdef SELFILE
  else if ( strncmp(params[0],"select", 6) == 0 ) {
    FILE *fp;
  
     FileOpenShouldCopy = 0;		/* default to open */
     fp = XsraSelFile((Widget) TopLevelWidget, "Open File :", "okay", "cancel",
       "Cannot open file : ", (char *) NULL, "r", isDviFileName, &fileName);
     if (fp == NULL)
  	    return;
     else {
       Arg argList[20];
       Cardinal args;
       char *p, *currentString;

       setDirectory (fileName);
       /* Peel off directory names and set file name with remainder */
       p = fileName;
       p += strlen (fileName) - 1;
      while( p != fileName && *p != '/' )
        --p;
      ...
      XtSetArg(argList[args], XtNstring, &currentString); args++;
      XtGetValues(fileNameText, argList, args);
      setFile( currentString, FileOpenShouldCopy );
      }
	...
	setFile( fileName, FileOpenShouldCopy );
      }
    }
  }
}
...
#ifdef SELFILE
  selectButton =
    XtCreateManagedWidget("selectButton",
			  commandWidgetClass, topPane,
			  0,0);
#endif
...
#ifdef SELFILE
static int
  isDviFileName (fileName, retFileName, statbuf)
char *fileName;
char **retFileName;
struct stat *statbuf;
{
  ...
  *retFileName = fileName;
  extIdx = strlen (fileName) - 4;
  return (S_ISDIR (statbuf->st_mode) || 
	 ((extIdx > 0) && !strcmp (fileName + extIdx, ".dvi")));
}
#endif
