  ...
  TeXPageSlamPages()
{
  ...
  for ( popup = 0; popup < TeXPages; popup++ ) {
    if ( IS_VALID_PAGE(TheTeXPages[popup]) ) {
     ...
     int mag;
{
  ...
  if ( ! xtexResources.autoSize ) {
    ...
    if ( tp -> userMag != mag ) {
      /*
       * Usually, you use the same fonts over and over. If you unref
       * them first and then ref them, you'll unload them & then load
       * them again. Major waste.
       */
      int oldMag = tp -> userMag;
    ...
    if ( tp -> userMag != mag ) {
      ...
      XUnmapWindow( XtDisplay(tp -> viewPort), XtWindow(tp -> viewPort));
    ...
    if ( ! tp -> horizBar ) {
      tp -> horizBar = XtNameToWidget((Widget) vp, "horizontal");
      if ( tp -> horizBar ) {
	XtInstallAccelerators(tp -> singleDviPage, tp -> horizBar);
      }
    }
    ...
    if ( ! tp -> vertBar ) {
      tp -> vertBar = XtNameToWidget((Widget) vp, "vertical");
      if (tp -> vertBar) {
	XtInstallAccelerators(tp -> singleDviPage, tp -> vertBar);
      }
    }
    ...
    if ( XtIsRealized( tp -> viewPort ) ) {
      XMapWindow( XtDisplay(tp -> viewPort), XtWindow(tp -> viewPort));
    }
  }
}
...
void actionRedisplay(w, event, params, num_params)
     Widget w;
     XEvent *event;
     String *params;
     Cardinal *num_params;
...
{
  TeXPage *tp = TeXPageLocate(w);
  if ( IS_VALID_PAGE(tp) && params && *params) {
...
static Bool actionBool(oldValue, params, num_params)
     Bool oldValue;
     String *params;
     Cardinal *num_params;
{
  static char *trues[] = {"true", "True", "yes", "1", 0};
  static char *falses[] = {"false", "False", "no", "0", 0};

  if ( *num_params == 1 ) {
    int i;
    for (i = 0; trues[i] != 0; i++ ) {
      if ( strcmp(trues[i], params[0]) == 0 ) return( True );
    }
    for (i = 0; falses[i] != 0; i++ ) {
      if ( strcmp(trues[i], params[0]) == 0 ) return( False );
    }
    return( False );
  } else {
    return( !oldValue);
...
static void actionChangeShowPostscript(w, event, params, num_params)
     Widget w;
     XEvent *event;
     String *params;
     Cardinal *num_params;
{
  TeXPage *tp = TeXPageLocate(w);
  xtexResources.showPostScript = actionBool( xtexResources.showPostScript,
					    params, num_params );
} 
...
static XtActionsRec texxPageActionsTable[] = {
  ...
  {"xtex-show-postscript",  actionChangeShowPostscript }
};
