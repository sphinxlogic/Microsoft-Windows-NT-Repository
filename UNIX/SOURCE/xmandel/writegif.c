...
{	FILE	*fp;
	...
	int		ncolors;
	int		ix, iy, shift;
	byte	*bytes, *these_bits, *input, *output;
	...
	for (i = 0;  i < 256;  i++)
		rmap[i] = gmap[i] = bmap[i] = 0;

	fp = fopen (fname, "w");
	if (fp == NULL)
	{	printf ("Cannot open gif output file\n");
		exit (0);
	}
	ncolors = getcolors();
	if (depth != 8)
	{	bytes = (byte *) malloc (image->pixheight *  image->pixwidth);
		for (iy = 0; iy < image->pixheight;  iy++)
		{
			output = bytes + iy * image->pixwidth;
			input = image->pix  + iy * image->bytewidth;
			switch (depth)
			{
			case 4:
				for (ix = 0; ix < image->pixwidth; ix++)
				{	if (ix % 2)
						shift = 0;
					else
						shift = 4;
					output[ix] = (input[ix >> 1] >> shift) & 0x0f;
				}
				break;
			...
			case 2:
				...
				{	switch (ix % 4)
					...
					output[ix] = (input[ix >> 2] >> shift) & 0x03;
				}
				break;

			case 1:
				for (ix = 0; ix < image->pixwidth; ix++)
					output[ix] = (input[ix >> 3]  >> (7 - ix % 8)) & 0x01;
			}
		}
		these_bits = bytes;
	}
	else
	{	these_bits = image->pix;
		bytes = NULL;
	}
	writegif (fp, these_bits, image->pixwidth, image->pixheight, rmap, gmap, bmap, ncolors, 0);
	if (bytes)
		free (bytes);
	fclose (fp);
}
...
static int getcolors ()
{
	...
	ncolors = DisplayCells (display, DefaultScreen (display));
	default_cmap = DefaultColormap (display, DefaultScreen (display));
	for (i = 0;  i < DisplayCells (display, 0);  i++)
	{	my_colors[i].pixel = i;
		my_colors[i].flags = DoRed | DoGreen | DoBlue;
	}

	XQueryColors (display, default_cmap, my_colors, ncolors);

	/*	scale the colours to a 0..255 range as expected by GIF
		X11 always gives RGB values as a 16 bit number.
	*/
	for (i = 0;  i < ncolors;  i++)
	{	rmap[i] = (double) my_colors[i].red / 256.0;
		gmap[i] = (double) my_colors[i].green / 256.0;
		bmap[i] = (double) my_colors[i].blue / 256.0;
	}
	return (ncolors);
}
...
int code;
{
  ...
  if( code == EOFCode ) {
    ...
    if( ferror( g_outfile ) )
    {	printf ("unable to write GIF file\n");
    	exit (1);
    }
  }
}
