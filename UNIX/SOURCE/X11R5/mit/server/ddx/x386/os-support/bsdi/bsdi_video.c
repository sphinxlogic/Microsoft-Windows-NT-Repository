...
/* $XFree86: mit/server/ddx/x386/os-support/bsdi/bsdi_video.c,v 2.14 1993/10/18 13:02:41 dawes Exp $ */
...
int ScreenNum;
{
	...
	for (i = 0; i < MAXSCREENS; i++)
	{
		...
		{
		    if (ioctl(x386Info.consoleFd, PCCONENABIOPL, 0) < 0)
		    {
	...
	if (ExtendedEnabled)
	{
		ioctl(x386Info.consoleFd, PCCONDISABIOPL, 0);
...
void xf86EnableInterrupts()
{
	if (!ExtendedEnabled)
	{
		ioctl(x386Info.consoleFd, PCCONENABIOPL, 0);
	}
