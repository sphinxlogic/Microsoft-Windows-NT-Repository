/*
 * $XFree86: mit/server/ddx/x386/common/xf86_KbdBSD.c,v 2.5 1993/09/30 17:49:38 dawes Exp $
 * Derived from x386Kbd.c by S_ren Schmidt (sos@login.dkuug.dk)
 * which is Copyright 1990,91 by Thomas Roell, Dinkelscherben, Germany.
 * and from x386KbdCODrv.c by Holger Veit
 *
  ...
  case CODRV01X:
    for (i=1; i<= 128; i++) {
	  ...
	  k[3] = NoSymbol;
        ...
        cokeymap.keynum = i;
        if (ioctl(x386Info.consoleFd, KBDGCKEY, &cokeymap) < 0)
          break;  /* Don't try again if it fails once */
