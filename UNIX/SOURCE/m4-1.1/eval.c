...
static eval_error xor_term ();
static eval_error and_term ();
static eval_error not_term ();
static eval_error logical_not_term ();
static eval_error cmp_term ();
static eval_error shift_term ();
...
evaluate (char *expr, int *val)
{
  ...
  switch (err)
    ...
    case NO_ERROR:
      break;
    case MISSING_RIGHT:
      m4error ("bad expression in eval (missing right paren): %s", expr);
      break;
    case SYNTAX_ERROR:
      m4error ("bad expression in eval: %s", expr);
      break;
    case UNKNOWN_INPUT:
      m4error ("bad expression in eval (bad input): %s", expr);
      break;
    case DIVIDE_ZERO:
      m4error ("divide by zero in eval: %s", expr);
      break;
    case MODULO_ZERO:
      m4error ("modulo by zero in eval: %s", expr);
      break;
    default:
...
static eval_error
logical_not_term (eval_token et, int *v1)
{
  eval_error er;
...
static eval_error
shift_term (eval_token et, int *v1)
{
  eval_token op;
  int v2;
  eval_error er;
  ...
  while ((op = eval_lex (&v2)) == LSHIFT || op == RSHIFT)
	...
	default:
	  internal_error ("Bad shift operator in shift_term ()");
	  break;
	}
    }
  if (op == ERROR)
