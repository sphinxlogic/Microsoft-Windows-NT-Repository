...
char	**av;
{
	...
	register off_t		*seekpts, li;	/* table of seek pointers */
	 ...
	 */
	++curseek;
	seekpts = (off_t *) malloc(sizeof *seekpts * curseek);	/* NOSTRICT */
	if (seekpts == NULL) {
		perror("calloc");
		exit(-1);
	}
	...
	(void) fseek(outf, (off_t) (sizeof Tbl + sizeof *seekpts * curseek), 0);
	(void) fseek(inf, (off_t) 0, 0);	/* goto start of input */
	...
	*seekpts = ftell(outf);
	...
	(void) fwrite((char *) seekpts, sizeof *seekpts, (int) curseek, outf);
	(void) fclose(outf);
