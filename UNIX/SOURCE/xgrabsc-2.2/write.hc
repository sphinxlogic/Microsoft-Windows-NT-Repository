 ...
 * ccs version:	1.8
 *
 * ccsid:	@(#)write.hc	1.8 - 8/12/92 14:15:01
 * from: 	ccs/s.write.hc
  ...
  FILE *outfile;
{
  ...
  if (lines % (PREVIEW_CODEWIDTH / 2) > 0) lines++;
  ...
  if (psimage == image->ximage)
    reverse = BlackPixel(hDisplay,hScreen)==1;
  else
    reverse = FALSE;
  ...
  int checkLimits;  /* TRUE if PS interpreter memory checks should be made */
{
  ...
  if (encode) {
    /* define a drawcolormappedimage procedure geared to this image
    */
    ...
    */
    ...
    if (binary) {
      fputs("  { currentfile buffer readstring pop pop    %% run length and index\n", outfile);
      fprintf(outfile,"    /ccount ccount %d add def                 %% count binary chars\n",encode ? 2 : 1);
      fprintf(outfile,
            "    ccount %d ge                             %% test for full line\n", IMAGE_CODEWIDTH);
      fputs("    { /ccount 0 def                          %% reset character counter\n",outfile);
      fputs("      currentfile endstr readline pop pop    %% skip newline\n",outfile);
      ...
      fputs("  { currentfile buffer readhexstring pop pop  %% index\n", outfile);
  ...
  FILE *outfile;
{
  ...
  color.flags = visMask;
  color.pad = 0;
  for (i=image->numcells; i<header.ncolors; i++) {
    color.pixel = i;
    if (*(char *) &swaptest)
      swapdws(&color.pixel, sizeof(color.pixel));
    fwrite(&color, sizeof(XColor), 1, outfile);
  ...
  FILE *outfile;
{
  ...
  case 3:
    fprintf(outfile, "/* XPM */\nstatic char * image_name [] = {\n\"%d %d %d %d\",\n",
      ximage->width, ximage->height, image->numcells, cpp);
    fputs("/* pixels*/\n", outfile);
    break;
  case 2:
