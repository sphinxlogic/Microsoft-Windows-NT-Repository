 ...
 * ccs version:	1.23
 *
 * ccsid:	@(#)xgrabsc.c	1.23 - 8/12/92 14:15:05
...
/* a version of XmuGetClientWindow - used so that Xmu and Xt need not be
 * linked to xgrabsc.
 *
 * The following two functions are from the X11R4 MIT distribution and
  ...
  char *env;
{
  ...
  if (!borders  &&
      (source == sourceKey || source == sourceWd)  &&
      childWindow != hRoot) {
  ...
  if ((source == sourceKey && childWindow != hRoot) ||
    ...
    if (childWindow == hRoot) {
      ...
      * determine the portion of the root window that should be grabbed */
      ...
      sourceWindow = vRoot;
      if (!borders) {
        xrect.x      = xwa.x + xwa.border_width;
        xrect.y      = xwa.y + xwa.border_width;
      ...
      /* if a subwindow of a window-manager window was found, adjust
        * the coordinates of the rectangle to be grabbed */
      if (clientWindow  &&  clientWindow != wmWindow) {
        if (!XGetWindowAttributes(hDisplay, wmWindow, &xwa)) {
          fprintf(stderr, "%s: unable to get window coordinates\n", programName);
          XCloseDisplay(hDisplay);
          exit(3);
        }
