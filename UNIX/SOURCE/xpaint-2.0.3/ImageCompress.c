...
/*
** this converts a RGB pixel into a 15-bit true color value
*/
...
/*
**  These macros exctrat the 0..255 range value from the "true color" version
*/
...
static void insertColorArea(unsigned long *counts, Area_t **rlargest, Area_t **rsmallest, Area_t *area)
{
	int		i;
...
*/
static int cmpColor(void *a, void *b)
{
...
static void freeColor(void *junk)
{
	/*
...
Image	*ImageCompress(Image *input, int ncolors)
{ 
	...
	*/
	...
	/*
	**  Creata a histogram of 15-bit color values.
	...
	*/
	...
	ctable = (color_t *)XtCalloc(sizeof(color_t), ncolors);
	nuniq  = 0;
	...
	if (nuniq <= ncolors) {
		/*
		**  Wow, this has enough colors to fit on the requested colormap
	...
	*/
	...
	/*
	**  Create the color areas an initalize the first element
	*/
	...
	/*
	**  While there are area's still to be broken down 
	...
	for (i = 0; i < allocated; i++) {
		...
		*/
		...
		for (j = 0; j < areas[i].npixels; j++) {
			...
			red   += RED_VALUE(pixel);
			green += GREEN_VALUE(pixel);
			blue  += BLUE_VALUE(pixel);
