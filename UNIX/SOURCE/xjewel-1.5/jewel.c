			...
			switch(key)
				...
				********************************/
				...
				case XK_u:
					...
					{ /* an attempt to iconify * /
					XClientMessageEvent xev;
					Atom wmchs;
					wmchs=XInternAtom(xw_display,
					    "WM_CHANGE_STATE", True);
					xev.type=ClientMessage;
					xev.message_type=wmchs;
					xev.window=xw_window;
					xev.format=32;
					xev.data.l[0]=IconicState;
					XSendEvent(xw_display,
					 RootWindow(xw_display, xw_screen),
					 False,(SubstructureRedirectMask |
					 SubstructureNotifyMask), &xev);
					/ *XUnmapWindow(xw_display, xw_window);*/
					}
					break;
				case XK_Cancel:
				case XK_E:
