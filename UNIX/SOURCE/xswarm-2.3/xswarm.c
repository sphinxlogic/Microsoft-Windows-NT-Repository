...
char	**geometry;
{
    ...
    while ((option = getopt(argc,argv,"hq?vprVxet:d:g:D:b:a:s:A:S:B:w:c:C:"))
	    != EOF)
    {
        switch (option)
        {
...
Window	current;
{
    ...
    if (0 != num_children) XFree( (char *) children);
}
...
Wait_For_Idleness()
{
	...
	while (found)
	{
		...
		case CreateNotify:
			...
			if (debugging) printf("Create Notify.\n\n");
...
Create_Big_Window()
{
    ...
    if (debugging) printf("Creating big window.\n\n");
...
char	*geometry;
{
    ...
    if (debugging) printf("Creating normal window.\n\n");
...
Animate()
{
    ...
    /* Allocate memory. */
    segs = (XSegment *) Safe_Malloc(sizeof(XSegment) * bees);
    old_segs = (XSegment *) Safe_Malloc(sizeof(XSegment) * bees);
	...
	if (stop)
	{
		...
		if (debugging)
		{
		    printf("Theoretical window info: x,y,width,height\n");
		    printf("%d,%d  %d,%d\n", winX, winY, winW, winH);
	    ...
	    XFreeGC(display.dpy, display.bee_gc);
	    XFreeGC(display.dpy, display.wasp_gc);
	    XFreeGC(display.dpy, display.erase_gc);
	    XFreeGC(display.dpy, display.bee_xor_gc);
	    XFreeGC(display.dpy, display.wasp_xor_gc);

	    if (debugging) printf("Freed memory.\n");
...
XEvent	*event;
{
	...
	case ClientMessage: /* sent by f.delete from twm */
		...
		if (debugging) printf("Configure Notify\n\n");
		...
		if (0 == timeout)
		{
		    ...
		    if (debugging)
		    {
			printf("Width, height, x, y = %d %d %d %d\n",
			    winW, winH, winX, winY);
		    }
		}
	    };
