...
static void 
CvtLabelsToPointer (args, num_args, fromVal, toVal)
     XrmValue *args;
     int *num_args;
     XrmValue *fromVal;
     XrmValue *toVal;
{
  ...
  while (i < MAXLABELS)
    ...
    /* Append null to end of string, step beyond '"' marking end of this label, increment "i" our
     * label counter. */
    *str_pos++ = '\0';
    instr++;
    i++;
...

{
  ...
  /* Draw traversal/enter highlight if actual exposure or if we had to clip text area */
...

{
  ...
  if ((curmb->core.width == newmb->core.width) &&
	...
	maxWidth = Max (maxWidth,
			XTextWidth (fs,
				    newmb->multi_button.labels[i],
				    strlen (newmb->multi_button.labels[i])));
     ...
     XwMultiButtonWidget mb;
{
  ...
  /* If user has not given us any labels then try to use widget's name as a label (of course this
   * too could be NULL but it still shouldn't hurt us). */
  if (mb->multi_button.labels == NULL)
    mb->multi_button.labels = &(mb->core.name);

  /* There is ALWAYS one label, even if that label is NULL (i.e., the application provides no
   * names and the button's name is NULL). */
  ...
  /* Allocate space for each label and copy pointer to new string into array allocated above.
   * Also compute, in pixels, width of widest label. */
  for (i = 0; i < mb->multi_button.num_labels; i++)
    {
