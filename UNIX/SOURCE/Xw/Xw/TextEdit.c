...
/*--------------------------------------------------------------------------+*/
static void _XwTextSetSelection(ctx, left, right)
/*--------------------------------------------------------------------------+*/
  XwTextEditWidget ctx;
  XwTextPosition left, right;
{
...
/*--------------------------------------------------------------------------+*/
static StuffFromBuffer(ctx, buffer)
/*--------------------------------------------------------------------------+*/
  XwTextEditWidget ctx;
  int buffer;
{
    ...
    Boolean status;
{
   ...
   if (status) {
     ...
     if ( XtIsRealized(w) ) InsertCursor(w, XwisOff);
   }
}
 ...
 Widget request, new;
{
    ...
    text->updateFrom = (XwTextPosition *) XtMalloc(1);
    text->updateTo = (XwTextPosition *) XtMalloc(1);
...
{ XwTextEditWidget ctx = (XwTextEditWidget)widget;
  ...
  else if (text->srctype == XwdiskSrc)
    text->source = XwDiskSourceCreate(widget, args , *num_args); 
     ...
     XwTextEditWidget ctx;
{
  ...
  XtRemoveAllCallbacks(ctx, XtNmotionVerification);
  XtRemoveAllCallbacks(ctx, XtNmodifyVerification);
  XtRemoveAllCallbacks(ctx, XtNleaveVerification);
}
...
XwTextEditClassRec XwtexteditClassRec = {
  ...
  },
  {
    ...
    /* set_selection     */     _XwTextSetSelection,
