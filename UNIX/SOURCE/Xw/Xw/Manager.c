...
/*   mw -> composite.num_mapped_children = 0; for R3 */
   ...
   if (mw -> manager.background_tile < XwFOREGROUND ||
       mw -> manager.background_tile > XwSLANT_LEFT)
   {
   ...
   if (mw -> manager.background_tile != XwBACKGROUND)
      mw -> core.background_pixmap =
          XwCreateTile (XtScreen ((Widget)(mw)),
                        mw -> manager.foreground,
                        mw -> core.background_pixel,
                        mw -> manager.background_tile); 
    ...
    Widget	w;
{
    int _XW_NARGS = 0;
    ArgList	args = NULL;
    Cardinal	*num_argsP = (Cardinal *) &_XW_NARGS;
 ...
 *
 * static Cardinal
 * _XwInsertOrder (cw, args, p_num_args)
 *
 *   This function searches through the supplied arglist, looking for an
 *   entry of type XtNinsertPosition.  If a match is found, then the
 *   corresponding value is extracted and used as the position to insert
 *   the child; if no match is found, then the value returned will indicate
 *   that the child should be inserted at the end.  Range checking is
 *   performed on the value:  if the value is less than zero, then it will
 *   be set to zero, and if the value is greater than the number of children
 *   already registered, then it will be set to the number of currently
 *   registered children.
 *
 ************************************************************************/


static Cardinal _XwInsertOrder (cw, args, p_num_args)

   CompositeWidget cw;         /* Composite widget being inserted into */
   ArgList         args;       /* Arglist containing potential position */
   Cardinal      * p_num_args; /* Number of argument pairs in arglist */

{
   Cardinal i;
   Cardinal position = cw->composite.num_children;

   /* Search the arglist for a match */
   for (i = 0; i < *p_num_args; i++)
   {
      if ((strcmp (args[i].name, XtNinsertPosition) == 0) &&
          (XwStrlen(args[i].name) == XwStrlen(XtNinsertPosition)))
      {
         /* Perform some range checking */
         position = args[i].value;

         if (position < 0)
            position = 0;
         else if (position > cw->composite.num_children)
            position = cw->composite.num_children;
      }
   }
...

{
	...
	(curmw->core.ancestor_sensitive != newmw->core.ancestor_sensitive) ||
        ...
        (curmw->manager.traversal_on != newmw->manager.traversal_on))
       {
         ...
         else 
         {
            ...
            if (XtIsSubclass((parent = (XwManagerWidget)XtParent(newmw)),
            ...
            if (traversalIsHere)
            {
               ...
               XtSetKeyboardFocus(parent, NULL);
            }
         }
        }
   ...
   if (newmw -> manager.background_tile < XwFOREGROUND ||
       newmw -> manager.background_tile > XwSLANT_LEFT)
   {
      XtWarning("Manager: Incorrect background tile.");
       ...
       curmw -> core.background_pixel)
   {
      ...
      else if (newmw -> manager.background_tile == XwBACKGROUND)
      {
         valueMask = CWBackPixel;
      ...
      else
      {
         newmw -> core.background_pixmap =   
            XwCreateTile (XtScreen ((Widget)(newmw)),
                          newmw -> manager.foreground, 
                          newmw -> core.background_pixel, 
                          newmw -> manager.background_tile);

         valueMask = CWBackPixmap;
         attributes.background_pixmap = newmw -> core.background_pixmap;
      }

      if (XtIsRealized (newmw))
    ...
    XwManagerWidget	w;
{
   ...
   else if ((XtIsSubclass((parent = (XwManagerWidget)XtParent(w)),
            XwmanagerWidgetClass)) &&
