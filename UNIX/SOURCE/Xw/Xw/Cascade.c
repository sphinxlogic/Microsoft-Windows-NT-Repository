...

{
  ...
  /* First, compute the minimum width (less padding) needed for these items; include the title in
   * these calculations. */
  ...
  for (i = 0; i < mw->manager.num_managed_children; i++)
    ...
    /* If we are in the middle of a geometry request from one of our children, then we need to
     * use the ideal size of the requesting widget, instead of the current widget.  Otherwise,
     * ask the widget for its ideal size. */
    if ((Widget) menuButton == mw->cascade.georeqWidget)
      idealWidth = mw->cascade.georeqWidth;
    else if (XtIsSubclass (menuButton, XwmenubuttonWidgetClass))
      {
...
static Boolean 
PreferredSize (mw, width, height, replyWidth, replyHeight)
     XwCascadeWidget mw;
...

{
  ...
  for (i = 0; i < mw->manager.num_managed_children; i++)
    {
    menuButton = (XwMenuButtonWidget) mw->manager.managed_children[i];
    XtMoveWidget (menuButton, xPos, yPos);

    /* The following kludge will force the widget's resize routine to always be called. */
    oldHeight = menuButton->core.height;
    menuButton->core.height = 0;
    menuButton->core.width = 0;

    XtResizeWidget (menuButton,
...

{
  ...
  /* Since the manager is solely responsible for controlling the position of its children, any
   * geometry request made which attempts to modify the child's position will not be satified;
   * instead, a compromise will be returned, with the position bits cleared. */
  ...
  if (request->request_mode & (CWWidth | CWHeight | CWBorderWidth))
    ...
    /* Only allow an item to shrink if it was the largest item. However, always allow the item to
     * get bigger. */

    if (!isPreferredSize)
      {
      /* The widget is attempting to make itself smaller than is allowed; therefore, we must
       * return a compromise size, which we know will succeed. */
	...
	{
	/* Even though the size request succeeded, the request cannot be satisfied, since someone
	 * before us has already set the result to XtGeometryAlmost; most likely, it was because
	...
	{
	/* All components of the geometry request could be satisfied, so make all necessary
	 * adjustments, and return a 'Yes' response to the requesting widget. */
	TryNewLayout (mw);
	Resize (mw);
	/* SetTitleAttributes (mw, FALSE); */
	width = w->core.width;
  ...
  /* Stacking changes are always allowed.  However, if the result has already been set to
   * XtGeometryAlmost, then some portion of the request has already failed, so we need to copy
   * the stacking request back into the 'reply' structure, so that they are not lost. */
     ...
     XwCascadeWidget mw;
{
  ...
  /* Tell the menu manager that the list of mapped & managed children has changed, */
     ...
     XwCascadeWidget request, new;
{
  ...
  /* KLUDGE!!!  This is necessary!!!   The shell must be realized so that the cascades
   * changed_managed routine is called and the keyboard accelerators are set up for the menu
   * manager. */
     ...
     XwCascadeWidget current, request, new;
{
  ...
  /* We never allow our traversal flags to be changed during SetValues(); this is enforced by our
   * superclass. */
  ...
  /* If the title position changes, but the size of the menupane does not (because no other
   * attributes have changed which could cause the size to change), then we need to force a call
   * to our resize() procedure, so that the menu buttons get repositioned relative to the new
   * title position. */
...
static void 
SetTitleAttributes (mw, setCoreFields)
...
static void 
Visibility (widget, event)

     Widget widget;
     XEvent *event;

{
  /* Noop; purpose is to prevent Manager's translation from taking effect. */
}

