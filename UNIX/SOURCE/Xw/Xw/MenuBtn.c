      ...
      for (i = 0; i < temp;)
      {
                                 ...
                                 mbutton->button.label[i+1]) == NULL)
        {
	      ...
	      if (XGetFontProperty(mbutton->button.font, XA_UNDERLINE_POSITION,
			           &mbutton->menubutton.underline_y) == 0)
 ...
 *   Description:
 *   -----------
 *   Creates image_GC and inverted_GC.
 *
 *   Inputs:
 *   ------
 *   mbutton = menubutton
...
XwMenuButtonWidget mbutton;
{
   ...
   values.tile = XwCreateTile (XtScreen((Widget)(mbutton)),
				      values.foreground, values.background,
				      XwFOREGROUND);
 
   mbutton->menubutton.inverted_GC = XtGetGC ((Widget) mbutton,
 ...
 Widget request, new;
{
   ...
   mbutton->menubutton.labelPixmap =
	 ...
	 mbutton->menubutton.cascadePixmap =
	    mbutton->menubutton.invertLabelPixmap =
		  ...
		  mbutton->menubutton.invertCascadePixmap = NULL;
    ...
    */
   if ((mbutton->menubutton.mnemonic) &&
       (*(mbutton->menubutton.mnemonic) != NULL))
   {
...
XwMenuButtonWidget mbutton;
{
   ...
   XtRemoveAllCallbacks (mbutton, XtNcascadeSelect);
   XtRemoveAllCallbacks (mbutton, XtNcascadeUnselect);
...
XSetWindowAttributes * attributes;
{
   ...
   XtCreateWindow (mbutton, InputOutput, (Visual *) CopyFromParent,
...

{
   ...
   if (XtIsSensitive(mbutton))
   {
	    ...
	    (mbutton->menubutton.menuMgr, mbutton, event) == FALSE)
	{
     ...
     XtCallCallbacks (mbutton, XtNselect, NULL);
  }
}

/*************************************<->*************************************
...
XEvent 		  * event;
{
                       ...
                       2 * mbutton->core.border_width))
   {
      ...
      XtCallCallbacks (mbutton, XtNcascadeUnselect, &params);
       ...
       (mbutton->primitive.traversal_type == XwHIGHLIGHT_OFF))
   {
                                ...
                                mbutton->button.internal_width)))
	 {
                             ...
                             mbutton->button.internal_width))
	    {
	       if (!mbutton->menubutton.cascadeEnabled)
	       {
		  XtCallCallbacks (mbutton, XtNcascadeSelect, NULL);
#ifdef DEBUG
                  printf ("enabled\n");
#endif
		  mbutton->menubutton.cascadeEnabled = TRUE;
...
Widget current, request, new;
{
	...
	newmbutton->menubutton.accelerator)
    {
	  ...
	  else
	  {
		...
		(*(((XwMenuMgrWidgetClass)
		   ...
		   (newmbutton->menubutton.menuMgr, newmbutton,
		    newmbutton->menubutton.accelerator,
		    newmbutton->menubutton.accelEventType,
		    newmbutton->menubutton.accelDetail,
		    newmbutton->menubutton.accelModifiers);
	  ...
	  if (curmbutton->menubutton.menuMgr)
		 ...
		 XtClass(curmbutton->menubutton.menuMgr))->
		    ...
		    (curmbutton->menubutton.menuMgr, curmbutton);
    ...
    if (curmbutton->menubutton.mnemonic != newmbutton->menubutton.mnemonic)
    {
      ...
      {
         if (*(newmbutton->menubutton.mnemonic) == NULL)
         {
            XtWarning 
    ...
    /*
     * If the foreground or background changed, recreate the GC's
     */
            ...
            curmbutton->core.background_pixel))
    {
       ...
       XtDestroyGC (curmbutton->menubutton.inverted_GC);
	...
	if (curmbutton->menubutton.menuMgr)
	   ...
	   (*(((XwMenuMgrWidgetClass)
	     XtClass(curmbutton->menubutton.menuMgr))->
	      ...
	      (curmbutton->menubutton.menuMgr, newmbutton);
        }
    }
