/*copyright	"%c%"*/
#ident	"@(#)r4Cdemos:worm/worm.c	1.3"
/*
	...
	char **argv;
{
...
#ifdef NCR
	if ((optind<argc)&&(argv[optind][0]!='-'))usage(cmd);
#endif
	...
	if (screen_saver)	{
		...
		if (geo == NULL)
			geo = defgeo;
		flags = XParseGeometry(geo, &x, &y, (unsigned int*)&xsize, (unsigned int*)&ysize);
	...
	for (;;) {
		...
		if (XPending(dpy))	{
			...
			if (e.type == VisibilityNotify)	{
				XVisibilityEvent *ev = (XVisibilityEvent *) &e.xvisibility;
				...
				XConfigureEvent *ev = (XConfigureEvent *) &e.xconfigure;

				xsize = ev->width; ysize = ev->height;
				for (i = 0; i < nworms; i++)	{
#ifdef NCR
					if(worm[i]->xcirc)free(worm[i]->xcirc);
					if(worm[i]->ycirc)free(worm[i]->ycirc);
					if(worm[i])free(worm[i]);
#else
					free(worm[i]->xcirc); 
					free(worm[i]->ycirc);
					free(worm[i]);
...
long	temp;
	...
	temp = colors[0].pixel;		/* start at 1 - don't want the black */
...
read_write_map:
	...
	for (i = ncolors; i >= MIN_COLORS; i--)	{
		if (XAllocColorCells(dpy, cmap, False, (unsigned long*)&pmasks, 0, (unsigned long*)pixels, i))
			break;
		}
	if (i < MIN_COLORS)	{
		fprintf(stderr,
