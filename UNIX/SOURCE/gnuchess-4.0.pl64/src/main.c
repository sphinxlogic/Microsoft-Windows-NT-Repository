...
unsigned long GenCnt, NodeCnt, ETnodes, EvalNodes, HashCnt, HashAdd, FHashCnt, FHashAdd, HashCol, THashCol, filesz, hashmask, hashbase;
long replus, reminus;
...
#ifdef BOOKFILE
  if (! bookfile)					    /* didn't pick up a book file, */
    bookfile = BOOKFILE;				    /* use the default */
#endif
  XC = 0;
  MaxResponseTime = 0;
#if defined CHESSTOOL || defined XBOARD
