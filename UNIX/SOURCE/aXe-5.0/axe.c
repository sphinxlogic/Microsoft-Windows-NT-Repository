...
Help(w, client_data, call_data)
     ...
     XtPointer client_data, call_data;
{
    AxeEditorHelpWindow("general", (Widget) client_data);
}
     ...
     char **argv;
{
	...
	XtVaSetValues(waiter,
		      XtNbackgroundPixmap, XCreatePixmapFromBitmapData(display, XDefaultRootWindow(display), axeLogo_bits, axeLogo_width, axeLogo_height, fg, bg, XDefaultDepthOfScreen(XtScreen(waiter))),
		      NULL);
