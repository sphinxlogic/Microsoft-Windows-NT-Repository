     ...
     Widget widget;
{
    ...
    return AxeiiTextReSearch(widget, forward,
     ...
     Boolean once;
{
	...
	if (XawTextReplace(widget, begin, end, &block) != XawEditDone)
	{
 ...
 */
Boolean
AxeiiTextReSearch(widget, forward, compexp, match, begin, end)
     Widget widget;
     Boolean forward;
     regexp *compexp;
     String *match;
     XawTextPosition *begin, *end;
{
    ...
    XawTextPosition eol, lastEol, swap, readfrom, toread;
    Boolean found;
    int length;
    ...
    textSrc = XawTextGetSource(widget);
    found = False;
    for (pos = XawTextGetInsertionPoint(widget),
                                      lastPos = pos - 1, eol = -1, lastEol = 0;
