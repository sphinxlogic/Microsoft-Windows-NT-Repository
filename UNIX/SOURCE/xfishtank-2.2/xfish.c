...
/* MAINLINE is a flag to the bit map files to just include the definitions
 * and not the bitmaps themselves (Greg Lehey, LEMIS, 3 May 1993) */
#define MAINLINE
#include "vroot.h"
#include "xfishy.h"
#include "bubbles.h"
...
/* to ensure that this happens, chop the rest off (Greg Lehey, LEMIS, 3 May 1993):
 * depending on which random function you use with SVR4, it can return up to 32
 * bits of information */
inline int RAND ()
{
  return rand () & 0x7fff;
...
#else
#define RAND rand					    /* standard rand () function */
#endif
...
{
	int ret;
#if defined(MOTOROLA) || defined(SCO) || defined (SVR4)
