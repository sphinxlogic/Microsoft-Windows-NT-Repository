...
_gdbm_get_bucket (dbf, dir_index)
     ...
     word_t dir_index;
{
  word_t bucket_adr;	/* The address of the correct hash bucket.  */
     ...
     word_t next_insert;
{
  ...
  word_t      dir_start0;	/* Used in updating the directory. */
  word_t      dir_start1;
  word_t      dir_end;

  word_t      *new_dir;		/* Pointer to the new directory. */
  off_t       dir_adr; 	/* Address of the new directory. */
  word_t      dir_size;	/* Size of the new directory. */
  word_t      old_adr[31]; 	/* Address of the old directories. */
  word_t      old_size[31]; 	/* Size of the old directories. */
  ...
  while (dbf->bucket->count == dbf->header->bucket_elems)
      ...
      if (dbf->header->dir_bits == dbf->bucket->bucket_bits)
	{
	  ...
	  new_dir  = (word_t *) malloc (dir_size);
	  if (new_dir == NULL) _gdbm_fatal (dbf, "malloc error");
	  for (index = 0;
	  	index < dbf->header->dir_size/sizeof (word_t); index++)
	    {
