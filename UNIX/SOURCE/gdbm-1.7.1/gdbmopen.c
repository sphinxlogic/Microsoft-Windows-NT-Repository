     ...
     void (*fatal_func) ();
{
  ...
  if (file_stat.st_size == 0)
      ...
      if (dbf->header == NULL)
	{
      ...
      dbf->header->dir_size = 8 * sizeof (word_t);
      ...
      if (dbf->header->dir_size != dbf->header->block_size)
	{
      ...
      dbf->dir = (word_t *) malloc (dbf->header->dir_size);
      if (dbf->dir == NULL)
	{
	  gdbm_close (dbf);
	  gdbm_errno = GDBM_MALLOC_ERROR;
      ...
      if (dbf->bucket == NULL)
	{
      ...
      for (index = 0; index < dbf->header->dir_size / sizeof (word_t); index++)
