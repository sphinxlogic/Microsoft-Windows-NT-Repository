...
bytefile dvifile  ; 
integer dviloc  ; 
ASCIIcode realnameoffile[FILENAMESIZE + 1]  ; 
integer i  ; 
cmdpar dvipar[256]  ; 
cmdcl dvicl[256]  ; 
...
{eightbits b  ; 
  ...
  while ( curloc < curlimit ) {
    ...
    if ( curnamelength < FILENAMESIZE ) 
    {
      incr ( curnamelength ) ; 
      curname [ curnamelength ] = xchr [ b ] ; 
    } 
    else overflow ( strnamelength , FILENAMESIZE ) ; 
  } 
  curloc = pcktstart [ e ] ; 
  ...
  while ( curnamelength < FILENAMESIZE ) {
