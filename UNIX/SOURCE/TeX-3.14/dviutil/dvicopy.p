...
curname:packed array[1..FILENAMESIZE]of char;curnamelength:int15;{:62}
{65:}{fres:int16u;resdigits:array[1..5]of char;nresdigits:int7;}{:65}
{72:}widths:array[widthpointer]of integer;
...
realnameoffile:packed array[0..FILENAMESIZE]of ASCIIcode;i:integer;
{:109}{118:}dvipar:packed array[eightbits]of cmdpar;{:118}{121:}
...
outfname:packed array[1..FILENAMESIZE]of char;outloc:integer;
outback:integer;outmaxv:int31;outmaxh:int31;outstack:int16u;
outpages:int16u;{:234}{245:}outfnts:array[fontnumber]of fontnumber;
outnf:fontnumber;outfnt:fontnumber;{:245}{23:}{48:}
...
if curnamelength<FILENAMESIZE then begin incr(curnamelength);
curname[curnamelength]:=xchr[b];
end else overflow(strnamelength,FILENAMESIZE);end;curloc:=pcktstart[e];
curlimit:=pcktstart[e+1];
while curloc<curlimit do begin{if curloc>=curlimit then confusion(
strpackets)else}begin b:=bytemem[curloc];incr(curloc);end;
begin c:=xchr[b];
{if c='?'then for ll:=nresdigits downto 1 do if curnamelength<
FILENAMESIZE then begin incr(curnamelength);
curname[curnamelength]:=resdigits[ll];
end else overflow(strnamelength,FILENAMESIZE)else}
if curnamelength<FILENAMESIZE then begin incr(curnamelength);
curname[curnamelength]:=c;end else overflow(strnamelength,FILENAMESIZE);
end;end;while curnamelength<FILENAMESIZE do begin incr(curnamelength);
