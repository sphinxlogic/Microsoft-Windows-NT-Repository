...
auxstacksize=20;maxbibfiles=20;poolsize=5120000;maxstrings=20000;
maxcites=3000;mincrossrefs=2;wizfnspace=3000;singlefnspace=100;
maxentints=25000;maxentstrs=25000;entstrsize=100;globstrsize=1000;
...
{37:}nameoffile:packed array[1..FILENAMESIZE]of char;
namelength:0..FILENAMESIZE;nameptr:integer;{:37}{42:}buffer:buftype;
last:bufpointer;{:42}{44:}svbuffer:buftype;svptr1:bufpointer;
svptr2:bufpointer;tmpptr,tmpendptr:integer;{:44}{49:}
...
begin writeln(logfile,'file name size ',FILENAMESIZE:0);
writeln(standardoutput,'file name size ',FILENAMESIZE:0);end;goto 9998;
end;end;{:60}{83:}procedure outtoken(var f:alphafile);var i:bufpointer;
...
begin if((strstart[filename+1]-strstart[filename])>FILENAMESIZE)then
begin begin write(logfile,'File=');write(standardoutput,'File=');end;
...
begin if(namelength+(strstart[ext+1]-strstart[ext])>FILENAMESIZE)then
begin begin write(logfile,'File=',nameoffile,', extension=');
...
while(nameptr<=FILENAMESIZE)do begin nameoffile[nameptr]:=' ';
nameptr:=nameptr+1;end;end;{:61}{62:}procedure addarea(area:strnumber);
var pptr:poolpointer;
begin if(namelength+(strstart[area+1]-strstart[area])>FILENAMESIZE)then
begin begin write(logfile,'File=');write(standardoutput,'File=');end;
...
begin{389:}
begin while((exbufxptr<exbufptr)and(lexclass[exbuf[exbufptr]]=1)and(
lexclass[exbuf[exbufptr]]=4))do exbufxptr:=exbufxptr+1;
...
begin while true do begin setpaths(BIBINPUTPATHBIT+TEXINPUTPATHBIT);
...
while(not eoln(standardinput))do begin if(auxnamelength=FILENAMESIZE)
then begin readln(standardinput);begin samtoolongfilenameprint;goto 46;
end;end;nameoffile[auxnamelength+1]:=getc(standardinput);
...
sauxextension])>FILENAMESIZE)or(auxnamelength+(strstart[slogextension+1]
-strstart[slogextension])>FILENAMESIZE)or(auxnamelength+(strstart[
sbblextension+1]-strstart[sbblextension])>FILENAMESIZE))then begin
...
if(not aopenin(bstfile,TEXINPUTPATH))then begin begin write(logfile,
...
;aclose(logfile);end{:456};9999:end.{:10}
