...
integer stringwidth  ; 
char nameoffile[FILENAMESIZE + 1]  ; 
integer namelength  ; 
integer nameptr  ; 
buftype buffer  ; 
bufpointer last  ; 
...
{{
    ...
    printoverflow () ; 
    {
      (void) fprintf( logfile , "%s%ld\n",  "file name size " , (long)FILENAMESIZE ) ; 
      (void) fprintf( standardoutput , "%s%ld\n",  "file name size " , (long)FILENAMESIZE ) ; 
    } 
    longjmp(jmp9998,1) ; 
  } 
...
{poolpointer pptr  ; 
  if ( ( ( strstart [ filename + 1 ] - strstart [ filename ] ) > FILENAMESIZE 
  ) ) 
  {
    {
...
{poolpointer pptr  ; 
  if ( ( namelength + ( strstart [ ext + 1 ] - strstart [ ext ] ) > 
  FILENAMESIZE ) ) 
  {
    {
...
{poolpointer pptr  ; 
  if ( ( namelength + ( strstart [ area + 1 ] - strstart [ area ] ) > 
  FILENAMESIZE ) ) 
  {
    {
      (void) Fputs( logfile ,  "File=" ) ; 
...
boolean streqbuf ( strnumber s , buftype buf , bufpointer bfptr , bufpointer 
len ) 
...
hashloc strlookup ( buftype buf , bufpointer j , bufpointer l , strilk ilk , 
boolean insertit ) 
...
void zinttoASCII ( integer theint , buftype intbuf , bufpointer intbegin , 
bufpointer * intend ) 
...
void zbuildin ( pdstype pds , pdslen len , hashloc * fnhashloc , bltinrange 
bltinnum ) 
...
{/* 16 17 52 */ poplitstk ( poplit1 , poptyp1 ) ; 
  ...
  else if ( ( poptyp3 != 1 ) ) 
  {
	...
	] == 1 ) && ( lexclass [ exbuf [ exbufptr ] ] == 4 ) ) ) exbufxptr = 
	exbufxptr + 1 ; 
...
{/* 41 46 */ while ( true ) {
    ...
    setpaths ( BIBINPUTPATHBIT + TEXINPUTPATHBIT ) ; 
    if ( ( argc > 1 ) ) 
    {
      argv ( 1 , nameoffile ) ; 
      auxnamelength = 1 ; 
    ...
    } 
    {
      ...
      sauxextension ] ) > FILENAMESIZE ) || ( auxnamelength + ( strstart [ 
      slogextension + 1 ] - strstart [ slogextension ] ) > FILENAMESIZE ) || ( 
      auxnamelength + ( strstart [ sbblextension + 1 ] - strstart [ 
      sbblextension ] ) > FILENAMESIZE ) ) ) 
      {
	samtoolongfilenameprint () ; 
	goto lab46 ; 
      } 
...
{/* 10 */ if ( ( bstseen ) ) 
  ...
  } 
  {
    ...
    if ( ( ! aopenin ( bstfile , TEXINPUTPATH ) ) ) 
    {
      {
