...
{integer i, j, m, n  ; 
  ...
  overflowlabeloffset = 10000000L ; 
  {
	...
	uexit ( 1 ) ; 
	if ( ( strcmp ( optiontable [ optionindex ] .name , 
...
void startgf ( ) 
{/* 30 */ char argbuffer[FILENAMESIZE + 1]  ; 
  integer argbufptr  ; 
  if ( optind == argc ) 
  {
    ...
    argbuffer [ FILENAMESIZE ] = ' ' ; 
    argbufptr = 1 ; 
    linelength = 0 ; 
    while ( ( argbufptr < FILENAMESIZE ) && ( argbuffer [ argbufptr ] == ' ' ) 
    ) argbufptr = argbufptr + 1 ; 
    while ( ( argbufptr < FILENAMESIZE ) && ( linelength < terminallinelength 
    ) && ( argbuffer [ argbufptr ] != ' ' ) ) {
