...
FILENAMESIZE]of char;nochange,nostyle:boolean;{:114}{29:}
procedure error;var k,l:0..bufsize;begin{30:}
...
whichfilenext:char;fname:array[1..FILENAMESIZE]of char;
...
slashpos:=-1;i:=1;
while(fname[i]<>' ')and(i<=FILENAMESIZE-5)do begin mffilename[i]:=fname[
i];if fname[i]='.'then dotpos:=i;if fname[i]='/'then slashpos:=i;i:=i+1;
...
while(fname[i]<>' ')and(i<=FILENAMESIZE-5)do begin changefilename[i]:=
fname[i];if fname[i]='.'then dotpos:=i;if fname[i]='/'then slashpos:=i;
i:=i+1;end;if(dotpos=-1)or(dotpos<slashpos)then begin dotpos:=i;
...
while(fname[i]<>' ')and(i<=FILENAMESIZE-5)do begin stylefilename[i]:=
fname[i];if fname[i]='.'then dotpos:=i;if fname[i]='/'then slashpos:=i;
i:=i+1;end;if(dotpos=-1)or(dotpos<slashpos)then begin dotpos:=i;
...
while(fname[i]<>' ')and(i<=FILENAMESIZE-5)do begin if fname[i]='c'then
begin foundchange:=false;
