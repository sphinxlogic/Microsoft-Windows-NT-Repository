...
UNIXfilename=packed array[1..FILENAMESIZE]of char;{:37}var{11:}
...
{:78}{82:}comment:packed array[1..24]of char;{:82}{87:}checksum:integer;
designsize:integer;hmag:integer;i:integer;{:87}procedure initialize;
...
for i:=1 to 8 do power[i]:=power[i-1]+power[i-1];{:79}{83:}
vstrcpy(comment+1,'GFtoPK 2.3 output from ');{:83};end;{:4}{30:}
...
slashpos:=-1;last:=1;
while(gfname[last]<>' ')and(last<=FILENAMESIZE-5)do begin if gfname[last
]='.'then dotpos:=last;if gfname[last]='/'then slashpos:=last;
last:=last+1;end;if slashpos=-1 then slashpos:=0;
...
if k<233 then begin putbyte(j,pkfile);pkloc:=pkloc+1 end;end;
...
pkloc:=pkloc+1 end{:84};end;{:51}{86:}begin initialize;convertgffile;
{85:}
for i:=0 to 255 do if status[i]=1 then if verbose then writeln(stdout,
'Character ',i:1,' missing raster information!'){:85};
