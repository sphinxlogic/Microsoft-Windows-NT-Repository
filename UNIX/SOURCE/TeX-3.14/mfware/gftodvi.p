...
nameoffile:packed array[1..FILENAMESIZE]of char;{:48}{49:}
...
{:220}{225:}verbose:boolean;{:225}{228:}overflowlabeloffset:integer;
...
if(strcmp(optiontable[optionindex].name,'overflow-label-offset')=0)then
...
for k:=namelength+1 to FILENAMESIZE do nameoffile[k]:=' ';end;{:92}{94:}
procedure startgf;label 30;
var argbuffer:packed array[1..FILENAMESIZE]of char;
argbufptr:1..FILENAMESIZE;
begin if optind=argc then begin write(stdout,'GF file name: ');inputln;
end else begin argv(optind,argbuffer);argbuffer[FILENAMESIZE]:=' ';
argbufptr:=1;linelength:=0;
while(argbufptr<FILENAMESIZE)and(argbuffer[argbufptr]=' ')do argbufptr:=
argbufptr+1;
while(argbufptr<FILENAMESIZE)and(linelength<terminallinelength)and(
argbuffer[argbufptr]<>' ')do begin buffer[linelength]:=xord[argbuffer[
