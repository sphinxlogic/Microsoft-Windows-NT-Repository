...
typedef char UNIXfilename [FILENAMESIZE + 1] ; 
ASCIIcode xord[256]  ; 
char xchr[256]  ; 
bytefile gffile  ; 
bytefile pkfile  ; 
...
{integer i  ; 
  ...
  while ( i++ < for_end ) ; } 
  vstrcpy ( comment + 1 , "GFtoPK 2.3 output from " ) ; 
} 
void opengffile ( ) 
{integer j  ; 
  verbose = false ; 
...
{integer dotpos, slashpos, last, gfindex, pkindex  ; 
  ...
  argv ( argc - 1 , pkname ) ; 
    ...
    slashpos = -1 ; 
    last = 1 ; 
    while ( ( gfname [ last ] != ' ' ) && ( last <= FILENAMESIZE - 5 ) ) {
...
{integer i, j, k  ; 
  ...
  {register integer for_end; k = 1 ; for_end = 23 ; if ( k <= for_end) do 
    if ( ( i > 0 ) || ( k <= 17 ) ) 
    {
