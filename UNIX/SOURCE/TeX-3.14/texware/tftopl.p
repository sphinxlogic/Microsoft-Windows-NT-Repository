{2:}program TFtoPL;label{3:}9999;{:3}const{4:}tfmsize=30000;
ligsize=5000;hashsize=5003;{:4}{106:}noptions=1;{:106}type{18:}
byte=0..255;index=0..tfmsize;{:18}var{6:}tfmfile:packed file of 0..255;
tfmname:packed array[1..FILENAMESIZE]of char;{:6}{8:}
lf,lh,bc,ec,nw,nh,nd,ni,nl,nk,ne,np:0..32767;{:8}{16:}plfile:text;
plname:array[1..FILENAMESIZE]of char;{:16}{19:}
tfm:array[-1000..tfmsize]of byte;{:19}{22:}
...
h,hh:0..hashsize;xligcycle,yligcycle:0..256;{:89}{103:}verbose:boolean;
{:103}procedure initialize;var{101:}
longoptions:array[0..noptions]of getoptstruct;getoptreturnval:integer;
optionindex:integer;{:101}
begin if(argc<2)or(argc>noptions+3)then begin writeln(
'Usage: tftopl [-verbose] <tfm file> [<property list file>].');uexit(1);
end;{104:}verbose:=false;{:104};{100:}begin{102:}
longoptions[0].name:='verbose';longoptions[0].hasarg:=0;
longoptions[0].flag:=addressofint(verbose);longoptions[0].val:=1;{:102}
{105:}longoptions[1].name:=0;longoptions[1].hasarg:=0;
longoptions[1].flag:=0;longoptions[1].val:=0;{:105};
...
end;until getoptreturnval=-1;end{:100};
if verbose then writeln('This is TFtoPL, C Version 3.1');{7:}
setpaths(TFMFILEPATHBIT);argv(optind,tfmname);
if testreadaccess(tfmname,TFMFILEPATH)then begin reset(tfmfile,tfmname);
end else begin printpascalstring(tfmname);
writeln(': TFM file not found.');uexit(1);end;{:7}{17:}
