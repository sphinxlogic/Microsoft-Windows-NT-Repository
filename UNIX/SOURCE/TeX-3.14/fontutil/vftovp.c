...
char vfname[FILENAMESIZE + 1]  ; 
file_ptr /* of  byte */ tfmfile  ; 
char tfmname[FILENAMESIZE + 1]  ; 
short lf, lh, bc, ec, nw, nh, nd, ni, nl, nk, ne, np  ; 
text vplfile  ; 
char vplname[FILENAMESIZE + 1]  ; 
byte 
...
{integer k  ; 
  ...
  if ( argc != 4 ) 
  {
    (void) fprintf(stdout, "%s\n", "Usage: <vfm file> <tfm file> <vpl file>." ) ; 
    uexit ( 1 ) ; 
  } 
  argv ( 1 , vfname ) ; 
...
{/* 9999 10 */ register boolean Result; integer vfptr  ; 
  ...
  {register integer for_end; k = 0 ; for_end = vfptr - 1 ; if ( k <= for_end) 
  do 
  ...
  do {
	...
	{register integer for_end; k = 1 ; for_end = FILENAMESIZE ; if ( k <= 
	for_end) do 
	  curname [ k ] = ' ' ; 
	while ( k++ < for_end ) ; } 
	r = 0 ; 
