...
void printexp ( halfword p , smallnumber verbosity ) 
...
void disperr ( halfword p , strnumber s ) 
{if ( interaction == 3 ) 
  ; 
  printnl ( 762 ) ; 
  printexp ( p , 1 ) ; 
...
halfword pplusfq ( halfword p , integer f , halfword q , smallnumber t , 
smallnumber tt ) 
...
halfword poverv ( halfword p , scaled v , smallnumber t0 , smallnumber t1 ) 
...
smallnumber undtype ( halfword p ) 
...
{halfword q  ; 
  ...
  while ( mem [ saveptr ] .hhfield .lhfield != 0 ) {
	...
	begindiagnostic () ; 
	printnl ( 515 ) ; 
	print ( intname [ q - ( 2369 ) ] ) ; 
	printchar ( 61 ) ; 
	printscaled ( mem [ saveptr + 1 ] .cint ) ; 
	printchar ( 125 ) ; 
	enddiagnostic ( false ) ; 
      } 
      internal [ q - ( 2369 ) ] = mem [ saveptr + 1 ] .cint ; 
    } 
    else {
