...
{This files defines symbols in addition to those in `common.defines',
 for use in the TeX and the Metafont change files.}

@define const MFBASEPATH;
@define const MFBASEPATHBIT;
@define const MFPOOLPATH;
@define const MFPOOLPATHBIT;
@define const TEXFORMATPATH;
@define const TEXFORMATPATHBIT;
@define const TEXPOOLPATH;
@define const TEXPOOLPATHBIT;
...
1,9998,9999;{:6}const{11:}memmax=60000;maxinternal=100;bufsize=500;
errorline=79;halferrorline=50;maxprintline=79;screenwidth=1664;
screendepth=1200;stacksize=30;maxstrings=2000;stringvacancies=8000;
poolsize=32000;movesize=5000;maxwiggle=300;gfbufsize=16384;
poolname='mf.pool';pathsize=300;bistacksize=785;headersize=100;
ligtablesize=5000;maxkerns=500;maxfontdimen=50;memtop=60000;{:11}
...
#include "memory.h";{:156}{186:}commandcode=1..85;{:186}{565:}
...
hash:array[1..2369]of twohalves;eqtb:array[1..2369]of twohalves;{:201}
...
{:680}{699:}bgloc,egloc:1..2369;{:699}{738:}condptr:halfword;
...
logopened:boolean;logname:strnumber;{:782}{785:}gfext:strnumber;{:785}
{791:}gffile:bytefile;outputfilename:strnumber;{:791}{796:}
...
{:199}{202:}hash[1].lhfield:=0;hash[1].rh:=0;eqtb[1].lhfield:=41;eqtb[1].rh:=0;
for k:=2 to 2369 do begin hash[k]:=hash[1];eqtb[k]:=eqtb[1];end;{:202}
...
begin if(s<0)or(s>=strptr)then s:=259;j:=strstart[s];
while j<strstart[s+1]do begin print(strpool[j]);incr(j);end;end;{:60}
...
end;{:773}{73:}procedure normalizeselector;forward;procedure getnext;
...
forward;ifdef('DEBUG')procedure debughelp;forward;endif('DEBUG'){43:}
...
printscaled(w.int div 4096);println;printint(w.hhfield.lhfield);printchar(61);
printint(w.hhfield.b0);printchar(58);printint(w.hhfield.b1);printchar(59);
printint(w.hhfield.rh);printchar(32);printint(w.qqqq.b0);printchar(58);
...
if mem[p].hhfield.b1=12 then if mem[p].hhfield.b0=16 then{220:}
...
else if mem[p].hhfield.b0<>4 then print(495)else begin printchar(34);
slowprint(mem[p+1].int);printchar(34);c:=4;
end else if(mem[p].hhfield.b1<>11)or(mem[p].hhfield.b0<1)or(mem[p].hhfield.b0>19)then
print(495)else begin gpointer:=p;printcapsule;c:=8;end{:219}
else begin r:=mem[p].hhfield.lhfield;if r>=2370 then{222:}
begin if r<2520 then begin print(497);r:=r-(2370);
end else if r<2670 then begin print(498);r:=r-(2520);
end else begin print(499);r:=r-(2670);end;printint(r);printchar(41);
...
others:printchar(32)end;print(r);end{:223};end;end{:218};class:=c;
p:=mem[p].hhfield.rh;end;if p<>0 then print(491);10:end;{:217}{665:}
...
println;showtokenlist(mem[memtop-2].hhfield.rh,0,errorline-10,0);end;end;
{:665}{:162}{163:}function getavail:halfword;var p:halfword;
begin p:=avail;
if p<>0 then avail:=mem[avail].hhfield.rh else if memend<memmax then begin
...
mem[p].hhfield.rh:=0;ifdef('STAT')incr(dynused);endif('STAT')getavail:=p;end;
...
repeat{169:}q:=p+mem[p].hhfield.lhfield;
while(mem[q].hhfield.rh=65535)do begin t:=mem[q+1].hhfield.rh;tt:=mem[q+1].hhfield.lhfield;
if q=rover then rover:=t;mem[t+1].hhfield.lhfield:=tt;mem[tt+1].hhfield.rh:=t;
q:=q+mem[q].hhfield.lhfield;end;r:=q-s;if r>toint(p+1)then{170:}
begin mem[p].hhfield.lhfield:=r-p;rover:=p;goto 40;end{:170};
if r=p then if mem[p+1].hhfield.rh<>p then{171:}begin rover:=mem[p+1].hhfield.rh;
t:=mem[p+1].hhfield.lhfield;mem[rover+1].hhfield.lhfield:=t;mem[t+1].hhfield.rh:=rover;goto 40;
end{:171};mem[p].hhfield.lhfield:=q-p{:169};p:=mem[p+1].hhfield.rh;until p=rover;
...
+1+(himemmin-lomemmax)div 2;if t>65535 then t:=65535;
p:=mem[rover+1].hhfield.lhfield;q:=lomemmax;mem[p+1].hhfield.rh:=q;
mem[rover+1].hhfield.lhfield:=q;mem[q+1].hhfield.rh:=rover;mem[q+1].hhfield.lhfield:=p;
mem[q].hhfield.rh:=65535;mem[q].hhfield.lhfield:=t-lomemmax;lomemmax:=t;
mem[lomemmax].hhfield.rh:=0;mem[lomemmax].hhfield.lhfield:=0;rover:=q;goto 20;end{:168}
;overflow(314,memmax+1);40:mem[r].hhfield.rh:=0;
ifdef('STAT')varused:=varused+s;endif('STAT')getnode:=r;10:end;{:167}
{172:}procedure freenode(p:halfword;s:halfword);var q:halfword;
begin mem[p].hhfield.lhfield:=s;mem[p].hhfield.rh:=65535;q:=mem[rover+1].hhfield.lhfield;
mem[p+1].hhfield.lhfield:=q;mem[p+1].hhfield.rh:=rover;mem[rover+1].hhfield.lhfield:=p;
mem[q+1].hhfield.rh:=p;ifdef('STAT')varused:=varused-s;endif('STAT')end;
{:172}{173:}ifdef('INIMF')procedure sortavail;var p,q,r:halfword;
oldrover:halfword;begin p:=getnode(1073741824);p:=mem[rover+1].hhfield.rh;
mem[rover+1].hhfield.rh:=65535;oldrover:=rover;while p<>oldrover do{174:}
if p<rover then begin q:=p;p:=mem[q+1].hhfield.rh;mem[q+1].hhfield.rh:=rover;
rover:=q;end else begin q:=rover;
while mem[q+1].hhfield.rh<p do q:=mem[q+1].hhfield.rh;r:=mem[p+1].hhfield.rh;
mem[p+1].hhfield.rh:=mem[q+1].hhfield.rh;mem[q+1].hhfield.rh:=p;p:=r;end{:174};
p:=rover;
while mem[p+1].hhfield.rh<>65535 do begin mem[mem[p+1].hhfield.rh+1].hhfield.lhfield:=p;
p:=mem[p+1].hhfield.rh;end;mem[p+1].hhfield.rh:=rover;mem[rover+1].hhfield.lhfield:=p;end;
...
repeat q:=r;r:=mem[r].hhfield.rh;ifdef('STAT')decr(dynused);
endif('STAT')if r<himemmin then goto 30;until r=memtop;
30:mem[q].hhfield.rh:=avail;avail:=p;end;end;
procedure flushnodelist(p:halfword);var q:halfword;
begin while p<>0 do begin q:=p;p:=mem[p].hhfield.rh;
if q<himemmin then freenode(q,2)else begin mem[q].hhfield.rh:=avail;avail:=q;
...
printint(q);goto 31;end;freearr[p]:=true;q:=p;p:=mem[q].hhfield.rh;end;
31:{:181};{182:}p:=rover;q:=0;clobbered:=false;
repeat if(p>=lomemmax)then clobbered:=true else if(mem[p+1].hhfield.rh>=
lomemmax)then clobbered:=true else if not((mem[p].hhfield.rh=65535))or(mem[p]
.hhfield.lhfield<2)or(p+mem[p].hhfield.lhfield>lomemmax)or(mem[mem[p+1].hhfield.rh+1].hhfield.lhfield<>p)
then clobbered:=true;if clobbered then begin printnl(316);printint(q);
goto 32;end;
for q:=p to p+mem[p].hhfield.lhfield-1 do begin if freearr[q]then begin printnl(
317);printint(q);goto 32;end;freearr[q]:=true;end;q:=p;
p:=mem[p+1].hhfield.rh;until p=rover;32:{:182};{183:}p:=0;
while p<=lomemmax do begin if(mem[p].hhfield.rh=65535)then begin printnl(318)
;printint(p);end;while(p<=lomemmax)and not freearr[p]do incr(p);
while(p<=lomemmax)and freearr[p]do incr(p);end{:183};{617:}q:=13;
p:=mem[q].hhfield.rh;
while p<>13 do begin if mem[p+1].hhfield.lhfield<>q then begin printnl(595);
printint(p);end;p:=mem[p+1].hhfield.rh;r:=himemmin;
repeat if mem[p].hhfield.lhfield>=r then begin printnl(596);printint(p);end;
r:=mem[p].hhfield.lhfield;q:=p;p:=mem[q].hhfield.rh;until r=0;end{:617};
...
begin for q:=0 to lomemmax do begin if mem[q].hhfield.rh=p then begin printnl
(320);printint(q);printchar(41);end;
if mem[q].hhfield.lhfield=p then begin printnl(321);printint(q);printchar(41);end;
end;
for q:=himemmin to memend do begin if mem[q].hhfield.rh=p then begin printnl(
320);printint(q);printchar(41);end;
if mem[q].hhfield.lhfield=p then begin printnl(321);printint(q);printchar(41);end;
end;{209:}
for q:=1 to 2369 do begin if eqtb[q].rh=p then begin printnl(457);
...
if hash[p].lhfield=0 then{207:}
begin if hash[p].rh>0 then begin repeat if(hashused=257)then overflow(
456,2100);decr(hashused);until hash[hashused].rh=0;hash[p].lhfield:=hashused;
p:=hashused;end;
...
ifdef('STAT')incr(stcount);endif('STAT')goto 40;end{:207};p:=hash[p].lhfield;
...
hash[cursym].rh:=s;end;eqtb[cursym].lhfield:=c;eqtb[cursym].rh:=o;end;
endif('INIMF'){:210}{215:}function newnumtok(v:scaled):halfword;
var p:halfword;begin p:=getnode(2);mem[p+1].int:=v;mem[p].hhfield.b0:=16;
mem[p].hhfield.b1:=12;newnumtok:=p;end;{:215}{216:}procedure tokenrecycle;
forward;procedure flushtokenlist(p:halfword);var q:halfword;
begin while p<>0 do begin q:=p;p:=mem[p].hhfield.rh;
if q>=himemmin then begin mem[q].hhfield.rh:=avail;avail:=q;
ifdef('STAT')decr(dynused);
endif('STAT')end else begin case mem[q].hhfield.b0 of 1,2,16:;
...
begin if mem[p].hhfield.lhfield=0 then flushtokenlist(p)else decr(mem[p].hhfield.lhfield);
...
print(656);
4:if m<=1 then if m=1 then print(659)else print(454)else if m=2370 then
print(657)else print(658);{:684}{689:}61:case m of 1:print(661);
2:printchar(64);3:print(662);others:print(660)end;{:689}{696:}
56:if m>=2370 then if m=2370 then print(673)else if m=2520 then print(
...
showtokenlist(mem[mem[m].hhfield.rh].hhfield.rh,0,1000,0);end;5:print(946);
40:print(intname[m]);{:1043}{1053:}
...
var r:halfword;begin p:=mem[p].hhfield.rh;
while mem[p].hhfield.lhfield>7 do begin r:=mem[p].hhfield.rh;mem[p].hhfield.rh:=0;
showtokenlist(p,0,l,0);mem[p].hhfield.rh:=r;p:=r;
if l>0 then l:=l-tally else goto 10;end;tally:=0;
case mem[p].hhfield.lhfield of 0:print(500);1,2,3:begin printchar(60);
printcmdmod(56,mem[p].hhfield.lhfield);print(501);end;4:print(502);5:print(503);
6:print(504);7:print(505);end;showtokenlist(mem[p].hhfield.rh,q,l-tally,0);
10:end;{:227}{232:}procedure initbignode(p:halfword);var q:halfword;
s:smallnumber;begin s:=bignodesize[mem[p].hhfield.b0];q:=getnode(s);
repeat s:=s-2;{586:}begin mem[q+s].hhfield.b0:=19;serialno:=serialno+64;
mem[q+s+1].int:=serialno;end{:586};mem[q+s].hhfield.b1:=(s)div 2+5;
mem[q+s].hhfield.rh:=0;until s=0;mem[q].hhfield.rh:=p;mem[p+1].int:=q;end;{:232}
{233:}function idtransform:halfword;var p,q,r:halfword;
begin p:=getnode(2);mem[p].hhfield.b0:=13;mem[p].hhfield.b1:=11;mem[p+1].int:=0;
initbignode(p);q:=mem[p+1].int;r:=q+12;repeat r:=r-2;mem[r].hhfield.b0:=16;
mem[r+1].int:=0;until r=q;mem[q+5].int:=65536;mem[q+11].int:=65536;
idtransform:=p;end;{:233}{234:}procedure newroot(x:halfword);
var p:halfword;begin p:=getnode(2);mem[p].hhfield.b0:=0;mem[p].hhfield.b1:=0;
mem[p].hhfield.rh:=x;eqtb[x].rh:=p;end;{:234}{235:}
procedure printvariablename(p:halfword);label 40,10;var q:halfword;
r:halfword;begin while mem[p].hhfield.b1>=5 do{237:}
begin case mem[p].hhfield.b1 of 5:printchar(120);6:printchar(121);
7:print(508);8:print(509);9:print(510);10:print(511);
11:begin print(512);printint(p-0);goto 10;end;end;print(513);
p:=mem[p-2*(mem[p].hhfield.b1-5)].hhfield.rh;end{:237};q:=0;
while mem[p].hhfield.b1>1 do{236:}
begin if mem[p].hhfield.b1=3 then begin r:=newnumtok(mem[p+2].int);
repeat p:=mem[p].hhfield.rh;until mem[p].hhfield.b1=4;
end else if mem[p].hhfield.b1=2 then begin p:=mem[p].hhfield.rh;goto 40;
end else begin if mem[p].hhfield.b1<>4 then confusion(507);r:=getavail;
mem[r].hhfield.lhfield:=mem[p+2].hhfield.lhfield;end;mem[r].hhfield.rh:=q;q:=r;
40:p:=mem[p+2].hhfield.rh;end{:236};r:=getavail;mem[r].hhfield.lhfield:=mem[p].hhfield.rh;
mem[r].hhfield.rh:=q;if mem[p].hhfield.b1=1 then print(506);
...
begin if internal[3]>0 then interesting:=true else begin t:=mem[p].hhfield.b1
;if t>=5 then if t<>11 then t:=mem[mem[p-2*(t-5)].hhfield.rh].hhfield.b1;
interesting:=(t<>11);end;end;{:238}{239:}
function newstructure(p:halfword):halfword;var q,r:halfword;
begin case mem[p].hhfield.b1 of 0:begin q:=mem[p].hhfield.rh;r:=getnode(2);
eqtb[q].rh:=r;end;3:{240:}begin q:=p;repeat q:=mem[q].hhfield.rh;
until mem[q].hhfield.b1=4;q:=mem[q+2].hhfield.rh;r:=q+1;repeat q:=r;
r:=mem[r].hhfield.rh;until r=p;r:=getnode(3);mem[q].hhfield.rh:=r;
mem[r+2].int:=mem[p+2].int;end{:240};4:{241:}begin q:=mem[p+2].hhfield.rh;
r:=mem[q+1].hhfield.lhfield;repeat q:=r;r:=mem[r].hhfield.rh;until r=p;r:=getnode(3);
mem[q].hhfield.rh:=r;mem[r+2]:=mem[p+2];
if mem[p+2].hhfield.lhfield=0 then begin q:=mem[p+2].hhfield.rh+1;
while mem[q].hhfield.rh<>p do q:=mem[q].hhfield.rh;mem[q].hhfield.rh:=r;end;end{:241};
others:confusion(514)end;mem[r].hhfield.rh:=mem[p].hhfield.rh;mem[r].hhfield.b0:=21;
mem[r].hhfield.b1:=mem[p].hhfield.b1;mem[r+1].hhfield.lhfield:=p;mem[p].hhfield.b1:=2;
q:=getnode(3);mem[p].hhfield.rh:=q;mem[r+1].hhfield.rh:=q;mem[q+2].hhfield.rh:=r;
mem[q].hhfield.b0:=0;mem[q].hhfield.b1:=4;mem[q].hhfield.rh:=17;mem[q+2].hhfield.lhfield:=0;
...
begin p:=mem[t].hhfield.lhfield;t:=mem[t].hhfield.rh;
if eqtb[p].lhfield mod 86<>41 then begin findvariable:=0;goto 10;end;
if eqtb[p].rh=0 then newroot(p);p:=eqtb[p].rh;pp:=p;
while t<>0 do begin{243:}
if mem[pp].hhfield.b0<>21 then begin if mem[pp].hhfield.b0>21 then begin
findvariable:=0;goto 10;end;ss:=newstructure(pp);if p=pp then p:=ss;
pp:=ss;end;if mem[p].hhfield.b0<>21 then p:=newstructure(p){:243};
if t<himemmin then{244:}begin n:=mem[t+1].int;
pp:=mem[mem[pp+1].hhfield.lhfield].hhfield.rh;q:=mem[mem[p+1].hhfield.lhfield].hhfield.rh;
saveword:=mem[q+2];mem[q+2].int:=2147483647;s:=p+1;repeat r:=s;
s:=mem[s].hhfield.rh;until n<=mem[s+2].int;
if n=mem[s+2].int then p:=s else begin p:=getnode(3);mem[r].hhfield.rh:=p;
mem[p].hhfield.rh:=s;mem[p+2].int:=n;mem[p].hhfield.b1:=3;mem[p].hhfield.b0:=0;end;
mem[q+2]:=saveword;end{:244}else{245:}begin n:=mem[t].hhfield.lhfield;
ss:=mem[pp+1].hhfield.lhfield;repeat rr:=ss;ss:=mem[ss].hhfield.rh;
until n<=mem[ss+2].hhfield.lhfield;if n<mem[ss+2].hhfield.lhfield then begin qq:=getnode(3);
mem[rr].hhfield.rh:=qq;mem[qq].hhfield.rh:=ss;mem[qq+2].hhfield.lhfield:=n;mem[qq].hhfield.b1:=4;
mem[qq].hhfield.b0:=0;mem[qq+2].hhfield.rh:=pp;ss:=qq;end;
if p=pp then begin p:=ss;pp:=ss;end else begin pp:=ss;s:=mem[p+1].hhfield.lhfield;
repeat r:=s;s:=mem[s].hhfield.rh;until n<=mem[s+2].hhfield.lhfield;
if n=mem[s+2].hhfield.lhfield then p:=s else begin q:=getnode(3);mem[r].hhfield.rh:=q;
mem[q].hhfield.rh:=s;mem[q+2].hhfield.lhfield:=n;mem[q].hhfield.b1:=4;mem[q].hhfield.b0:=0;
mem[q+2].hhfield.rh:=p;p:=q;end;end;end{:245};t:=mem[t].hhfield.rh;end;
if mem[pp].hhfield.b0>=21 then if mem[pp].hhfield.b0=21 then pp:=mem[pp+1].hhfield.lhfield
else begin findvariable:=0;goto 10;end;
if mem[p].hhfield.b0=21 then p:=mem[p+1].hhfield.lhfield;
if mem[p].hhfield.b0=0 then begin if mem[pp].hhfield.b0=0 then begin mem[pp].hhfield.b0
:=15;mem[pp+1].int:=0;end;mem[p].hhfield.b0:=mem[pp].hhfield.b0;mem[p+1].int:=0;
...
repeat q:=mem[p].hhfield.rh;if(p=0)or(q=0)then begin printnl(259);goto 30;
end;{258:}printtwo(mem[p+1].int,mem[p+2].int);
case mem[p].hhfield.b1 of 0:begin if mem[p].hhfield.b0=4 then print(517);
if(mem[q].hhfield.b0<>0)or(q<>h)then q:=0;goto 31;end;1:{261:}
begin print(523);printtwo(mem[p+5].int,mem[p+6].int);print(522);
if mem[q].hhfield.b0<>1 then print(524)else printtwo(mem[q+3].int,mem[q+4]
.int);goto 31;end{:261};4:{262:}
if(mem[p].hhfield.b0<>1)and(mem[p].hhfield.b0<>4)then print(517){:262};3,2:{263:}
begin if mem[p].hhfield.b0=4 then print(524);
if mem[p].hhfield.b1=3 then begin print(520);printscaled(mem[p+5].int);
...
if mem[q].hhfield.b0<=1 then print(518)else if(mem[p+6].int<>65536)or(mem[q+4
...
end{:260};31:{:258};p:=q;if(p<>h)or(mem[h].hhfield.b0<>0)then{259:}
begin printnl(519);if mem[p].hhfield.b0=2 then begin nsincos(mem[p+3].int);
printchar(123);printscaled(ncos);printchar(44);printscaled(nsin);
printchar(125);end else if mem[p].hhfield.b0=3 then begin print(520);
printscaled(mem[p+3].int);printchar(125);end;end{:259};until p=h;
if mem[h].hhfield.b0<>0 then print(384);30:enddiagnostic(true);end;{:257}
{332:}{333:}procedure printweight(q:halfword;xoff:integer);
var w,m:integer;d:integer;begin d:=mem[q].hhfield.lhfield;w:=d mod 8;
m:=(d div 8)-mem[curedges+3].hhfield.lhfield;
...
p:=mem[curedges].hhfield.lhfield;n:=mem[curedges+1].hhfield.rh-4096;
while p<>curedges do begin q:=mem[p+1].hhfield.lhfield;r:=mem[p+1].hhfield.rh;
if(q>1)or(r<>memtop)then begin printnl(532);printint(n+yoff);
printchar(58);while q>1 do begin printweight(q,xoff);q:=mem[q].hhfield.rh;
end;print(533);while r<>memtop do begin printweight(r,xoff);
r:=mem[r].hhfield.rh;end;end;p:=mem[p].hhfield.lhfield;decr(n);end;enddiagnostic(true);
...
n:=mem[h].hhfield.lhfield;w:=mem[h].hhfield.rh;if not odd(k)then w:=mem[w].hhfield.lhfield;
for m:=1 to n+1 do begin if odd(k)then ww:=mem[w].hhfield.rh else ww:=mem[w]
.hhfield.lhfield;
...
;w:=ww;end;end;if nothingprinted then begin w:=mem[p+1].hhfield.rh;
...
while true do begin v:=abs(mem[p+1].int);q:=mem[p].hhfield.lhfield;
...
if mem[q].hhfield.b0<>19 then confusion(586);printvariablename(q);
v:=mem[q+1].int mod 64;while v>0 do begin print(587);v:=v-2;end;
p:=mem[p].hhfield.rh;end;10:end;{:589}{801:}{805:}
procedure printdp(t:smallnumber;p:halfword;verbosity:smallnumber);
var q:halfword;begin q:=mem[p].hhfield.rh;
if(mem[q].hhfield.lhfield=0)or(verbosity>0)then printdependency(p,t)else print(761
);end;{:805}{799:}function stashcurexp:halfword;var p:halfword;
begin case curtype of 3,5,7,12,10,13,14,17,18,19:p:=curexp;
others:begin p:=getnode(2);mem[p].hhfield.b1:=11;mem[p].hhfield.b0:=curtype;
mem[p+1].int:=curexp;end end;curtype:=1;mem[p].hhfield.rh:=1;stashcurexp:=p;
end;{:799}{800:}procedure unstashcurexp(p:halfword);
begin curtype:=mem[p].hhfield.b0;
...
restorecurexp:=true;end;t:=mem[p].hhfield.b0;
if t<17 then v:=mem[p+1].int else if t<19 then v:=mem[p+1].hhfield.rh;{802:}
...
while(mem[v].hhfield.b1=11)and(v<>p)do v:=mem[v+1].int;printvariablename(v);
...
q:=v+bignodesize[t];
repeat if mem[v].hhfield.b0=16 then printscaled(mem[v+1].int)else if mem[v]
.hhfield.b0=19 then printvariablename(v)else printdp(mem[v].hhfield.b0,mem[v+1].hhfield
...
pp:=mem[p].hhfield.lhfield;qq:=mem[q].hhfield.lhfield;
...
then if watchcoefs then begin mem[qq].hhfield.b0:=0;fixneeded:=true;end;
mem[r].hhfield.rh:=s;r:=s;end;pp:=mem[p].hhfield.lhfield;q:=mem[q].hhfield.rh;
qq:=mem[q].hhfield.lhfield;end{:595}else if mem[pp+1].int<mem[qq+1].int then{596:}
...
mem[s].hhfield.lhfield:=qq;mem[s+1].int:=v;
if abs(v)>=626349397 then if watchcoefs then begin mem[qq].hhfield.b0:=0;
fixneeded:=true;end;mem[r].hhfield.rh:=s;r:=s;end;q:=mem[q].hhfield.rh;
qq:=mem[q].hhfield.lhfield;end{:596}else begin mem[r].hhfield.rh:=p;r:=p;
p:=mem[p].hhfield.rh;pp:=mem[p].hhfield.lhfield;end;
...
f));mem[r].hhfield.rh:=p;depfinal:=p;pplusfq:=mem[memtop-1].hhfield.rh;end;{:594}
...
while mem[p].hhfield.lhfield<>0 do begin if scalingdown then if abs(v)<524288 then
w:=makescaled(mem[p+1].int,v*4096)else w:=makescaled(roundfraction(mem[p
+1].int),v)else w:=makescaled(mem[p+1].int,v);
if abs(w)<=threshold then begin s:=mem[p].hhfield.rh;freenode(p,2);p:=s;
end else begin if abs(w)>=626349397 then begin fixneeded:=true;
mem[mem[p].hhfield.lhfield].hhfield.b0:=0;end;mem[r].hhfield.rh:=p;r:=p;mem[p+1].int:=w;
p:=mem[p].hhfield.rh;end;end;mem[r].hhfield.rh:=p;
mem[p+1].int:=makescaled(mem[p+1].int,v);poverv:=mem[memtop-1].hhfield.rh;
...
begin mem[mem[q].hhfield.rh+1].hhfield.lhfield:=mem[p+1].hhfield.lhfield;
mem[mem[p+1].hhfield.lhfield].hhfield.rh:=mem[q].hhfield.rh;t:=mem[p].hhfield.b0;
mem[p].hhfield.b0:=16;mem[p+1].int:=mem[q+1].int;freenode(q,2);
...
begin r:=mem[13].hhfield.rh;s:=0;while r<>13 do begin t:=r;{605:}r:=t+1;
while true do begin q:=mem[r].hhfield.rh;x:=mem[q].hhfield.lhfield;if x=0 then goto 30;
if mem[x].hhfield.b0<=1 then begin if mem[x].hhfield.b0<1 then begin p:=getavail;
mem[p].hhfield.rh:=s;s:=p;mem[s].hhfield.lhfield:=x;mem[x].hhfield.b0:=1;end;
mem[q+1].int:=mem[q+1].int div 4;
if mem[q+1].int=0 then begin mem[r].hhfield.rh:=mem[q].hhfield.rh;freenode(q,2);
q:=r;end;end;r:=q;end;30:{:605};r:=mem[q].hhfield.rh;
if q=mem[t+1].hhfield.rh then makeknown(t,q);end;
while s<>0 do begin p:=mem[s].hhfield.rh;x:=mem[s].hhfield.lhfield;
begin mem[s].hhfield.rh:=avail;avail:=s;ifdef('STAT')decr(dynused);
endif('STAT')end;s:=p;mem[x].hhfield.b0:=19;mem[x+1].int:=mem[x+1].int+2;end;
fixneeded:=false;end;{:604}{268:}procedure tossknotlist(p:halfword);
var q:halfword;r:halfword;begin q:=p;repeat r:=mem[q].hhfield.rh;
freenode(q,7);q:=r;until q=p;end;{:268}{385:}
procedure tossedges(h:halfword);var p,q:halfword;begin q:=mem[h].hhfield.rh;
while q<>h do begin flushlist(mem[q+1].hhfield.rh);
if mem[q+1].hhfield.lhfield>1 then flushlist(mem[q+1].hhfield.lhfield);p:=q;q:=mem[q].hhfield.rh;
freenode(p,2);end;freenode(h,6);end;{:385}{487:}
procedure tosspen(p:halfword);var k:1..8;w,ww:halfword;
begin if p<>3 then begin for k:=1 to 8 do begin w:=mem[p+k].hhfield.rh;
repeat ww:=mem[w].hhfield.rh;freenode(w,3);w:=ww;until w=mem[p+k].hhfield.rh;end;
...
vv:integer;q,r,s,pp:halfword;begin t:=mem[p].hhfield.b0;
...
flushstring(v);end;
6:if mem[v].hhfield.lhfield=0 then tosspen(v)else decr(mem[v].hhfield.lhfield);
...
begin q:=mem[p+1].hhfield.rh;while mem[q].hhfield.lhfield<>0 do q:=mem[q].hhfield.rh;
mem[mem[p+1].hhfield.lhfield].hhfield.rh:=mem[q].hhfield.rh;
mem[mem[q].hhfield.rh+1].hhfield.lhfield:=mem[p+1].hhfield.lhfield;mem[q].hhfield.rh:=0;
flushnodelist(mem[p+1].hhfield.rh);end{:811};19:{812:}begin maxc[17]:=0;
maxc[18]:=0;maxlink[17]:=0;maxlink[18]:=0;q:=mem[13].hhfield.rh;
while q<>13 do begin s:=q+1;while true do begin r:=mem[s].hhfield.rh;
if mem[r].hhfield.lhfield=0 then goto 30;
if mem[r].hhfield.lhfield<>p then s:=r else begin t:=mem[q].hhfield.b0;
mem[s].hhfield.rh:=mem[r].hhfield.rh;mem[r].hhfield.lhfield:=q;
if abs(mem[r+1].int)>maxc[t]then{814:}
begin if maxc[t]>0 then begin mem[maxptr[t]].hhfield.rh:=maxlink[t];
maxlink[t]:=maxptr[t];end;maxc[t]:=abs(mem[r+1].int);maxptr[t]:=r;
end{:814}else begin mem[r].hhfield.rh:=maxlink[t];maxlink[t]:=r;end;end;end;
30:q:=mem[r].hhfield.rh;end;if(maxc[17]>0)or(maxc[18]>0)then{815:}
begin if(maxc[17]>=268435456)or(maxc[17]div 4096>=maxc[18])then t:=17
else t:=18;{816:}s:=maxptr[t];pp:=mem[s].hhfield.lhfield;v:=mem[s+1].int;
if t=17 then mem[s+1].int:=-268435456 else mem[s+1].int:=-65536;
r:=mem[pp+1].hhfield.rh;mem[s].hhfield.rh:=r;
while mem[r].hhfield.lhfield<>0 do r:=mem[r].hhfield.rh;q:=mem[r].hhfield.rh;
mem[r].hhfield.rh:=0;mem[q+1].hhfield.lhfield:=mem[pp+1].hhfield.lhfield;
mem[mem[pp+1].hhfield.lhfield].hhfield.rh:=q;begin mem[pp].hhfield.b0:=19;
...
if maxc[t]>0 then begin mem[maxptr[t]].hhfield.rh:=maxlink[t];
maxlink[t]:=maxptr[t];end;if t<>17 then{818:}
for t:=17 to 18 do begin r:=maxlink[t];
while r<>0 do begin q:=mem[r].hhfield.lhfield;
mem[q+1].hhfield.rh:=pplusfq(mem[q+1].hhfield.rh,makefraction(mem[r+1].int,-v),s,t
,17);if mem[q+1].hhfield.rh=depfinal then makeknown(q,depfinal);q:=r;
r:=mem[r].hhfield.rh;freenode(q,2);end;end{:818}else{819:}
for t:=17 to 18 do begin r:=maxlink[t];
while r<>0 do begin q:=mem[r].hhfield.lhfield;
if t=17 then begin if curexp=q then if curtype=17 then curtype:=18;
mem[q+1].hhfield.rh:=poverv(mem[q+1].hhfield.rh,65536,17,18);mem[q].hhfield.b0:=18;
mem[r+1].int:=roundfraction(mem[r+1].int);end;
mem[q+1].hhfield.rh:=pplusfq(mem[q+1].hhfield.rh,makescaled(mem[r+1].int,-v),s,18,
18);if mem[q+1].hhfield.rh=depfinal then makeknown(q,depfinal);q:=r;
r:=mem[r].hhfield.rh;freenode(q,2);end;end{:819};flushnodelist(s);
if fixneeded then fixdependencies;begin if aritherror then cleararith;
end;end{:815};end{:812};20,21:confusion(763);
22,23:deletemacref(mem[p+1].int);end;mem[p].hhfield.b0:=0;end;{:809}{808:}
...
6:if mem[curexp].hhfield.lhfield=0 then tosspen(curexp)else decr(mem[curexp].hhfield.lhfield
);
...
var q,r:halfword;
begin if mem[p].hhfield.b0<>21 then recyclevalue(p)else begin q:=mem[p+1].hhfield
.rh;while mem[q].hhfield.b1=3 do begin flushbelowvariable(q);r:=q;
q:=mem[q].hhfield.rh;freenode(r,3);end;r:=mem[p+1].hhfield.lhfield;q:=mem[r].hhfield.rh;
recyclevalue(r);if mem[p].hhfield.b1<=1 then freenode(r,2)else freenode(r,3);
repeat flushbelowvariable(q);r:=q;q:=mem[q].hhfield.rh;freenode(r,3);
until q=17;mem[p].hhfield.b0:=0;end;end;{:247}
procedure flushvariable(p,t:halfword;discardsuffixes:boolean);label 10;
var q,r:halfword;n:halfword;
begin while t<>0 do begin if mem[p].hhfield.b0<>21 then goto 10;
n:=mem[t].hhfield.lhfield;t:=mem[t].hhfield.rh;if n=0 then begin r:=p+1;
q:=mem[r].hhfield.rh;
while mem[q].hhfield.b1=3 do begin flushvariable(q,t,discardsuffixes);
if t=0 then if mem[q].hhfield.b0=21 then r:=q else begin mem[r].hhfield.rh:=mem[q]
.hhfield.rh;freenode(q,3);end else r:=q;q:=mem[r].hhfield.rh;end;end;
p:=mem[p+1].hhfield.lhfield;repeat r:=p;p:=mem[p].hhfield.rh;until mem[p+2].hhfield.lhfield>=n;
if mem[p+2].hhfield.lhfield<>n then goto 10;end;
if discardsuffixes then flushbelowvariable(p)else begin if mem[p].hhfield.b0=
21 then p:=mem[p+1].hhfield.lhfield;recyclevalue(p);end;10:end;{:246}{248:}
function undtype(p:halfword):smallnumber;
begin case mem[p].hhfield.b0 of 0,1:undtype:=0;2,3:undtype:=3;4,5:undtype:=5;
6,7,8:undtype:=7;9,10:undtype:=10;11,12:undtype:=12;
13,14,15:undtype:=mem[p].hhfield.b0;16,17,18,19:undtype:=15;end;end;{:248}
{249:}procedure clearsymbol(p:halfword;saving:boolean);var q:halfword;
begin q:=eqtb[p].rh;
case eqtb[p].lhfield mod 86 of 10,53,44,49:if not saving then deletemacref(q)
;41:if q<>0 then if saving then mem[q].hhfield.b1:=1 else begin
flushbelowvariable(q);freenode(q,2);end;others:end;eqtb[p]:=eqtb[2369];
end;{:249}{252:}procedure savevariable(q:halfword);var p:halfword;
begin if saveptr<>0 then begin p:=getnode(2);mem[p].hhfield.lhfield:=q;
mem[p].hhfield.rh:=saveptr;mem[p+1].hhfield:=eqtb[q];saveptr:=p;end;
clearsymbol(q,(saveptr<>0));end;{:252}{253:}
procedure saveinternal(q:halfword);var p:halfword;
begin if saveptr<>0 then begin p:=getnode(2);mem[p].hhfield.lhfield:=2369+q;
mem[p].hhfield.rh:=saveptr;mem[p+1].int:=internal[q];saveptr:=p;end;end;
{:253}{254:}procedure unsave;var q:halfword;p:halfword;
begin while mem[saveptr].hhfield.lhfield<>0 do begin q:=mem[saveptr].hhfield.lhfield;
if q>2369 then begin if internal[8]>0 then begin begindiagnostic;
printnl(515);print(intname[q-(2369)]);printchar(61);
...
clearsymbol(q,false);eqtb[q]:=mem[saveptr+1].hhfield;
if eqtb[q].lhfield mod 86=41 then begin p:=eqtb[q].rh;
if p<>0 then mem[p].hhfield.b1:=0;end;end;p:=mem[saveptr].hhfield.rh;
freenode(saveptr,2);saveptr:=p;end;p:=mem[saveptr].hhfield.rh;
begin mem[saveptr].hhfield.rh:=avail;avail:=saveptr;
...
while true do begin mem[qq].hhfield.b0:=mem[pp].hhfield.b0;
mem[qq].hhfield.b1:=mem[pp].hhfield.b1;mem[qq+1].int:=mem[pp+1].int;
...
if mem[pp].hhfield.rh=p then begin mem[qq].hhfield.rh:=q;copypath:=q;goto 10;end;
mem[qq].hhfield.rh:=getnode(7);qq:=mem[qq].hhfield.rh;pp:=mem[pp].hhfield.rh;end;
10:end;{:265}{266:}function htapypoc(p:halfword):halfword;label 10;
var q,pp,qq,rr:halfword;begin q:=getnode(7);qq:=q;pp:=p;
while true do begin mem[qq].hhfield.b1:=mem[pp].hhfield.b0;
mem[qq].hhfield.b0:=mem[pp].hhfield.b1;mem[qq+1].int:=mem[pp+1].int;
...
if mem[pp].hhfield.rh=p then begin mem[q].hhfield.rh:=qq;pathtail:=pp;htapypoc:=q;
goto 10;end;rr:=getnode(7);mem[rr].hhfield.rh:=qq;qq:=rr;pp:=mem[pp].hhfield.rh;
...
takefraction(deltax[k],sf),ss);mem[p].hhfield.b1:=1;mem[q].hhfield.b0:=1;end;
...
while true do begin t:=mem[s].hhfield.rh;if k=0 then{285:}
case mem[s].hhfield.b1 of 2:if mem[t].hhfield.b0=2 then{301:}
...
3:if mem[t].hhfield.b0=3 then{302:}begin mem[p].hhfield.b1:=1;mem[q].hhfield.b0:=1;
...
end{:285}else case mem[s].hhfield.b0 of 5,4:{287:}begin{288:}
...
if mem[r].hhfield.b1=3 then begin ww[k]:=0;
...
end{:290};if mem[s].hhfield.b0=5 then{291:}begin aa:=0;bb:=268435456;
...
s:=p;k:=0;repeat t:=mem[s].hhfield.rh;nsincos(theta[k]);st:=nsin;ct:=ncos;
...
repeat q:=mem[p].hhfield.rh;
if mem[p+1].int=mem[q+1].int then if mem[p+2].int=mem[q+2].int then if
mem[p].hhfield.b1>1 then begin mem[p].hhfield.b1:=1;
if mem[p].hhfield.b0=4 then begin mem[p].hhfield.b0:=3;mem[p+3].int:=65536;end;
mem[q].hhfield.b0:=1;if mem[q].hhfield.b1=4 then begin mem[q].hhfield.b1:=3;
...
while true do begin if mem[h].hhfield.b0<>4 then goto 30;
if mem[h].hhfield.b1<>4 then goto 30;h:=mem[h].hhfield.rh;
if h=knots then begin mem[h].hhfield.b0:=5;goto 30;end;end;30:{:272};p:=h;
repeat{273:}q:=mem[p].hhfield.rh;
if mem[p].hhfield.b1>=2 then begin while(mem[q].hhfield.b0=4)and(mem[q].hhfield.b1=4)do
q:=mem[q].hhfield.rh;{278:}{281:}k:=0;s:=p;n:=pathsize;
repeat t:=mem[s].hhfield.rh;deltax[k]:=mem[t+1].int-mem[s+1].int;
...
until(k>=n)and(mem[s].hhfield.b0<>5);
if k=n then psi[n]:=0 else psi[k]:=psi[1]{:281};{282:}
if mem[q].hhfield.b0=4 then begin delx:=mem[q+5].int-mem[q+1].int;
dely:=mem[q+6].int-mem[q+2].int;
if(delx=0)and(dely=0)then begin mem[q].hhfield.b0:=3;mem[q+3].int:=65536;
end else begin mem[q].hhfield.b0:=2;mem[q+3].int:=narg(delx,dely);end;end;
if(mem[p].hhfield.b1=4)and(mem[p].hhfield.b0=1)then begin delx:=mem[p+1].int-mem[p
+3].int;dely:=mem[p+2].int-mem[p+4].int;
if(delx=0)and(dely=0)then begin mem[p].hhfield.b1:=3;mem[p+5].int:=65536;
end else begin mem[p].hhfield.b1:=2;mem[p+5].int:=narg(delx,dely);end;
...
{:321}{326:}procedure initedges(h:halfword);begin mem[h].hhfield.lhfield:=h;
mem[h].hhfield.rh:=h;mem[h+1].hhfield.lhfield:=8191;mem[h+1].hhfield.rh:=1;
mem[h+2].hhfield.lhfield:=8191;mem[h+2].hhfield.rh:=1;mem[h+3].hhfield.lhfield:=4096;
mem[h+3].hhfield.rh:=0;mem[h+4].int:=0;mem[h+5].hhfield.rh:=h;mem[h+5].hhfield.lhfield:=0;
end;{:326}{328:}procedure fixoffset;var p,q:halfword;delta:integer;
begin delta:=8*(mem[curedges+3].hhfield.lhfield-4096);mem[curedges+3].hhfield.lhfield:=4096;
q:=mem[curedges].hhfield.rh;while q<>curedges do begin p:=mem[q+1].hhfield.rh;
while p<>memtop do begin mem[p].hhfield.lhfield:=mem[p].hhfield.lhfield-delta;
p:=mem[p].hhfield.rh;end;p:=mem[q+1].hhfield.lhfield;
while p>1 do begin mem[p].hhfield.lhfield:=mem[p].hhfield.lhfield-delta;p:=mem[p].hhfield.rh;end;
q:=mem[q].hhfield.rh;end;end;{:328}{329:}
...
if ml<mem[curedges+2].hhfield.lhfield then mem[curedges+2].hhfield.lhfield:=ml;
if mr>mem[curedges+2].hhfield.rh then mem[curedges+2].hhfield.rh:=mr;
temp:=mem[curedges+3].hhfield.lhfield-4096;
if not(abs(mem[curedges+2].hhfield.lhfield+temp-4096)<4096)or not(abs(mem[curedges
+2].hhfield.rh+temp-4096)<4096)then fixoffset;
if mem[curedges].hhfield.rh=curedges then begin mem[curedges+1].hhfield.lhfield:=nr+1;
mem[curedges+1].hhfield.rh:=nr;end;if nl<mem[curedges+1].hhfield.lhfield then{330:}
begin delta:=mem[curedges+1].hhfield.lhfield-nl;mem[curedges+1].hhfield.lhfield:=nl;
p:=mem[curedges].hhfield.rh;repeat q:=getnode(2);mem[q+1].hhfield.rh:=memtop;
mem[q+1].hhfield.lhfield:=1;mem[p].hhfield.lhfield:=q;mem[q].hhfield.rh:=p;p:=q;decr(delta);
until delta=0;mem[p].hhfield.lhfield:=curedges;mem[curedges].hhfield.rh:=p;
if mem[curedges+5].hhfield.rh=curedges then mem[curedges+5].hhfield.lhfield:=nl-1;
end{:330};if nr>mem[curedges+1].hhfield.rh then{331:}
begin delta:=nr-mem[curedges+1].hhfield.rh;mem[curedges+1].hhfield.rh:=nr;
p:=mem[curedges].hhfield.lhfield;repeat q:=getnode(2);mem[q+1].hhfield.rh:=memtop;
mem[q+1].hhfield.lhfield:=1;mem[p].hhfield.rh:=q;mem[q].hhfield.lhfield:=p;p:=q;decr(delta);
until delta=0;mem[p].hhfield.rh:=curedges;mem[curedges].hhfield.lhfield:=p;
if mem[curedges+5].hhfield.rh=curedges then mem[curedges+5].hhfield.lhfield:=nr+1;
...
mem[hh+4]:=mem[h+4];mem[hh+5].hhfield.lhfield:=mem[hh+1].hhfield.rh+1;
mem[hh+5].hhfield.rh:=hh;p:=mem[h].hhfield.rh;qq:=hh;
while p<>h do begin pp:=getnode(2);mem[qq].hhfield.rh:=pp;mem[pp].hhfield.lhfield:=qq;
{335:}r:=mem[p+1].hhfield.rh;rr:=pp+1;while r<>memtop do begin ss:=getavail;
mem[rr].hhfield.rh:=ss;rr:=ss;mem[rr].hhfield.lhfield:=mem[r].hhfield.lhfield;r:=mem[r].hhfield.rh;
end;mem[rr].hhfield.rh:=memtop;r:=mem[p+1].hhfield.lhfield;rr:=memtop-1;
while r>1 do begin ss:=getavail;mem[rr].hhfield.rh:=ss;rr:=ss;
mem[rr].hhfield.lhfield:=mem[r].hhfield.lhfield;r:=mem[r].hhfield.rh;end;mem[rr].hhfield.rh:=r;
mem[pp+1].hhfield.lhfield:=mem[memtop-1].hhfield.rh{:335};p:=mem[p].hhfield.rh;qq:=pp;end;
mem[qq].hhfield.rh:=hh;mem[hh].hhfield.lhfield:=qq;copyedges:=hh;end;{:334}{336:}
procedure yreflectedges;var p,q,r:halfword;
begin p:=mem[curedges+1].hhfield.lhfield;
mem[curedges+1].hhfield.lhfield:=8191-mem[curedges+1].hhfield.rh;
mem[curedges+1].hhfield.rh:=8191-p;
mem[curedges+5].hhfield.lhfield:=8191-mem[curedges+5].hhfield.lhfield;
p:=mem[curedges].hhfield.rh;q:=curedges;repeat r:=mem[p].hhfield.rh;
mem[p].hhfield.rh:=q;mem[q].hhfield.lhfield:=p;q:=p;p:=r;until q=curedges;
mem[curedges+4].int:=0;end;{:336}{337:}procedure xreflectedges;
var p,q,r,s:halfword;m:integer;begin p:=mem[curedges+2].hhfield.lhfield;
mem[curedges+2].hhfield.lhfield:=8192-mem[curedges+2].hhfield.rh;
mem[curedges+2].hhfield.rh:=8192-p;m:=(4096+mem[curedges+3].hhfield.lhfield)*8+8;
mem[curedges+3].hhfield.lhfield:=4096;p:=mem[curedges].hhfield.rh;repeat{339:}
q:=mem[p+1].hhfield.rh;r:=memtop;while q<>memtop do begin s:=mem[q].hhfield.rh;
mem[q].hhfield.rh:=r;r:=q;mem[r].hhfield.lhfield:=m-mem[q].hhfield.lhfield;q:=s;end;
mem[p+1].hhfield.rh:=r{:339};{338:}q:=mem[p+1].hhfield.lhfield;
while q>1 do begin mem[q].hhfield.lhfield:=m-mem[q].hhfield.lhfield;q:=mem[q].hhfield.rh;
end{:338};p:=mem[p].hhfield.rh;until p=curedges;mem[curedges+4].int:=0;end;
{:337}{340:}procedure yscaleedges(s:integer);
var p,q,pp,r,rr,ss:halfword;t:integer;
begin if(s*(mem[curedges+1].hhfield.rh-4095)>=4096)or(s*(mem[curedges+1].hhfield
.lhfield-4096)<=-4096)then begin begin if interaction=3 then;printnl(261);
print(534);end;begin helpptr:=3;helpline[2]:=535;helpline[1]:=536;
helpline[0]:=537;end;putgeterror;
end else begin mem[curedges+1].hhfield.rh:=s*(mem[curedges+1].hhfield.rh-4095)
+4095;mem[curedges+1].hhfield.lhfield:=s*(mem[curedges+1].hhfield.lhfield-4096)+4096;{341:}
p:=curedges;repeat q:=p;p:=mem[p].hhfield.rh;
for t:=2 to s do begin pp:=getnode(2);mem[q].hhfield.rh:=pp;mem[p].hhfield.lhfield:=pp;
mem[pp].hhfield.rh:=p;mem[pp].hhfield.lhfield:=q;q:=pp;{335:}r:=mem[p+1].hhfield.rh;
rr:=pp+1;while r<>memtop do begin ss:=getavail;mem[rr].hhfield.rh:=ss;rr:=ss;
mem[rr].hhfield.lhfield:=mem[r].hhfield.lhfield;r:=mem[r].hhfield.rh;end;mem[rr].hhfield.rh:=memtop;
r:=mem[p+1].hhfield.lhfield;rr:=memtop-1;while r>1 do begin ss:=getavail;
mem[rr].hhfield.rh:=ss;rr:=ss;mem[rr].hhfield.lhfield:=mem[r].hhfield.lhfield;r:=mem[r].hhfield.rh;
end;mem[rr].hhfield.rh:=r;mem[pp+1].hhfield.lhfield:=mem[memtop-1].hhfield.rh{:335};end;
until mem[p].hhfield.rh=curedges{:341};mem[curedges+4].int:=0;end;end;{:340}
{342:}procedure xscaleedges(s:integer);var p,q:halfword;t:0..65535;
w:0..7;delta:integer;
begin if(s*(mem[curedges+2].hhfield.rh-4096)>=4096)or(s*(mem[curedges+2].hhfield
.lhfield-4096)<=-4096)then begin begin if interaction=3 then;printnl(261);
print(534);end;begin helpptr:=3;helpline[2]:=538;helpline[1]:=536;
helpline[0]:=537;end;putgeterror;
end else if(mem[curedges+2].hhfield.rh<>4096)or(mem[curedges+2].hhfield.lhfield<>4096)
then begin mem[curedges+2].hhfield.rh:=s*(mem[curedges+2].hhfield.rh-4096)+4096;
mem[curedges+2].hhfield.lhfield:=s*(mem[curedges+2].hhfield.lhfield-4096)+4096;
delta:=8*(4096-s*mem[curedges+3].hhfield.lhfield)+0;mem[curedges+3].hhfield.lhfield:=4096;
{343:}q:=mem[curedges].hhfield.rh;repeat p:=mem[q+1].hhfield.rh;
while p<>memtop do begin t:=mem[p].hhfield.lhfield;w:=t mod 8;
mem[p].hhfield.lhfield:=(t-w)*s+w+delta;p:=mem[p].hhfield.rh;end;p:=mem[q+1].hhfield.lhfield;
while p>1 do begin t:=mem[p].hhfield.lhfield;w:=t mod 8;
mem[p].hhfield.lhfield:=(t-w)*s+w+delta;p:=mem[p].hhfield.rh;end;q:=mem[q].hhfield.rh;
...
begin p:=mem[h].hhfield.rh;while p<>h do begin q:=mem[p+1].hhfield.lhfield;
while q>1 do begin mem[q].hhfield.lhfield:=8-2*((mem[q].hhfield.lhfield)mod 8)+mem[q].hhfield.lhfield;
q:=mem[q].hhfield.rh;end;q:=mem[p+1].hhfield.rh;
if q<>memtop then begin repeat mem[q].hhfield.lhfield:=8-2*((mem[q].hhfield.lhfield)mod 8)+
mem[q].hhfield.lhfield;q:=mem[q].hhfield.rh;until q=memtop;{345:}u:=p+1;
q:=mem[u].hhfield.rh;r:=q;s:=mem[r].hhfield.rh;
while true do if mem[s].hhfield.lhfield>mem[r].hhfield.lhfield then begin mem[u].hhfield.rh:=q;
if s=memtop then goto 30;u:=r;q:=s;r:=q;s:=mem[r].hhfield.rh;
end else begin t:=s;s:=mem[t].hhfield.rh;mem[t].hhfield.rh:=q;q:=t;end;
30:mem[r].hhfield.rh:=memtop{:345};end;p:=mem[p].hhfield.rh;end;mem[h+4].int:=0;
end;{:344}{346:}procedure sortedges(h:halfword);label 30;var k:halfword;
p,q,r,s:halfword;begin r:=mem[h+1].hhfield.lhfield;mem[h+1].hhfield.lhfield:=0;
p:=mem[r].hhfield.rh;mem[r].hhfield.rh:=memtop;mem[memtop-1].hhfield.rh:=r;
while p>1 do begin k:=mem[p].hhfield.lhfield;q:=memtop-1;repeat r:=q;
q:=mem[r].hhfield.rh;until k<=mem[q].hhfield.lhfield;mem[r].hhfield.rh:=p;r:=mem[p].hhfield.rh;
mem[p].hhfield.rh:=q;p:=r;end;{347:}begin r:=h+1;q:=mem[r].hhfield.rh;
p:=mem[memtop-1].hhfield.rh;while true do begin k:=mem[p].hhfield.lhfield;
while k>mem[q].hhfield.lhfield do begin r:=q;q:=mem[r].hhfield.rh;end;mem[r].hhfield.rh:=p;
s:=mem[p].hhfield.rh;mem[p].hhfield.rh:=q;if s=memtop then goto 30;r:=p;p:=s;end;
...
begin mind:=65535;maxd:=0;minn:=65535;maxn:=0;p:=mem[curedges].hhfield.rh;
n:=mem[curedges+1].hhfield.lhfield;
while p<>curedges do begin if mem[p+1].hhfield.lhfield>1 then sortedges(p);
if mem[p+1].hhfield.rh<>memtop then{349:}begin r:=memtop-1;q:=mem[p+1].hhfield.rh;
ww:=0;m:=1000000;prevw:=0;
while true do begin if q=memtop then mm:=1000000 else begin d:=mem[q].hhfield
.lhfield;mm:=d div 8;ww:=ww+(d mod 8)-4;end;if mm>m then begin{350:}
if w<>prevw then begin s:=getavail;mem[r].hhfield.rh:=s;
mem[s].hhfield.lhfield:=8*m+4+w-prevw;r:=s;prevw:=w;end{:350};
if q=memtop then goto 30;end;m:=mm;
if ww>=wlo then if ww<=whi then w:=win else w:=wout else w:=wout;
s:=mem[q].hhfield.rh;begin mem[q].hhfield.rh:=avail;avail:=q;
ifdef('STAT')decr(dynused);endif('STAT')end;q:=s;end;
30:mem[r].hhfield.rh:=memtop;mem[p+1].hhfield.rh:=mem[memtop-1].hhfield.rh;
if r<>memtop-1 then{351:}begin if minn=65535 then minn:=n;maxn:=n;
if mind>mem[mem[memtop-1].hhfield.rh].hhfield.lhfield then mind:=mem[mem[memtop-1].hhfield
.rh].hhfield.lhfield;if maxd<mem[r].hhfield.lhfield then maxd:=mem[r].hhfield.lhfield;end{:351};
end{:349};p:=mem[p].hhfield.rh;incr(n);end;{352:}if minn>maxn then{353:}
begin p:=mem[curedges].hhfield.rh;while p<>curedges do begin q:=mem[p].hhfield.rh;
freenode(p,2);p:=q;end;initedges(curedges);end{:353}
else begin n:=mem[curedges+1].hhfield.lhfield;mem[curedges+1].hhfield.lhfield:=minn;
while minn>n do begin p:=mem[curedges].hhfield.rh;
mem[curedges].hhfield.rh:=mem[p].hhfield.rh;mem[mem[p].hhfield.rh].hhfield.lhfield:=curedges;
freenode(p,2);incr(n);end;n:=mem[curedges+1].hhfield.rh;
mem[curedges+1].hhfield.rh:=maxn;mem[curedges+5].hhfield.lhfield:=maxn+1;
mem[curedges+5].hhfield.rh:=curedges;
while maxn<n do begin p:=mem[curedges].hhfield.lhfield;
mem[curedges].hhfield.lhfield:=mem[p].hhfield.lhfield;mem[mem[p].hhfield.lhfield].hhfield.rh:=curedges;
freenode(p,2);decr(n);end;
mem[curedges+2].hhfield.lhfield:=((mind)div 8)-mem[curedges+3].hhfield.lhfield+4096;
mem[curedges+2].hhfield.rh:=((maxd)div 8)-mem[curedges+3].hhfield.lhfield+4096;
...
mspread:=mem[curedges+2].hhfield.rh-mem[curedges+2].hhfield.lhfield;
if mspread>movesize then overflow(539,movesize);
for j:=0 to mspread do move[j]:=memtop{:356};{355:}p:=getnode(2);
mem[p+1].hhfield.rh:=memtop;mem[p+1].hhfield.lhfield:=0;mem[p].hhfield.lhfield:=curedges;
mem[mem[curedges].hhfield.rh].hhfield.lhfield:=p;p:=getnode(2);mem[p+1].hhfield.rh:=memtop;
mem[p].hhfield.lhfield:=mem[curedges].hhfield.lhfield;{:355};{365:}
mmagic:=mem[curedges+2].hhfield.lhfield+mem[curedges+3].hhfield.lhfield-4096;
nmagic:=8*mem[curedges+1].hhfield.rh+12{:365};repeat q:=mem[p].hhfield.lhfield;
if mem[q+1].hhfield.lhfield>1 then sortedges(q);{358:}r:=mem[p+1].hhfield.rh;
freenode(p,2);p:=r;pd:=mem[p].hhfield.lhfield;pm:=pd div 8;r:=mem[q+1].hhfield.rh;
rd:=mem[r].hhfield.lhfield;rm:=rd div 8;w:=0;
...
for k:=1 to extras do begin s:=getavail;mem[s].hhfield.lhfield:=nmagic+xw;
mem[s].hhfield.rh:=move[j];move[j]:=s;end;s:=getavail;
mem[s].hhfield.lhfield:=nmagic+ww;mem[s].hhfield.rh:=move[j];move[j]:=s;incr(m);
until m=mm;end{:362};if pd<rd then begin dw:=(pd mod 8)-4;{360:}
s:=mem[p].hhfield.rh;begin mem[p].hhfield.rh:=avail;avail:=p;
ifdef('STAT')decr(dynused);endif('STAT')end;p:=s;pd:=mem[p].hhfield.lhfield;
pm:=pd div 8{:360};end else begin if r=memtop then goto 30;
dw:=-((rd mod 8)-4);{359:}r:=mem[r].hhfield.rh;rd:=mem[r].hhfield.lhfield;
rm:=rd div 8{:359};end;m:=mm;w:=w+dw;end;30:{:358};p:=q;
nmagic:=nmagic-8;until mem[p].hhfield.lhfield=curedges;freenode(p,2);{364:}
move[mspread]:=0;j:=0;while move[j]=memtop do incr(j);
if j=mspread then initedges(curedges)else begin mm:=mem[curedges+2].hhfield
.lhfield;mem[curedges+2].hhfield.lhfield:=mem[curedges+1].hhfield.lhfield;
mem[curedges+2].hhfield.rh:=mem[curedges+1].hhfield.rh+1;
mem[curedges+3].hhfield.lhfield:=4096;jj:=mspread-1;
while move[jj]=memtop do decr(jj);mem[curedges+1].hhfield.lhfield:=j+mm;
mem[curedges+1].hhfield.rh:=jj+mm;q:=curedges;repeat p:=getnode(2);
mem[q].hhfield.rh:=p;mem[p].hhfield.lhfield:=q;mem[p+1].hhfield.rh:=move[j];
mem[p+1].hhfield.lhfield:=0;incr(j);q:=p;until j>jj;mem[q].hhfield.rh:=curedges;
mem[curedges].hhfield.lhfield:=q;mem[curedges+5].hhfield.lhfield:=mem[curedges+1].hhfield.rh+1;
mem[curedges+5].hhfield.rh:=curedges;mem[curedges+4].int:=0;end;{:364};end;
{:354}{366:}procedure mergeedges(h:halfword);label 30;
var p,q,r,pp,qq,rr:halfword;n:integer;k:halfword;delta:integer;
begin if mem[h].hhfield.rh<>h then begin if(mem[h+2].hhfield.lhfield<mem[curedges+2].hhfield
.lhfield)or(mem[h+2].hhfield.rh>mem[curedges+2].hhfield.rh)or(mem[h+1].hhfield.lhfield<mem[
curedges+1].hhfield.lhfield)or(mem[h+1].hhfield.rh>mem[curedges+1].hhfield.rh)then edgeprep(
mem[h+2].hhfield.lhfield-4096,mem[h+2].hhfield.rh-4096,mem[h+1].hhfield.lhfield-4096,mem[h+1].hhfield
.rh-4095);if mem[h+3].hhfield.lhfield<>mem[curedges+3].hhfield.lhfield then{367:}
begin pp:=mem[h].hhfield.rh;delta:=8*(mem[curedges+3].hhfield.lhfield-mem[h+3].hhfield.lhfield);
repeat qq:=mem[pp+1].hhfield.rh;
while qq<>memtop do begin mem[qq].hhfield.lhfield:=mem[qq].hhfield.lhfield+delta;
qq:=mem[qq].hhfield.rh;end;qq:=mem[pp+1].hhfield.lhfield;
while qq>1 do begin mem[qq].hhfield.lhfield:=mem[qq].hhfield.lhfield+delta;
qq:=mem[qq].hhfield.rh;end;pp:=mem[pp].hhfield.rh;until pp=h;end{:367};
n:=mem[curedges+1].hhfield.lhfield;p:=mem[curedges].hhfield.rh;pp:=mem[h].hhfield.rh;
while n<mem[h+1].hhfield.lhfield do begin incr(n);p:=mem[p].hhfield.rh;end;repeat{368:}
qq:=mem[pp+1].hhfield.lhfield;
if qq>1 then if mem[p+1].hhfield.lhfield<=1 then mem[p+1].hhfield.lhfield:=qq else begin
while mem[qq].hhfield.rh>1 do qq:=mem[qq].hhfield.rh;
mem[qq].hhfield.rh:=mem[p+1].hhfield.lhfield;mem[p+1].hhfield.lhfield:=mem[pp+1].hhfield.lhfield;end;
mem[pp+1].hhfield.lhfield:=0;qq:=mem[pp+1].hhfield.rh;
if qq<>memtop then begin if mem[p+1].hhfield.lhfield=1 then mem[p+1].hhfield.lhfield:=0;
mem[pp+1].hhfield.rh:=memtop;r:=p+1;q:=mem[r].hhfield.rh;
if q=memtop then mem[p+1].hhfield.rh:=qq else while true do begin k:=mem[qq]
.hhfield.lhfield;while k>mem[q].hhfield.lhfield do begin r:=q;q:=mem[r].hhfield.rh;end;
mem[r].hhfield.rh:=qq;rr:=mem[qq].hhfield.rh;mem[qq].hhfield.rh:=q;
if rr=memtop then goto 30;r:=qq;qq:=rr;end;end;30:{:368};
pp:=mem[pp].hhfield.rh;p:=mem[p].hhfield.rh;until pp=h;end;end;{:366}{369:}
function totalweight(h:halfword):integer;var p,q:halfword;n:integer;
m:0..65535;begin n:=0;p:=mem[h].hhfield.rh;
while p<>h do begin q:=mem[p+1].hhfield.rh;while q<>memtop do{370:}
begin m:=mem[q].hhfield.lhfield;n:=n-((m mod 8)-4)*(m div 8);q:=mem[q].hhfield.rh;
end{:370};q:=mem[p+1].hhfield.lhfield;while q>1 do{370:}begin m:=mem[q].hhfield.lhfield;
n:=n-((m mod 8)-4)*(m div 8);q:=mem[q].hhfield.rh;end{:370};p:=mem[p].hhfield.rh;
...
m,n0,n1:integer;begin d:=mem[r].hhfield.lhfield;w:=(d mod 8)-4;
m:=(d div 8)-mem[curedges+3].hhfield.lhfield;if w=curwt then begin n0:=n+1;n1:=n;
...
begin base:=8*mem[curedges+3].hhfield.lhfield+4-curwt;
if m0<=m1 then edgeprep(m0,m1,n0,n1)else edgeprep(m1,m0,n0,n1);{377:}
n:=mem[curedges+5].hhfield.lhfield-4096;p:=mem[curedges+5].hhfield.rh;
if n<>n0 then if n<n0 then repeat incr(n);p:=mem[p].hhfield.rh;
until n=n0 else repeat decr(n);p:=mem[p].hhfield.lhfield;until n=n0{:377};
y0:=65536-y0;while true do begin r:=getavail;
mem[r].hhfield.rh:=mem[p+1].hhfield.lhfield;mem[p+1].hhfield.lhfield:=r;
tx:=takefraction(delx,makefraction(y0,dely));
if abvscd(delx,y0,dely,tx)<0 then decr(tx);
mem[r].hhfield.lhfield:=8*roundunscaled(x0+tx)+base;y1:=y1-65536;
if internal[10]>0 then tracenewedge(r,n);if y1<65536 then goto 30;
p:=mem[p].hhfield.rh;y0:=y0+65536;incr(n);end;30:end{:375}else{376:}
begin base:=8*mem[curedges+3].hhfield.lhfield+4+curwt;
if m0<=m1 then edgeprep(m0,m1,n1,n0)else edgeprep(m1,m0,n1,n0);decr(n0);
{377:}n:=mem[curedges+5].hhfield.lhfield-4096;p:=mem[curedges+5].hhfield.rh;
if n<>n0 then if n<n0 then repeat incr(n);p:=mem[p].hhfield.rh;
until n=n0 else repeat decr(n);p:=mem[p].hhfield.lhfield;until n=n0{:377};
while true do begin r:=getavail;mem[r].hhfield.rh:=mem[p+1].hhfield.lhfield;
mem[p+1].hhfield.lhfield:=r;tx:=takefraction(delx,makefraction(y0,dely));
if abvscd(delx,y0,dely,tx)<0 then incr(tx);
mem[r].hhfield.lhfield:=8*roundunscaled(x0-tx)+base;y1:=y1+65536;
if internal[10]>0 then tracenewedge(r,n);if y1>=0 then goto 31;
p:=mem[p].hhfield.lhfield;y0:=y0+65536;decr(n);end;31:end{:376};
mem[curedges+5].hhfield.rh:=p;mem[curedges+5].hhfield.lhfield:=n+4096;end;end;{:374}
...
edgeandweight:=8*(m0+mem[curedges+3].hhfield.lhfield)+4-curwt;
repeat edgeandweight:=edgeandweight+dx*move[k];begin r:=avail;
if r=0 then r:=getavail else begin avail:=mem[r].hhfield.rh;mem[r].hhfield.rh:=0;
ifdef('STAT')incr(dynused);endif('STAT')end;end;
mem[r].hhfield.rh:=mem[p+1].hhfield.lhfield;mem[r].hhfield.lhfield:=edgeandweight;
if internal[10]>0 then tracenewedge(r,n);mem[p+1].hhfield.lhfield:=r;
p:=mem[p].hhfield.rh;incr(k);incr(n);until k=delta;end;goto 30{:381};
61:{382:}n0:=-n0-1;{377:}n:=mem[curedges+5].hhfield.lhfield-4096;
p:=mem[curedges+5].hhfield.rh;if n<>n0 then if n<n0 then repeat incr(n);
p:=mem[p].hhfield.rh;until n=n0 else repeat decr(n);p:=mem[p].hhfield.lhfield;
...
mem[r].hhfield.rh:=mem[p+1].hhfield.lhfield;mem[r].hhfield.lhfield:=edgeandweight;
if internal[10]>0 then tracenewedge(r,n);mem[p+1].hhfield.lhfield:=r;
p:=mem[p].hhfield.lhfield;incr(k);decr(n);until k=delta;end;goto 30{:382};
62:{383:}edgeandweight:=8*(n0+mem[curedges+3].hhfield.lhfield)+4-curwt;n0:=m0;
k:=0;{377:}n:=mem[curedges+5].hhfield.lhfield-4096;p:=mem[curedges+5].hhfield.rh;
if n<>n0 then if n<n0 then repeat incr(n);p:=mem[p].hhfield.rh;
until n=n0 else repeat decr(n);p:=mem[p].hhfield.lhfield;until n=n0{:377};
...
mem[r].hhfield.rh:=mem[p+1].hhfield.lhfield;mem[r].hhfield.lhfield:=edgeandweight;
if internal[10]>0 then tracenewedge(r,n);mem[p+1].hhfield.lhfield:=r;
p:=mem[p].hhfield.rh;decr(j);incr(n);end;edgeandweight:=edgeandweight+dx;
incr(k);until k>delta;goto 30{:383};63:{384:}
edgeandweight:=8*(n0+mem[curedges+3].hhfield.lhfield)+4+curwt;n0:=-m0-1;k:=0;
{377:}n:=mem[curedges+5].hhfield.lhfield-4096;p:=mem[curedges+5].hhfield.rh;
if n<>n0 then if n<n0 then repeat incr(n);p:=mem[p].hhfield.rh;
until n=n0 else repeat decr(n);p:=mem[p].hhfield.lhfield;until n=n0{:377};
repeat j:=move[k];while j>0 do begin begin r:=avail;
...
mem[r].hhfield.rh:=mem[p+1].hhfield.lhfield;mem[r].hhfield.lhfield:=edgeandweight;
if internal[10]>0 then tracenewedge(r,n);mem[p+1].hhfield.lhfield:=r;
p:=mem[p].hhfield.lhfield;decr(j);decr(n);end;edgeandweight:=edgeandweight+dx;
incr(k);until k>delta;goto 30{:384};30:mem[curedges+5].hhfield.lhfield:=n+4096;
mem[curedges+5].hhfield.rh:=p;end;{:378}{387:}procedure skew(x,y:scaled;
...
while true do begin q:=mem[p].hhfield.rh;if mem[p].hhfield.b1=0 then goto 45;
...
printint(mem[q].hhfield.b0-1);end{:397};p:=q;end;
...
p:=curspec;t:=256;repeat p:=mem[p].hhfield.rh;
if mem[p].hhfield.b0<>0 then begin if mem[p].hhfield.b0<t then f:=p;
t:=mem[p].hhfield.b0;end;until p=curspec{:399};{400:}p:=curspec;q:=p;
repeat p:=mem[p].hhfield.rh;if mem[p].hhfield.b0=0 then q:=p;until p=f{:400};t:=0;
repeat if mem[p].hhfield.b0<>0 then begin if mem[p].hhfield.b0<>t then begin t:=
mem[p].hhfield.b0;printchar(32);printint(t-1);end;if q<>0 then begin{401:}
if mem[mem[q].hhfield.rh].hhfield.b0=0 then begin print(557);
print(octantdir[mem[q+3].int]);q:=mem[q].hhfield.rh;
while mem[mem[q].hhfield.rh].hhfield.b0=0 do begin printchar(32);
print(octantdir[mem[q+3].int]);q:=mem[q].hhfield.rh;end;printchar(41);
end{:401};printchar(32);print(octantdir[mem[q+3].int]);q:=0;end;
end else if q=0 then q:=p;p:=mem[p].hhfield.rh;until p=f;printchar(32);
printint(mem[p].hhfield.b0-1);if q<>0 then{401:}
if mem[mem[q].hhfield.rh].hhfield.b0=0 then begin print(557);
print(octantdir[mem[q+3].int]);q:=mem[q].hhfield.rh;
while mem[mem[q].hhfield.rh].hhfield.b0=0 do begin printchar(32);
print(octantdir[mem[q+3].int]);q:=mem[q].hhfield.rh;end;printchar(41);
...
begin q:=mem[p].hhfield.rh;mem[p].hhfield.b1:=mem[q].hhfield.b1;
mem[p].hhfield.rh:=mem[q].hhfield.rh;mem[p+1].int:=mem[q+1].int;
...
q,r:halfword;begin q:=mem[p].hhfield.rh;r:=getnode(7);mem[p].hhfield.rh:=r;
mem[r].hhfield.rh:=q;mem[r].hhfield.b0:=mem[q].hhfield.b0;mem[r].hhfield.b1:=mem[p].hhfield.b1;
...
firsty:=mem[curspec+2].int;repeat 22:q:=mem[p].hhfield.rh;{407:}
...
del2:=-del2;del3:=-del3;destx:=-destx;mem[p].hhfield.b1:=2;end{:409};
t:=crossingpoint(del1,del2,del3);if t<268435456 then{411:}
begin splitcubic(p,t,destx,desty);r:=mem[p].hhfield.rh;
if mem[r].hhfield.b1>1 then mem[r].hhfield.b1:=1 else mem[r].hhfield.b1:=2;
...
begin splitcubic(r,t,destx,desty);s:=mem[r].hhfield.rh;
if mem[s+1].int<destx then mem[s+1].int:=destx;
if mem[s+1].int<mem[r+1].int then mem[s+1].int:=mem[r+1].int;
mem[s].hhfield.b1:=mem[p].hhfield.b1;mem[s+3].int:=mem[s+1].int;
...
pp:=p;repeat qq:=mem[pp].hhfield.rh;
abnegate(mem[qq+1].int,mem[qq+2].int,mem[qq].hhfield.b1,mem[pp].hhfield.b1);
...
desty:=-desty;mem[pp].hhfield.b1:=mem[pp].hhfield.b1+2;end{:414};
t:=crossingpoint(del1,del2,del3);if t<268435456 then{415:}
begin splitcubic(pp,t,destx,desty);r:=mem[pp].hhfield.rh;
if mem[r].hhfield.b1>2 then mem[r].hhfield.b1:=mem[r].hhfield.b1-2 else mem[r].hhfield.b1:=
mem[r].hhfield.b1+2;
...
mem[s].hhfield.b1:=mem[pp].hhfield.b1;mem[s+4].int:=mem[s+2].int;
...
end else if not odd(mem[pp].hhfield.b1)then{414:}
begin mem[pp+2].int:=-mem[pp+2].int;mem[pp+6].int:=-mem[pp+6].int;
mem[qq+4].int:=-mem[qq+4].int;del1:=-del1;del2:=-del2;del3:=-del3;
desty:=-desty;mem[pp].hhfield.b1:=mem[pp].hhfield.b1+2;end{:414}{:417};pp:=qq;
until pp=q;if constantx then{418:}begin pp:=p;repeat qq:=mem[pp].hhfield.rh;
if mem[pp].hhfield.b1>2 then begin mem[pp].hhfield.b1:=mem[pp].hhfield.b1+1;
...
destx,desty:scaled;begin p:=curspec;repeat q:=mem[p].hhfield.rh;
mem[p+1].int:=mem[p+1].int-mem[p+2].int;
mem[p+5].int:=mem[p+5].int-mem[p+6].int;
mem[q+3].int:=mem[q+3].int-mem[q+4].int;{420:}{421:}
if q=curspec then begin unskew(mem[q+1].int,mem[q+2].int,mem[q].hhfield.b1);
skew(curx,cury,mem[p].hhfield.b1);destx:=curx;desty:=cury;
end else begin abnegate(mem[q+1].int,mem[q+2].int,mem[q].hhfield.b1,mem[p].hhfield
...
desty:=destx+desty;destx:=-destx;mem[p].hhfield.b1:=mem[p].hhfield.b1+4;end{:423};
t:=crossingpoint(del1,del2,del3);if t<268435456 then{424:}
begin splitcubic(p,t,destx,desty);r:=mem[p].hhfield.rh;
if mem[r].hhfield.b1>4 then mem[r].hhfield.b1:=mem[r].hhfield.b1-4 else mem[r].hhfield.b1:=
mem[r].hhfield.b1+4;
...
if mem[s+6].int>mem[q+4].int then mem[s+6].int:=mem[q+4].int;end{:425}
else begin if mem[r+1].int>destx then begin mem[r+1].int:=destx;
mem[r+3].int:=-mem[r+1].int;mem[r+5].int:=mem[r+1].int;end;
if mem[q+3].int>destx then mem[q+3].int:=destx else if mem[q+3].int<mem[
...
repeat q:=mem[p].hhfield.rh;{434:}
if odd(mem[p].hhfield.b1)<>odd(mem[q].hhfield.b1)then begin if odd(mem[q].hhfield.b1)
...
compromise(mem[mem[curpen+5].hhfield.rh+2].int,mem[mem[curpen+7].hhfield.rh+2].int
)else if odd(mem[q].hhfield.b1)then penedge:=mem[mem[curpen+7].hhfield.rh+2].int
else penedge:=mem[mem[curpen+5].hhfield.rh+2].int;a:=goodval(b,penedge);
end{:435}else a:=b;
...
if odd(mem[p].hhfield.b1)then begin b:=before[curroundingptr];
...
mem[p+5].int:=takefraction(alpha,mem[p+5].int-b)+a;p:=mem[p].hhfield.rh;
mem[p+3].int:=takefraction(alpha,mem[p+3].int-b)+a;
until p=nodetoround[curroundingptr+1];end;until curroundingptr=0;
end{:436};p:=curspec;curroundingptr:=0;repeat q:=mem[p].hhfield.rh;{437:}
if(mem[p].hhfield.b1>2)<>(mem[q].hhfield.b1>2)then begin if mem[q].hhfield.b1<=2 then b
:=mem[q+2].int else b:=-mem[q+2].int;
if(abs(mem[q+2].int-mem[q+6].int)<655)or(abs(mem[q+2].int+mem[q+4].int)<
655)then{438:}
begin if curpen=3 then penedge:=0 else if curpathtype=0 then penedge:=
compromise(mem[mem[curpen+2].hhfield.rh+2].int,mem[mem[curpen+1].hhfield.rh+2].int
)else if mem[q].hhfield.b1<=2 then penedge:=mem[mem[curpen+1].hhfield.rh+2].int
else penedge:=mem[mem[curpen+2].hhfield.rh+2].int;a:=goodval(b,penedge);
end{:438}else a:=b;
if abs(a)>maxallowed then if a>0 then a:=maxallowed else a:=-maxallowed;
...
if(after[curroundingptr]<>before[curroundingptr])or(after[curroundingptr
+1]<>before[curroundingptr+1])then begin p:=nodetoround[curroundingptr];
if mem[p].hhfield.b1<=2 then begin b:=before[curroundingptr];
a:=after[curroundingptr];end else begin b:=-before[curroundingptr];
a:=-after[curroundingptr];end;
if before[curroundingptr]=before[curroundingptr+1]then alpha:=268435456
else alpha:=makefraction(after[curroundingptr+1]-after[curroundingptr],
...
mem[p+6].int:=takefraction(alpha,mem[p+6].int-b)+a;p:=mem[p].hhfield.rh;
...
begin p:=curspec;curroundingptr:=0;repeat q:=mem[p].hhfield.rh;{441:}
if mem[p].hhfield.b1<>mem[q].hhfield.b1 then begin if mem[q].hhfield.b1>4 then b:=-mem[
q+1].int else b:=mem[q+1].int;
if abs(mem[q].hhfield.b1-mem[p].hhfield.b1)=4 then if(abs(mem[q+1].int-mem[q+5]
.int)<655)or(abs(mem[q+1].int+mem[q+3].int)<655)then{442:}
begin if curpen=3 then penedge:=0 else if curpathtype=0 then{443:}
case mem[q].hhfield.b1 of 1,5:penedge:=compromise(mem[mem[mem[curpen+1].hhfield.rh
].hhfield.lhfield+1].int,-mem[mem[mem[curpen+4].hhfield.rh].hhfield.lhfield+1].int);
4,8:penedge:=-compromise(mem[mem[mem[curpen+1].hhfield.rh].hhfield.lhfield+1].int,-mem[
mem[mem[curpen+4].hhfield.rh].hhfield.lhfield+1].int);
6,2:penedge:=compromise(mem[mem[mem[curpen+2].hhfield.rh].hhfield.lhfield+1].int,-mem[
mem[mem[curpen+3].hhfield.rh].hhfield.lhfield+1].int);
7,3:penedge:=-compromise(mem[mem[mem[curpen+2].hhfield.rh].hhfield.lhfield+1].int,-mem[
mem[mem[curpen+3].hhfield.rh].hhfield.lhfield+1].int);end{:443}
else if mem[q].hhfield.b1<=4 then penedge:=mem[mem[mem[curpen+mem[q].hhfield.b1]
.hhfield.rh].hhfield.lhfield+1].int else penedge:=-mem[mem[mem[curpen+mem[q].hhfield.b1].hhfield
.rh].hhfield.lhfield+1].int;
if odd(mem[q].hhfield.b1)then a:=goodval(b,penedge+(curgran)div 2)else a:=
...
].hhfield.b1)else unskew(mem[pp+1].int,mem[pp+2].int,mem[pp].hhfield.b1);
skew(curx,cury,mem[p].hhfield.b1);bb:=curx;aa:=bb;dd:=cury;cc:=dd;
if mem[p].hhfield.b1>4 then begin b:=-b;a:=-a;end;
end else begin if mem[p].hhfield.b1>4 then begin bb:=-bb;aa:=-aa;b:=-b;a:=-a;
end;if pp=nodetoround[0]then dd:=firsty-bb else dd:=mem[pp+2].int-bb;
if odd(aa-bb)then if mem[p].hhfield.b1>4 then cc:=dd-(aa-bb+1)div 2 else cc:=
dd-(aa-bb-1)div 2 else cc:=dd-(aa-bb)div 2;end;d:=mem[p+2].int;
if odd(a-b)then if mem[p].hhfield.b1>4 then c:=d-(a-b-1)div 2 else c:=d-(a-b
...
mem[p+6].int:=takefraction(beta,mem[p+6].int-d)+c;p:=mem[p].hhfield.rh;
mem[p+3].int:=takefraction(alpha,mem[p+3].int-b)+a;
mem[p+4].int:=takefraction(beta,mem[p+4].int-d)+c;until p=pp;end;end;
end{:444};end;{:440}{451:}procedure newboundary(p:halfword;
octant:smallnumber);var q,r:halfword;begin q:=mem[p].hhfield.rh;
r:=getnode(7);mem[r].hhfield.rh:=q;mem[p].hhfield.rh:=r;
mem[r].hhfield.b0:=mem[q].hhfield.b0;mem[r+3].int:=mem[q+3].int;
mem[r+4].int:=mem[q+4].int;mem[r].hhfield.b1:=0;mem[q].hhfield.b0:=0;
mem[r+5].int:=octant;mem[q+3].int:=mem[q].hhfield.b1;
unskew(mem[q+1].int,mem[q+2].int,mem[q].hhfield.b1);skew(curx,cury,octant);
...
maxallowed;end;p:=mem[p].hhfield.rh;mem[p].hhfield.b0:=k;
...
].int then begin unskew(mem[q+1].int,mem[q+2].int,mem[q].hhfield.b1);
...
q:=mem[p].hhfield.rh;repeat r:=mem[q].hhfield.rh;
if(mem[p].hhfield.b1<>mem[q].hhfield.b1)or(q=r)then{452:}
begin newboundary(p,mem[p].hhfield.b1);s:=mem[p].hhfield.rh;
o1:=octantnumber[mem[p].hhfield.b1];o2:=octantnumber[mem[q].hhfield.b1];
...
if dx2=0 then if dy2=0 then begin if mem[r].hhfield.b1=0 then begin curx:=mem
[r+1].int;cury:=mem[r+2].int;
end else begin unskew(mem[r+1].int,mem[r+2].int,mem[r].hhfield.b1);
skew(curx,cury,mem[q].hhfield.b1);end;dx2:=curx-mem[q+1].int;
...
end{:457};unskew(dx1,dy1,mem[p].hhfield.b1);del:=pythadd(curx,cury);
dx1:=makefraction(curx,del);dy1:=makefraction(cury,del);
unskew(dx2,dy2,mem[q].hhfield.b1);del:=pythadd(curx,cury);
...
newboundary(s,octantcode[o1]);s:=mem[s].hhfield.rh;
mem[s+3].int:=mem[s+5].int;end{:458};
30:if q=r then begin q:=mem[q].hhfield.rh;r:=q;p:=s;mem[s].hhfield.rh:=q;
mem[q+3].int:=mem[q+5].int;mem[q].hhfield.b0:=0;freenode(curspec,7);
curspec:=q;end;{459:}p:=mem[p].hhfield.rh;repeat s:=mem[p].hhfield.rh;
...
while mem[curspec].hhfield.b0<>0 do curspec:=mem[curspec].hhfield.rh;
...
while mem[q].hhfield.b1<>0 do q:=mem[q].hhfield.rh{:466};if q<>p then begin{467:}
...
movetoedges(m0,n0,m1,n1);end;p:=mem[q].hhfield.rh;until p=h;tossknotlist(h);
...
mem[r].hhfield.rh:=mem[w].hhfield.rh;mem[mem[w].hhfield.rh].hhfield.lhfield:=r;mem[r].hhfield.lhfield:=w;
mem[w].hhfield.rh:=r;end;{:476}{477:}function makepen(h:halfword):halfword;
label 30,31,45,40;var o,oo,k:smallnumber;p:halfword;q,r,s,w,hh:halfword;
n:integer;dx,dy:scaled;mc:scaled;begin{479:}q:=h;r:=mem[q].hhfield.rh;
mc:=abs(mem[h+1].int);if q=r then begin hh:=h;mem[h].hhfield.b1:=0;
if mc<abs(mem[h+2].int)then mc:=abs(mem[h+2].int);end else begin o:=0;
hh:=0;while true do begin s:=mem[r].hhfield.rh;
...
if dx<dy then octant:=octant+4{:480};mem[q].hhfield.b1:=octant;
...
mem[p].hhfield.lhfield:=0;if mem[q].hhfield.rh<>q then mem[p].hhfield.rh:=1;
...
mem[r+2].int:=cury;if n=0 then mem[h].hhfield.rh:=r else{482:}
if odd(k)then begin mem[w].hhfield.rh:=r;mem[r].hhfield.lhfield:=w;
end else begin mem[w].hhfield.lhfield:=r;mem[r].hhfield.rh:=w;end{:482};w:=r;
if mem[q].hhfield.b1<>octant then goto 31;q:=mem[q].hhfield.rh;incr(n);end;
31:{483:}r:=mem[h].hhfield.rh;if odd(k)then begin mem[w].hhfield.rh:=r;
mem[r].hhfield.lhfield:=w;end else begin mem[w].hhfield.lhfield:=r;mem[r].hhfield.rh:=w;
mem[h].hhfield.rh:=w;r:=w;end;
if(mem[r+2].int<>mem[mem[r].hhfield.rh+2].int)or(n=0)then begin dupoffset(r);
incr(n);end;r:=mem[r].hhfield.lhfield;
if mem[r+1].int<>mem[mem[r].hhfield.lhfield+1].int then dupoffset(r)else decr(n){:
483};if n>=255 then overflow(578,255);mem[h].hhfield.lhfield:=n;end{:481};goto 40;
...
begin p:=getnode(7);mem[p].hhfield.b0:=1;mem[p].hhfield.b1:=1;mem[p+1].int:=x;
...
mem[p].hhfield.rh:=trivialknot(curx,cury);p:=mem[p].hhfield.rh;end{:485};w:=ww;
end;end;if p=memtop-1 then begin w:=mem[penhead+1].hhfield.rh;
p:=trivialknot(mem[w+1].int+mem[w+2].int,mem[w+2].int);
mem[memtop-1].hhfield.rh:=p;end;mem[p].hhfield.rh:=mem[memtop-1].hhfield.rh;
makepath:=mem[memtop-1].hhfield.rh;end;{:484}{488:}
...
w:=mem[mem[h].hhfield.rh].hhfield.rh;ww:=mem[w].hhfield.rh;n:=mem[h].hhfield.lhfield;
while n>1 do begin if abvscd(x,mem[ww+2].int-mem[w+2].int,y,mem[ww+1]
.int-mem[w+1].int)<>s then goto 30;w:=ww;ww:=mem[w].hhfield.rh;decr(n);end;
30:unskew(mem[w+1].int,mem[w+2].int,octant);10:end;{:488}{491:}{493:}
procedure splitforoffset(p:halfword;t:fraction);var q:halfword;
r:halfword;begin q:=mem[p].hhfield.rh;
splitcubic(p,t,mem[q+1].int,mem[q+2].int);r:=mem[p].hhfield.rh;
...
v:integer;begin while true do begin mem[p].hhfield.b1:=k;
if rising then if k=n then goto 10 else ww:=mem[w].hhfield.rh else if k=1
then goto 10 else ww:=mem[w].hhfield.lhfield;{498:}du:=mem[ww+1].int-mem[w+1].int;
...
mem[p].hhfield.b1:=k;p:=mem[p].hhfield.rh;v:=x0-takefraction(x0-x1,t);
...
mem[mem[p].hhfield.rh].hhfield.b1:=k;v:=x1-takefraction(x1-x2,t);
...
{:495}begin p:=c;n:=mem[h].hhfield.lhfield;lh:=mem[h].hhfield.rh;
while mem[p].hhfield.b1<>0 do begin q:=mem[p].hhfield.rh;{494:}
if n<=1 then mem[p].hhfield.b1:=1 else begin{496:}
...
if dx=0 then{505:}
finoffsetprep(p,n,mem[mem[lh].hhfield.lhfield].hhfield.lhfield,-x0,-x1,-x2,-y0,-y1,-y2,false
,n){:505}else begin{502:}k:=1;w:=mem[lh].hhfield.rh;
while true do begin if k=n then goto 30;ww:=mem[w].hhfield.rh;
...
{503:}if k=1 then t:=268435457 else begin ww:=mem[w].hhfield.lhfield;{498:}
...
begin splitforoffset(p,t);r:=mem[p].hhfield.rh;x1a:=x0-takefraction(x0-x1,t);
...
finoffsetprep(mem[r].hhfield.rh,k,w,x0a,x1a,x2,y0a,y1a,y2,true,n);x2:=x0a;
y2:=y0a;end{:504};
finoffsetprep(r,k-1,ww,-x0,-x1,-x2,-y0,-y1,-y2,false,n);end{:503};end;
45:end{:494};{492:}repeat r:=mem[p].hhfield.rh;
...
{:511}begin{519:}k:=mem[h].hhfield.lhfield+1;ww:=mem[h].hhfield.rh;w:=mem[ww].hhfield.lhfield;
...
while mem[r].hhfield.b1<>k do{521:}begin xx:=mem[r+1].int+mem[w+1].int;
...
endif('STAT')if mem[r].hhfield.b1<k then begin decr(k);w:=mem[w].hhfield.lhfield;
...
end else begin incr(k);w:=mem[w].hhfield.rh;xp:=mem[r+1].int+mem[w+1].int;
...
movetoedges(m0,n0,m1,n1);if mem[q+6].int=1 then begin w:=mem[h].hhfield.rh;
skewlineedges(q,w,mem[w].hhfield.lhfield);end{:523};end;{:518}
...
while mem[q].hhfield.b1<>0 do q:=mem[q].hhfield.rh{:466};{508:}w:=mem[h].hhfield.rh;
if mem[p+4].int=1 then w:=mem[w].hhfield.lhfield;
ifdef('STAT')if internal[10]>65536 then{509:}begin printnl(579);
print(octantdir[octant]);print(557);printint(mem[h].hhfield.lhfield);print(580);
if mem[h].hhfield.lhfield<>1 then printchar(115);print(581);
unskew(mem[p+1].int+mem[w+1].int,mem[p+2].int+mem[w+2].int,octant);
printtwo(curx,cury);ww:=mem[h].hhfield.rh;
if mem[q+6].int=1 then ww:=mem[ww].hhfield.lhfield;print(582);
unskew(mem[q+1].int+mem[ww+1].int,mem[q+2].int+mem[ww+2].int,octant);
printtwo(curx,cury);end{:509};endif('STAT')ww:=mem[h].hhfield.rh;www:=ww;
if odd(octantnumber[octant])then www:=mem[www].hhfield.lhfield else ww:=mem[ww].hhfield
.lhfield;if w<>ww then skewlineedges(p,w,ww);
...
{466:}q:=p;while mem[q].hhfield.b1<>0 do q:=mem[q].hhfield.rh{:466};{512:}
if odd(octantnumber[octant])then begin{513:}k:=0;w:=mem[h].hhfield.rh;
ww:=mem[w].hhfield.lhfield;
mm0:=floorunscaled(mem[p+1].int+mem[w+1].int-xycorr[octant]);
mm1:=floorunscaled(mem[q+1].int+mem[ww+1].int-xycorr[octant]);
for n:=0 to n1-n0 do envmove[n]:=mm0;envmove[n1-n0]:=mm1;moveptr:=0;
m:=mm0{:513};r:=p;mem[q].hhfield.b1:=mem[h].hhfield.lhfield+1;
while true do begin if r=q then smoothtop:=moveptr;
while mem[r].hhfield.b1<>k do{515:}begin xx:=mem[r+1].int+mem[w+1].int;
yy:=mem[r+2].int+mem[w+2].int+32768;
ifdef('STAT')if internal[10]>65536 then begin printnl(584);printint(k);
print(585);unskew(xx,yy-32768,octant);printtwo(curx,cury);end;
endif('STAT')if mem[r].hhfield.b1>k then begin incr(k);w:=mem[w].hhfield.rh;
xp:=mem[r+1].int+mem[w+1].int;yp:=mem[r+2].int+mem[w+2].int+32768;
...
31:end;end{:516};end else begin decr(k);w:=mem[w].hhfield.lhfield;
...
move[moveptr]:=move[moveptr]-d1;
if internal[35]>0 then smoothmoves(smoothbot,smoothtop);
movetoedges(m0,n0,m1,n1);if mem[q+6].int=0 then begin w:=mem[h].hhfield.rh;
skewlineedges(q,mem[w].hhfield.lhfield,w);end{:517};end else dualmoves(h,p,q);
mem[q].hhfield.b1:=0{:512};p:=mem[q].hhfield.rh;until p=spechead;
...
if symmetric then s:=0 else s:=getnode(7);h:=p;mem[p].hhfield.rh:=q;
mem[q].hhfield.rh:=r;mem[r].hhfield.rh:=s;{529:}if beta=0 then beta:=1;
...
mem[p].hhfield.rh:=s;mem[s].hhfield.rh:=q;
...
end else p:=q;{532:}while true do begin q:=mem[p].hhfield.rh;
if q=0 then goto 30;
if mem[q+4].int=0 then begin mem[p].hhfield.rh:=mem[q].hhfield.rh;
mem[p+6].int:=mem[q+6].int;mem[p+5].int:=mem[q+5].int;freenode(q,7);
end else begin r:=mem[q].hhfield.rh;if r=0 then goto 30;
if mem[r+4].int=0 then begin mem[p].hhfield.rh:=r;freenode(q,7);p:=r;
end else goto 40;end;end;40:{:532};end;30:{:531};if symmetric then{536:}
begin s:=0;q:=h;while true do begin r:=getnode(7);mem[r].hhfield.rh:=s;s:=r;
mem[s+1].int:=mem[q+1].int;mem[s+2].int:=-mem[q+2].int;
if q=p then goto 31;q:=mem[q].hhfield.rh;if mem[q+2].int=0 then goto 31;end;
31:mem[p].hhfield.rh:=s;beta:=-mem[h+2].int;
while mem[p+2].int<>beta do p:=mem[p].hhfield.rh;q:=mem[p].hhfield.rh;end{:536};
{537:}if q<>0 then begin if mem[h+5].int=0 then begin p:=h;
h:=mem[h].hhfield.rh;freenode(p,7);mem[q+1].int:=-mem[h+1].int;end;p:=q;
end else q:=p;r:=mem[h].hhfield.rh;repeat s:=getnode(7);mem[p].hhfield.rh:=s;p:=s;
mem[p+1].int:=-mem[r+1].int;mem[p+2].int:=-mem[r+2].int;r:=mem[r].hhfield.rh;
until r=q;mem[p].hhfield.rh:=h{:537};makeellipse:=h;end;{:527}{539:}
...
while true do begin if mem[p].hhfield.b1=0 then goto 45;q:=mem[p].hhfield.rh;
...
var q,qq:halfword;begin timetogo:=5000;maxt:=2;{558:}q:=mem[p].hhfield.rh;
qq:=mem[pp].hhfield.rh;bisectptr:=20;
...
if mem[h].hhfield.b1=0 then begin mem[h+5].int:=mem[h+1].int;
mem[h+3].int:=mem[h+1].int;mem[h+6].int:=mem[h+2].int;
mem[h+4].int:=mem[h+2].int;mem[h].hhfield.b1:=1;end;
if mem[hh].hhfield.b1=0 then begin mem[hh+5].int:=mem[hh+1].int;
mem[hh+3].int:=mem[hh+1].int;mem[hh+6].int:=mem[hh+2].int;
mem[hh+4].int:=mem[hh+2].int;mem[hh].hhfield.b1:=1;end;{:563};tolstep:=0;
repeat n:=-65536;p:=h;repeat if mem[p].hhfield.b1<>0 then begin nn:=-65536;
pp:=hh;repeat if mem[pp].hhfield.b1<>0 then begin cubicintersection(p,pp);
if curt>0 then begin curt:=curt+n;curtt:=curtt+nn;goto 10;end;end;
nn:=nn+65536;pp:=mem[pp].hhfield.rh;until pp=hh;end;n:=n+65536;
p:=mem[p].hhfield.rh;until p=h;tolstep:=tolstep+3;until tolstep>3;
...
if mem[curedges+3].hhfield.rh=k then if mem[curedges+4].int=windowtime[k]then
alreadythere:=true;
if not alreadythere then blankrectangle(leftcol[k],rightcol[k],toprow[k]
,botrow[k]);{581:}madjustment:=mwindow[k]-mem[curedges+3].hhfield.lhfield;
rightedge:=8*(rightcol[k]-madjustment);mincol:=leftcol[k]{:581};
p:=mem[curedges].hhfield.rh;r:=nwindow[k]-(mem[curedges+1].hhfield.lhfield-4096);
while(p<>curedges)and(r>=toprow[k])do begin if r<botrow[k]then{578:}
begin if mem[p+1].hhfield.lhfield>1 then sortedges(p)else if mem[p+1].hhfield.lhfield=1 then
if alreadythere then goto 30;mem[p+1].hhfield.lhfield:=1;{582:}n:=0;ww:=0;m:=-1;
w:=0;q:=mem[p+1].hhfield.rh;rowtransition[0]:=mincol;
while true do begin if q=memtop then d:=rightedge else d:=mem[q].hhfield.lhfield;
...
if d>=rightedge then goto 40;ww:=ww+(d mod 8)-4;q:=mem[q].hhfield.rh;end;
40:{584:}
...
30:end{:578};p:=mem[p].hhfield.rh;decr(r);end;updatescreen;
incr(windowtime[k]);mem[curedges+3].hhfield.rh:=k;
mem[curedges+4].int:=windowtime[k];end;end;{:577}{591:}
function maxcoef(p:halfword):fraction;var x:fraction;begin x:=0;
while mem[p].hhfield.lhfield<>0 do begin if abs(mem[p+1].int)>x then x:=abs(mem[p
+1].int);p:=mem[p].hhfield.rh;end;maxcoef:=x;end;{:591}{597:}
...
begin v:=mem[p+1].int+mem[q+1].int;mem[p+1].int:=v;s:=p;p:=mem[p].hhfield.rh;
pp:=mem[p].hhfield.lhfield;
if abs(v)<threshold then freenode(s,2)else begin if abs(v)>=626349397
then if watchcoefs then begin mem[qq].hhfield.b0:=0;fixneeded:=true;end;
mem[r].hhfield.rh:=s;r:=s;end;q:=mem[q].hhfield.rh;qq:=mem[q].hhfield.lhfield;end{:598}
else if mem[pp+1].int<mem[qq+1].int then begin s:=getnode(2);
mem[s].hhfield.lhfield:=qq;mem[s+1].int:=mem[q+1].int;q:=mem[q].hhfield.rh;
qq:=mem[q].hhfield.lhfield;mem[r].hhfield.rh:=s;r:=s;end else begin mem[r].hhfield.rh:=p;
r:=p;p:=mem[p].hhfield.rh;pp:=mem[p].hhfield.lhfield;end;
30:mem[p+1].int:=slowadd(mem[p+1].int,mem[q+1].int);mem[r].hhfield.rh:=p;
depfinal:=p;pplusq:=mem[memtop-1].hhfield.rh;end;{:597}{599:}
...
if t1=17 then threshold:=1342 else threshold:=4;r:=memtop-1;
while mem[p].hhfield.lhfield<>0 do begin if scalingdown then w:=takefraction(v,mem
...
.int:=takefraction(mem[p+1].int,v);ptimesv:=mem[memtop-1].hhfield.rh;end;
...
while mem[mem[s].hhfield.lhfield+1].int>sx do begin r:=s;s:=mem[s].hhfield.rh;end;
if mem[s].hhfield.lhfield<>x then pwithxbecomingq:=p else begin mem[memtop-1].hhfield
.rh:=p;mem[r].hhfield.rh:=mem[s].hhfield.rh;v:=mem[s+1].int;freenode(s,2);
pwithxbecomingq:=pplusfq(mem[memtop-1].hhfield.rh,v,q,t,17);end;end;{:601}
{606:}procedure newdep(q,p:halfword);var r:halfword;
begin mem[q+1].hhfield.rh:=p;mem[q+1].hhfield.lhfield:=13;r:=mem[13].hhfield.rh;
mem[depfinal].hhfield.rh:=r;mem[r+1].hhfield.lhfield:=depfinal;mem[13].hhfield.rh:=q;end;
{:606}{607:}function constdependency(v:scaled):halfword;
begin depfinal:=getnode(2);mem[depfinal+1].int:=v;
mem[depfinal].hhfield.lhfield:=0;constdependency:=depfinal;end;{:607}{608:}
...
);mem[q+1].int:=twotothe[28-m];mem[q].hhfield.lhfield:=p;
mem[q].hhfield.rh:=constdependency(0);singledependency:=q;end;end;{:608}
{609:}function copydeplist(p:halfword):halfword;label 30;var q:halfword;
begin q:=getnode(2);depfinal:=q;
while true do begin mem[depfinal].hhfield.lhfield:=mem[p].hhfield.lhfield;
mem[depfinal+1].int:=mem[p+1].int;if mem[depfinal].hhfield.lhfield=0 then goto 30;
mem[depfinal].hhfield.rh:=getnode(2);depfinal:=mem[depfinal].hhfield.rh;
p:=mem[p].hhfield.rh;end;30:copydeplist:=q;end;{:609}{610:}
...
w:integer;begin{611:}q:=p;r:=mem[p].hhfield.rh;v:=mem[q+1].int;
while mem[r].hhfield.lhfield<>0 do begin if abs(mem[r+1].int)>abs(v)then begin q:=
r;v:=mem[r+1].int;end;r:=mem[r].hhfield.rh;end{:611};x:=mem[q].hhfield.lhfield;
n:=mem[x+1].int mod 64;{612:}s:=memtop-1;mem[s].hhfield.rh:=p;r:=p;
repeat if r=q then begin mem[s].hhfield.rh:=mem[r].hhfield.rh;freenode(r,2);
end else begin w:=makefraction(mem[r+1].int,v);
if abs(w)<=1342 then begin mem[s].hhfield.rh:=mem[r].hhfield.rh;freenode(r,2);
end else begin mem[r+1].int:=-w;s:=r;end;end;r:=mem[s].hhfield.rh;
until mem[r].hhfield.lhfield=0;
if t=18 then mem[r+1].int:=-makescaled(mem[r+1].int,v)else if v<>
-268435456 then mem[r+1].int:=-makefraction(mem[r+1].int,v);
finalnode:=r;p:=mem[memtop-1].hhfield.rh{:612};if internal[2]>0 then{613:}
...
{614:}prevr:=13;r:=mem[13].hhfield.rh;while r<>13 do begin s:=mem[r+1].hhfield.rh;
q:=pwithxbecomingq(s,x,p,mem[r].hhfield.b0);
if mem[q].hhfield.lhfield=0 then makeknown(r,q)else begin mem[r+1].hhfield.rh:=q;
repeat q:=mem[q].hhfield.rh;until mem[q].hhfield.lhfield=0;prevr:=q;end;
r:=mem[prevr].hhfield.rh;end{:614};{615:}if n>0 then{616:}begin s:=memtop-1;
mem[memtop-1].hhfield.rh:=p;r:=p;
repeat if n>30 then w:=0 else w:=mem[r+1].int div twotothe[n];
if(abs(w)<=1342)and(mem[r].hhfield.lhfield<>0)then begin mem[s].hhfield.rh:=mem[r].hhfield
.rh;freenode(r,2);end else begin mem[r+1].int:=w;s:=r;end;
r:=mem[s].hhfield.rh;until mem[s].hhfield.lhfield=0;p:=mem[memtop-1].hhfield.rh;end{:616};
if mem[p].hhfield.lhfield=0 then begin mem[x].hhfield.b0:=16;
...
curtype:=16;freenode(x,2);end;end else begin mem[x].hhfield.b0:=17;
...
begin q:=getnode(2);mem[q].hhfield.b1:=11;mem[q].hhfield.b0:=mem[p].hhfield.b0;
...
var t:smallnumber;q,r:halfword;begin t:=mem[p].hhfield.b0-1;q:=mem[p+1].int;
if flushp then mem[p].hhfield.b0:=1 else p:=q;repeat r:=mem[q+1].int;
mem[q].hhfield.b0:=t;case t of 2:mem[q+1].int:=v;4:begin mem[q+1].int:=v;
begin if strref[v]<127 then incr(strref[v]);end;end;
6:begin mem[q+1].int:=v;incr(mem[v].hhfield.lhfield);end;
...
if p<>0 then if mem[p].hhfield.rh=1 then printexp(p,0)else showtokenlist(p,0,
...
if curinput.namefield<>0 then print(hash[curinput.namefield].rh)else{640
:}begin p:=paramstack[curinput.limitfield];
if p=0 then showtokenlist(paramstack[curinput.limitfield+1],0,20,tally)
else begin q:=p;while mem[q].hhfield.rh<>0 do q:=mem[q].hhfield.rh;
mem[q].hhfield.rh:=paramstack[curinput.limitfield+1];
showtokenlist(p,0,20,tally);mem[q].hhfield.rh:=0;end;end{:640};print(500);
...
if p<>0 then if mem[p].hhfield.rh=1 then begin recyclevalue(p);freenode(p,2);
...
mem[curexp].hhfield.b0:=curtype;mem[curexp].hhfield.b1:=11;newdep(curexp,p);end;
{:856}{858:}procedure install(r,q:halfword);var p:halfword;
begin if mem[q].hhfield.b0=16 then begin mem[r+1].int:=mem[q+1].int;
mem[r].hhfield.b0:=16;
end else if mem[q].hhfield.b0=19 then begin p:=singledependency(q);
if p=depfinal then begin mem[r].hhfield.b0:=16;mem[r+1].int:=0;freenode(p,2);
end else begin mem[r].hhfield.b0:=17;newdep(r,p);end;
end else begin mem[r].hhfield.b0:=mem[q].hhfield.b0;
newdep(r,copydeplist(mem[q+1].hhfield.rh));end;end;{:858}
procedure makeexpcopy(p:halfword);label 20;var q,r,t:halfword;
begin 20:curtype:=mem[p].hhfield.b0;
...
6:begin curexp:=mem[p+1].int;incr(mem[curexp].hhfield.lhfield);end;
...
mem[t].hhfield.b1:=11;mem[t].hhfield.b0:=curtype;initbignode(t);
...
17,18:encapsulate(copydeplist(mem[p+1].hhfield.rh));
15:begin begin mem[p].hhfield.b0:=19;serialno:=serialno+64;
...
saveexp:=curexp;makeexpcopy(curmod);p:=stashcurexp;mem[p].hhfield.rh:=0;
curtype:=savetype;curexp:=saveexp;end else begin p:=getnode(2);
mem[p+1].int:=curmod;mem[p].hhfield.b1:=12;
if curcmd=42 then mem[p].hhfield.b0:=16 else mem[p].hhfield.b0:=4;
end else begin begin p:=avail;
if p=0 then p:=getavail else begin avail:=mem[p].hhfield.rh;mem[p].hhfield.rh:=0;
ifdef('STAT')incr(dynused);endif('STAT')end;end;mem[p].hhfield.lhfield:=cursym;
...
mem[p].hhfield.lhfield:=cursym;begintokenlist(p,19);end{:662};
...
2:begin print(627);helpline[3]:=628;cursym:=2363;end;3:begin print(629);
helpline[3]:=630;
if warninginfo=0 then cursym:=2367 else begin cursym:=2359;
eqtb[2359].rh:=warninginfo;end;end;4,5:begin print(631);
if scannerstatus=5 then print(hash[warninginfo].rh)else
printvariablename(warninginfo);cursym:=2365;end;6:begin print(632);
print(hash[warninginfo].rh);print(633);helpline[3]:=634;cursym:=2364;
end;{:664}end;inserror;end{:663}else begin begin if interaction=3 then;
printnl(261);print(615);end;printint(warninginfo);begin helpptr:=3;
helpline[2]:=616;helpline[1]:=617;helpline[0]:=618;end;
if cursym=0 then helpline[2]:=619;cursym:=2366;inserror;end;
deletionsallowed:=true;checkoutervalidity:=false;end;end;{:661}{666:}
procedure firmuptheline;forward;{:666}{667:}procedure getnext;
label 20,10,40,25,85,86,87,30;var k:0..bufsize;c:ASCIIcode;
class:ASCIIcode;n,f:integer;begin 20:cursym:=0;
if(curinput.indexfield<=15)then{669:}
...
.hhfield.lhfield;curinput.locfield:=mem[curinput.locfield].hhfield.rh;
if cursym>=2370 then if cursym>=2520 then{677:}
begin if cursym>=2670 then cursym:=cursym-150;
begintokenlist(paramstack[curinput.limitfield+cursym-(2520)],18);
...
begin if mem[curinput.locfield].hhfield.b1=12 then begin curmod:=mem[curinput
.locfield+1].int;
if mem[curinput.locfield].hhfield.b0=16 then curcmd:=42 else begin curcmd:=39
;begin if strref[curmod]<127 then incr(strref[curmod]);end;end;
end else begin curmod:=curinput.locfield;curcmd:=38;end;
curinput.locfield:=mem[curinput.locfield].hhfield.rh;goto 10;end{:678}
else begin endtokenlist;goto 20;end{:676};{668:}curcmd:=eqtb[cursym].lhfield;
...
balance:=1;mem[memtop-2].hhfield.rh:=0;while true do begin getnext;
if cursym>0 then begin{686:}begin q:=substlist;
while q<>0 do begin if mem[q].hhfield.lhfield=cursym then begin cursym:=mem[q+1]
.int;curcmd:=7;goto 40;end;q:=mem[q].hhfield.rh;end;40:end{:686};
...
2519+curmod;end{:690};end;mem[p].hhfield.rh:=curtok;p:=mem[p].hhfield.rh;end;
30:mem[p].hhfield.rh:=tailend;flushnodelist(substlist);
scantoks:=mem[memtop-2].hhfield.rh;end;{:685}{691:}procedure getsymbol;
...
flushstring(curmod);end;cursym:=2357;inserror;goto 20;end;end;{:691}
...
getsymbol;q:=getnode(2);mem[q].hhfield.lhfield:=cursym;mem[q+1].int:=2370;
getclearsymbol;warninginfo:=cursym;getsymbol;p:=getnode(2);
mem[p].hhfield.lhfield:=cursym;mem[p+1].int:=2371;mem[p].hhfield.rh:=q;getnext;
checkequals;scannerstatus:=5;q:=getavail;mem[q].hhfield.lhfield:=0;r:=getavail;
mem[q].hhfield.rh:=r;mem[r].hhfield.lhfield:=0;mem[r].hhfield.rh:=scantoks(16,p,0,0);
scannerstatus:=0;eqtb[warninginfo].lhfield:=m;eqtb[warninginfo].rh:=q;
...
if curcmd<>41 then clearsymbol(x,false);h:=getavail;mem[h].hhfield.lhfield:=x;
...
mem[t].hhfield.rh:=getavail;t:=mem[t].hhfield.rh;mem[t].hhfield.lhfield:=cursym;end;
30:if eqtb[x].lhfield<>41 then clearsymbol(x,false);
...
begin m:=curmod;c:=0;mem[memtop-2].hhfield.rh:=0;q:=getavail;mem[q].hhfield.lhfield:=0;
r:=0;{700:}if m=1 then begin getclearsymbol;warninginfo:=cursym;getnext;
scannerstatus:=5;n:=0;eqtb[warninginfo].lhfield:=10;eqtb[warninginfo].rh:=q;
end else begin p:=scandeclaredvariable;
flushvariable(eqtb[mem[p].hhfield.lhfield].rh,mem[p].hhfield.rh,true);
...
mem[warninginfo].hhfield.b0:=20+n;mem[warninginfo+1].int:=q;end{:700};k:=n;
...
helpline[0]:=683;end;backerror;base:=2370;end;{704:}
repeat mem[q].hhfield.rh:=getavail;q:=mem[q].hhfield.rh;mem[q].hhfield.lhfield:=base+k;
getsymbol;p:=getnode(2);mem[p+1].int:=base+k;mem[p].hhfield.lhfield:=cursym;
if k=150 then overflow(684,150);incr(k);mem[p].hhfield.rh:=r;r:=p;getnext;
...
if k=150 then overflow(684,150);incr(k);getsymbol;mem[p].hhfield.lhfield:=cursym;
mem[p].hhfield.rh:=r;r:=p;getnext;if c=4 then if curcmd=69 then begin c:=5;
p:=getnode(2);if k=150 then overflow(684,150);mem[p+1].int:=2370+k;
getsymbol;mem[p].hhfield.lhfield:=cursym;mem[p].hhfield.rh:=r;r:=p;getnext;end;
end{:705};checkequals;p:=getavail;mem[p].hhfield.lhfield:=c;mem[q].hhfield.rh:=p;{698:}
if m=1 then mem[p].hhfield.rh:=scantoks(16,r,0,n)else begin q:=getavail;
mem[q].hhfield.lhfield:=bgloc;mem[p].hhfield.rh:=q;p:=getavail;mem[p].hhfield.lhfield:=egloc;
mem[q].hhfield.rh:=scantoks(16,r,p,n);end;
...
begin if n<>0 then print(hash[n].rh)else begin p:=mem[a].hhfield.lhfield;
if p=0 then print(hash[mem[mem[mem[a].hhfield.rh].hhfield.lhfield].hhfield.lhfield].rh)else begin
q:=p;while mem[q].hhfield.rh<>0 do q:=mem[q].hhfield.rh;
mem[q].hhfield.rh:=mem[mem[a].hhfield.rh].hhfield.lhfield;showtokenlist(p,0,1000,0);
mem[q].hhfield.rh:=0;end;end;end;{:722}{723:}procedure printarg(q:halfword;
n:integer;b:halfword);
begin if mem[q].hhfield.rh=1 then printnl(497)else if(b<2670)and(b<>7)then
printnl(498)else printnl(499);printint(n);print(700);
if mem[q].hhfield.rh=1 then printexp(q,1)else showtokenlist(q,0,1000,0);end;
{:723}{730:}procedure scantextarg(ldelim,rdelim:halfword);label 30;
var balance:integer;p:halfword;begin warninginfo:=ldelim;
scannerstatus:=3;p:=memtop-2;balance:=1;mem[memtop-2].hhfield.rh:=0;
...
;mem[p].hhfield.rh:=curtok;p:=mem[p].hhfield.rh;end;
30:curexp:=mem[memtop-2].hhfield.rh;curtype:=20;scannerstatus:=0;end;{:730}
...
tail:halfword;begin r:=mem[defref].hhfield.rh;incr(mem[defref].hhfield.lhfield);
if arglist=0 then n:=0 else{724:}begin n:=1;tail:=arglist;
while mem[tail].hhfield.rh<>0 do begin incr(n);tail:=mem[tail].hhfield.rh;end;
...
repeat q:=mem[p].hhfield.lhfield;printarg(q,n,0);incr(n);p:=mem[p].hhfield.rh;
until p=0;end;enddiagnostic(false);end{:721};{725:}curcmd:=83;
while mem[r].hhfield.lhfield>=2370 do begin{726:}
...
if mem[r].hhfield.lhfield>=2520 then begin curexp:=0;curtype:=20;
end else begin curexp:=0;curtype:=16;end;backerror;curcmd:=62;goto 40;
end;ldelim:=cursym;rdelim:=curmod;end;{729:}
if mem[r].hhfield.lhfield>=2670 then scantextarg(ldelim,rdelim)else begin getxnext
;if mem[r].hhfield.lhfield>=2520 then scansuffix else scanexpression;end{:729};
if curcmd<>82 then{727:}
if(curcmd<>62)or(curmod<>ldelim)then if mem[mem[r].hhfield.rh].hhfield.lhfield>=2370
...
begin p:=getavail;
if curtype=20 then mem[p].hhfield.lhfield:=curexp else mem[p].hhfield.lhfield:=stashcurexp;
if internal[9]>0 then begin begindiagnostic;
printarg(mem[p].hhfield.lhfield,n,mem[r].hhfield.lhfield);enddiagnostic(false);end;
if arglist=0 then arglist:=p else mem[tail].hhfield.rh:=p;tail:=p;incr(n);
end{:728}{:726};r:=mem[r].hhfield.rh;end;
...
if mem[r].hhfield.lhfield<>0 then{733:}
begin if mem[r].hhfield.lhfield<7 then begin getxnext;
if mem[r].hhfield.lhfield<>6 then if(curcmd=51)or(curcmd=77)then getxnext;end;
case mem[r].hhfield.lhfield of 1:scanprimary;2:scansecondary;3:scantertiary;
4:scanexpression;5:{734:}begin scanexpression;p:=getavail;
mem[p].hhfield.lhfield:=stashcurexp;if internal[9]>0 then begin begindiagnostic;
printarg(mem[p].hhfield.lhfield,n,0);enddiagnostic(false);end;
if arglist=0 then arglist:=p else mem[tail].hhfield.rh:=p;tail:=p;incr(n);
...
printarg(mem[p].hhfield.lhfield,n,mem[r].hhfield.lhfield);enddiagnostic(false);end;
if arglist=0 then arglist:=p else mem[tail].hhfield.rh:=p;tail:=p;incr(n);
end{:728};end{:733};r:=mem[r].hhfield.rh{:725};{736:}
while(curinput.indexfield>15)and(curinput.locfield=0)do endtokenlist;
...
repeat paramstack[paramptr]:=mem[p].hhfield.lhfield;incr(paramptr);
p:=mem[p].hhfield.rh;until p=0;flushlist(arglist);end{:736};end;{:720}
...
cursym:=2362;inserror;end else begin begin if interaction=3 then;
...
{745:}begin p:=condptr;ifline:=mem[p+1].int;curif:=mem[p].hhfield.b1;
iflimit:=mem[p].hhfield.b0;condptr:=mem[p].hhfield.rh;freenode(p,2);end{:745};
...
until p<>0;if p<>mem[loopptr].hhfield.lhfield then fatalerror(696);stopiteration;
...
if mem[q].hhfield.rh=p then begin mem[q].hhfield.b0:=l;goto 10;end;
q:=mem[q].hhfield.rh;end;end;10:end;{:746}{747:}procedure checkcolon;
...
mem[p].hhfield.rh:=condptr;mem[p].hhfield.b0:=iflimit;mem[p].hhfield.b1:=curif;
...
begin p:=condptr;ifline:=mem[p+1].int;curif:=mem[p].hhfield.b1;
iflimit:=mem[p].hhfield.b0;condptr:=mem[p].hhfield.rh;freenode(p,2);end{:745};
...
if m=1 then begin mem[s+1].hhfield.lhfield:=1;p:=0;getxnext;goto 40;end;getsymbol;
p:=getnode(2);mem[p].hhfield.lhfield:=cursym;mem[p+1].int:=m;getxnext;
...
{764:}mem[s+1].hhfield.lhfield:=0;q:=s+1;mem[q].hhfield.rh:=0;repeat getxnext;
...
mem[s+1].hhfield.lhfield:=pp;goto 30;end{:765};curexp:=stashcurexp;end;
mem[q].hhfield.rh:=getavail;q:=mem[q].hhfield.rh;mem[q].hhfield.lhfield:=curexp;curtype:=1;
...
end{:756};{758:}q:=getavail;mem[q].hhfield.lhfield:=2358;scannerstatus:=6;
warninginfo:=n;mem[s].hhfield.lhfield:=scantoks(4,p,q,0);scannerstatus:=0;
mem[s].hhfield.rh:=loopptr;loopptr:=s{:758};resumeiteration;end;{:755}{760:}
procedure resumeiteration;label 45,10;var p,q:halfword;
begin p:=mem[loopptr+1].hhfield.lhfield;if p>1 then begin curexp:=mem[p+1].int;
if{761:}
...
end else if p<1 then begin p:=mem[loopptr+1].hhfield.rh;if p=0 then goto 45;
mem[loopptr+1].hhfield.rh:=mem[p].hhfield.rh;q:=mem[p].hhfield.lhfield;
begin mem[p].hhfield.rh:=avail;avail:=p;ifdef('STAT')decr(dynused);
endif('STAT')end;end else begin begintokenlist(mem[loopptr].hhfield.lhfield,16);
goto 10;end;begintokenlist(mem[loopptr].hhfield.lhfield,17);stackargument(q);
if internal[7]>65536 then{762:}begin begindiagnostic;printnl(733);
if(q<>0)and(mem[q].hhfield.rh=1)then printexp(q,1)else showtokenlist(q,0,50,0
);printchar(125);enddiagnostic(false);end{:762};goto 10;
45:stopiteration;10:end;{:760}{763:}procedure stopiteration;
var p,q:halfword;begin p:=mem[loopptr+1].hhfield.lhfield;
if p>1 then freenode(p,4)else if p<1 then begin q:=mem[loopptr+1].hhfield.rh;
while q<>0 do begin p:=mem[q].hhfield.lhfield;
if p<>0 then if mem[p].hhfield.rh=1 then begin recyclevalue(p);freenode(p,2);
end else flushtokenlist(p);p:=q;q:=mem[q].hhfield.rh;
begin mem[p].hhfield.rh:=avail;avail:=p;ifdef('STAT')decr(dynused);
endif('STAT')end;end;end;p:=loopptr;loopptr:=mem[p].hhfield.rh;
flushtokenlist(mem[p].hhfield.lhfield);freenode(p,2);end;{:763}{770:}
...
for k:=namelength+1 to FILENAMESIZE do nameoffile[k]:=' ';end;{:774}
{778:}procedure packbufferedname(n:smallnumber;a,b:integer);
...
MFbasedefault[j]];incr(k);
if k<=FILENAMESIZE then nameoffile[k]:=xchr[c];end;
if k<=FILENAMESIZE then namelength:=k else namelength:=FILENAMESIZE;
for k:=namelength+1 to FILENAMESIZE do nameoffile[k]:=' ';end;{:778}
{780:}function makenamestring:strnumber;var k:1..FILENAMESIZE;
...
promptfilename(751,749);end{:789};logname:=amakenamestring(logfile);
selector:=2;logopened:=true;{790:}
begin write(logfile,'This is METAFONT, C Version 2.7');print(baseident);
...
if namelength<FILENAMESIZE+4 then begin nameoffile[namelength+1]:=46;
...
print(curinput.namefield);flush(stdout);{794:}begin line:=1;
...
begin mem[p].hhfield.b0:=curtype;
if curtype=16 then mem[p+1].int:=curexp else begin if curtype=19 then{
829:}begin q:=singledependency(curexp);
if q=depfinal then begin mem[p].hhfield.b0:=16;mem[p+1].int:=0;freenode(q,2);
end else begin mem[p].hhfield.b0:=17;newdep(p,q);end;recyclevalue(curexp);
end{:829}else begin mem[p+1]:=mem[curexp+1];
mem[mem[p+1].hhfield.lhfield].hhfield.rh:=p;end;freenode(curexp,2);end;curtype:=1;end;
{:827}{848:}procedure backexpr;var p:halfword;begin p:=stashcurexp;
mem[p].hhfield.rh:=0;begintokenlist(p,19);end;{:848}{849:}
...
begin q:=getavail;r:=getavail;mem[q].hhfield.rh:=r;mem[q].hhfield.lhfield:=p;
mem[r].hhfield.lhfield:=stashcurexp;macrocall(c,q,n);end;{:863}{865:}
...
if mem[q].hhfield.b0=0 then begin begin if interaction=3 then;printnl(261);
print(803);end;begin helpptr:=2;helpline[1]:=804;helpline[0]:=574;end;
putgeterror;curexp:=3;goto 50;end else if mem[q].hhfield.b0=4 then{866:}
...
mem[p+2].int:=mem[p+2].int+ty;p:=mem[p].hhfield.rh;until p=q;end{:867};
...
if mem[p].hhfield.b0=16 then curx:=mem[p+1].int else begin disperr(p,812);
begin helpptr:=5;helpline[4]:=813;helpline[3]:=808;helpline[2]:=809;
helpline[1]:=810;helpline[0]:=811;end;putgeterror;recyclevalue(p);
curx:=0;end;
if mem[p+2].hhfield.b0=16 then cury:=mem[p+3].int else begin disperr(p+2,814)
...
mem[q].hhfield.b0:=0;mem[q].hhfield.b1:=0;mem[q].hhfield.rh:=q;knownpair;
...
curexp:=getnode(7);mem[curexp].hhfield.b0:=4;mem[curexp].hhfield.b1:=4;
mem[curexp].hhfield.rh:=curexp;mem[curexp+1].int:=0;mem[curexp+2].int:=0;
...
if mem[p].hhfield.b0=16 then if mem[p+2].hhfield.b0=16 then begin nicepair:=true;
...
if mem[p].hhfield.lhfield=0 then goto 10;p:=mem[p].hhfield.rh;end;10:end;{:904}{908:}
procedure pairtopath;begin curexp:=newknot;curtype:=9;end;{:908}{910:}
procedure takepart(c:quarterword);var p:halfword;
begin p:=mem[curexp+1].int;mem[18].int:=p;mem[17].hhfield.b0:=curtype;
mem[p].hhfield.rh:=17;freenode(curexp,2);makeexpcopy(p+2*(c-53));
...
begin p:=curexp;if mem[p].hhfield.b0=0 then n:=-65536 else n:=0;
repeat p:=mem[p].hhfield.rh;n:=n+65536;until p=curexp;pathlength:=n;end;
...
repeat q:=q-2;if mem[q].hhfield.b0<>16 then goto 30;until q=p;b:=30;30:end;
...
if curtype=17 then negatedeplist(mem[curexp+1].hhfield.rh)else if curtype=14
then begin p:=mem[curexp+1].int;
if mem[p].hhfield.b0=16 then mem[p+1].int:=-mem[p+1].int else negatedeplist(
mem[p+1].hhfield.rh);
if mem[p+2].hhfield.b0=16 then mem[p+3].int:=-mem[p+3].int else negatedeplist
(mem[p+3].hhfield.rh);end;recyclevalue(q);freenode(q,2);end;
17,18:negatedeplist(mem[curexp+1].hhfield.rh);16:curexp:=-curexp;
...
else if mem[curexp].hhfield.b0=0 then flushcurexp(0)else begin curpen:=3;
...
68:begin if curtype<>9 then flushcurexp(31)else if mem[curexp].hhfield.b0<>0
...
if mem[p].hhfield.b1=0 then p:=mem[p].hhfield.rh;tossknotlist(curexp);curexp:=p;
...
r:=q+bignodesize[mem[p].hhfield.b0];repeat r:=r-2;
if mem[r].hhfield.b0=19 then begin tarnished:=1;goto 10;end;until r=q;
tarnished:=0;10:end;{:928}{930:}{935:}procedure depfinish(v,q:halfword;
t:smallnumber);var p:halfword;vv:scaled;
begin if q=0 then p:=curexp else p:=q;mem[p+1].hhfield.rh:=v;mem[p].hhfield.b0:=t;
if mem[v].hhfield.lhfield=0 then begin vv:=mem[v+1].int;
if q=0 then flushcurexp(vv)else begin recyclevalue(p);mem[q].hhfield.b0:=16;
...
if t<17 then v:=curexp else v:=mem[curexp+1].hhfield.rh;
end else begin t:=mem[q].hhfield.b0;
if t<17 then v:=mem[q+1].int else v:=mem[q+1].hhfield.rh;end;
if t=16 then begin if c=70 then v:=-v;
if mem[p].hhfield.b0=16 then begin v:=slowadd(mem[p+1].int,v);
if q=0 then curexp:=v else mem[q+1].int:=v;goto 10;end;{931:}
r:=mem[p+1].hhfield.rh;while mem[r].hhfield.lhfield<>0 do r:=mem[r].hhfield.rh;
mem[r+1].int:=slowadd(mem[r+1].int,v);if q=0 then begin q:=getnode(2);
curexp:=q;curtype:=mem[p].hhfield.b0;mem[q].hhfield.b1:=11;end;
mem[q+1].hhfield.rh:=mem[p+1].hhfield.rh;mem[q].hhfield.b0:=mem[p].hhfield.b0;
mem[q+1].hhfield.lhfield:=mem[p+1].hhfield.lhfield;mem[mem[p+1].hhfield.lhfield].hhfield.rh:=q;
mem[p].hhfield.b0:=16;{:931};end else begin if c=70 then negatedeplist(v);
{932:}if mem[p].hhfield.b0=16 then{933:}
begin while mem[v].hhfield.lhfield<>0 do v:=mem[v].hhfield.rh;
mem[v+1].int:=slowadd(mem[p+1].int,mem[v+1].int);end{:933}
else begin s:=mem[p].hhfield.b0;r:=mem[p+1].hhfield.rh;
...
begin if p=0 then q:=curexp else if mem[p].hhfield.b0<>16 then q:=p else
begin if visscaled then mem[p+1].int:=takescaled(mem[p+1].int,v)else mem
[p+1].int:=takefraction(mem[p+1].int,v);goto 10;end;t:=mem[q].hhfield.b0;
q:=mem[q+1].hhfield.rh;s:=t;
...
begin if mem[p].hhfield.b0=14 then begin q:=stashcurexp;unstashcurexp(p);
p:=q;end;r:=mem[curexp+1].int;u:=mem[r+1].int;v:=mem[r+3].int;{947:}
mem[r+2].hhfield.b0:=mem[p].hhfield.b0;newdep(r+2,copydeplist(mem[p+1].hhfield.rh));
mem[r].hhfield.b0:=mem[p].hhfield.b0;mem[r+1]:=mem[p+1];
mem[mem[p+1].hhfield.lhfield].hhfield.rh:=r;freenode(p,2){:947};depmult(r,u,true);
...
t:=mem[q].hhfield.b0;q:=mem[q+1].hhfield.rh;s:=t;
...
84:if mem[p].hhfield.b0=16 then{958:}
...
end{:958};85:if mem[p].hhfield.b0>14 then begin install(q+6,p);goto 30;end;
86:if mem[p].hhfield.b0>14 then begin install(q+4,p);install(q+10,p);goto 30;
end;87:if mem[p].hhfield.b0=14 then begin r:=mem[p+1].int;install(q,r);
install(q+2,r+2);goto 30;end;
89:if mem[p].hhfield.b0>14 then begin install(q+4,p);goto 30;end;
90:if mem[p].hhfield.b0>14 then begin install(q+10,p);goto 30;end;
91:if mem[p].hhfield.b0=14 then{959:}begin r:=mem[p+1].int;install(q+4,r);
install(q+10,r);install(q+8,r+2);
if mem[r+2].hhfield.b0=16 then mem[r+3].int:=-mem[r+3].int else negatedeplist
(mem[r+3].hhfield.rh);install(q+6,r+2);goto 30;end{:959};88:;{:957}end;
...
if mem[r].hhfield.b0<>16 then goto 10;until r=q;txx:=mem[q+5].int;
...
repeat if mem[q].hhfield.b0<>0 then trans(q+3,q+4);trans(q+1,q+2);
if mem[q].hhfield.b1<>0 then trans(q+5,q+6);q:=mem[q].hhfield.rh;until q=curexp;
...
if(toint(mem[curedges+2].hhfield.lhfield)+tx<=0)or(mem[curedges+2].hhfield.rh+tx>=8192)
or(toint(mem[curedges+1].hhfield.lhfield)+ty<=0)or(mem[curedges+1].hhfield.rh+ty>=8191)
...
end else begin if tx<>0 then begin if not(abs(mem[curedges+3].hhfield.lhfield-tx
-4096)<4096)then fixoffset;
mem[curedges+2].hhfield.lhfield:=mem[curedges+2].hhfield.lhfield+tx;
mem[curedges+2].hhfield.rh:=mem[curedges+2].hhfield.rh+tx;
mem[curedges+3].hhfield.lhfield:=mem[curedges+3].hhfield.lhfield-tx;mem[curedges+4].int:=0;
end;if ty<>0 then begin mem[curedges+1].hhfield.lhfield:=mem[curedges+1].hhfield.lhfield+ty;
mem[curedges+1].hhfield.rh:=mem[curedges+1].hhfield.rh+ty;
mem[curedges+5].hhfield.lhfield:=mem[curedges+5].hhfield.lhfield+ty;mem[curedges+4].int:=0;
...
if u<>0 then if mem[q].hhfield.b0=16 then delta:=delta+takescaled(mem[q+1]
.int,u)else begin{969:}
if mem[p].hhfield.b0<>18 then begin if mem[p].hhfield.b0=16 then newdep(p,
constdependency(mem[p+1].int))else mem[p+1].hhfield.rh:=ptimesv(mem[p+1].hhfield
.rh,65536,17,18,true);mem[p].hhfield.b0:=18;end{:969};
mem[p+1].hhfield.rh:=pplusfq(mem[p+1].hhfield.rh,u,mem[q+1].hhfield.rh,18,mem[q].hhfield.b0)
;end;
if mem[p].hhfield.b0=16 then mem[p+1].int:=mem[p+1].int+delta else begin r:=
mem[p+1].hhfield.rh;while mem[r].hhfield.lhfield<>0 do r:=mem[r].hhfield.rh;
delta:=mem[r+1].int+delta;
if r<>mem[p+1].hhfield.rh then mem[r+1].int:=delta else begin recyclevalue(p)
;mem[p].hhfield.b0:=16;mem[p+1].int:=delta;end;end;
if fixneeded then fixdependencies;end;{:968}{971:}
procedure addmultdep(p:halfword;v:scaled;r:halfword);
begin if mem[r].hhfield.b0=16 then mem[depfinal+1].int:=mem[depfinal+1].int+
takescaled(mem[r+1].int,v)else begin mem[p+1].hhfield.rh:=pplusfq(mem[p+1].hhfield
.rh,v,mem[r+1].hhfield.rh,18,mem[r].hhfield.b0);if fixneeded then fixdependencies;
end;end;{:971}{972:}procedure bilin2(p,t:halfword;v:scaled;
u,q:halfword);var vv:scaled;begin vv:=mem[p+1].int;mem[p].hhfield.b0:=18;
newdep(p,constdependency(0));if vv<>0 then addmultdep(p,vv,t);
if v<>0 then addmultdep(p,v,u);if q<>0 then addmultdep(p,65536,q);
if mem[p+1].hhfield.rh=depfinal then begin vv:=mem[depfinal+1].int;
recyclevalue(p);mem[p].hhfield.b0:=16;mem[p+1].int:=vv;end;end;{:972}{974:}
...
var q,r,pp,qq:halfword;s:smallnumber;begin s:=bignodesize[mem[p].hhfield.b0];
q:=mem[p+1].int;r:=q+s;repeat r:=r-2;if mem[r].hhfield.b0<>16 then{967:}
...
if a<=b then reversed:=false else begin reversed:=true;k:=a;a:=b;b:=k;
end;{979:}if a<0 then if mem[curexp].hhfield.b0=0 then begin a:=0;
if b<0 then b:=0;end else repeat a:=a+l;b:=b+l;until a>=0;
if b>l then if mem[curexp].hhfield.b0=0 then begin b:=l;if a>l then a:=l;
end else while a>=l do begin a:=a-l;b:=b-l;end{:979};q:=curexp;
while a>=65536 do begin q:=mem[q].hhfield.rh;a:=a-65536;b:=b-65536;end;
if b=a then{981:}begin if a>0 then begin qq:=mem[q].hhfield.rh;
splitcubic(q,a*4096,mem[qq+1].int,mem[qq+2].int);q:=mem[q].hhfield.rh;end;
pp:=copyknot(q);qq:=pp;end{:981}else{980:}begin pp:=copyknot(q);qq:=pp;
repeat q:=mem[q].hhfield.rh;rr:=qq;qq:=copyknot(q);mem[rr].hhfield.rh:=qq;
b:=b-65536;until b<=0;if a>0 then begin ss:=pp;pp:=mem[pp].hhfield.rh;
splitcubic(ss,a*4096,mem[pp+1].int,mem[pp+2].int);pp:=mem[ss].hhfield.rh;
...
.int);freenode(qq,7);qq:=mem[rr].hhfield.rh;end;end{:980};mem[pp].hhfield.b0:=0;
mem[qq].hhfield.b1:=0;mem[qq].hhfield.rh:=pp;tossknotlist(curexp);
if reversed then begin curexp:=mem[htapypoc(pp)].hhfield.rh;tossknotlist(pp);
end else curexp:=pp;end;{:978}{982:}procedure pairvalue(x,y:scaled);
var p:halfword;begin p:=getnode(2);flushcurexp(p);curtype:=14;
mem[p].hhfield.b0:=14;mem[p].hhfield.b1:=11;initbignode(p);p:=mem[p+1].int;
mem[p].hhfield.b0:=16;mem[p+1].int:=x;mem[p+2].hhfield.b0:=16;mem[p+3].int:=y;end;
...
if mem[p].hhfield.b0=0 then n:=-65536 else n:=0;repeat p:=mem[p].hhfield.rh;
n:=n+65536;until p=curexp;
if n=0 then v:=0 else if v<0 then if mem[p].hhfield.b0=0 then v:=0 else v:=n
-1-((-v-1)mod n)else if v>n then if mem[p].hhfield.b0=0 then v:=n else v:=v
mod n;p:=curexp;while v>=65536 do begin p:=mem[p].hhfield.rh;v:=v-65536;end;
if v<>0 then{986:}begin q:=mem[p].hhfield.rh;
splitcubic(p,v*4096,mem[q+1].int,mem[q+2].int);p:=mem[p].hhfield.rh;end{:986}
;{987:}case c of 97:pairvalue(mem[p+1].int,mem[p+2].int);
98:if mem[p].hhfield.b0=0 then pairvalue(mem[p+1].int,mem[p+2].int)else
pairvalue(mem[p+3].int,mem[p+4].int);
99:if mem[p].hhfield.b1=0 then pairvalue(mem[p+1].int,mem[p+2].int)else
...
case mem[p].hhfield.b0 of 13,14:oldp:=tarnished(p);19:oldp:=1;
...
if(curtype<14)or(mem[p].hhfield.b0<14)then if(curtype=11)and(mem[p].hhfield.b0=11)
then begin if c=70 then negateedges(curexp);curedges:=curexp;
mergeedges(mem[p+1].int);
end else badbinary(p,c)else if curtype=14 then if mem[p].hhfield.b0<>14 then
badbinary(p,c)else begin q:=mem[p+1].int;r:=mem[curexp+1].int;
addorsubtract(q,r,c);addorsubtract(q+2,r+2,c);
end else if mem[p].hhfield.b0=14 then badbinary(p,c)else addorsubtract(p,0,c)
{:929};{936:}
77,78,79,80,81,82:begin if(curtype>14)and(mem[p].hhfield.b0>14)then
addorsubtract(p,0,70)else if curtype<>mem[p].hhfield.b0 then begin badbinary(
p,c);goto 30;
...
if mem[r].hhfield.b0<>16 then goto 31;if mem[r+1].int<>0 then goto 31;
...
30:end;{:936}{940:}
76,75:if(mem[p].hhfield.b0<>2)or(curtype<>2)then badbinary(p,c)else if mem[p
+1].int=c-45 then curexp:=mem[p+1].int;{:940}{941:}
71:if(curtype<14)or(mem[p].hhfield.b0<14)then badbinary(p,71)else if(curtype=
16)or(mem[p].hhfield.b0=16)then{942:}
begin if mem[p].hhfield.b0=16 then begin v:=mem[p+1].int;freenode(p,2);
...
else if(nicepair(p,mem[p].hhfield.b0)and(curtype>14))or(nicepair(curexp,
curtype)and(mem[p].hhfield.b0>14))then begin hardtimes(p);goto 10;
end else badbinary(p,71);{:941}{948:}
72:if(curtype<>16)or(mem[p].hhfield.b0<14)then badbinary(p,72)else begin v:=
...
73,74:if(curtype=16)and(mem[p].hhfield.b0=16)then if c=73 then curexp:=
pythadd(mem[p+1].int,curexp)else curexp:=pythsub(mem[p+1].int,curexp)
else badbinary(p,c);{:951}{952:}
84,85,86,87,88,89,90,91:if(mem[p].hhfield.b0=9)or(mem[p].hhfield.b0=8)or(mem[p].hhfield
.b0=6)then begin pathtrans(p,c);goto 10;
end else if(mem[p].hhfield.b0=14)or(mem[p].hhfield.b0=13)then bigtrans(p,c)else if
mem[p].hhfield.b0=11 then begin edgestrans(p,c);goto 10;
end else badbinary(p,c);{:952}{975:}
83:if(curtype=4)and(mem[p].hhfield.b0=4)then cat(p)else badbinary(p,83);
94:if nicepair(p,mem[p].hhfield.b0)and(curtype=4)then chopstring(mem[p+1].int
)else badbinary(p,94);95:begin if curtype=14 then pairtopath;
if nicepair(p,mem[p].hhfield.b0)and(curtype=9)then choppath(mem[p+1].int)else
badbinary(p,95);end;{:975}{983:}
97,98,99:begin if curtype=14 then pairtopath;
if(curtype=9)and(mem[p].hhfield.b0=16)then findpoint(mem[p+1].int,c)else
badbinary(p,c);end;100:begin if curtype=8 then materializepen;
if(curtype=6)and nicepair(p,mem[p].hhfield.b0)then setupoffset(mem[p+1].int)
else badbinary(p,100);end;96:begin if curtype=14 then pairtopath;
if(curtype=9)and nicepair(p,mem[p].hhfield.b0)then setupdirectiontime(mem[p+1
].int)else badbinary(p,96);end;{:983}{988:}
92:begin if mem[p].hhfield.b0=14 then begin q:=stashcurexp;unstashcurexp(p);
pairtopath;p:=stashcurexp;unstashcurexp(q);end;
if curtype=14 then pairtopath;
if(curtype=9)and(mem[p].hhfield.b0=9)then begin pathintersection(mem[p+1].int
...
end{:1166};{1167:}prevn:=4096;p:=mem[curedges].hhfield.lhfield;
n:=mem[curedges+1].hhfield.rh-4096;while p<>curedges do begin{1169:}
if mem[p+1].hhfield.lhfield>1 then sortedges(p);q:=mem[p+1].hhfield.rh;w:=0;
prevm:=-268435456;ww:=0;prevw:=0;m:=prevm;
repeat if q=memtop then mm:=268435456 else begin d:=mem[q].hhfield.lhfield;
...
begin if prevn=4096 then begin gfboc(mem[curedges+2].hhfield.lhfield+xoff-4096,mem
[curedges+2].hhfield.rh+xoff-4096,mem[curedges+1].hhfield.lhfield+yoff-4096,n+yoff);
curminm:=mem[curedges+2].hhfield.lhfield-4096+mem[curedges+3].hhfield.lhfield;
...
prevm:=m;prevw:=w;end{:1171};m:=mm;end;w:=ww;q:=mem[q].hhfield.rh;
until mm=268435456;if w<>0 then printnl(1054);
if prevm-toint(mem[curedges+3].hhfield.lhfield)+xoff>gfmaxm then gfmaxm:=prevm-mem
[curedges+3].hhfield.lhfield+xoff{:1169};p:=mem[p].hhfield.lhfield;decr(n);end;
...
t:=mem[l].hhfield.b0;if t=16 then begin t:=17;
p:=constdependency(-mem[l+1].int);q:=p;
end else if t=19 then begin t:=17;p:=singledependency(l);
mem[p+1].int:=-mem[p+1].int;q:=depfinal;
end else begin p:=mem[l+1].hhfield.rh;q:=p;
while true do begin mem[q+1].int:=-mem[q+1].int;
if mem[q].hhfield.lhfield=0 then goto 30;q:=mem[q].hhfield.rh;end;
30:mem[mem[l+1].hhfield.lhfield].hhfield.rh:=mem[q].hhfield.rh;
mem[mem[q].hhfield.rh+1].hhfield.lhfield:=mem[l+1].hhfield.lhfield;mem[l].hhfield.b0:=16;end{:1007};
{1009:}
if r=0 then if curtype=16 then begin mem[q+1].int:=mem[q+1].int+curexp;
goto 31;end else begin tt:=curtype;
if tt=19 then pp:=singledependency(curexp)else pp:=mem[curexp+1].hhfield.rh;
end else if mem[r].hhfield.b0=16 then begin mem[q+1].int:=mem[q+1].int+mem[r
+1].int;goto 31;end else begin tt:=mem[r].hhfield.b0;
if tt=19 then pp:=singledependency(r)else pp:=mem[r+1].hhfield.rh;end;
...
,17)else begin q:=p;
while mem[q].hhfield.lhfield<>0 do begin mem[q+1].int:=roundfraction(mem[q+1].int)
;q:=mem[q].hhfield.rh;end;t:=18;p:=pplusq(p,pp,t);end;watchcoefs:=true;
{:1010};if copied then flushnodelist(pp);31:{:1009};
if mem[p].hhfield.lhfield=0 then{1008:}
...
if r=0 then if curtype<>16 then if mem[curexp].hhfield.b0=16 then begin pp:=
...
var t:smallnumber;v:integer;p,q:halfword;begin 20:t:=mem[lhs].hhfield.b0;
...
if mem[lhs].hhfield.b0<=14 then printtype(mem[lhs].hhfield.b0)else print(339);
...
if curtype=10 then if mem[lhs].hhfield.b0=14 then begin p:=stashcurexp;
...
if mem[lhs].hhfield.lhfield>2369 then print(intname[mem[lhs].hhfield.lhfield-(2369)])else
showtokenlist(lhs,0,1000,0);print(460);printexp(0,0);printchar(125);
enddiagnostic(false);end{:998};if mem[lhs].hhfield.lhfield>2369 then{999:}
if curtype=16 then internal[mem[lhs].hhfield.lhfield-(2369)]:=curexp else begin
disperr(0,884);print(intname[mem[lhs].hhfield.lhfield-(2369)]);print(885);
...
mem[p].hhfield.b0:=curtype;mem[p+1].int:=0;makeexpcopy(p);p:=stashcurexp;
...
flushvariable(eqtb[mem[p].hhfield.lhfield].rh,mem[p].hhfield.rh,false);
q:=findvariable(p);if q<>0 then begin mem[q].hhfield.b0:=t;mem[q+1].int:=0;
...
t:halfword;begin m:=curmod;repeat getsymbol;t:=eqtb[cursym].lhfield;
if m=0 then begin if t>=86 then eqtb[cursym].lhfield:=t-86;
end else if t<86 then eqtb[cursym].lhfield:=t+86;getxnext;until curcmd<>82;
...
eqtb[ldelim].lhfield:=31;eqtb[ldelim].rh:=rdelim;eqtb[rdelim].lhfield:=62;
...
case curcmd of 10,53,44,49:incr(mem[curmod].hhfield.lhfield);others:end;
clearsymbol(l,false);eqtb[l].lhfield:=curcmd;
...
getclearsymbol;incr(intptr);eqtb[cursym].lhfield:=40;eqtb[cursym].rh:=intptr;
...
if eqtb[cursym].lhfield>=86 then print(938);printcmdmod(curcmd,curmod);
...
begin if mem[p].hhfield.b0=21 then{1047:}begin q:=mem[p+1].hhfield.lhfield;
repeat dispvar(q);q:=mem[q].hhfield.rh;until q=17;q:=mem[p+1].hhfield.rh;
while mem[q].hhfield.b1=3 do begin dispvar(q);q:=mem[q].hhfield.rh;end;end{:1047}
else if mem[p].hhfield.b0>=22 then{1048:}begin printnl(283);
printvariablename(p);if mem[p].hhfield.b0>22 then print(662);print(950);
if fileoffset>=maxprintline-20 then n:=5 else n:=maxprintline-fileoffset
-15;showmacro(mem[p+1].int,0,n);end{:1048}
else if mem[p].hhfield.b0<>0 then begin printnl(283);printvariablename(p);
...
var p:halfword;begin p:=mem[13].hhfield.rh;
while p<>13 do begin if interesting(p)then begin printnl(283);
printvariablename(p);
if mem[p].hhfield.b0=17 then printchar(61)else print(765);
printdependency(mem[p+1].hhfield.rh,mem[p].hhfield.b0);end;p:=mem[p+1].hhfield.rh;
while mem[p].hhfield.lhfield<>0 do p:=mem[p].hhfield.rh;p:=mem[p].hhfield.rh;end;getxnext;
...
end else if mem[p].hhfield.b0<>11 then begin begin if interaction=3 then;
printnl(261);print(787);end;showtokenlist(t,0,1000,0);print(965);
printtype(mem[p].hhfield.b0);printchar(41);begin helpptr:=2;helpline[1]:=966;
...
1063:}
begin if mem[curpen].hhfield.lhfield=0 then tosspen(curpen)else decr(mem[curpen]
.hhfield.lhfield);curpen:=curexp;end{:1063};{1064:}findedgesvar(lhs);
if curedges=0 then tossknotlist(rhs)else begin lhs:=0;
curpathtype:=addtotype;
if mem[rhs].hhfield.b0=0 then if curpathtype=0 then{1065:}
if mem[rhs].hhfield.rh=rhs then{1066:}begin mem[rhs+5].int:=mem[rhs+1].int;
mem[rhs+6].int:=mem[rhs+2].int;mem[rhs+3].int:=mem[rhs+1].int;
mem[rhs+4].int:=mem[rhs+2].int;mem[rhs].hhfield.b0:=1;mem[rhs].hhfield.b1:=1;
end{:1066}else begin p:=htapypoc(rhs);q:=mem[p].hhfield.rh;
mem[pathtail+5].int:=mem[q+5].int;mem[pathtail+6].int:=mem[q+6].int;
mem[pathtail].hhfield.b1:=mem[q].hhfield.b1;mem[pathtail].hhfield.rh:=mem[q].hhfield.rh;
freenode(q,7);mem[p+5].int:=mem[rhs+5].int;mem[p+6].int:=mem[rhs+6].int;
mem[p].hhfield.b1:=mem[rhs].hhfield.b1;mem[p].hhfield.rh:=mem[rhs].hhfield.rh;
...
if(turningnumber<0)and(mem[curpen].hhfield.rh=0)then curwt:=-curwt else begin
if turningnumber=0 then if(internal[39]<=65536)and(mem[curpen].hhfield.rh=0)
...
45:end{:1064};
if mem[curpen].hhfield.lhfield=0 then tosspen(curpen)else decr(mem[curpen].hhfield.lhfield);
...
printnl(baseident){:1200};{1190:}dumpint(426472440);dumpint(0);
dumpint(memtop);dumpint(2100);dumpint(1777);dumpint(15){:1190};{1192:}
...
varused:=varused+q-p;p:=q+mem[q].hhfield.lhfield;q:=mem[q+1].hhfield.rh;until q=rover;
...
begin p:=memtop-1;while true do begin q:=mem[p].hhfield.rh;
if v<=mem[q+1].int then goto 40;p:=q;end;
40:if v<mem[q+1].int then begin r:=getnode(2);mem[r+1].int:=v;
mem[r].hhfield.rh:=q;mem[p].hhfield.rh:=r;end;sortin:=mem[p].hhfield.rh;end;{:1117}
{1118:}function mincover(d:scaled):integer;var p:halfword;l:scaled;
m:integer;begin m:=0;p:=mem[memtop-1].hhfield.rh;perturbation:=2147483647;
while p<>19 do begin incr(m);l:=mem[p+1].int;repeat p:=mem[p].hhfield.rh;
...
perturbation:=0;q:=memtop-1;m:=0;p:=mem[memtop-1].hhfield.rh;
while p<>19 do begin incr(m);l:=mem[p+1].int;mem[p].hhfield.lhfield:=m;
if mem[mem[p].hhfield.rh+1].int<=l+d then{1122:}begin repeat p:=mem[p].hhfield.rh;
mem[p].hhfield.lhfield:=m;decr(excess);if excess=0 then d:=0;
until mem[mem[p].hhfield.rh+1].int>l+d;v:=l+(mem[p+1].int-l)div 2;
if mem[p+1].int-v>perturbation then perturbation:=mem[p+1].int-v;r:=q;
repeat r:=mem[r].hhfield.rh;mem[r+1].int:=v;until r=p;mem[q].hhfield.rh:=p;
end{:1122};q:=p;p:=mem[p].hhfield.rh;end;skimp:=m;end;{:1121}{1123:}
...
undumpint(x);if x<>2100 then goto 6666;undumpint(x);
if x<>1777 then goto 6666;undumpint(x);if x<>15 then goto 6666{:1191};
...
if(p>lomemmax)or((q>=mem[q+1].hhfield.rh)and(mem[q+1].hhfield.rh<>rover))then goto
6666;q:=mem[q+1].hhfield.rh;until q=rover;
...
if(x<1)or(x>2357)then goto 6666 else hashused:=x;end;p:=0;
...
for p:=hashused+1 to 2369 do begin undumphh(hash[p]);undumphh(eqtb[p]);
...
begin undumpint(x);if(x<0)or(x>2357)then goto 6666 else startsym:=x;end;
...
end;begin undumpint(x);if(x<1)or(x>2369)then goto 6666 else bgloc:=x;
end;begin undumpint(x);if(x<1)or(x>2369)then goto 6666 else egloc:=x;
...
begin p:=getnode(2);mem[p].hhfield.b0:=14;mem[p].hhfield.b1:=11;initbignode(p);
...
mem[p].hhfield.lhfield:=0;mem[p].hhfield.rh:=saveptr;saveptr:=p;end;repeat dostatement;
...
if curcmd=77 then begin curexp:=getavail;mem[curexp].hhfield.lhfield:=q+2369;
...
if prehead=0 then prehead:=getavail else begin avail:=mem[prehead].hhfield.rh
;mem[prehead].hhfield.rh:=0;ifdef('STAT')incr(dynused);endif('STAT')end;end;
tail:=prehead;posthead:=0;tt:=1;while true do begin t:=curtok;
mem[tail].hhfield.rh:=t;if tt<>0 then begin{850:}begin p:=mem[prehead].hhfield.rh;
q:=mem[p].hhfield.lhfield;tt:=0;if eqtb[q].lhfield mod 86=41 then begin q:=eqtb[q].rh;
if q=0 then goto 32;while true do begin p:=mem[p].hhfield.rh;
if p=0 then begin tt:=mem[q].hhfield.b0;goto 32;end;
if mem[q].hhfield.b0<>21 then goto 32;q:=mem[mem[q+1].hhfield.lhfield].hhfield.rh;
if p>=himemmin then begin repeat q:=mem[q].hhfield.rh;
until mem[q+2].hhfield.lhfield>=mem[p].hhfield.lhfield;
if mem[q+2].hhfield.lhfield>mem[p].hhfield.lhfield then goto 32;end;end;end;32:end{:850};
if tt>=22 then{845:}begin mem[tail].hhfield.rh:=0;
if tt>22 then begin posthead:=getavail;tail:=posthead;
mem[tail].hhfield.rh:=t;tt:=0;macroref:=mem[q+1].int;
incr(mem[macroref].hhfield.lhfield);end else{853:}begin p:=getavail;
mem[prehead].hhfield.lhfield:=mem[prehead].hhfield.rh;mem[prehead].hhfield.rh:=p;
mem[p].hhfield.lhfield:=t;macrocall(mem[q+1].int,prehead,0);getxnext;goto 20;
...
backexpr;curcmd:=63;curmod:=0;cursym:=2360;end{:847}
...
begin backinput;p:=getavail;q:=mem[posthead].hhfield.rh;
mem[prehead].hhfield.lhfield:=mem[prehead].hhfield.rh;mem[prehead].hhfield.rh:=posthead;
mem[posthead].hhfield.lhfield:=q;mem[posthead].hhfield.rh:=p;
mem[p].hhfield.lhfield:=mem[q].hhfield.rh;mem[q].hhfield.rh:=0;
macrocall(macroref,prehead,0);decr(mem[macroref].hhfield.lhfield);getxnext;
goto 20;end{:854};q:=mem[prehead].hhfield.rh;begin mem[prehead].hhfield.rh:=avail;
...
curmod:=0;cursym:=2360;end{:847};unstashcurexp(p);
...
then begin p:=getavail;mem[p].hhfield.lhfield:=cursym;end else goto 30;
mem[t].hhfield.rh:=p;t:=p;getxnext;end;30:curexp:=mem[h].hhfield.rh;
begin mem[h].hhfield.rh:=avail;avail:=h;ifdef('STAT')decr(dynused);
...
d:=curcmd;if d=53 then begin macname:=cursym;incr(mem[c].hhfield.lhfield);end;
getxnext;scanprimary;if d<>53 then dobinary(p,c)else begin backinput;
binarymac(p,c,macname);decr(mem[c].hhfield.lhfield);getxnext;goto 20;end;goto 22;
...
d:=curcmd;if d=44 then begin macname:=cursym;incr(mem[c].hhfield.lhfield);end;
...
if d=49 then begin macname:=cursym;incr(mem[c].hhfield.lhfield);end;
if(d<48)or((d=48)and((mem[p].hhfield.b0=14)or(mem[p].hhfield.b0=9)))then{869:}
begin cyclehit:=false;{870:}begin unstashcurexp(p);
if curtype=14 then p:=newknot else if curtype=9 then p:=curexp else goto
10;q:=p;while mem[q].hhfield.rh<>p do q:=mem[q].hhfield.rh;
if mem[p].hhfield.b0<>0 then begin r:=copyknot(p);mem[q].hhfield.rh:=r;q:=r;end;
mem[p].hhfield.b0:=4;mem[q].hhfield.b1:=4;end{:870};25:{874:}
if curcmd=46 then{879:}begin t:=scandirection;
if t<>4 then begin mem[q].hhfield.b1:=t;mem[q+5].int:=curexp;
if mem[q].hhfield.b0=4 then begin mem[q].hhfield.b0:=t;mem[q+3].int:=curexp;end;
...
else if curcmd=57 then{884:}begin mem[q].hhfield.b1:=1;t:=1;getxnext;
...
begin t:=scandirection;if mem[q].hhfield.b1<>1 then x:=curexp else t:=1;
end{:880}else if mem[q].hhfield.b1<>1 then begin t:=4;x:=0;end{:874};
...
while mem[qq].hhfield.rh<>pp do qq:=mem[qq].hhfield.rh;
if mem[pp].hhfield.b0<>0 then begin r:=copyknot(pp);mem[qq].hhfield.rh:=r;qq:=r;
end;mem[pp].hhfield.b0:=4;mem[qq].hhfield.b1:=4;end{:885};end;{887:}
...
{889:}if mem[pp].hhfield.b1=4 then if(t=3)or(t=2)then begin mem[pp].hhfield.b1:=t;
mem[pp+5].int:=x;end{:889};if d=48 then{890:}
begin if mem[q].hhfield.b0=4 then if mem[q].hhfield.b1=4 then begin mem[q].hhfield.b0:=
3;mem[q+3].int:=65536;end;
if mem[pp].hhfield.b1=4 then if t=4 then begin mem[pp].hhfield.b1:=3;
mem[pp+5].int:=65536;end;mem[q].hhfield.b1:=mem[pp].hhfield.b1;
mem[q].hhfield.rh:=mem[pp].hhfield.rh;mem[q+5].int:=mem[pp+5].int;
mem[q+6].int:=mem[pp+6].int;freenode(pp,7);if qq=pp then qq:=q;end{:890}
else begin{888:}
if mem[q].hhfield.b1=4 then if(mem[q].hhfield.b0=3)or(mem[q].hhfield.b0=2)then begin
mem[q].hhfield.b1:=mem[q].hhfield.b0;mem[q+5].int:=mem[q+3].int;end{:888};
mem[q].hhfield.rh:=pp;mem[pp+4].int:=y;if t<>4 then begin mem[pp+3].int:=x;
mem[pp].hhfield.b0:=t;end;end;q:=qq;end{:887};
if curcmd>=46 then if curcmd<=48 then if not cyclehit then goto 25;
26:{891:}if cyclehit then begin if d=48 then p:=q;
end else begin mem[p].hhfield.b0:=0;
if mem[p].hhfield.b1=4 then begin mem[p].hhfield.b1:=3;mem[p+5].int:=65536;end;
mem[q].hhfield.b1:=0;if mem[q].hhfield.b0=4 then begin mem[q].hhfield.b0:=3;
mem[q+3].int:=65536;end;mem[q].hhfield.rh:=p;end;makechoices(p);curtype:=9;
...
decr(mem[c].hhfield.lhfield);getxnext;goto 20;end;end;goto 22;end;10:end;{:868}
...
mem[rover].hhfield.rh:=65535;lomemmax:=himemmin-1;
if lomemmax-rover>65535 then lomemmax:=65535+rover;
mem[rover].hhfield.lhfield:=lomemmax-rover;mem[rover+1].hhfield.lhfield:=rover;
mem[rover+1].hhfield.rh:=rover;mem[lomemmax].hhfield.rh:=0;
mem[lomemmax].hhfield.lhfield:=0{:1207};{1124:}mem[memtop-1].hhfield.rh:=19;
for k:=bc to ec do if charexists[k]then tfmwidth[k]:=sortin(tfmwidth[k])
;nw:=skimp(255)+1;dimenhead[1]:=mem[memtop-1].hhfield.rh;
if perturbation>=4096 then tfmwarning(20){:1124};fixdesignsize;
fixchecksum;if internal[33]>0 then begin{1126:}mem[memtop-1].hhfield.rh:=19;
...
dimenhead[2]:=mem[memtop-1].hhfield.rh;
if perturbation>=4096 then tfmwarning(21);mem[memtop-1].hhfield.rh:=19;
for k:=bc to ec do if charexists[k]then if tfmdepth[k]=0 then tfmdepth[k
]:=15 else tfmdepth[k]:=sortin(tfmdepth[k]);nd:=skimp(15)+1;
dimenhead[3]:=mem[memtop-1].hhfield.rh;
if perturbation>=4096 then tfmwarning(22);mem[memtop-1].hhfield.rh:=19;
for k:=bc to ec do if charexists[k]then if tfmitalcorr[k]=0 then
tfmitalcorr[k]:=15 else tfmitalcorr[k]:=sortin(tfmitalcorr[k]);
ni:=skimp(63)+1;dimenhead[4]:=mem[memtop-1].hhfield.rh;
...
begin bwritebyte(tfmfile,mem[tfmwidth[k]].hhfield.lhfield);
bwritebyte(tfmfile,(mem[tfmheight[k]].hhfield.lhfield)*16+mem[tfmdepth[k]].hhfield.lhfield);
bwritebyte(tfmfile,(mem[tfmitalcorr[k]].hhfield.lhfield)*4+chartag[k]);
...
p:=mem[p].hhfield.rh;end;end{:1136};{1139:}
...
curif:=mem[condptr].hhfield.b1;condptr:=mem[condptr].hhfield.rh;end;
...
eqtb[2367]:=eqtb[cursym];egloc:=cursym;primitive(470,26,0);
...
primitive(453,16,0);eqtb[2365]:=eqtb[cursym];primitive(657,4,2370);
primitive(658,4,2520);primitive(659,4,1);primitive(454,4,0);
eqtb[2364]:=eqtb[cursym];{:683}{688:}primitive(660,61,0);
primitive(661,61,1);primitive(64,61,2);primitive(662,61,3);{:688}{695:}
primitive(673,56,2370);primitive(674,56,2520);primitive(675,56,2670);
...
primitive(451,2,2);eqtb[2366]:=eqtb[cursym];primitive(716,2,3);
...
eqtb[2361]:=eqtb[cursym];primitive(385,45,73);primitive(309,45,74);
...
mem[rover].hhfield.rh:=65535;mem[rover].hhfield.lhfield:=1000;
mem[rover+1].hhfield.lhfield:=rover;mem[rover+1].hhfield.rh:=rover;
lomemmax:=rover+1000;mem[lomemmax].hhfield.rh:=0;mem[lomemmax].hhfield.lhfield:=0;
for k:=memtop-2 to memtop do mem[k]:=mem[lomemmax];avail:=0;
memend:=memtop;himemmin:=memtop-2;varused:=23;
dynused:=memtop+1-memtop-2;{:176}{193:}intname[1]:=408;intname[2]:=409;
...
hashused:=2357;stcount:=0;hash[2368].rh:=450;hash[2366].rh:=451;
hash[2367].rh:=452;hash[2365].rh:=453;hash[2364].rh:=454;
hash[2363].rh:=59;hash[2362].rh:=58;hash[2361].rh:=47;hash[2360].rh:=91;
hash[2359].rh:=41;hash[2357].rh:=455;eqtb[2359].lhfield:=62;{:203}{229:}
mem[19].hhfield.lhfield:=2370;mem[19].hhfield.rh:=0;{:229}{324:}
mem[memtop].hhfield.lhfield:=65535;{:324}{475:}mem[3].hhfield.lhfield:=0;mem[3].hhfield.rh:=0;
mem[4].hhfield.lhfield:=1;mem[4].hhfield.rh:=0;for k:=5 to 11 do mem[k]:=mem[4];
mem[12].int:=0;mem[0].hhfield.rh:=0;mem[0].hhfield.lhfield:=0;mem[1].int:=0;
mem[2].int:=0;{:475}{587:}serialno:=0;mem[13].hhfield.rh:=13;
mem[14].hhfield.lhfield:=13;mem[13].hhfield.lhfield:=0;mem[14].hhfield.rh:=0;{:587}{702:}
mem[21].hhfield.b1:=0;mem[21].hhfield.rh:=2368;eqtb[2368].rh:=21;
eqtb[2368].lhfield:=41;{:702}{759:}eqtb[2358].lhfield:=91;hash[2358].rh:=732;
{:759}{911:}mem[17].hhfield.b1:=11;{:911}{1116:}mem[20].int:=1073741824;
{:1116}{1127:}mem[16].int:=0;mem[15].hhfield.lhfield:=0;{:1127}{1185:}
...
2:printint(mem[n].hhfield.lhfield);3:printint(mem[n].hhfield.rh);
4:begin printint(eqtb[n].lhfield);printchar(58);printint(eqtb[n].rh);end;
...
if 1100>memtop then bad:=4;if 1777>2100 then bad:=5;
...
if(255<255)or(65535<65535)then bad:=17;{:154}{204:}
if 2369+maxinternal>65535 then bad:=21;{:204}{214:}
if 2820>65535 then bad:=22;{:214}{310:}
if 15*11>bistacksize then bad:=31;{:310}{553:}
if 20+17*45>bistacksize then bad:=32;{:553}{777:}
if basedefaultlength>FILENAMESIZE then bad:=41;{:777}
...
write(stdout,'This is METAFONT, C Version 2.7');
if baseident>0 then print(baseident);println;flush(stdout);{:61}{783:}
jobname:=0;logopened:=false;{:783}{792:}outputfilename:=0;{:792};{1211:}
begin{657:}begin inputptr:=0;maxinstack:=0;inopen:=0;openparens:=0;
