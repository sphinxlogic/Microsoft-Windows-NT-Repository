...
#endif /* DEBUG */
void showtokenlist ( integer p , integer q , integer l , integer nulltally ) 
...
{/* 40 10 20 */ register halfword Result; halfword p  ; 
  ...
  do {
    ...
    while ( ( mem [ q ] .hhfield .v.RH == 65535L ) ) {
...
{/* 31 32 */ halfword p, q, r  ; 
  ...
  do {
    ...
    clobbered = true ; 
    else if ( ! ( ( mem [ p ] .hhfield .v.RH == 65535L ) ) || ( mem [ p ] 
...
{integer q  ; 
  ...
  while ( q++ < for_end ) ; } 
  {register integer for_end; q = 1 ; for_end = 2369 ; if ( q <= for_end) do 
    {
      if ( eqtb [ q ] .v.RH == p ) 
      {
	printnl ( 457 ) ; 
...
void flushtokenlist ( halfword p ) 
{halfword q  ; 
  while ( p != 0 ) {
...
void printpath ( halfword h , strnumber s , boolean nuline ) 
{/* 30 31 */ halfword p, q  ; 
...
void printedges ( strnumber s , boolean nuline , integer xoff , integer yoff ) 
{halfword p, q, r  ; 
  integer n  ; 
...
void unskew ( scaled x , scaled y , smallnumber octant ) 
{switch ( octant ) 
  {case 1 : 
    {
      curx = x + y ; 
...
void printpen ( halfword p , strnumber s , boolean nuline ) 
...
void printdependency ( halfword p , smallnumber t ) 
{/* 10 */ integer v  ; 
  halfword pp, q  ; 
  pp = p ; 
  while ( true ) {
...
void printdp ( smallnumber t , halfword p , smallnumber verbosity ) 
{halfword q  ; 
  q = mem [ p ] .hhfield .v.RH ; 
