...
void skew ( scaled x , scaled y , smallnumber octant ) 
{switch ( octant ) 
  {case 1 : 
    {
      curx = x - y ; 
...
void abnegate ( scaled x , scaled y , smallnumber octantbefore , smallnumber 
octantafter ) 
...
{halfword p, q, r, s  ; 
  ...
  do {
	...
	desty = desty + destx ; 
	destx = - (integer) destx ; 
	if ( mem [ r + 6 ] .cint > desty ) 
	mem [ r + 6 ] .cint = desty ; 
	if ( mem [ q + 4 ] .cint > desty ) 
	mem [ q + 4 ] .cint = desty ; 
	else if ( mem [ q + 4 ] .cint < mem [ r + 2 ] .cint ) 
	mem [ q + 4 ] .cint = mem [ r + 2 ] .cint ; 
	if ( mem [ r + 6 ] .cint > mem [ q + 4 ] .cint ) 
	mem [ r + 6 ] .cint = mem [ q + 4 ] .cint ; 
	del2 = del2 - takefraction ( del2 - del3 , t ) ; 
	if ( del2 > 0 ) 
	del2 = 0 ; 
	t = crossingpoint ( 0 , - (integer) del2 , - (integer) del3 ) ; 
	if ( t < 268435456L ) 
	{
	  ...
	  mem [ s + 5 ] .cint = mem [ s + 1 ] .cint ; 
	  desty = desty + destx ; 
	  if ( mem [ s + 6 ] .cint > desty ) 
	  mem [ s + 6 ] .cint = desty ; 
	  if ( mem [ q + 4 ] .cint > desty ) 
	  mem [ q + 4 ] .cint = desty ; 
	  else if ( mem [ q + 4 ] .cint < mem [ s + 2 ] .cint ) 
	  mem [ q + 4 ] .cint = mem [ s + 2 ] .cint ; 
	  if ( mem [ s + 6 ] .cint > mem [ q + 4 ] .cint ) 
	  mem [ s + 6 ] .cint = mem [ q + 4 ] .cint ; 
	} 
	else {
