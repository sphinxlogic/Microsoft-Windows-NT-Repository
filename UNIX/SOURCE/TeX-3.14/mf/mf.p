...
else begin r:=mem[p].hh.lh;if r>=2370 then{222:}
...
mem[rover+1].hh.rh:=65535;oldrover:=rover;while p<>oldrover do{174:}
...
lomemmax)then clobbered:=true else if not((mem[p].hh.rh=65535))or(mem[p]
...
while p<=lomemmax do begin if(mem[p].hh.rh=65535)then begin printnl(318)
...
r:=mem[p].hh.lh;q:=p;p:=mem[q].hh.rh;until r=0;end{:617};
...
456,2100);decr(hashused);until hash[hashused].rh=0;hash[p].lh:=hashused;
p:=hashused;end;
...
2519+curmod;end{:690};end;mem[p].hh.rh:=curtok;p:=mem[p].hh.rh;end;
...
getsymbol;q:=getnode(2);mem[q].hh.lh:=cursym;mem[q+1].int:=2370;
getclearsymbol;warninginfo:=cursym;getsymbol;p:=getnode(2);
mem[p].hh.lh:=cursym;mem[p+1].int:=2371;mem[p].hh.rh:=q;getnext;
...
begin if mem[q].hh.rh=1 then printnl(497)else if(b<2670)and(b<>7)then
...
while mem[r].hh.lh>=2370 do begin{726:}
...
if mem[r].hh.lh>=2670 then scantextarg(ldelim,rdelim)else begin getxnext
;if mem[r].hh.lh>=2520 then scansuffix else scanexpression;end{:729};
if curcmd<>82 then{727:}
if(curcmd<>62)or(curmod<>ldelim)then if mem[mem[r].hh.rh].hh.lh>=2370
...
if mem[lhs].hh.lh>2369 then print(intname[mem[lhs].hh.lh-(2369)])else
showtokenlist(lhs,0,1000,0);print(460);printexp(0,0);printchar(125);
enddiagnostic(false);end{:998};if mem[lhs].hh.lh>2369 then{999:}
if curtype=16 then internal[mem[lhs].hh.lh-(2369)]:=curexp else begin
disperr(0,884);print(intname[mem[lhs].hh.lh-(2369)]);print(885);
...
hash[2359].rh:=41;hash[2357].rh:=455;eqtb[2359].lh:=62;{:203}{229:}
mem[19].hh.lh:=2370;mem[19].hh.rh:=0;{:229}{324:}
mem[memtop].hh.lh:=65535;{:324}{475:}mem[3].hh.lh:=0;mem[3].hh.rh:=0;
...
mem[21].hh.b1:=0;mem[21].hh.rh:=2368;eqtb[2368].rh:=21;
eqtb[2368].lh:=41;{:702}{759:}eqtb[2358].lh:=91;hash[2358].rh:=732;
