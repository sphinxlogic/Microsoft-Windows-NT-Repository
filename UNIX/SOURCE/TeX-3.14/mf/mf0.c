...
{integer i  ; 
  ...
  eqtb [ 1 ] .lhfield = 41 ; 
  eqtb [ 1 ] .v.RH = 0 ; 
  {register integer for_end; k = 2 ; for_end = 2369 ; if ( k <= for_end) do 
    {
      hash [ k ] = hash [ 1 ] ; 
      eqtb [ k ] = eqtb [ 1 ] ; 
    } 
...
void printtwo ( scaled x , scaled y ) 
{printchar ( 40 ) ; 
  printscaled ( x ) ; 
  printchar ( 44 ) ; 
  printscaled ( y ) ; 
...
void printdiagnostic ( strnumber s , strnumber t , boolean nuline ) 
{begindiagnostic () ; 
  if ( nuline ) 
  printnl ( s ) ; 
  else print ( s ) ; 
...
integer strvsstr ( strnumber s , strnumber t ) 
...
integer slowadd ( integer x , integer y ) 
...
fraction makefraction ( integer p , integer q ) 
...
integer takefraction ( integer q , fraction f ) 
...
integer takescaled ( integer q , scaled f ) 
{register integer Result; integer p  ; 
  boolean negative  ; 
  integer n  ; 
  integer becareful  ; 
...
scaled makescaled ( integer p , integer q ) 
...
fraction velocity ( fraction st , fraction ct , fraction sf , fraction cf , 
scaled t ) 
...
integer abvscd ( integer a , integer b , integer c , integer d ) 
...
integer floorunscaled ( scaled x ) 
...
integer roundunscaled ( scaled x ) 
{register integer Result; integer becareful  ; 
  if ( x >= 32768L ) 
...
scaled roundfraction ( fraction x ) 
{register scaled Result; integer becareful  ; 
  if ( x >= 2048 ) 
...
integer pythadd ( integer a , integer b ) 
...
integer pythsub ( integer a , integer b ) 
{/* 30 */ register integer Result; fraction r  ; 
  boolean big  ; 
  a = abs ( a ) ; 
  b = abs ( b ) ; 
...
angle narg ( integer x , integer y ) 
