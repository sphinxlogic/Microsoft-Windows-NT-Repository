...
webname,chgname,pascalfilename,poolfilename:array[1..FILENAMESIZE]of
char;{:189}{30:}{procedure debughelp;forward;}{:30}{31:}procedure error;
...
fname:array[1..FILENAMESIZE]of char;foundweb,foundchange:boolean;
...
slashpos:=-1;i:=1;
while(fname[i]<>' ')and(i<=FILENAMESIZE-5)do begin webname[i]:=fname[i];
...
slashpos:=-1;i:=1;
while(fname[i]<>' ')and(i<=FILENAMESIZE-5)do begin chgname[i]:=fname[i];
if fname[i]='.'then dotpos:=i;if fname[i]='/'then slashpos:=i;i:=i+1;
end;chgname[i]:=' ';
if(dotpos=-1)or(dotpos<slashpos)then begin dotpos:=i;
...
error;history:=3;uexit(1);end;tokmem[z,tokptr[z]]:=buffer[loc];
...
loc:=loc+2;end;end;if loc>=limit then begin writeln(stdout);
...
writeln(stdout,'This is TANGLE, C Version 4.2');{183:}phaseone:=true;
